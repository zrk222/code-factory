...
- **Assembly resource/lifecycle status (T2 complete):** `_run_cli` delegates to `assembly_process.run_cli`, limiting retained stdout and stderr to 4 MiB each, using a 300-second execution deadline and finite OS cleanup/reader waits. Each invocation is bound to a POSIX process group or Windows Job Object; overflow, cancellation, timeout, read failure and unconfirmed cleanup cannot pass. The detailed runner receipt exposes `cleanup_confirmed`, `streams_closed`, exit code and reason without breaking the legacy tuple API. This is process hygiene, not a sandbox; hostile descendants that escape the operating-system cleanup boundary are not claimed absent.
...
- **Remaining process boundary:** proof-reuse filesystem races and Windows process cleanup are covered by T1/T2. Native Linux/macOS parity and malicious descendants that escape the operating-system cleanup boundary remain unverified and are explicitly scheduled in T3; no sandbox or hostile-process absence is claimed.
...
- Final 0.46.2 release-gate regression after Oracle, Metadata, Studio HTTP framing, capability execution containment, and review-thread hardening: `python -m pytest -q` — **1324 passed, 3 skipped**.
