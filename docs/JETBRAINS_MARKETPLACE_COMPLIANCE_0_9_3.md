# FactoryLine 0.9.3 — Marketplace compliance evidence map

| Requirement or review risk | 0.9.3 evidence | Status and boundary |
| --- | --- | --- |
| Identity and version | `editors/intellij/build.gradle.kts` declares `0.9.3`; `plugin.xml` keeps id `app.factoryline`; `editors/intellij/CHANGELOG.md` leads with `0.9.3`. | Local metadata is aligned; a Marketplace listing read-back is still required. |
| User-facing value | `plugin.xml` and the IntelliJ README lead with First Proof, six audit lanes, and the senior evidence controls. | Copy is source-reviewable; moderation and listing presentation remain external. |
