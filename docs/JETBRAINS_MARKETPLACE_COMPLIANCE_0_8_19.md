...
| --- | --- | --- |
| New SaaS controls must not turn the IDE plugin into an identity, billing, or credential integration. | **Verify SaaS journey** accepts only two existing project-local JSON files, requires a second execution confirmation, and runs the local CLI with a workspace-local receipt path. **View SaaS status** reads that receipt. The verifier rejects raw tokens, secrets, cookies, authorization headers, and code verifiers; Graph Ops projects hash-valid receipt metadata. | **Local source, command-construction, and unit-test evidence. Provider certification remains out of scope.** |
