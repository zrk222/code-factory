# FactoryLine 0.8.16 — Marketplace compliance checklist
...
| Privacy, telemetry, and performance | A production-source scan finds no `java.net`, `HttpClient`, `Socket`, or `URL` API reference. Guardian and Engineering Judgment are explicit-confirmation, local observation/read-only surfaces; tests cover command construction, schema rejection, and UI/action registration. | Runtime behavior in every IDE and Marketplace's independent security/performance analysis cannot be proven locally. |
