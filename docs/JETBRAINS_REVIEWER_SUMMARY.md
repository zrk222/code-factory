# FactoryLine 0.8.15 — JetBrains reviewer summary
...
| A hard-won architectural decision can disappear from a handoff, then a later diff silently violates its stated proof obligations. | **Engineering Judgment** shows only repository-tracked, schema-bound Capsule state. Separate workspace-confirmed local actions either inspect Capsules or compile a deterministic Safety Case from one selected native Change List. The view cannot create, promote, waive, infer, apply, or execute a decision. | `FactoryLineJudgment.kt`, `FactoryLineCore.kt`, `FactoryLineActions.kt`, `FactoryLineCoreTest.judgment*`, and `docs/ENGINEERING_JUDGMENT.md`. |
...
FactoryLine 0.8.15 is a local, supervised JetBrains adapter. It does not
