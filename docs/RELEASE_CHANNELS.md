...
Code Factory core v0.46.4, FactoryLine for VS Code v0.9.6, and FactoryLine for
JetBrains v0.9.4 publish from one
...
| VS Code / Open VSX | `factoryline-vscode-0.9.6.vsix` | GitHub release bundle; protected publisher workflows publish the immutable, verified VSIX when their scoped credentials are configured | Installable VSIX and public marketplace version |
| JetBrains | `factoryline-intellij-0.9.4.zip` | Submit only after the live Marketplace binary-slot and metadata gates permit the update | Compatible ZIP plus an accepted Marketplace upload receipt; public availability still requires moderation |
