...
Code Factory v0.21.0 publishes one verified source state through channel-specific
...
| --- | --- | --- | --- |
| GitHub | Source tag, wheel, sdist, VSIX, JetBrains ZIP, media | Publish `v0.21.0`; `publish.yml` attaches the verified bundle | Public release URL and green workflow |
| PyPI | `factoryline-code-factory==0.21.0` | Trusted Publishing from `publish.yml` | PyPI project version and attestation |
...
| VS Code | `factoryline-vscode-0.6.0.vsix` | GitHub release bundle; Marketplace requires a separately configured publisher token | Installable VSIX or public Marketplace version |
| JetBrains | `factoryline-intellij-0.6.0.zip` | One-time manual Marketplace listing, then scoped workflow updates | Public plugin/version page after moderation |
