...
Code Factory core v0.46.3, FactoryLine for VS Code v0.9.5, and FactoryLine for
JetBrains v0.9.3 publish from one
...
| --- | --- | --- | --- |
| GitHub | Source tag, wheel, sdist, VSIX, JetBrains ZIP, media | Publish core tag `v0.46.3`; `publish.yml` attaches the verified bundle | Public release URL and green workflow |
| PyPI | `factoryline-code-factory==0.46.3` | Trusted Publishing from `publish.yml` | PyPI project version and attestation |
...
| VS Code / Open VSX | `factoryline-vscode-0.9.5.vsix` | GitHub release bundle; protected publisher workflows publish the immutable, verified VSIX when their scoped credentials are configured | Installable VSIX and public marketplace version |
| JetBrains | `factoryline-intellij-0.9.3.zip` | Submit only after the live Marketplace binary-slot and metadata gates permit the update | Compatible ZIP plus an accepted Marketplace upload receipt; public availability still requires moderation |
