...
Code Factory core v0.46.4, FactoryLine for VS Code/Open VSX v0.9.7, and
FactoryLine for JetBrains v0.9.5 publish from one
...
| --- | --- | --- | --- |
| GitHub | Source tag, wheel, sdist, VSIX, JetBrains ZIP, media | Publish core tag `v0.46.4`; `publish.yml` attaches the verified bundle | Public release URL and green workflow |
| PyPI | `factoryline-code-factory==0.46.4` | Trusted Publishing from `publish.yml` | PyPI project version and attestation |
...
| VS Code / Open VSX | `factoryline-vscode-0.9.7.vsix` | GitHub release bundle; protected publisher workflows publish the immutable, verified VSIX when their scoped credentials are configured | Installable VSIX and public marketplace version |
| JetBrains | `factoryline-intellij-0.9.5.zip` | Submit only after the live Marketplace binary-slot and metadata gates permit the update | Compatible ZIP plus an accepted Marketplace upload receipt; public availability still requires moderation |
...
stages, scoped source preconditions, real before/after artifacts, provider-
declared tool and checkpoint hashes, and resume lineage. Code Factory binds
those facts to the current Oracle Contract and rejects drift. The adapter and
its local Proof Worklog are deliberately read-only: they do not run a provider,
execute commands, post to a tracker, inherit agent authority, or make approval,
merge, publication, or deployment decisions.
...
was still pending review. The prepared 0.9.0 successor must pass the live
