...
from factoryline.contract import Receipt, ensure_layout  # noqa: E402
from factoryline.passport import build_passport, verify_passport  # noqa: E402
from factoryline.proof import build_trace  # noqa: E402
from factoryline.challenge import challenge_trace  # noqa: E402
    ...
    with tempfile.TemporaryDirectory(prefix="prooflab-e2e-") as temp:
        ...
        run_module(
            ...
            [
                "challenge",
                "prooflab",
                str(root / "prooflab.ssat.yaml"),
                "--root",
                str(root),
                "--out",
                str(challenges / "forgeline.json"),
            ],
            root,
        )
        ...
        run_module(
            ...
            [
                ...
                str(root / "DESIGN.md"),
                "--out",
                str(challenges / "prestige-tokens.json"),
            ],
            root,
        )
        ...
        factory_path.write_text(
            json.dumps(factory_challenge, indent=2), encoding="utf-8"
        )
        challenge_paths = sorted(challenges.glob("*.json"))
        passport = build_passport(
            root, "prooflab", root / trace["trace_path"], challenge_paths
        )
            ...
            json.dumps(
                {
                    ...
                    "challenge_stages": [
                        f"{item['brick']}:{item['stage']}"
                        for item in passport["challenges"]
                    ],
