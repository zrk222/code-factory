...
from factoryline.contract import Receipt  # noqa: E402
from factoryline.oracle_firewall import (  # noqa: E402
    capture_intent_handoff,
    seal_oracle_contract,
)
from factoryline.release_contract import _sha  # noqa: E402
from factoryline.verification import verify_feature  # noqa: E402
...
STAGES = [
    *(f"specline:{stage}" for stage in SPEC),
    *(f"forgeline:{stage}" for stage in (*FORGE, "ship")),
]


def write_complete(root: Path, binding: dict[str, str]) -> None:
    for stage in SPEC:
        Receipt("specline", stage, "release-train", True, inputs=dict(binding)).write(
            root
        )
    for stage in FORGE:
        Receipt("forgeline", stage, "release-train", True, inputs=dict(binding)).write(
            root
        )
    Receipt(
        "forgeline",
...
def write_release_contract(root: Path) -> dict[str, str]:
    brief = root / "brief.md"
    brief.write_text(
        "Preserve intent traceability for the release train.", encoding="utf-8"
    )
    agent = {
    ...
    oracle_path = Path(
        seal_oracle_contract(
            root, source, Path(".factory/oracles/contracts/release-train.json")
        )["path"]
    )
...
def main() -> int:
    with tempfile.TemporaryDirectory(prefix="factory-release-train-") as temporary:
        ...
        if incomplete["shippable"]:
            raise RuntimeError(
                "incomplete evidence incorrectly became locally shippable"
            )
        ...
        if not complete["release_ready"]:
            raise RuntimeError(
                f"complete evidence was rejected: {complete['blockers']}"
            )
        ...
        if stale["release_ready"] or not any(
            item["code"] == "RECEIPT_ORACLE_BINDING_MISMATCH"
            for item in stale["blockers"]
        ):
            raise RuntimeError(
                "stale receipt binding incorrectly remained release-ready"
            )
    print(
        "release-train-e2e: incomplete rejected; strict bound pipeline accepted; stale receipt rejected"
    )
    return 0
