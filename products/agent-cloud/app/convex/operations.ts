...
type OperationalHealthData = {
  ...
  authoritativeSources: Doc<"authoritativeSources">[];
  backups: Doc<"backupSnapshots">[];
  drills: Doc<"restoreDrills">[];
  account: Doc<"creditAccounts"> | null;
};

async function collectOperationalHealthData(ctx: QueryCtx, workspaceId: Id<"workspaces">): Promise<OperationalHealthData> {
  const [jobs, connectors, authoritativeSources, backups, drills, account] = await Promise.all([
    ctx.db.query("executionJobs").withIndex("by_workspace_created", (q) => q.eq("workspaceId", workspaceId)).order("desc").take(100),
    ctx.db.query("knowledgeConnectors").withIndex("by_workspace_status", (q) => q.eq("workspaceId", workspaceId)).collect(),
    ctx.db.query("authoritativeSources").withIndex("by_workspace_status", (q) => q.eq("workspaceId", workspaceId)).collect(),
    ctx.db.query("backupSnapshots").withIndex("by_workspace_requested", (q) => q.eq("workspaceId", workspaceId)).order("desc").take(10),
    ctx.db.query("restoreDrills").withIndex("by_workspace_requested", (q) => q.eq("workspaceId", workspaceId)).order("desc").take(10),
    ctx.db.query("creditAccounts").withIndex("by_workspace", (q) => q.eq("workspaceId", workspaceId)).unique(),
  ]);
  return { jobs, connectors, authoritativeSources, backups, drills, account };
}

function jobHealthSummary(jobs: Doc<"executionJobs">[]) {
...
function operationalHealthFindings(data: OperationalHealthData, queueAgeSeconds: number, failureRatePercent: number, blockedRequiredGroups: ReturnType<typeof assessSourceGroups>) {
  const findings: string[] = [];
  if (queueAgeSeconds > 300) findings.push("QUEUE_AGE_HIGH");
  if (failureRatePercent > 10) findings.push("FAILURE_RATE_HIGH");
  if (blockedRequiredGroups.length > 0) findings.push("AUTHORITATIVE_SOURCES_NOT_READY");
  if (!data.backups.some((item) => item.state === "completed")) findings.push("BACKUP_NOT_VERIFIED");
  if (!data.drills.some((item) => item.state === "passed")) findings.push("RESTORE_DRILL_NOT_VERIFIED");
  if (data.account?.status !== "active") findings.push("CREDIT_ACCOUNT_INACTIVE");
  return findings;
}

function operationalHealthStatus(findings: string[]) {
  if (findings.some((item) => item !== "BACKUP_NOT_VERIFIED" && item !== "RESTORE_DRILL_NOT_VERIFIED")) return "degraded" as const;
  if (findings.length > 0) return "attention" as const;
  return "healthy" as const;
}

function operationalHealthReport(data: OperationalHealthData) {
  const jobs = jobHealthSummary(data.jobs);
  const sourceGroups = assessSourceGroups(data.authoritativeSources, Date.now());
  const blockedRequiredGroups = sourceGroups.filter((group) => group.requiredForRuns && group.state === "blocked");
  const findings = operationalHealthFindings(data, jobs.queueAgeSeconds, jobs.failureRatePercent, blockedRequiredGroups);
  return {
    marker: "OPERATIONS_HEALTH_EXPLAINED" as const,
    status: operationalHealthStatus(findings),
    queue: { queued: jobs.queued, running: jobs.running, queueAgeSeconds: jobs.queueAgeSeconds },
    reliability: { terminalJobs: jobs.terminal.length, failedJobs: jobs.failed, failureRatePercent: jobs.failureRatePercent },
    connectors: { ready: data.connectors.filter((item) => item.status === "ready").length, setupRequired: data.connectors.filter((item) => item.status === "setup-required").length },
    sourceAssurance: { configuredSources: data.authoritativeSources.length, requiredGroups: sourceGroups.filter((group) => group.requiredForRuns).length, readyRequiredGroups: sourceGroups.filter((group) => group.requiredForRuns && group.state === "ready").length, blockedRequiredGroups },
    latestBackup: data.backups[0] ?? null,
    latestRestoreDrill: data.drills[0] ?? null,
    findings,
  };
}
...
export const health = query({
  ...
  handler: async (ctx, args) => {
    ...
    return operationalHealthReport(await collectOperationalHealthData(ctx, args.workspaceId));
  },
});
