...
- The system shall emit `PROOF_REVIEW_FACTS_PRESERVED` and return the existing 1 through 100 normalized workspace-relative path bound, exact Graph Ops impact facts, coverage facts, plan-only rerun recommendations, and all-false external-effect authority object from factory.change_review.v1.
