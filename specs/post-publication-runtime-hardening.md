...
- When `REQ_POSIX_PARITY` validates command-runner behavior on Linux and macOS, it shall sample native PID lineage no more often than every 0.25 seconds with a 1-second command deadline, parse each native `ps` lineage row as exactly 3 numeric fields (`pid`, `ppid`, `pgid`) plus one non-empty `lstart` identity, return `timed_out` for a 1 seconds fixture, `output_limit_exceeded` for a 4194305 bytes fixture, `cancelled` for 1 cancellation fixture and `cleanup_confirmed=false` for 1 surviving-child fixture.
