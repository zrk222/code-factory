...
- Contract fields and parser subset are documented in docs/DEEP_AUDIT_INGESTION.md; the evaluator is documented in docs/DEEP_AUDIT_DECISIONS.md. External scanner execution remains outside this module.
- Evaluation conservatively counts all mapped results toward max_total, including absent, pass and suppressed labels; max_new counts new, updated and unbaselined results. Labels cannot silently remove evidence from signed thresholds.
- Repair priority orders signed severities critical, high, medium, low using internal ranks 0, 1, 2, 3; ranks are sort keys, not defect thresholds.
- Read-only status inspects at most 1024 receipt files (probing 1025 to detect overflow), bounded to 10,000,000 bytes each; linked directories, malformed or inconsistent receipts return INCOMPLETE. Self-hash validation is not signer authentication or freshness validation.
