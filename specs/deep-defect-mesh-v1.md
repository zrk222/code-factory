...
- When `REQ_VERIFY` inspects the plan, it shall verify Ed25519 signature trust, an aware issuance time, an expiry no more than 24 hours after issuance, current non-expiry, exact candidate source hashes and byte sizes, a candidate manifest digest, unique identifiers, exact fields, bounded values, and authoritative rule origins from `human_confirmed`, `trusted_source`, or `observed_production`. [R20]
