...
_EVIDENCE_KINDS = frozenset(
    {"counterexample", "test_result", "trace", "proof_receipt", "external_artifact"}
)
_AUTHORITY = {
    "execution": False,
    "repair": False,
...
def _canonical(value: object) -> bytes:
    try:
        ...
        raise ProofDeltaError(
            "PROOF_DELTA_INVALID", f"receipt must be canonical JSON: {exc}"
        ) from exc
...
def _relative(
    root: Path, value: Path | str, field: str, *, exists: bool = True
) -> tuple[Path, str]:
    raw = Path(value)
    ...
    try:
        ...
        raise ProofDeltaError(
            "PROOF_DELTA_PATH_OUTSIDE_ROOT", f"{field} must stay inside the workspace"
        ) from exc
    ...
    if exists and (not path.is_file() or path.stat().st_size > MAX_SOURCE_BYTES):
        raise ProofDeltaError(
            "PROOF_DELTA_SOURCE_INVALID",
            f"{field} must be a readable file at most {MAX_SOURCE_BYTES} bytes",
        )
...
def _load_json(path: Path, field: str) -> dict[str, Any]:
    try:
        ...
        raise ProofDeltaError(
            "PROOF_DELTA_SOURCE_INVALID", f"{field} must be readable JSON: {exc}"
        ) from exc
    if not isinstance(value, dict):
        raise ProofDeltaError(
            "PROOF_DELTA_SOURCE_INVALID", f"{field} must contain one JSON object"
        )
    return value


def _reference(
    root: Path, value: Path | str, field: str
) -> tuple[dict[str, str], dict[str, Any]]:
    path, relative = _relative(root, value, field)
    return {"path": relative, "sha256": _sha_bytes(path.read_bytes())}, _load_json(
        path, field
    )
...
def _sha_text(value: object, field: str) -> str:
    if not isinstance(value, str) or not _SHA.fullmatch(value):
        raise ProofDeltaError(
            "PROOF_DELTA_INVALID", f"{field} must be a lowercase SHA-256"
        )
    return value
...
def _changed_paths(value: object, field: str) -> list[str]:
    if not isinstance(value, list) or not value:
        raise ProofDeltaError(
            "PROOF_DELTA_CANDIDATE_INVALID",
            f"{field} must contain at least one workspace-relative path",
        )
    paths: list[str] = []
    for item in value:
        if (
            not isinstance(item, str)
            or not _PATH.fullmatch(item)
            or Path(item).is_absolute()
            or ".." in Path(item).parts
        ):
            raise ProofDeltaError(
                "PROOF_DELTA_CANDIDATE_INVALID",
                f"{field} contains an invalid workspace-relative path",
            )
        paths.append(Path(item).as_posix())
    if paths != sorted(set(paths)):
        raise ProofDeltaError(
            "PROOF_DELTA_CANDIDATE_INVALID", f"{field} must be sorted and unique"
        )
    return paths
...
def _evidence(root: Path, value: object, field: str) -> list[dict[str, str]]:
    if not isinstance(value, list) or not 1 <= len(value) <= MAX_EVIDENCE:
        raise ProofDeltaError(
            "PROOF_DELTA_EVIDENCE_INVALID",
            f"{field} must contain 1 to {MAX_EVIDENCE} evidence references",
        )
    ...
    for index, item in enumerate(value):
        if not isinstance(item, dict) or set(item) != {"path", "sha256", "kind"}:
            raise ProofDeltaError(
                "PROOF_DELTA_EVIDENCE_INVALID",
                f"{field}[{index}] must contain path, sha256, and kind",
            )
        kind = item.get("kind")
        if kind not in _EVIDENCE_KINDS:
            raise ProofDeltaError(
                "PROOF_DELTA_EVIDENCE_INVALID", f"{field}[{index}].kind is unsupported"
            )
        ...
        if _sha_bytes(path.read_bytes()) != digest:
            raise ProofDeltaError(
                "PROOF_DELTA_EVIDENCE_STALE",
                f"{field}[{index}] bytes do not match sha256",
            )
        key = (relative, digest)
        if key in seen:
            raise ProofDeltaError(
...
def _candidate(
    root: Path, reference: dict[str, str], mission_id: str, field: str
) -> dict[str, Any]:
    if set(reference) != {"path", "sha256"}:
        raise ProofDeltaError(
            "PROOF_DELTA_INVALID", f"{field} reference must contain path and sha256"
        )
    path, relative = _relative(root, reference["path"], field)
    digest = _sha_text(reference["sha256"], f"{field}.sha256")
    if _sha_bytes(path.read_bytes()) != digest:
        raise ProofDeltaError(
            "PROOF_DELTA_SOURCE_STALE", f"{field} receipt bytes changed"
        )
    value = _load_json(path, field)
    if value.get("schema") != CANDIDATE_SCHEMA or value.get("mission_id") != mission_id:
        raise ProofDeltaError(
            "PROOF_DELTA_CANDIDATE_INVALID",
            f"{field} must be a mission-bound candidate receipt",
        )
    ...
    if not isinstance(candidate, dict) or set(candidate) != {
        ...
        raise ProofDeltaError(
            "PROOF_DELTA_CANDIDATE_INVALID",
            f"{field}.candidate must contain diff_sha256 and changed_paths",
        )
    return {
        "path": relative,
        "sha256": digest,
        "candidate": {
            "diff_sha256": _sha_text(
                candidate.get("diff_sha256"), f"{field}.candidate.diff_sha256"
            ),
            "changed_paths": _changed_paths(
                candidate.get("changed_paths"), f"{field}.candidate.changed_paths"
            ),
        },
...
def _failure(root: Path, reference: dict[str, str], mission_id: str) -> dict[str, str]:
    if set(reference) != {"path", "sha256"}:
        raise ProofDeltaError(
            "PROOF_DELTA_INVALID", "failure reference must contain path and sha256"
        )
    path, relative = _relative(root, reference["path"], "failure")
    digest = _sha_text(reference["sha256"], "failure.sha256")
    if _sha_bytes(path.read_bytes()) != digest:
        raise ProofDeltaError(
            "PROOF_DELTA_SOURCE_STALE", "failure receipt bytes changed"
        )
    value = _load_json(path, "failure")
    if (
        value.get("schema") != VALIDATION_FAILURE_SCHEMA
        or value.get("mission_id") != mission_id
    ):
        raise ProofDeltaError(
            "PROOF_DELTA_FAILURE_INVALID",
            "failure must be a mission-bound validation failure receipt",
        )
...
def _mission(root: Path, reference: dict[str, str], mission_id: str) -> dict[str, str]:
    if set(reference) != {"path", "mission_sha256"}:
        raise ProofDeltaError(
            "PROOF_DELTA_INVALID",
            "mission reference must contain path and mission_sha256",
        )
    ...
    if not verification.get("valid"):
        raise ProofDeltaError(
            "PROOF_DELTA_MISSION_INVALID",
            "; ".join(verification.get("errors", ["mission verification failed"])),
        )
    value = _load_json(path, "mission")
    if value.get("id") != mission_id or value.get("mission_sha256") != expected:
        raise ProofDeltaError(
            "PROOF_DELTA_MISSION_INVALID",
            "mission identity or hash does not match the receipt",
        )
    return {"path": relative, "mission_sha256": expected}


def _new_evidence(
    prior: list[dict[str, str]], repair: list[dict[str, str]]
) -> list[dict[str, str]]:
...
def _mission_identity(
    root: Path, mission_path: Path
    ...
    if (
        ...
        or not _SHA.fullmatch(mission_sha)
    ):
        raise ProofDeltaError(
            "PROOF_DELTA_MISSION_INVALID", "mission must have an id and mission_sha256"
        )
    return (
        {"path": reference["path"], "mission_sha256": mission_sha},
        mission_value,
        mission_id,
    )
...
def _require_criterion(mission: dict[str, Any], criterion_id: str) -> None:
    if not isinstance(criterion_id, str) or not criterion_id or len(criterion_id) > 120:
        raise ProofDeltaError(
            "PROOF_DELTA_CRITERION_INVALID",
            "criterion_id must contain 1 to 120 characters",
        )
    ...
    if criterion_id not in {
        ...
        raise ProofDeltaError(
            "PROOF_DELTA_CRITERION_INVALID",
            "criterion_id is not declared by the mission completion contract",
        )


def _candidate_from_path(
    root: Path, path: Path, mission_id: str, field: str
) -> dict[str, Any]:
...
def _delta_marker(disposition: str) -> str:
    return (
        "PROOF_DELTA_ADVANCE"
        if disposition == "advance"
        else "PROOF_DELTA_NO_EVIDENCE_GAIN"
    )


def _write_delta(
    root: Path, out: Path, result: dict[str, Any], disposition: str
) -> dict[str, Any]:
        ...
        try:
            if path.read_bytes() == encoded:
                return {
                    "path": str(path),
                    **result,
                    "marker": _delta_marker(disposition),
                    "idempotent": True,
                }
        except OSError as exc:
            raise ProofDeltaError(
                "PROOF_DELTA_WRITE_FAILED", f"cannot read existing receipt: {exc}"
            ) from exc
        raise ProofDeltaError(
            "PROOF_DELTA_ARTIFACT_EXISTS", f"refusing to replace {path}"
        )
    try:
        path.write_bytes(encoded)
    except OSError as exc:
        raise ProofDeltaError(
            "PROOF_DELTA_WRITE_FAILED", f"cannot write receipt: {exc}"
        ) from exc
    return {
        "path": str(path),
        **result,
...
def create_proof_delta(
    root: Path,
    mission_path: Path,
    prior_candidate_path: Path,
    ...
    prior = _candidate_from_path(
        workspace, prior_candidate_path, mission_id, "prior_candidate"
    )
    repair = _candidate_from_path(
        workspace, repair_candidate_path, mission_id, "repair_candidate"
    )
    ...
    core = {
        ...
        "markers": [
            "PROOF_DELTA_HASH_BOUND",
            "PROOF_DELTA_FRESH_CONTEXT_REQUIRED",
            "PROOF_DELTA_INDEPENDENT_VALIDATION_REQUIRED",
        ],
    }
...
def _validate_delta_shape(value: dict[str, Any]) -> None:
    ...
    if set(value) != required or value.get("schema") != PROOF_DELTA_SCHEMA:
        raise ProofDeltaError(
            "PROOF_DELTA_INVALID",
            f"receipt must contain exactly the {PROOF_DELTA_SCHEMA} fields",
        )
...
def _validate_delta_authority(value: dict[str, Any]) -> None:
    if (
        ...
        raise ProofDeltaError(
            "PROOF_DELTA_INVALID", "fresh context and retained authority are required"
        )
...
def _current_delta_inputs(
    ...
    criteria = (
        _load_json((workspace / mission["path"]).resolve(), "mission")
        .get("completion_contract", {})
        .get("criteria", [])
    )
    if value["criterion_id"] not in {
        item.get("id") for item in criteria if isinstance(item, dict)
    }:
        raise ProofDeltaError(
            "PROOF_DELTA_CRITERION_INVALID",
            "criterion_id is not declared by the current mission",
        )
    failure = _failure(workspace, value["failure"], value["mission_id"])
    prior_reference = (
        {key: value["prior_candidate"].get(key) for key in ("path", "sha256")}
        if isinstance(value.get("prior_candidate"), dict)
        else {}
    )
    repair_reference = (
        {key: value["repair_candidate"].get(key) for key in ("path", "sha256")}
        if isinstance(value.get("repair_candidate"), dict)
        else {}
    )
    prior = _candidate(
        workspace, prior_reference, value["mission_id"], "prior_candidate"
    )
    repair = _candidate(
        workspace, repair_reference, value["mission_id"], "repair_candidate"
    )
    return failure, prior, repair, mission


def _validate_delta_evidence(
    value: dict[str, Any], prior: dict[str, Any], repair: dict[str, Any]
) -> tuple[str, list[dict[str, str]]]:
    if value["prior_candidate"] != prior or value["repair_candidate"] != repair:
        raise ProofDeltaError(
            "PROOF_DELTA_INTEGRITY_INVALID",
            "candidate bindings do not match current candidate receipts",
        )
    expected_new = _new_evidence(prior["evidence"], repair["evidence"])
    if value["new_evidence"] != expected_new:
        raise ProofDeltaError(
            "PROOF_DELTA_INTEGRITY_INVALID",
            "new_evidence does not equal the candidate evidence delta",
        )
    changed = repair["candidate"]["diff_sha256"] != prior["candidate"]["diff_sha256"]
    ...
    if value.get("disposition") != expected or value.get("reason") != _reason(expected):
        raise ProofDeltaError(
            "PROOF_DELTA_INTEGRITY_INVALID", "disposition does not match bound evidence"
        )
