...
def _app_jwt(app_id: int, private_key_pem: str, now: int) -> str:
    ...
    payload = _b64(
        json.dumps(
            {"iat": now - 30, "exp": now + 570, "iss": str(app_id)},
            separators=(",", ":"),
        ).encode()
    )
    try:
        ...
        raise PRAssuranceError(
            "E_GITHUB_KEY", "GitHub App private key is invalid"
        ) from exc
...
class GitHubAppPublisher:
    ...
    def __init__(
        ...
        *,
        ...
        if app_id <= 0 or "PRIVATE KEY" not in private_key_pem:
            raise PRAssuranceError(
                "E_GITHUB_CONFIG", "positive app id and PEM private key are required"
            )
        parts = urlsplit(api_base)
        if parts.scheme != "https" or not parts.netloc or parts.path not in {"", "/"}:
            raise PRAssuranceError(
                "E_GITHUB_CONFIG", "GitHub API base must be an HTTPS origin"
            )
        self.app_id = app_id
    ...
    def publish(
        self, installation_id: int, repository: str, request: dict[str, Any]
    ) -> int:
        ...
        if installation_id <= 0 or not REPOSITORY_RE.fullmatch(repository):
            raise PRAssuranceError(
                "E_GITHUB_TARGET", "GitHub installation and repository are invalid"
            )
        installation_token = self._installation_token(installation_id)
        payload = {
            key: value
            for key, value in request.items()
            if key not in {"schema", "marker"}
        }
    ...
    def _installation_token(self, installation_id: int) -> str:
        ...
        if response.status_code != 201:
            raise PRAssuranceError(
                "E_GITHUB_TOKEN", "GitHub installation token exchange failed"
            )
        body = response.json()
        ...
        if not isinstance(installation_token, str) or not installation_token:
            raise PRAssuranceError(
                "E_GITHUB_TOKEN", "GitHub token response omitted the credential"
            )
    ...
    def _check_id(published: Any) -> int:
        if published.status_code != 201:
            raise PRAssuranceError(
                "E_GITHUB_PUBLISH", "GitHub Check publication failed"
            )
        ...
        if not isinstance(check_id, int) or check_id <= 0:
            raise PRAssuranceError(
                "E_GITHUB_RESPONSE", "GitHub Check response omitted a positive id"
            )
        return check_id
