...
def _capsule_core(value: object) -> dict[str, Any]:
    ...
    row = _exact_keys(
        value, base_keys | (v2_keys if is_v2 else set()), "capsule proposal"
    )
    supersedes = row["supersedes"]
    if supersedes is not None:
        supersedes = _identifier(supersedes, "supersedes")
    paths = (
        [_relative(item, "scope_paths") for item in row["scope_paths"]]
        if isinstance(row["scope_paths"], list)
        else []
    )
    ...
    if is_v2:
        ...
        if category not in _CATEGORIES:
            raise JudgmentError(
                "JUDGMENT_CAPSULE_INVALID",
                "category is not a declared Capsule category",
            )
        kinds = [
        ...
        core.update(
            {
                ...
                "incident_refs": _strings(
                    row["incident_refs"], "incident_refs", minimum=0
                ),
            }
        )
    return core
...
def _change_profile(
    ...
    try:
        ...
        value = _exact_keys(
            row, {"schema", "changed", "profile_sha256"}, "change profile"
        )
        ...
        if (
            not isinstance(value["changed"], list)
            or not 1 <= len(value["changed"]) <= MAX_ITEMS
        ):
            raise JudgmentError(
                "JUDGMENT_CHANGE_PROFILE_INVALID",
...
def _proof_receipt(root: Path, supplied: Path) -> dict[str, Any]:
    ...
    if (
        ...
        or len(value["evidence"]) > MAX_ITEMS
    ):
        raise JudgmentError(
            "JUDGMENT_PROOF_RECEIPT_INVALID",
    ...
    *,
    ...
    for capsule in store["capsules"]:
        ...
        paths = [
            ...
            if any(_path_matches(scope, item) for scope in capsule_core["scope_paths"])
        ]
        if not paths:
            continue
        ...
        for obligation in capsule_core["proof_obligations"]:
            ...
            if receipt:
                ...
                missing.append(
                    {
                        "capsule_id": capsule_core["id"],
                        "obligation_id": obligation["id"],
                        "owner": capsule_core["owner"],
                    }
                )
        ...
        matching.append(
            {
                "id": capsule_core["id"],
                "title": capsule_core["title"],
                "owner": capsule_core["owner"],
                "matched_paths": paths,
                "review_by": capsule_core["review_by"],
    ...
    if profile["state"] == "valid":
        for entry in profile["entries"]:
            relevant = [
                capsule
                for capsule in matching
                if entry["path"] in capsule["matched_paths"]
            ]
            for kind in entry["change_kinds"]:
                capsule_ids = sorted(
                    capsule["id"]
                    for capsule in relevant
                    if kind in capsule["change_kinds"]
                )
                if capsule_ids:
                    known_kinds.append(
                        {
    ...
    has_novel_architecture_boundary = any(
        item["kind"] == "architecture-boundary" for item in novel_kinds
    )
    ...
    drift = [
        {"capsule_id": item["id"], "state": item["drift_state"]} for item in matching
    ]
    ...
    core = {
        ...
        "profile": profile,
        "novelty": {
            "known_change_kinds": known_kinds,
            "novel_change_kinds": novel_kinds,
            "unclassified_changed_paths": unclassified,
        },
        "drift": drift,
