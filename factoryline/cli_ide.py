...
def run(a) -> int:
    ...
    if a.cmd == "repair":
        from .repair_sandbox import RepairSandboxError, create_repair_scope, inspect_repair_candidate, write_repair_candidate_artifacts, write_repair_scope_artifacts
    ...
    from .jetbrains_handshake import JetBrainsHandshakeError, build_agent_proof_mission, evaluate_jetbrains_handshake, jetbrains_handshake_projection, write_jetbrains_handshake
    ...
    try:
        if a.jetbrains_cmd == "mission":
            ...
            payload = evaluate_jetbrains_handshake(root, Path(a.scope), a.changed, Path(analysis_path), Path(a.e2e_receipt) if a.e2e_receipt else None, analysis_provider=analysis_provider, max_new_errors=a.max_new_errors, max_new_warnings=a.max_new_warnings)
