...
MAX_BYTES = 1_048_576
BEATS = frozenset(
    {"mission", "tension", "guidance", "agency", "transformation", "celebration"}
)
CLAIM_KINDS = frozenset({"experience", "feature", "measured"})
HIGH_RISK_CLAIM = re.compile(
    r"\b(?:guarantee(?:d)?|apple\s+approved|never\s+reject(?:ed)?|save\s+(?:\d+|many)\s+(?:hours?|days?)|\d+\s*%)\b",
    re.IGNORECASE,
)
...
def _text(value: object, field: str, limit: int) -> str:
    ...
    if not result or len(result) > limit:
        raise RevenueForgeError(
            "APPFORGE_STOREFRONT_STORY_INVALID",
...
def _timestamp(value: object) -> str:
    ...
    try:
        ...
        raise RevenueForgeError(
            "APPFORGE_STOREFRONT_STORY_INVALID", "review.confirmed_at must be RFC3339"
        ) from exc
    return result
...
def _local(root: Path, path: Path, exists: bool = True) -> Path:
    ...
    try:
        ...
        raise RevenueForgeError(
            "APPFORGE_STOREFRONT_STORY_PATH_REJECTED",
            "paths must remain inside the workspace",
        ) from exc
    if exists and not target.is_file():
        raise RevenueForgeError(
            "APPFORGE_STOREFRONT_STORY_INPUT_UNAVAILABLE",
...
def _read(root: Path, path: Path, schema: str) -> tuple[dict[str, Any], Path]:
    ...
    if source.stat().st_size > MAX_BYTES:
        raise RevenueForgeError(
            "APPFORGE_STOREFRONT_STORY_INPUT_TOO_LARGE", "input exceeds 1 MiB"
        )
    try:
        value = json.loads(source.read_text(encoding="utf-8-sig"))
    except (OSError, UnicodeDecodeError, json.JSONDecodeError) as exc:
        raise RevenueForgeError(
            "APPFORGE_STOREFRONT_STORY_INPUT_INVALID", "input must be valid JSON"
        ) from exc
    if not isinstance(value, dict) or value.get("schema") != schema:
        raise RevenueForgeError(
            "APPFORGE_STOREFRONT_STORY_SCHEMA_REJECTED", f"expected {schema}"
        )
    return value, source
...
def _captures(media: dict[str, Any]) -> dict[tuple[str, str], str]:
    ...
    if not isinstance(sets, dict):
        raise RevenueForgeError(
            "APPFORGE_STOREFRONT_STORY_MEDIA_INVALID",
            "store-media receipt has no media sets",
        )
    ...
    for set_id, values in sets.items():
        ...
        if not isinstance(captures, list):
            raise RevenueForgeError(
                "APPFORGE_STOREFRONT_STORY_MEDIA_INVALID",
                "store-media capture list is invalid",
            )
        for capture in captures:
            if not isinstance(capture, dict):
                raise RevenueForgeError(
                    "APPFORGE_STOREFRONT_STORY_MEDIA_INVALID",
                    "store-media capture is invalid",
                )
            key = (str(set_id), _text(capture.get("id"), "media.capture.id", 80))
            if key in found:
                raise RevenueForgeError(
                    "APPFORGE_STOREFRONT_STORY_MEDIA_INVALID",
                    "store-media capture id is not unique",
                )
...
def _contract(
    ...
    if (
        ...
        or value.get("store_media_receipt_sha256") != media_sha
    ):
        raise RevenueForgeError(
            "APPFORGE_STOREFRONT_STORY_CONTRACT_INVALID",
            "contract must bind the exact candidate and store-media receipt",
        )
    scenes = value.get("scenes")
    if not isinstance(scenes, list) or not scenes or len(scenes) > 20:
        raise RevenueForgeError(
            "APPFORGE_STOREFRONT_STORY_CONTRACT_INVALID",
    ...
    if seen != set(captures):
        raise RevenueForgeError(
            "APPFORGE_STOREFRONT_STORY_COVERAGE_MISSING",
            "every ready store-media capture must have one truthful story scene",
        )
    return normalized
...
def _evidence(
    ...
    if (
        ...
        raise RevenueForgeError(
            "APPFORGE_STOREFRONT_STORY_EVIDENCE_INVALID",
            "evidence must bind the exact candidate and story contract",
        )
    review = value.get("review")
    if (
        not isinstance(review, dict)
        or set(review)
        != {"reviewed_by", "confirmed_at", "storyboard_truth", "claims_checked"}
        or review.get("storyboard_truth") is not True
        or review.get("claims_checked") is not True
    ):
        raise RevenueForgeError(
...
def verify_storefront_story(
    ...
    if (
        not _sealed(media, MEDIA_RECEIPT_SCHEMA, "APPFORGE_STORE_MEDIA_READY")
        or media.get("candidate") != candidate
    ):
        raise RevenueForgeError(
            "APPFORGE_STOREFRONT_STORY_MEDIA_INVALID",
            "store-media receipt must be hash-valid, ready, and bound to the exact candidate",
        )
    captures = _captures(media)
    contract_raw, contract_source = _read(workspace, contract_path, CONTRACT_SCHEMA)
    scenes = _contract(
        workspace, contract_raw, candidate, media["receipt_sha256"], captures
    )
    evidence, evidence_source = _read(workspace, evidence_path, EVIDENCE_SCHEMA)
    review = _evidence(
        workspace, evidence, candidate, _file_sha(contract_source), scenes
    )
    findings: list[dict[str, str]] = []
    for scene in scenes:
        copy = f"{scene['headline']} {scene['supporting_copy']}"
        if HIGH_RISK_CLAIM.search(copy) and scene["claim_kind"] != "measured":
            findings.append(
                {
                    "code": "APPFORGE_STOREFRONT_STORY_UNSOURCED_CLAIM",
                    "detail": f"{scene['capture_id']} contains a high-risk factual claim without measured provenance",
                }
            )
    result: dict[str, Any] = {
        ...
        if not findings
        else "APPFORGE_STOREFRONT_STORY_BLOCKED",
        "ok": not findings,
        "action_summary": "Bind each ready App Store capture to one distinct user journey, concise story beat, and locally reviewed claim posture; do not generate imagery, add frames, upload media, contact Apple, or claim marketing or App Review approval.",
        "candidate": candidate,
        "store_media_receipt_sha256": media["receipt_sha256"],
        "store_media_path_sha256": _file_sha(media_source),
        ...
        "authority": {
            **AUTHORITY,
            ...
            "apple_asset_download": False,
            "app_store_connect_write": False,
            "app_review_submit": False,
            "apple_approval_claim": False,
        },
        "claim_boundary": "Local screenshot-story and file-reference evidence only. It does not verify what an image depicts, prove an external evidence file supports the claim text, generate frames, upload Store media, or establish App Review approval.",
    }
...
def storefront_story_projection(root: Path) -> dict[str, Any]:
    ...
    invalid: list[str] = []
    for path in sorted(
        (workspace / ".factory" / "appforge").rglob("*storefront-story*.json")
    )[:100]:
        try:
        ...
        if (
            ...
            and value.get("schema") == RECEIPT_SCHEMA
            and isinstance(supplied, str)
            and _sha(
            ...
            invalid.append(path.relative_to(workspace).as_posix())
    return {
        "schema": "factory.appforge.storefront-story-projection.v1",
        "marker": "APPFORGE_STOREFRONT_STORY_READ_ONLY",
        ...
        "claim_boundary": "Read-only local screenshot-story status; not image semantic verification, claim substantiation, App Store Connect state, App Review, or Apple approval.",
    }
