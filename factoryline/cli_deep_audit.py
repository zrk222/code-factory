...
def add_parser(sub: Any) -> None:
    ...
    runtime = sub.add_parser("runtime-audit", help="run or inspect six signed senior-engineering audit lanes")
    runtime_sub = runtime.add_subparsers(required=True, dest="runtime_audit_cmd")
    for action in ("inspect", "run"):
        command = runtime_sub.add_parser(action, help=f"{action} one signed runtime audit plan")
        ...
        command.add_argument("--json", action="store_true")
    status = runtime_sub.add_parser("status", help="read the latest self-hash-verified local runtime audit receipt")
    ...
    attest = senior_sub.add_parser("attest", help="verify one DSSE-signed independent execution attestation")
    ...
    boundary = senior_sub.add_parser("boundary", help="verify one DSSE-signed runtime-boundary attestation")
    ...
    benchmark = senior_sub.add_parser("benchmark", help="evaluate a manifest-bound buggy/fixed defect corpus")
    benchmark.add_argument("manifest")
    benchmark.add_argument("--observations", required=True)
    benchmark.add_argument("--out")
    benchmark.add_argument("--json", action="store_true")
    schedule = senior_sub.add_parser("schedule", help="plan dependency-aware proof routing without executing gates")
    ...
    shadow = senior_sub.add_parser("shadow", help="compare incremental and full plan obligations/findings")
    shadow.add_argument("incremental")
    shadow.add_argument("full")
    shadow.add_argument("--out")
    shadow.add_argument("--json", action="store_true")
    replay = senior_sub.add_parser("replay", help="reproduce one contract-bound candidate in a fresh process workspace")
    replay.add_argument("manifest")
    replay.add_argument("--root", default=".")
    replay.add_argument("--execute", action="store_true", help="run the declared argv; without this flag only validate the replay plan")
    replay.add_argument("--out")
    replay.add_argument("--json", action="store_true")
    repair = senior_sub.add_parser("repair", help="compare the original failure, repair, and negative controls")
    repair.add_argument("manifest")
    repair.add_argument("--root", default=".")
    repair.add_argument("--execute", action="store_true", help="run all declared replay legs; without this flag only validate the comparison")
    repair.add_argument("--out")
    repair.add_argument("--json", action="store_true")
    reuse = senior_sub.add_parser("reuse", help="explain exact proof reuse or the reason a gate must run")
    ...
    brief = senior_sub.add_parser("brief", help="turn one failed receipt into an evidence-linked failure briefing")
...
def _run_deep(args: Any) -> int:
    ...
    try:
        ...
        if args.deep_cmd == "compare":
            result = compare_deep_audit_repairs(root, args.before, args.after, before_attestation=args.before_attestation, after_attestation=args.after_attestation, trust_root_path=Path(args.trust_root) if args.trust_root else None, require_attestation=args.require_attestation, max_age_seconds=args.max_age_seconds)
            result["action_summary"] = "Compared signed deep-audit observations; no analyzer or repair ran."
            return _emit(result, 0 if result["state"] == "approval_required" else 1)
        if args.deep_cmd == "status":
            ...
            result = verify_deep_audit_attestation(root, Path(args.attestation), Path(args.trust_root), Path(args.receipt), max_age_seconds=args.max_age_seconds)
            code = 0 if result["state"] == "VERIFIED" else 1
            result["action_summary"] = "Verified one offline DSSE deep-audit attestation; no analyzer or repair ran."
        else:
            result = execute_deep_audit(Path(args.plan), Path(args.trust_root), args.trust_root_sha256, root)
            code = 0 if result["receipt"]["decision"] == "READY_FOR_HUMAN_REVIEW" else 1
            result["action_summary"] = "Evaluated signed reports and saved a review receipt; no analyzer or repair ran."
    except (DeepAuditAttestationError, RuntimeAuditError, OSError, ValueError, KeyError, TypeError) as exc:
        return _emit({"code": getattr(exc, "code", "E_DEEP_AUDIT"), "message": str(exc), "authority": "none"}, 2)
...
def _run_runtime(args: Any) -> int:
    ...
    try:
        if args.runtime_audit_cmd == "status":
            ...
            result = verify_runtime_audit_plan(Path(args.plan), Path(args.trust_root), args.trust_root_sha256, root, args.environment_sha256, require_intake=args.require_intake)
            ...
            result = execute_runtime_audit(Path(args.plan), Path(args.trust_root), args.trust_root_sha256, root, args.environment_sha256, Path(args.out), require_intake=args.require_intake)
            code = 0 if result["receipt"]["decision"] == "READY_FOR_HUMAN_REVIEW" else 1
    except (RuntimeAuditError, OSError, ValueError, KeyError, TypeError) as exc:
        return _emit({"schema": "factory.runtime-audit.error.v1", "code": getattr(exc, "code", "E_RUNTIME_AUDIT"), "message": str(exc), "authority": "none"}, 2)
    return _emit(result, code)
...
def _run_senior(args: Any) -> int:
    ...
    try:
        if args.senior_cmd == "attest":
            result = verify_signed_execution_attestation(Path(args.receipt), Path(args.trust_root), candidate_sha256=args.candidate_sha256, plan_sha256=args.plan_sha256)
            ...
            code = 0
        elif args.senior_cmd == "boundary":
            result = verify_signed_runtime_attestation(Path(args.receipt), Path(args.trust_root), candidate_sha256=args.candidate_sha256, plan_sha256=args.plan_sha256, environment_sha256=args.environment_sha256)
            ...
            code = 0
        elif args.senior_cmd == "benchmark":
            result = evaluate_benchmark(load_benchmark_json(Path(args.manifest)), load_benchmark_json(Path(args.observations)), out=Path(args.out) if args.out else None)
        ...
        elif args.senior_cmd == "schedule":
            result = plan_incremental(Path(args.root).resolve(), load_schedule_json(Path(args.manifest)), out=Path(args.out) if args.out else None)
            result["action_summary"] = "Planned dependency-aware proof routing; no gate command ran."
            code = 1 if result["counts"]["BLOCK"] else 0
        elif args.senior_cmd == "shadow":
            result = compare_shadow(load_schedule_json(Path(args.incremental)), load_schedule_json(Path(args.full)), out=Path(args.out) if args.out else None)
        ...
        elif args.senior_cmd == "replay":
            result = run_replay(Path(args.root).resolve(), load_assurance_json(Path(args.manifest)), execute=args.execute, out=Path(args.out) if args.out else None)
            result["action_summary"] = "Replayed the declared candidate in a fresh process workspace; no release authority was granted." if args.execute else "Validated a contract-bound replay plan; no candidate command ran."
            code = 0 if result["state"] in {"PLAN_ONLY", "PASS"} else 1
        elif args.senior_cmd == "repair":
