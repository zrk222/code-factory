...
def _timestamp(value: object, field: str) -> str:
    if not isinstance(value, str):
        _reject(
            "GITHUB_ASSURANCE_INPUT_INVALID", f"{field} must be an RFC3339 timestamp"
        )
    try:
...
def _list_of_names(value: object, field: str) -> list[str]:
    if not isinstance(value, list) or not all(
        isinstance(item, str) and item and len(item) <= 160 for item in value
    ):
        _reject(
            "GITHUB_ASSURANCE_INPUT_INVALID",
            f"{field} must be a list of short non-empty strings",
        )
...
def _validate_policy_snapshot(value: object) -> dict[str, Any]:
    ...
    if not isinstance(value, dict) or set(value) != {
        ...
        _reject(
            "GITHUB_ASSURANCE_INPUT_INVALID",
            "policy snapshot must use the exact v1 shape",
        )
    ...
    if not isinstance(value["scope"], dict) or set(value["scope"]) != {
        ...
        _reject(
            "GITHUB_ASSURANCE_INPUT_INVALID",
            "snapshot scope must contain owner and repository",
        )
    if not all(
        isinstance(value["scope"][key], str)
        and _IDENTIFIER.fullmatch(value["scope"][key])
    ...
    if not isinstance(value["capture"], dict) or set(value["capture"]) != {
        ...
        _reject(
            "GITHUB_ASSURANCE_INPUT_INVALID",
            "snapshot capture must use the exact v1 shape",
        )
    capture = value["capture"]
    if capture["source"] not in {"github_api_export", "manual_export"}:
        _reject(
            "GITHUB_ASSURANCE_INPUT_INVALID",
            "capture source must be github_api_export or manual_export",
        )
    if not isinstance(capture["captured_by"], str) or not _IDENTIFIER.fullmatch(
        capture["captured_by"]
    ):
        _reject("GITHUB_ASSURANCE_INPUT_INVALID", "capture actor is invalid")
    if (
        not isinstance(capture["source_reference"], str)
        or not capture["source_reference"]
    ...
    rulesets = (
        ...
        else _reject("GITHUB_ASSURANCE_INPUT_INVALID", "rulesets must be a list")
    )
    if len(rulesets) > 100 or [item["id"] for item in rulesets] != sorted(
        item["id"] for item in rulesets
    ):
        _reject(
            "GITHUB_ASSURANCE_INPUT_INVALID",
            "rulesets must be sorted by unique id and bounded",
        )
...
def _finding(
    identifier: str, severity: str, message: str, ruleset: str
) -> dict[str, str]:
    return {
        "id": identifier,
        "severity": severity,
...
def _detect_policy_drift(baseline: object, current: object) -> list[dict[str, str]]:
    ...
    before, after = (
        _validate_policy_snapshot(baseline),
        _validate_policy_snapshot(current),
    )
...
def _validate_assurance_exception(
    ...
    if (
        ...
        or value["approval"]["state"] != "approved"
        or not isinstance(value["approval"]["approved_by"], str)
        or not _IDENTIFIER.fullmatch(value["approval"]["approved_by"])
    ):
        _reject(
            "GITHUB_ASSURANCE_EXCEPTION_INVALID",
    ...
    if parsed <= moment:
        _reject(
            "GITHUB_ASSURANCE_EXCEPTION_EXPIRED",
...
def _load_json(path: Path) -> dict[str, Any]:
    try:
        ...
        raise GitHubAssuranceDossierError(
            "GITHUB_ASSURANCE_INPUT_INVALID", f"cannot read JSON input: {path}"
        ) from exc
...
def _review(value: object) -> dict[str, Any]:
    if not isinstance(value, dict) or value.get("schema") != GITHUB_PROOF_REVIEW_SCHEMA:
        _reject(
            "GITHUB_ASSURANCE_INPUT_INVALID",
            "the first dossier version requires factory.github_proof_review.v1 evidence",
        )
...
def _render_markdown(core: dict[str, Any]) -> str:
    findings = (
        "\n".join(
            f"- **{item['severity']}** `{item['id']}` - {item['message']}"
            for item in core["drift"]["findings"]
        )
        or "- None."
    )
    exceptions = (
        "\n".join(
            f"- `{item['id']}` approved by `{item['approved_by']}`, expires `{item['expires_at']}`"
            for item in core["exceptions"]
        )
        or "- None."
    )
    disabled = ", ".join(
        key.replace("_", " ")
        for key, enabled in sorted(core["authority"].items())
        if not enabled
    )
    return "\n".join(
        [
            "# FactoryLine Merge Evidence Dossier",
            "",
            f"Commit: `{core['head_sha']}`",
            f"Dossier SHA-256: `{core['dossier_sha256']}`",
            "",
            "## Policy comparison",
            "",
            f"Current snapshot: `{core['policy']['current_sha256']}`",
            f"Baseline snapshot: `{core['policy'].get('baseline_sha256', 'not supplied')}`",
            "",
            "## Policy drift",
            "",
            findings,
            ...
            "## Authority boundary",
            "",
            f"Advisory evidence only. This dossier has no {disabled} authority. It does not fetch or prove the live GitHub configuration, change a rule, approve, merge, sign, or modify source.",
            "",
            "```mermaid",
            core["mermaid"].rstrip(),
            "```",
...
def _policy_context(
    current_policy: object, baseline_policy: object | None
) -> tuple[dict[str, Any], str, list[dict[str, str]], str | None]:
...
def _bound_exceptions(
    ...
    accepted = [
        _validate_assurance_exception(
            item, policy_sha256=current_sha, head_sha=head_sha, now=now
        )
        for item in items
    ]
    if any(not set(item["finding_ids"]).issubset(finding_ids) for item in accepted):
        _reject(
            "GITHUB_ASSURANCE_EXCEPTION_INVALID",
            "exception names a finding not present in this dossier",
        )
    return accepted


def _dossier_decision(
    has_baseline: bool, findings: list[dict[str, str]], exceptions: list[dict[str, Any]]
) -> tuple[str, tuple[str, str], list[dict[str, str]]]:
    covered = {finding_id for item in exceptions for finding_id in item["finding_ids"]}
    unresolved = [
        item
        for item in findings
        if item["severity"] == "high" and item["id"] not in covered
    ]
    if not has_baseline:
        return (
            "review_required",
            (
                ...
                "No baseline snapshot was supplied; a human must record a comparable policy export.",
            ),
            unresolved,
        )
    if unresolved:
        return (
            ...
            (
                ...
                "High-severity policy drift remains unexceptioned.",
            ),
            unresolved,
        )
    if exceptions:
        return (
            "exception_accepted",
            (
                "retain_dossier_for_human_merge_decision",
                "Evidence carries a named, expiring exception; a human still decides whether to merge.",
            ),
            unresolved,
        )
    return (
        ...
        (
            "retain_dossier_for_human_merge_decision",
            "Evidence is aligned; a human still decides whether to merge.",
        ),
        unresolved,
    )
...
def _dossier_core(
    ...
    return {
        ...
        "markers": [
            "GITHUB_ASSURANCE_DOSSIER_V1",
            "GITHUB_POLICY_DRIFT_DETERMINISTIC",
            "GITHUB_ASSURANCE_EXCEPTIONS_NAMED_EXPIRING",
            "GITHUB_ASSURANCE_LOCAL_ONLY",
            "GITHUB_ASSURANCE_HUMAN_MERGE_REQUIRED",
        ],
        ...
        "policy": {
            ...
            **({"baseline_sha256": baseline_sha} if baseline_sha else {}),
        },
        "drift": {
            "baseline_supplied": has_baseline,
            "findings": findings,
            "unresolved_high_count": len(unresolved),
        },
        "exceptions": [
            {
                "id": item["id"],
        ...
        "status": status,
        "next_action": {"action": action[0], "reason": action[1]},
        "authority": AUTHORITY,
        "scope_limits": [
            "Snapshots are supplied local exports, not a live GitHub policy read.",
            "Only deterministic rule deltas represented by the v1 snapshot shape are evaluated.",
            "A dossier never authorizes a merge, policy change, source write, test execution, signing, or deployment.",
        ],
...
def build_assurance_dossier(
    proof_review: object,
    current_policy: object,
    baseline_policy: object | None = None,
    exceptions: list[object] | None = None,
    now: datetime | None = None,
) -> dict[str, Any]:
    """Join local proof review and policy snapshots into a non-authoritative dossier."""
    review = _review(proof_review)
    current, current_sha, findings, baseline_sha = _policy_context(
        current_policy, baseline_policy
    )
    valid_exceptions = _bound_exceptions(
        exceptions or [],
        current_sha,
        review["head_sha"],
    ...
    status, action, unresolved = _dossier_decision(
        baseline_policy is not None, findings, valid_exceptions
    )
    core = _dossier_core(
        review,
        current,
...
def _validate_assurance_dossier(value: object) -> dict[str, Any]:
    if (
        ...
        or value.get("schema") != GITHUB_ASSURANCE_DOSSIER_SCHEMA
    ):
        _reject(
            "GITHUB_ASSURANCE_INPUT_INVALID",
            "a factory.github_assurance_dossier.v1 payload is required",
        )
    core = {
        key: item
        for key, item in value.items()
        if key not in {"dossier_sha256", "dossier_markdown", "artifacts"}
    }
    if value.get("dossier_sha256") != _sha(core):
        _reject(
            "GITHUB_ASSURANCE_INPUT_INVALID",
            "dossier SHA-256 does not match canonical facts",
        )
    expected = _render_markdown({**core, "dossier_sha256": value["dossier_sha256"]})
    if value.get("dossier_markdown") != expected:
        _reject(
            "GITHUB_ASSURANCE_INPUT_INVALID",
            "dossier markdown does not match canonical facts",
        )
    return value
...
def write_assurance_dossier_artifacts(dossier: object, out_dir: Path) -> dict[str, Any]:
    ...
    paths = {
        "json": destination / f"{stem}.dossier.json",
    ...
    contents = {
        "json": json.dumps(serializable, ensure_ascii=False, indent=2, sort_keys=True)
        + "\n",
        "markdown": payload["dossier_markdown"],
    ...
    return {
        "marker": "GITHUB_ASSURANCE_DOSSIER_ARTIFACTS_WRITTEN",
        "paths": {name: str(path) for name, path in paths.items()},
        "sha256": digests,
    }


def _build_assurance_dossier_from_paths(
    proof_review_path: Path,
    current_policy_path: Path,
    baseline_policy_path: Path | None = None,
    exception_paths: list[Path] | None = None,
    now: datetime | None = None,
) -> dict[str, Any]:
    return build_assurance_dossier(
        _load_json(proof_review_path),
        _load_json(current_policy_path),
        _load_json(baseline_policy_path) if baseline_policy_path else None,
        [_load_json(path) for path in (exception_paths or [])],
        now=now,
    )
