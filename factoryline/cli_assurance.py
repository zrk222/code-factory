...
def add_parser(sub: Any) -> None:
    ...
    parser = sub.add_parser("assurance", help="produce deterministic assurance artifacts")
    ...
    mutation = commands.add_parser("policy-mutate", help="emit explicit policy mutations for a challenge run")
    mutation.add_argument("policy")
    mutation.add_argument("--out", required=True)
    supply_chain = commands.add_parser("supply-chain", help="verify a local source, dependency, vulnerability and reproducible-build manifest")
    ...
    verify = commands.add_parser("supply-chain-verify", help="verify an independently collected signed supply-chain attestation")
...
def run(args: Any) -> int:
    ...
    from .supply_chain import verify_signed_supply_chain_attestation, write_supply_chain_receipt
    ...
    try:
        if args.assurance_cmd == "supply-chain":
            ...
            result = write_supply_chain_receipt(root, manifest, Path(args.out), candidate_sha256=args.candidate_sha256)
            print(json.dumps(result, indent=2, sort_keys=True) if args.json else f"supply-chain: {result.get('decision')} ({result.get('path')})")
        ...
        if args.assurance_cmd == "supply-chain-verify":
            ...
            result = verify_signed_supply_chain_attestation(attestation, trust_root, root, candidate_sha256=args.candidate_sha256)
            print(json.dumps(result, indent=2, sort_keys=True) if args.json else f"supply-chain signature: {result.get('state')} ({result.get('attestation_id')})")
            return 0
        if args.assurance_cmd == "graph":
            result = build_evidence_graph(json.loads(Path(args.records).read_text(encoding="utf-8")), tenant_id=args.tenant)
        elif args.assurance_cmd == "sbom":
            result = build_cyclonedx_sbom(json.loads(Path(args.components).read_text(encoding="utf-8")))
        elif args.assurance_cmd == "vex":
            result = build_vex(json.loads(Path(args.entries).read_text(encoding="utf-8")))
        else:
            result = {"schema": "factory.assurance.policy-mutations.v1", "mutations": policy_mutations(json.loads(Path(args.policy).read_text(encoding="utf-8")))}
        ...
        print(json.dumps({"schema": "factory.assurance.result.v1", "verdict": "ERROR", "error": {"code": getattr(exc, "code", "E_ASSURANCE"), "message": getattr(exc, "message", str(exc))}}, indent=2))
        return 1
    print(json.dumps({"schema": "factory.assurance.result.v1", "verdict": "WRITTEN", "path": str(Path(args.out).resolve())}, indent=2))
    return 0
