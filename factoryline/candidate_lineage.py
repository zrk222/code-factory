...
def _path_from_path_value(root: Path, value: Path, field: str) -> Path:
...
def _path_from_text_value(root: Path, value: object, field: str) -> Path:
...
def _validate_resolved_path(root: Path, resolved: Path, field: str) -> tuple[Path, str]:
    try:
...
def _path(root: Path, value: object, field: str) -> tuple[Path, str]:
    resolved = (
        _path_from_path_value(root, value, field)
        if isinstance(value, Path)
        else _path_from_text_value(root, value, field)
    )
    return _validate_resolved_path(root, resolved, field)
...
def _evidence_identity(raw: object, index: int) -> tuple[str, str, dict[str, Any]]:
    ...
    return identifier, kind, raw


def _verified_evidence_file(
    root: Path, raw: dict[str, Any], index: int
    ...
    value = _json(path)
    return path, relative, expected_file_sha, value


def _verify_evidence_kind(
    root: Path,
    path: Path,
    kind: str,
    candidate: str,
...
def _candidate_binding(value: dict[str, Any], candidate: str, index: int) -> str:
...
def _evidence_row(
    root: Path, raw: object, candidate: str, index: int
) -> dict[str, str]:
    identifier, kind, evidence = _evidence_identity(raw, index)
    path, relative, expected_file_sha, value = _verified_evidence_file(
        root, evidence, index
    )
    value = _verify_evidence_kind(root, path, kind, candidate, value, index)
    declared = _candidate_binding(value, candidate, index)
    return {
        "id": identifier,
        "kind": kind,
        "path": relative,
...
def _evidence(root: Path, rows: object, candidate: str) -> list[dict[str, str]]:
    ...
    for index, raw in enumerate(rows):
        item = _evidence_row(root, raw, candidate, index)
