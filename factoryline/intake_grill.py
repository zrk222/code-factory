...
_SECRET = re.compile(
    r"(?:\b(?:sk|pk|ghp|github_pat|xox[baprs]|AIza)[_-][A-Za-z0-9_-]{12,}|-----BEGIN(?: [A-Z]+)? PRIVATE KEY-----)",
    re.I,
)
_AUTHORITY = {
...
def _project_id(value: str) -> str:
    ...
    if not _PROJECT.fullmatch(lowered):
        raise ProductMissionError(
            "INTAKE_PROJECT_INVALID",
            "project must be 1-80 lowercase letters, digits, or hyphens",
        )
    return lowered
...
def _relative(
    ...
    try:
        ...
        raise ProductMissionError(
            "INTAKE_PATH_OUTSIDE_ROOT", f"{field} must stay inside the workspace"
        ) from exc
    ...
    if exists and not path.is_file():
        raise ProductMissionError(
            "INTAKE_SOURCE_NOT_FOUND", f"{field} must name a readable file"
        )
...
def _read_prd(root: Path, value: Path | str) -> tuple[Path, str, bytes]:
    ...
    try:
        ...
        raise ProductMissionError(
            "INTAKE_PRD_ENCODING_INVALID", "PRD must be valid UTF-8"
        ) from exc
    except OSError as exc:
        raise ProductMissionError(
            "INTAKE_SOURCE_NOT_FOUND", "PRD cannot be read"
        ) from exc
    if not source or len(source) > MAX_SOURCE_BYTES:
        raise ProductMissionError(
            "INTAKE_PRD_SIZE_INVALID", f"PRD must be 1-{MAX_SOURCE_BYTES} UTF-8 bytes"
        )
...
def _atomic_write(path: Path, data: bytes, *, force: bool) -> Path:
    ...
    if path.exists():
        ...
        if not force:
            raise ProductMissionError(
                "INTAKE_ARTIFACT_EXISTS", f"refusing to replace {path}; use --force"
            )
...
def _text(value: str, field: str, *, minimum: int = 8, maximum: int = MAX_TEXT) -> str:
    ...
    if (
        ...
        raise ProductMissionError(
            ...
            f"{field} must contain {minimum}-{maximum} non-secret characters",
        )
    return cleaned
...
def _clear_decision(value: str, field: str, *, observable: bool = False) -> str:
    ...
    try:
        return require_clear(
            value, field=field, require_action=True, require_observable=observable
        )
    except IntentQualityError as exc:
        raise ProductMissionError(
            "INTAKE_INTENT_UNCLEAR", f"{field}: {exc.message}"
        ) from exc
    ...
    return [
        {
            ...
            "choices": [item["id"] for item in shortlist],
            "depends_on": [],
        ...
        },
        {
            ...
            "choices": sorted(_EXTERNAL_EFFECTS),
            "depends_on": ["Q-INTENT"],
        },
        {
            "id": "Q-PR-REVIEW",
...
def _core(receipt: dict[str, Any]) -> dict[str, Any]:
    return {
        ...
        if key not in {"intake_sha256", "generated_at"}
    }


def _load_receipt(
    root: Path, value: Path | str, field: str, schema: str
    ...
    try:
        ...
        raise ProductMissionError(
            "INTAKE_RECEIPT_INVALID", f"{field} must be readable JSON"
        ) from exc
    if not isinstance(receipt, dict) or receipt.get("schema") != schema:
        raise ProductMissionError(
            "INTAKE_RECEIPT_INVALID", f"{field} must use {schema}"
        )
    ...
    *,
    ...
    if path.is_file() and not force:
        try:
            ...
            raise ProductMissionError(
                "INTAKE_ARTIFACT_INVALID", f"cannot read existing receipt: {path}"
            ) from exc
        if (
            isinstance(existing, dict)
            and existing.get("schema") == schema
            and existing.get(hash_field) == digest
        ):
...
def grill_intake(
    ...
    analysis = analyze_product_text(
        source.decode("utf-8"), source_name=path.name, project=project
    )
    ...
    core = {
        ...
        "markers": [
            "INTAKE_SOURCE_BOUND",
            "INTAKE_DECISION_TREE",
            "INTAKE_HUMAN_CONFIRMATION_REQUIRED",
            "INTAKE_ZERO_EXECUTION_AUTHORITY",
        ],
    ...
    markdown_path = (
        raw_out.resolve()
        if raw_out and raw_out.is_absolute()
        else (workspace / raw_out if raw_out else directory / f"{source_sha}.md")
    )
    if out_path:
        _relative(workspace, markdown_path, "out", exists=False)
    result, reused = _sealed_receipt(
        receipt_path,
        INTAKE_GRILL_SCHEMA,
        core,
        "intake_sha256",
    ...
    _atomic_write(
        receipt_path,
        json.dumps(result, ensure_ascii=False, sort_keys=True).encode("utf-8") + b"\n",
        force=force,
    )
...
def verify_intake_grill(root: Path, receipt_path: Path) -> dict[str, Any]:
    ...
    workspace = Path(root).resolve()
    path, relative, receipt = _load_receipt(
        workspace, receipt_path, "intake", INTAKE_GRILL_SCHEMA
    )
    errors: list[str] = []
    ...
    return {
        "schema": "factory.intake-grill.verification.v1",
        "valid": not errors,
        "marker": "INTAKE_VERIFIED" if not errors else "INTAKE_DRIFT",
        "path": relative,
        "errors": errors,
    }
...
def _confirmation_core(receipt: dict[str, Any]) -> dict[str, Any]:
    return {
        key: value
        for key, value in receipt.items()
        if key not in {"confirmation_sha256", "confirmed_at"}
    }
...
def confirm_intake(
    ...
    approved_by: str,
    rationale: str,
    re_evaluate_when: str | None = None,
    out_path: Path | None = None,
    force: bool = False,
) -> dict[str, Any]:
    ...
    if not intake_verification["valid"]:
        raise ProductMissionError(
            "INTAKE_SOURCE_DRIFT", "; ".join(intake_verification["errors"])
        )
    intake_file, intake_relative, intake = _load_receipt(
        workspace, intake_path, "intake", INTAKE_GRILL_SCHEMA
    )
    allowed = {
        item["id"]
        for item in intake.get("framework_shortlist", [])
        if isinstance(item, dict) and isinstance(item.get("id"), str)
    }
    if framework not in allowed:
        raise ProductMissionError(
            "INTAKE_FRAMEWORK_NOT_SHORTLISTED",
            "framework must be one of the source-bound shortlist choices",
        )
    if external_effects not in _EXTERNAL_EFFECTS:
        raise ProductMissionError(
            "INTAKE_EXTERNAL_EFFECTS_INVALID",
    ...
    decision = {
        ...
        "re_evaluate_when": _text(re_evaluate_when, "re_evaluate_when")
        if re_evaluate_when
        else None,
    }
    ...
    core = {
        ...
        "decision": decision,
        "markers": [
            "INTAKE_CONFIRMATION_SOURCE_BOUND",
            "INTAKE_NAMED_HUMAN_BOUND",
            "INTAKE_INTENT_CLARITY_CHECKED",
            "INTAKE_ACCEPTANCE_OBSERVABLE_CHECKED",
            "INTAKE_REEVALUATION_EXPLICIT",
            "INTAKE_ZERO_EXECUTION_AUTHORITY",
        ],
        "authority": _AUTHORITY,
    }
    ...
    path = (
        raw_out.resolve()
        if raw_out and raw_out.is_absolute()
        else (workspace / raw_out if raw_out else default)
    )
    _relative(workspace, path, "out", exists=False)
    confirmation, reused = _sealed_receipt(
        path,
        INTAKE_CONFIRMATION_SCHEMA,
        core,
        "confirmation_sha256",
        "confirmed_at",
        force=force,
    ...
    _atomic_write(
        path,
        json.dumps(confirmation, ensure_ascii=False, sort_keys=True).encode("utf-8")
        + b"\n",
        force=force,
    )
...
def _confirmation_base_errors(
    confirmation: dict[str, Any],
) -> tuple[list[str], dict[str, Any] | None]:
    errors: list[str] = []
    if confirmation.get("confirmation_sha256") != _sha(
        _confirmation_core(confirmation)
    ):
        errors.append("confirmation hash mismatch")
    if (
        confirmation.get("authority") != _AUTHORITY
...
def _confirmation_binding_errors(
    workspace: Path, confirmation: dict[str, Any], decision: dict[str, Any] | None
) -> list[str]:
    errors: list[str] = []
    ...
    if not isinstance(intake, dict) or set(intake) != {"path", "intake_sha256"}:
        ...
        return errors
    try:
        intake_path, _, intake_receipt = _load_receipt(
            workspace, intake["path"], "intake", INTAKE_GRILL_SCHEMA
        )
    ...
    choices = {
        item.get("id")
        for item in intake_receipt.get("framework_shortlist", [])
    ...
    if not isinstance(source, dict) or source.get("sha256") != intake_receipt.get(
        "source", {}
    ).get("sha256"):
...
def verify_intake_confirmation(root: Path, receipt_path: Path) -> dict[str, Any]:
    ...
    workspace = Path(root).resolve()
    _, relative, confirmation = _load_receipt(
        workspace, receipt_path, "confirmation", INTAKE_CONFIRMATION_SCHEMA
    )
    ...
    return {
        "schema": "factory.intake-confirmation.verification.v1",
        "valid": not errors,
        "marker": "INTAKE_CONFIRMATION_VERIFIED"
        if not errors
        else "INTAKE_CONFIRMATION_DRIFT",
        "path": relative,
        "errors": errors,
...
def intake_status(root: Path, prd_path: Path | None = None) -> dict[str, Any]:
    ...
    for path in sorted(
        (workspace / ".factory" / "intake-confirmations").glob("*/*.json")
    ):
        check = verify_intake_confirmation(workspace, path)
        confirmation = check.get("confirmation")
        if source_sha is not None and (
            not confirmation
            or confirmation.get("source", {}).get("sha256") != source_sha
        ):
            continue
        rows.append(
            {
                ...
                "valid": check["valid"],
                "marker": check["marker"],
                "project": confirmation.get("project") if confirmation else None,
                "framework": confirmation.get("decision", {}).get("framework")
                if confirmation
                else None,
            }
        )
    ...
    return {
        "schema": "factory.intake.status.v1",
        "found": bool(latest),
