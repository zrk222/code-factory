...
def render(message: str, *, uppercase: bool = False) -> dict:
    normalized = message.strip()
    if not normalized:
        raise ValueError("message is required")
    return {"message": normalized.upper() if uppercase else normalized, "characters": len(normalized)}
...
def main(argv: list[str] | None = None) -> int:
    ...
    print(json.dumps(render(args.message, uppercase=args.uppercase), sort_keys=True))
    return 0
...
def test_cli_result_is_deterministic():
    assert render(" hello ") == {"message": "hello", "characters": 5}
    assert render("hello", uppercase=True)["message"] == "HELLO"
...
modules:
  - name: cli_app
    ...
    functions:
      - name: render
        args: ["message: str", "uppercase: bool"]
        returns: "dict"
        doc: "Normalize one command input deterministically."
dependencies: []
invariants:
  - name: no_dynamic_execution
    forbid_pattern: "\\b(eval|exec)\\s*\\("
...
def test_health_and_echo_contracts():
    assert client.get("/healthz").json()["ok"] is True
    first = client.post("/v1/echo", json={"message": "hello"})
    second = client.post("/v1/echo", json={"message": "hello"})
    assert first.status_code == 200
    assert first.json() == second.json() == {"message": "hello", "characters": 5}
  ...
  - name: api
    ...
    functions:
      - name: healthz
        args: []
        returns: "dict"
        doc: "Return stable API health and identity."
      - name: echo
        args: ["request: EchoRequest"]
        returns: "dict"
        doc: "Return a validated deterministic response."
dependencies: []
invariants:
  - name: no_dynamic_execution
    forbid_pattern: "\\b(eval|exec)\\s*\\("
...
def dispatch(request: dict) -> dict:
    ...
    if method == "initialize":
        result = {"protocolVersion": "2025-03-26", "capabilities": {"tools": {}}, "serverInfo": {"name": "factory-mcp", "version": "0.1.0"}}
    elif method == "tools/list":
        result = {"tools": [{"name": "echo", "description": "Return local text unchanged.", "inputSchema": {"type": "object", "required": ["message"], "properties": {"message": {"type": "string", "minLength": 1}}}}]}
    elif method == "tools/call" and request.get("params", {}).get("name") == "echo":
        message = str(request.get("params", {}).get("arguments", {}).get("message", "")).strip()
        if not message:
            return {"jsonrpc": "2.0", "id": request_id, "error": {"code": -32602, "message": "message is required"}}
...
def main() -> int:
    for line in sys.stdin:
        if line.strip():
            print(json.dumps(dispatch(json.loads(line)), sort_keys=True), flush=True)
    return 0
...
def test_mcp_tool_discovery_and_call_are_deterministic():
    listing = dispatch({"jsonrpc": "2.0", "id": 1, "method": "tools/list"})
    ...
    assert dispatch(request) == dispatch(request)
    assert dispatch(request)["result"]["content"][0]["text"] == "hello"
  ...
  - name: mcp_server
    ...
    functions:
      - name: dispatch
        args: ["request: dict"]
        returns: "dict"
        doc: "Dispatch one local JSON-RPC request deterministically."
dependencies: []
invariants:
  - name: stdio_only
