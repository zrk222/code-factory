...
def add_parser(sub: Any) -> None:
    ...
    oracle_init = oracle_sub.add_parser(
        "init", help="create a full, intentionally incomplete source-bound Oracle Firewall workspace"
    )
    ...
    continuity = sub.add_parser("proof-continuity", help="seal and monitor a senior-engineering source-to-decision audit chain")
    continuity_sub = continuity.add_subparsers(required=True, dest="proof_continuity_cmd")
    continuity_seal = continuity_sub.add_parser("seal", help="seal one source-to-obligation-to-evidence audit receipt without running work")
    ...
    continuity_observe = continuity_sub.add_parser("observe", help="record later local evidence and reopen the audit on contradiction")
    ...
    continuity_status = continuity_sub.add_parser("status", help="read continuity audit and incident state without changing artifacts")
...
def run(args: Any) -> int:
    ...
    if args.cmd == "proof-continuity":
        ...
        from .proof_continuity_ledger import ProofContinuityError, proof_continuity_projection, record_proof_continuity_observation, seal_proof_continuity
        ...
        try:
            if args.proof_continuity_cmd == "seal":
                ...
                result = record_proof_continuity_observation(root, Path(args.contract), Path(args.observation), Path(args.out))
            else:
    ...
    if args.cmd == "oracle":
        ...
        try:
            if args.oracle_cmd == "init":
                ...
                result = initialize_oracle_firewall(root, Path(args.out_dir), Path(args.source), agent, args.id, args.scope, appforge=args.appforge)
            elif args.oracle_cmd == "handoff":
                agent = json.loads((root / args.agent).read_text(encoding="utf-8-sig"))
                result = capture_intent_handoff(root, Path(args.source), agent, args.id, Path(args.out) if args.out else None)
                ...
                result = compare_oracle_contracts(root, Path(args.prior), Path(args.candidate), Path(args.out) if args.out else None)
            elif args.oracle_cmd == "challenge":
                result = compile_oracle_challenge(root, Path(args.contract), Path(args.out) if args.out else None) if args.oracle_challenge_cmd == "compile" else verify_oracle_challenge_result(root, Path(args.plan), Path(args.result))
            elif args.oracle_cmd == "incident":
                agent = json.loads((root / args.agent).read_text(encoding="utf-8-sig"))
                result = record_oracle_incident(root, agent, Path(args.contract), Path(args.drift), Path(args.out) if args.out else None)
            else:
    ...
    try:
        if args.semantic_cmd == "handoff":
            ...
            result = verify_semantic_handoff(root, Path(args.path)) if args.kind == "handoff" else verify_authority_lease(root, Path(args.path))
        elif args.semantic_cmd == "check":
            result = authorize_semantic_action(root, Path(args.lease), json.loads((root / args.request).read_text(encoding="utf-8-sig")))
        elif args.semantic_cmd == "record":
            result = record_semantic_action_decision(root, Path(args.lease), json.loads((root / args.request).read_text(encoding="utf-8-sig")), Path(args.out))
        else:
