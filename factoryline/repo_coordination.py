...
def _path(value: object, label: str) -> str:
    if not isinstance(value, str) or not value.strip() or len(value) > 512:
        raise RepoCoordinationError(
            "E_REPO_COORDINATION_SCHEMA", f"{label} must be a workspace-relative path"
        )
    path = Path(value.replace("\\", "/"))
    if path.is_absolute() or ".." in path.parts:
        raise RepoCoordinationError(
            "E_REPO_COORDINATION_PATH", f"{label} escapes the workspace"
        )
...
def _inside(root: Path, relative: str) -> Path:
    ...
    try:
        ...
        raise RepoCoordinationError(
            "E_REPO_COORDINATION_PATH", "path escapes the workspace"
        ) from exc
    if not target.is_dir():
        raise RepoCoordinationError(
            "E_REPO_COORDINATION_PATH", f"repository is unavailable: {relative}"
        )
    return target
...
def _git(root: Path, *args: str) -> str:
    ...
    if run.returncode:
        raise RepoCoordinationError(
            "E_REPO_COORDINATION_GIT", run.stderr.strip() or "git inspection failed"
        )
...
def coordinate_repositories(root: Path, manifest_path: Path) -> dict[str, Any]:
    ...
    while pending:
        ready = [
            ...
            if all(dependency not in pending for dependency in row["depends_on"])
        ]
        if not ready:
            findings.append(
                {
                    "code": "REPO_COORDINATION_CYCLE",
                    "severity": "blocking",
                    "message": "Repository dependencies contain a cycle; no sequential plan is safe.",
                }
            )
            break
        for row in sorted(ready, key=lambda item: item["id"]):
            ...
            if _git(repo, "rev-parse", "--is-inside-work-tree") != "true":
                raise RepoCoordinationError(
                    "E_REPO_COORDINATION_GIT", f"{row['id']} is not a Git worktree"
                )
            ...
            if status == "blocked":
                findings.append(
                    {
                        "code": "REPO_COORDINATION_HEAD_DRIFT",
                        "severity": "blocking",
                        "repository": row["id"],
                        "expected_head_sha": row["expected_head_sha"],
                        "observed_head_sha": actual,
                        "message": "Pinned repository head differs; refresh human-reviewed coordination before sequential work.",
                    }
                )
            ordered.append(
                {
    ...
    core = {
        "schema": PLAN_SCHEMA,
        "marker": "REPO_COORDINATION_READY"
        if not blockers
        else "REPO_COORDINATION_BLOCKED",
        "manifest": {
            "id": manifest["id"],
            "path": manifest["path"],
        ...
        "claim_boundary": "Read-only local Git inspection. The plan never checks out, pulls, pushes, merges, rebases, creates worktrees, starts agents, or changes a repository.",
    }
...
def repo_coordination_template() -> dict[str, Any]:
    ...
    return {
        "schema": "factory.repo-coordination-template.v1",
        "manifest_schema": MANIFEST_SCHEMA,
        "authority": dict(AUTHORITY),
        "claim_boundary": "Template only. It creates no cross-repository action or connection.",
        "manifest": {
            "schema": MANIFEST_SCHEMA,
            "id": "replace-with-coordination-id",
            "repositories": [
                {
                    "id": "foundation",
