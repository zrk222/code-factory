...
def records(
    value: object, field: str, required: set[str], maximum: int = 256
) -> list[dict[str, Any]]:
...
def _stateful(config: dict[str, Any]) -> None:
    exact_keys(
        config, {"invariant_ids", "required_actions", "min_examples", "min_actions"}
    )
    require_unique_strings(
        config["invariant_ids"], "invariant_ids", minimum=1, maximum=128
    )
    require_unique_strings(
        config["required_actions"], "required_actions", minimum=1, maximum=128
    )
...
def _tenant(config: dict[str, Any]) -> None:
    ...
    require_unique_strings(
        config["forbidden_fields"], "forbidden_fields", minimum=1, maximum=128
    )
    statuses = config["denial_statuses"]
    if (
        not isinstance(statuses, list)
        or not statuses
        or any(type(s) is not int or s not in {401, 403, 404} for s in statuses)
        or len(set(statuses)) != len(statuses)
    ):
        raise RuntimeAuditError(
            "E_POLICY", "denial_statuses must be distinct 401, 403 or 404 values"
        )
    categories = {
        "api",
        "cache",
...
def _compatibility(config: dict[str, Any]) -> None:
    ...
    for item in records(
        config["interactions"],
        "interactions",
        {"id", "consumer_id", "consumer_version", "expected_sha256"},
    ):
...
def _migration(config: dict[str, Any]) -> None:
    ...
    require_digest(
        config["expected_after_schema_sha256"], "expected_after_schema_sha256"
    )
    ...
    require_unique_strings(
        config["required_catalog_objects"],
        "required_catalog_objects",
        minimum=1,
        maximum=256,
    )
...
def _performance(config: dict[str, Any]) -> None:
    exact_keys(
        config,
        {
            ...
            "max_retained_growth_ratio",
            "max_loadgen_cpu_pct",
            "max_loadgen_memory_pct",
    ...
    for item in records(
        config["thresholds"],
        "thresholds",
        {"id", "metric", "mode", "operator", "value", "origin"},
        128,
    ):
        require_str(item["metric"], "metric")
        if (
            item["mode"] not in {"absolute", "ratio"}
            or item["operator"] not in {"lte", "gte", "eq"}
            or item["origin"]
            not in {
                "human_confirmed",
                "trusted_source",
            ...
            raise RuntimeAuditError(
                "E_POLICY", "unknown threshold mode/operator/provenance"
            )
        require_number(item["value"], "value")
    require_unique_strings(
        config["resource_metrics"], "resource_metrics", minimum=1, maximum=16
    )
    require_int(
