...
_VERDICTS = frozenset(
    {"passed", "needs_revision", "failed", "stalled", "budget_exhausted"}
)
...
def _digest(value: object, field: str) -> str:
    ...
    if not re.fullmatch(r"[0-9a-f]{64}", result):
        raise VerifierPlaneError(
            "VERIFIER_INPUT_INVALID", f"{field} must be a SHA-256 hexadecimal digest"
        )
    return result
...
def _number(value: object, field: str, *, integer: bool = False) -> float | int:
    if (
        ...
        raise VerifierPlaneError(
            "VERIFIER_INPUT_INVALID", f"{field} must be a finite non-negative number"
        )
    if integer:
        if not isinstance(value, int):
            raise VerifierPlaneError(
                "VERIFIER_INPUT_INVALID", f"{field} must be a non-negative integer"
            )
        return value
    return float(value)
...
def _root(root: Path) -> Path:
    ...
    if not workspace.is_dir():
        raise VerifierPlaneError(
            "VERIFIER_ROOT_INVALID", f"root must be an existing directory: {workspace}"
        )
    return workspace


def _under(
    root: Path, value: Path, code: str, label: str, *, directory: bool | None = None
) -> Path:
...
def _load(path: Path, schema: str, label: str) -> dict[str, Any]:
    try:
        ...
        raise VerifierPlaneError(
            "VERIFIER_INPUT_INVALID", f"cannot read {label}: {exc}"
        ) from exc
    if not isinstance(value, dict) or value.get("schema") != schema:
        raise VerifierPlaneError(
            "VERIFIER_INPUT_INVALID", f"{label} must use schema {schema}"
        )
    return value
...
def _tree_digest(root: Path) -> str:
    ...
    for path in sorted(item for item in root.rglob("*") if item.is_file()):
        ...
        if any(
            part in {".git", ".factory", "node_modules", "__pycache__"}
...
def _file_descriptors(
    root: Path, paths: Iterable[Path], *, label: str, maximum: int
) -> list[dict[str, str]]:
    values = list(paths)
    if not values or len(values) > maximum:
        raise VerifierPlaneError(
            "VERIFIER_INPUT_INVALID", f"{label} must contain 1 through {maximum} files"
        )
    ...
    for item in values:
        ...
        if relative in seen:
            raise VerifierPlaneError(
                "VERIFIER_INPUT_INVALID", f"{label} must not contain duplicate files"
            )
        seen.add(relative)
...
def _bounded_budgets(
    *, max_attempts: int, max_wall_seconds: int, max_tokens: int, max_cost_usd: float
) -> dict[str, int | float]:
    values = {
    ...
    for key, ceiling in _DEFAULT_BUDGETS.items():
        if values[key] < 1 or values[key] > ceiling:
            raise VerifierPlaneError(
                "VERIFIER_BUDGET_INVALID", f"{key} must be from 1 through {ceiling:g}"
            )
    return values
...
def _atomic_json(path: Path, value: dict[str, Any], *, force: bool) -> None:
    if path.exists() and not force:
        raise VerifierPlaneError(
            "VERIFIER_SESSION_EXISTS", f"session already exists: {path}"
        )
...
def create_verifier_session(
    ...
    *,
    ...
    mission_file = _under(
        workspace, mission_path, "VERIFIER_PATH_REJECTED", "mission", directory=False
    )
    candidate = _under(
        workspace,
        candidate_root,
        "VERIFIER_PATH_REJECTED",
        "candidate_root",
        directory=True,
    )
    ...
    bundle = _file_descriptors(
        workspace, verifier_bundle, label="verifier_bundle", maximum=_MAX_BUNDLE_FILES
    )
    if any(
        (workspace / item["path"]).resolve().is_relative_to(candidate)
        for item in bundle
    ):
        raise VerifierPlaneError(
            "VERIFIER_BUNDLE_WRITABLE",
            "verifier bundle files must be outside candidate_root",
        )
    ...
    result = {
        **core,
        ...
        "created_at": _now(),
        "markers": ["VERIFIER_SESSION_BOUND", "VERIFIER_RUNTIME_UNATTESTED"],
    }
    path = (
        workspace
        / ".factory"
        / "verifier-sessions"
        / f"{mission_id}-{session_sha[:12]}.session.json"
    )
...
def _session(root: Path, path: Path) -> tuple[Path, dict[str, Any]]:
    session_path = _under(
        root, path, "VERIFIER_PATH_REJECTED", "session", directory=False
    )
    ...
    core = {
        ...
        if key not in {"session_sha256", "created_at", "markers"}
    }
    if _sha_bytes(_canonical(core)) != digest:
        raise VerifierPlaneError(
            "VERIFIER_SESSION_DRIFT",
            "session digest does not match canonical session bytes",
        )
    return session_path, session


def _paths(
    root: Path, values: object, *, label: str, maximum: int
) -> list[dict[str, str]]:
    ...
    if len(values) > maximum:
        raise VerifierPlaneError(
            "VERIFIER_INPUT_INVALID", f"{label} must contain at most {maximum} files"
        )
    for index, value in enumerate(values):
        if not isinstance(value, dict):
            raise VerifierPlaneError(
                "VERIFIER_INPUT_INVALID", f"{label}[{index}] must be an object"
            )
        raw = _text(value.get("path"), f"{label}[{index}].path", maximum=400)
        relative = Path(raw)
        if relative.is_absolute() or ".." in relative.parts:
            raise VerifierPlaneError(
                "VERIFIER_PATH_REJECTED", f"{label}[{index}] must be workspace-relative"
            )
        path = _under(
            root,
            root / relative,
        ...
        if _sha_path(path) != supplied:
            raise VerifierPlaneError(
                "VERIFIER_EVIDENCE_DRIFT",
                f"{label}[{index}] digest does not match current bytes",
            )
        descriptors.append({"path": _relative(root, path), "sha256": supplied})
    if len({item["path"] for item in descriptors}) != len(descriptors):
        raise VerifierPlaneError(
            "VERIFIER_INPUT_INVALID", f"{label} must not contain duplicate paths"
        )
    return sorted(descriptors, key=lambda item: item["path"])
...
def _worker_result(
    ...
    if (
        _digest(value.get("session_sha256"), "worker_result.session_sha256")
        != session["session_sha256"]
    ):
        raise VerifierPlaneError(
            "VERIFIER_SESSION_BINDING", "worker result is not bound to the session"
        )
    worker_id = _text(value.get("worker_id"), "worker_result.worker_id", maximum=96)
    candidate = _under(
        root,
        root
        / _text(session.get("candidate_root"), "session.candidate_root", maximum=400),
        "VERIFIER_PATH_REJECTED",
        "candidate_root",
        directory=True,
    )
    supplied_tree = _digest(
        value.get("candidate_tree_sha256"), "worker_result.candidate_tree_sha256"
    )
    ...
    if not isinstance(writes, list):
        raise VerifierPlaneError(
            "VERIFIER_INPUT_INVALID", "worker_result.declared_writes must be a list"
        )
    for raw in writes:
        ...
        if relative.is_absolute() or ".." in relative.parts:
            raise VerifierPlaneError(
                "VERIFIER_PATH_REJECTED",
                "worker_result.declared_writes must be candidate-relative",
            )
        _under(
            candidate,
            candidate / relative,
            "VERIFIER_PATH_REJECTED",
            "worker_result.declared_writes",
            directory=False,
        )
    ...
    if not isinstance(raw_usage, dict):
        raise VerifierPlaneError(
            "VERIFIER_INPUT_INVALID", "worker_result.usage must be an object"
        )
    usage = {
        "attempt": _number(
            raw_usage.get("attempt"), "worker_result.usage.attempt", integer=True
        ),
        "wall_seconds": _number(
            raw_usage.get("wall_seconds"),
            "worker_result.usage.wall_seconds",
            integer=True,
        ),
        "tokens": _number(
            raw_usage.get("tokens"), "worker_result.usage.tokens", integer=True
        ),
    ...
    if (
        ...
        or usage["wall_seconds"] > budgets["max_wall_seconds"]
        or usage["tokens"] > budgets["max_tokens"]
        or usage["cost_usd"] > budgets["max_cost_usd"]
    ):
        raise VerifierPlaneError(
    ...
    if not isinstance(raw_progress, dict):
        raise VerifierPlaneError(
            "VERIFIER_INPUT_INVALID", "worker_result.progress must be an object"
        )
    progress = {
        field: _number(
            raw_progress.get(field), f"worker_result.progress.{field}", integer=True
        )
...
def _bundle_current(root: Path, session: dict[str, Any]) -> str:
    ...
    if digest != session["verifier_bundle_sha256"]:
        raise VerifierPlaneError(
            "VERIFIER_BUNDLE_DRIFT",
            "verifier bundle bytes differ from the bound session",
        )
    return digest
...
def _checks(value: object) -> list[dict[str, Any]]:
    if not isinstance(value, list) or not value:
        raise VerifierPlaneError(
            "VERIFIER_INPUT_INVALID", "verifier_result.checks must be a non-empty list"
        )
    ...
    for index, item in enumerate(value):
        if not isinstance(item, dict) or not isinstance(item.get("passed"), bool):
            raise VerifierPlaneError(
                "VERIFIER_INPUT_INVALID",
                f"verifier_result.checks[{index}] must contain boolean passed",
            )
        checks.append(
            {
                "id": _text(
                    item.get("id"), f"verifier_result.checks[{index}].id", maximum=96
                ),
                "passed": item["passed"],
            }
        )
    if len({item["id"] for item in checks}) != len(checks):
        raise VerifierPlaneError(
            "VERIFIER_INPUT_INVALID", "verifier_result.checks must not repeat IDs"
        )
    return sorted(checks, key=lambda item: item["id"])


def _verifier_input(
    root: Path, path: Path, session: dict[str, Any], worker_sha: str, worker_id: str
) -> tuple[Path, dict[str, Any], str]:
    verifier_path = _under(
        root, path, "VERIFIER_PATH_REJECTED", "verifier_result", directory=False
    )
    value = _load(verifier_path, RESULT_SCHEMA, "verifier_result")
    if (
        _digest(value.get("session_sha256"), "verifier_result.session_sha256")
        != session["session_sha256"]
    ):
        raise VerifierPlaneError(
            "VERIFIER_SESSION_BINDING", "verifier result is not bound to the session"
        )
    if (
        _digest(
            value.get("worker_result_sha256"), "verifier_result.worker_result_sha256"
        )
        != worker_sha
    ):
        raise VerifierPlaneError(
            "VERIFIER_WORKER_BINDING",
            "verifier result is not bound to current worker result bytes",
        )
    verifier_id = _text(
        value.get("verifier_id"), "verifier_result.verifier_id", maximum=96
    )
    if verifier_id == worker_id:
        raise VerifierPlaneError(
            "VERIFIER_IDENTITY_DISTINCT", "worker and verifier identities must differ"
        )
    if (
        value.get("fresh_session") is not True
        or value.get("context_wall") != "isolated"
    ):
        raise VerifierPlaneError(
            "VERIFIER_CONTEXT_WALL",
            "verifier result requires fresh_session=true and context_wall=isolated",
        )
    return verifier_path, value, verifier_id


def _verifier_details(
    root: Path, session: dict[str, Any], value: dict[str, Any]
) -> tuple[str, str, list[dict[str, str]], list[dict[str, Any]], str, str | None]:
    bundle_digest = _bundle_current(root, session)
    if (
        _digest(
            value.get("verifier_bundle_sha256"),
            "verifier_result.verifier_bundle_sha256",
        )
        != bundle_digest
    ):
        raise VerifierPlaneError(
            "VERIFIER_BUNDLE_DRIFT",
            "verifier result bundle digest differs from bound bundle",
        )
    toolchain_digest = _digest(
        value.get("toolchain_sha256"), "verifier_result.toolchain_sha256"
    )
    evidence = _paths(
        root,
        value.get("evidence"),
        label="verifier_result.evidence",
        maximum=_MAX_EVIDENCE_FILES,
    )
    ...
    if verdict not in _VERDICTS:
        raise VerifierPlaneError(
            "VERIFIER_INPUT_INVALID",
            f"verifier_result.verdict must be one of {sorted(_VERDICTS)}",
        )
    all_passed = all(item["passed"] for item in checks)
    if verdict == "passed" and not all_passed:
        raise VerifierPlaneError(
            "VERIFIER_VERDICT_INVALID",
            "passed verdict requires every declared check to pass",
        )
    if verdict == "needs_revision" and all_passed:
        raise VerifierPlaneError(
            "VERIFIER_VERDICT_INVALID",
            "needs_revision verdict requires at least one failed check",
        )
    ...
    if harness is not None:
        if not isinstance(harness, dict):
            raise VerifierPlaneError(
                "VERIFIER_INPUT_INVALID", "harness_attestation must be an object"
            )
...
def verify_verifier_result(
    session_path: Path, worker_result_path: Path, verifier_result_path: Path, root: Path
) -> dict[str, Any]:
    ...
    try:
        checked_worker_path, worker, worker_sha = _worker_result(
            workspace, worker_result_path, session
        )
    except VerifierPlaneError as exc:
        exc.marker = "VERIFIER_WORKER_RESULT_REJECTED"
        raise
    verifier_path, value, verifier_id = _verifier_input(
        workspace, verifier_result_path, session, worker_sha, worker.worker_id
    )
    bundle_digest, toolchain_digest, evidence, checks, verdict, harness_sha = (
        _verifier_details(workspace, session, value)
    )
    core = {
    ...
    return {
        **core,
        ...
        "markers": [
            ...
            "VERIFIER_HARNESS_ATTESTATION_BOUND"
            if harness_sha
            else "VERIFIER_RUNTIME_UNATTESTED",
        ],
    }
...
def _normalized_attempt(item: object, index: int) -> dict[str, Any]:
    if not isinstance(item, dict):
        raise VerifierPlaneError(
            "VERIFIER_INPUT_INVALID", f"attempts[{index}] must be an object"
        )
    progress = item.get("progress")
    if not isinstance(progress, dict):
        raise VerifierPlaneError(
            "VERIFIER_INPUT_INVALID", f"attempts[{index}].progress must be an object"
        )
    return {
        "attempt": _number(
            item.get("attempt", index), f"attempts[{index}].attempt", integer=True
        ),
        "failure_signature": _text(
            item.get("failure_signature"),
            f"attempts[{index}].failure_signature",
            maximum=160,
        ),
        "progress": {
            field: _number(
                progress.get(field), f"attempts[{index}].progress.{field}", integer=True
            )
            for field in ("passed_checks", "failed_checks", "criteria_covered")
        },
    }
...
def _normalized_attempts(attempts: object) -> list[dict[str, Any]]:
    if (
        ...
        or len(attempts) < 1
        or len(attempts) > _DEFAULT_BUDGETS["max_attempts"]
    ):
        raise VerifierPlaneError(
            "VERIFIER_INPUT_INVALID",
            f"attempts must contain 1 through {_DEFAULT_BUDGETS['max_attempts']} records",
        )
    normalized = [
        _normalized_attempt(item, index) for index, item in enumerate(attempts, 1)
    ]
    if [item["attempt"] for item in normalized] != sorted(
        item["attempt"] for item in normalized
    ) or len({item["attempt"] for item in normalized}) != len(normalized):
        raise VerifierPlaneError(
            "VERIFIER_INPUT_INVALID",
            "attempts must have strictly increasing unique attempt numbers",
        )
    return normalized
...
def _progress_delta(
    ...
    improved = (
        ...
        or current["progress"]["criteria_covered"]
        > previous["progress"]["criteria_covered"]
    )
...
def evaluate_progress(attempts: list[dict[str, Any]]) -> dict[str, Any]:
    ...
    return {
        ...
        "markers": ["VERIFIER_PROGRESS_STALLED"]
        if stalled
        else ["VERIFIER_PROGRESS_CONTINUES"],
        "scope_limits": [
            "Progress compares declared deterministic counts and exact failure signatures; it does not judge reasoning quality."
        ],
    }
