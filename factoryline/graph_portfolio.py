...
def _blocked_ancestors(
    ...
    blockers = sorted(
        node_id for node_id in nodes if _disposition(source_nodes[node_id]) == "BLOCK"
    )
...
def _common(snapshot: object) -> dict[str, Any]:
    graph_sha256 = (
        snapshot.get("base_graph_sha256", snapshot.get("graph_sha256"))
...
def _blocked_plan(
    common: dict[str, Any], marker: str, cycles: list[list[str]]
) -> dict[str, Any]:
    core = {
        **common,
        "verdict": "BLOCKED",
...
def _schedule(
    ...
    for node_id in ordered:
        prior, parent = max(
            ((earliest[item], item) for item in predecessors[node_id]),
            default=(0, None),
        )
    ...
    for node_id in reversed(ordered):
        ...
        downstream[node_id] = own + max(
            (downstream[child] for child in successors[node_id]), default=0
        )
    return earliest, parents, downstream


def _critical_path(
    nodes: list[str], earliest: dict[str, int], parents: dict[str, str | None]
) -> tuple[list[str], int]:
    horizon = max(earliest.values(), default=0)
    terminal = min(
        (node_id for node_id in nodes if earliest.get(node_id) == horizon), default=None
    )
    path: list[str] = []
...
def _workset(
    ...
    for sequence, node_id in enumerate(ordered, 1):
        ...
        disposition = (
            "BLOCK" if blocked[node_id] else _disposition(source_nodes[node_id])
        )
        result.append(
            {
                "sequence": sequence,
                ...
                "descendant_count": len(descendants[node_id]),
                "shared_candidate": len(descendants[node_id]) >= 2,
...
def _parallel_waves(workset: list[dict[str, Any]]) -> list[dict[str, Any]]:
    ready = [
        item
        for item in workset
        if not item["blocked_by"] and item["disposition"] != "BLOCK"
    ]
    ...
    return [
        {
            "index": depth + 1,
            "node_ids": [
                item["node_id"] for item in ready if item["structural_depth"] == depth
            ],
...
def graph_portfolio_plan(
    snapshot: dict[str, Any], duration_observations: object = None
) -> dict[str, Any]:
    """Compile a deterministic structural plan without writing or executing."""
    common = _common(snapshot)
    nodes, successors, source_nodes = _dependency_graph(
    ...
    earliest, parents, downstream = _schedule(
        ordered, successors, _predecessors(nodes, successors), durations
    )
    ...
    core = {
        **common,
        ...
        "workset": workset,
        "parallel_waves": _parallel_waves(workset),
...
def _quantitative(horizon: int, durations: dict[str, int] | None) -> dict[str, Any]:
    ...
    return {
        "markers": [
            "GRAPH_PORTFOLIO_DURATION_OBSERVATIONS_BOUND",
            "GRAPH_PORTFOLIO_SAVINGS_UNMEASURED",
        ],
