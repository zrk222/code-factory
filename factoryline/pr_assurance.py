...
def _positive_int(value: Any, field: str) -> int:
    if not isinstance(value, int) or isinstance(value, bool) or value <= 0:
        raise PRAssuranceError(
            "E_WEBHOOK_PAYLOAD", f"{field} must be a positive integer"
        )
    return value


def _verify_webhook_envelope(
    body: bytes, headers: Mapping[str, str], secret: bytes
) -> str:
    if not isinstance(body, bytes) or not body or len(body) > MAX_WEBHOOK_BYTES:
        raise PRAssuranceError(
            "E_WEBHOOK_BODY", f"webhook body must contain 1-{MAX_WEBHOOK_BYTES} bytes"
        )
    if not isinstance(secret, bytes) or len(secret) < 16:
        raise PRAssuranceError(
            "E_WEBHOOK_SECRET", "webhook secret must contain at least 16 bytes"
        )
    supplied = _header(headers, "X-Hub-Signature-256")
    expected = "sha256=" + hmac.new(secret, body, hashlib.sha256).hexdigest()
    if not supplied or not hmac.compare_digest(supplied, expected):
        raise PRAssuranceError(
            "E_WEBHOOK_SIGNATURE", "GitHub webhook signature is invalid"
        )
    if _header(headers, "X-GitHub-Event") != "pull_request":
        raise PRAssuranceError(
            "E_WEBHOOK_EVENT", "only pull_request events are accepted"
        )
...
def _parse_webhook_body(body: bytes) -> dict[str, Any]:
    try:
        ...
        raise PRAssuranceError(
            "E_WEBHOOK_PAYLOAD", "webhook body is not valid UTF-8 JSON"
        ) from exc
...
def _normalize_pr_payload(
    payload: dict[str, Any], delivery_id: str, body: bytes
) -> PullRequestEvent:
    action = _required_text(payload.get("action"), "action")
    if action not in ALLOWED_ACTIONS:
        raise PRAssuranceError(
            "E_WEBHOOK_ACTION", f"unsupported pull_request action: {action}"
        )
    ...
    head_sha = _required_text(
        (pull_request.get("head") or {}).get("sha"), "pull_request.head.sha"
    )
    if not SHA_RE.fullmatch(head_sha):
        raise PRAssuranceError(
            "E_WEBHOOK_PAYLOAD",
            "pull_request.head.sha must be a 40 or 64 character hex digest",
        )
    return PullRequestEvent(
        delivery_id=delivery_id,
        repository=repository,
        pull_request=_positive_int(
            pull_request.get("number") or payload.get("number"), "pull request number"
        ),
        installation_id=_positive_int(
            (payload.get("installation") or {}).get("id"), "installation.id"
        ),
        head_sha=head_sha.lower(),
        actor=_required_text(
            (payload.get("sender") or {}).get("login"), "sender.login"
        ),
        action=action,
...
def _reject_duplicates(pairs: list[tuple[str, Any]]) -> dict[str, Any]:
    ...
    for key, item in pairs:
        if key in result:
            raise PRAssuranceError(
                "E_OIDC_MALFORMED", f"OIDC token repeats JSON member {key!r}"
            )
        result[key] = item
    return result
...
def _json_segment(value: str) -> dict[str, Any]:
    try:
        decoded = json.loads(
            _decode_segment(value, "E_OIDC_MALFORMED"),
            object_pairs_hook=_reject_duplicates,
        )
    except (UnicodeDecodeError, json.JSONDecodeError) as exc:
        raise PRAssuranceError(
            "E_OIDC_MALFORMED", "OIDC token segment is not a JSON object"
        ) from exc
    if not isinstance(decoded, dict):
        raise PRAssuranceError(
            "E_OIDC_MALFORMED", "OIDC token segment must be a JSON object"
        )
    return decoded
...
def _trusted_rsa_key(jwks: dict[str, Any], kid: str) -> dict[str, Any]:
    ...
    if len(matches) != 1:
        raise PRAssuranceError(
            "E_OIDC_KEY", "OIDC kid must resolve to exactly one trusted key"
        )
    key = matches[0]
    if (
        key.get("kty") != "RSA"
        ...
        raise PRAssuranceError(
            "E_OIDC_KEY", "trusted OIDC key is not an RS256 signing key"
        )
    return key


def _verify_oidc_signature(
    key: dict[str, Any], signature: str, signing_input: bytes
) -> None:
    try:
        ...
        if modulus.bit_length() < 2048:
            raise PRAssuranceError(
                "E_OIDC_KEY", "trusted RSA key must contain at least 2048 bits"
            )
        public_key = rsa.RSAPublicNumbers(exponent, modulus).public_key()
        public_key.verify(
            _decode_segment(signature, "E_OIDC_MALFORMED"),
            signing_input,
            padding.PKCS1v15(),
            hashes.SHA256(),
        )
        ...
        raise PRAssuranceError(
            "E_OIDC_SIGNATURE", "OIDC token signature is invalid"
        ) from exc
...
def _parse_oidc_token(
    ...
    if not expected_issuer or not expected_audience:
        raise PRAssuranceError(
            "E_OIDC_CONFIG", "expected issuer and audience are required"
        )
    if not 0 <= clock_skew_seconds <= 300:
        raise PRAssuranceError(
            "E_OIDC_CONFIG", "clock skew must be between 0 and 300 seconds"
        )
    ...
    return (
        ...
        encoded_payload,
        encoded_signature,
        _json_segment(encoded_header),
        _json_segment(encoded_payload),
    )
...
def verify_oidc_token(
    ...
    *,
    ...
    encoded_header, encoded_payload, encoded_signature, header, claims = (
        _parse_oidc_token(token, expected_issuer, expected_audience, clock_skew_seconds)
    )
    if header.get("alg") != "RS256":
        raise PRAssuranceError(
            "E_OIDC_ALGORITHM", "only RS256 OIDC tokens are accepted"
        )
    kid = header.get("kid")
    if not isinstance(kid, str) or not kid:
        raise PRAssuranceError("E_OIDC_KEY", "OIDC token kid is required")
    _verify_oidc_signature(
        _trusted_rsa_key(jwks, kid),
        encoded_signature,
        f"{encoded_header}.{encoded_payload}".encode("ascii"),
    )
    current = int(time.time()) if now is None else int(now)
    _validate_oidc_claims(
        claims, expected_issuer, expected_audience, current, clock_skew_seconds
    )
...
def github_check_request(
    ...
    *,
    ...
    if status not in {"pending", "approved", "rejected"}:
        raise PRAssuranceError(
            "E_CHECK_STATUS", "check status must be pending, approved, or rejected"
        )
...
class PRAssuranceStore:
    ...
    def register_installation(self, tenant_id: str, installation_id: int) -> None:
        ...
        with self._connect() as db:
            row = db.execute(
                "SELECT tenant_id FROM github_installation_tenants WHERE installation_id = ?",
                (installation_id,),
            ).fetchone()
            if row and row["tenant_id"] != tenant_id:
                raise PRAssuranceError(
                    "E_INSTALLATION_TENANT",
                    "GitHub installation is bound to another tenant",
                )
            if not row:
                db.execute(
                    "INSERT INTO github_installation_tenants VALUES (?, ?)",
    ...
    def require_installation(self, tenant_id: str, installation_id: int) -> None:
        ...
        if not row or row["tenant_id"] != tenant_id:
            raise PRAssuranceError(
                "E_INSTALLATION_TENANT",
                "GitHub installation is not bound to this tenant",
            )
    ...
    def reserve_delivery(self, tenant_id: str, event: PullRequestEvent) -> None:
        ...
        try:
            ...
            raise PRAssuranceError(
                "E_WEBHOOK_REPLAY", "GitHub delivery id has already been consumed"
            ) from exc

    def bind_delivery(
        self, tenant_id: str, delivery_id: str, evidence_id: str, approval_id: str
    ) -> None:
        ...
        if changed != 1:
            raise PRAssuranceError(
                "E_ASSURANCE_STATE", "delivery reservation is not bindable"
            )

    def by_approval(
        self, tenant_id: str, approval_id: str
    ) -> tuple[PullRequestEvent, dict[str, Any]]:
        ...
        if not row:
            raise PRAssuranceError(
                "E_NOT_FOUND", "PR assurance approval was not found in this tenant"
            )
        return PullRequestEvent(**json.loads(row["event_json"])), dict(row)

    def consume_jti(
        self, issuer: str, jti: str, subject: str, approval_id: str
    ) -> None:
        """Consume one verified OIDC token identifier or reject an identity replay."""
        try:
            with self._connect() as db:
                db.execute(
                    "INSERT INTO oidc_token_uses VALUES (?, ?, ?, ?)",
                    (issuer, jti, subject, approval_id),
                )
        except sqlite3.IntegrityError as exc:
            raise PRAssuranceError(
                "E_OIDC_REPLAY", "OIDC jti has already been consumed"
            ) from exc
    ...
    def set_state(self, tenant_id: str, approval_id: str, state: str) -> None:
        ...
        if changed != 1:
            raise PRAssuranceError(
                "E_ASSURANCE_STATE", "PR assurance request is not pending"
            )
...
def ingest_pull_request(
    ...
    principal = Principal(
        subject=f"github-app:{event.installation_id}",
        tenant_id=tenant_id,
        roles=("operator",),
    )
    evidence_id = (
        "pr-"
        + hashlib.sha256(f"{tenant_id}:{event.delivery_id}".encode()).hexdigest()[:32]
    )
    ...
    approval = store.evidence.request_approval(
        ...
        tenant_id,
        evidence_id,
        f"independent assurance decision for {event.repository}#{event.pull_request}",
    )
    store.bind_delivery(
        tenant_id, event.delivery_id, evidence_id, approval["approval_id"]
    )
    return {
        "schema": PR_ASSURANCE_SCHEMA,
        "markers": [
...
def decide_pull_request(
    ...
    *,
    ...
    principal = principal_from_verified_oidc(
        claims, expected_issuer=expected_issuer, role_map=role_map
    )
    ...
    approved = store.evidence.decide_approval(
        principal, tenant_id, approval_id, decision, reason
    )
    ...
    marker = (
        "APPROVAL_APPROVED" if approved["status"] == "approved" else "APPROVAL_REJECTED"
    )
    return {
        "schema": PR_ASSURANCE_SCHEMA,
