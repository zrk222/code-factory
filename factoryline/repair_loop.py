...
def compare_deep_audit_repairs(
    root: Path, before_path: str, after_path: str, **kwargs
) -> dict:
    ...
    return compare_deep_audits(root, before_path, after_path, **kwargs)
...
def _canonical(value: object) -> bytes:
    try:
        ...
        raise RepairLoopError(
            "E_REPAIR_LOOP_SCHEMA", "input must be canonical JSON"
        ) from exc
...
def _id(value: object, label: str) -> str:
    if not isinstance(value, str) or not _ID.fullmatch(value):
        raise RepairLoopError(
            "E_REPAIR_LOOP_SCHEMA", f"{label} must be a safe identifier"
        )
    return value
...
def _hash(value: object, label: str) -> str:
    if not isinstance(value, str) or not _SHA.fullmatch(value):
        raise RepairLoopError(
            "E_REPAIR_LOOP_SCHEMA", f"{label} must be a lowercase SHA-256"
        )
    return value
...
def _path(value: object, label: str) -> str:
    if not isinstance(value, str) or not value.strip() or len(value) > 512:
        raise RepairLoopError(
            "E_REPAIR_LOOP_SCHEMA", f"{label} must be a workspace-relative path"
        )
...
def _inside(root: Path, relative: str, *, required: bool = False) -> Path:
    ...
    try:
        ...
        raise RepairLoopError(
            "E_REPAIR_LOOP_PATH", "path escapes the workspace"
        ) from exc
    if required and not target.is_file():
        raise RepairLoopError(
            "E_REPAIR_LOOP_EVIDENCE", f"required evidence is unavailable: {relative}"
        )
    return target
...
def _bound_file(root: Path, value: object, label: str) -> dict[str, str]:
    if not isinstance(value, dict) or set(value) != {"path", "sha256"}:
        raise RepairLoopError(
            "E_REPAIR_LOOP_SCHEMA", f"{label} must contain exact path and sha256"
        )
    relative, expected = (
    ...
    if actual != expected:
        raise RepairLoopError(
            "E_REPAIR_LOOP_EVIDENCE", f"{label} does not match local bytes"
        )
...
def _bound_failure_capsule(root: Path, value: object) -> dict[str, str]:
    ...
    try:
        payload = json.loads(
            _inside(root, bound["path"], required=True).read_text(encoding="utf-8")
        )
        ...
        raise RepairLoopError(
            "E_REPAIR_LOOP_REPRODUCTION",
            "reproduction must be a current, hash-valid failure capsule",
        ) from exc
    return bound
...
def _bound_e2e_receipt(root: Path, value: object, label: str) -> dict[str, str]:
    ...
    try:
        ...
        raise RepairLoopError(
            "E_REPAIR_LOOP_RECHECK", f"{label} must be a hash-valid E2E proof receipt"
        ) from exc
    if verified.get("marker") != "E2E_PROOF_PASS" or verified.get("ok") is not True:
        raise RepairLoopError(
            "E_REPAIR_LOOP_RECHECK",
            f"{label} must prove its declared positive and negative controls",
        )
    return bound


def _candidate_patch(
    root: Path, value: object, allowed_paths: list[str]
) -> dict[str, Any]:
    ...
    try:
        ...
        raise RepairLoopError(
            "E_REPAIR_LOOP_CANDIDATE",
            "repair.candidate must be UTF-8 unified diff text",
        ) from exc
    matches = re.findall(
    ...
    if not matches or not re.search(r"^@@ -", text, flags=re.MULTILINE):
        raise RepairLoopError(
            "E_REPAIR_LOOP_CANDIDATE",
            "repair.candidate must contain at least one unified-diff file and hunk",
        )
    paths: list[str] = []
    for before, after in matches:
        if before != after:
            raise RepairLoopError(
                "E_REPAIR_LOOP_CANDIDATE", "repair.candidate must not rename files"
            )
        ...
        if not any(
            ...
            raise RepairLoopError(
                "E_REPAIR_LOOP_SCOPE",
                f"repair.candidate changes undeclared path: {relative}",
            )
...
def _bound_challenge(root: Path, value: object, contract_sha256: str) -> dict[str, str]:
    ...
    try:
        ...
        raise RepairLoopError(
            "E_REPAIR_LOOP_CHALLENGE",
            "challenge_plan must be a current independent Oracle challenge",
        ) from exc
    if plan.get("contract", {}).get("contract_sha256") != contract_sha256:
        raise RepairLoopError(
            "E_REPAIR_LOOP_CHALLENGE",
            "challenge_plan must bind the repair loop's sealed Oracle contract",
        )
    return bound
...
def _read_manifest(root: Path, source: Path) -> tuple[dict[str, Any], str]:
    try:
        ...
        raise RepairLoopError(
            "E_REPAIR_LOOP_PATH", "manifest escapes the workspace"
        ) from exc
    path = _inside(root, relative, required=True)
    if path.stat().st_size > MAX_BYTES:
        raise RepairLoopError(
            "E_REPAIR_LOOP_SCHEMA", "manifest exceeds the 1 MiB limit"
        )
    try:
        value = json.loads(path.read_text(encoding="utf-8"))
    except (OSError, UnicodeDecodeError, json.JSONDecodeError) as exc:
        raise RepairLoopError(
            "E_REPAIR_LOOP_SCHEMA", "manifest must be UTF-8 JSON"
        ) from exc
...
def _manifest(root: Path, source: Path) -> dict[str, Any]:
    ...
    contract_path, contract_sha = (
        _path(oracle["contract_path"], "oracle.contract_path"),
        _hash(oracle["contract_sha256"], "oracle.contract_sha256"),
    )
    try:
    ...
    if (
        not verified.get("ok")
        or verified.get("contract", {}).get("contract_sha256") != contract_sha
    ):
        raise RepairLoopError(
    ...
    obligations = sorted(
        {
            ...
            for item in issue["affected_obligations"]
        }
    )
    ...
    if (
        ...
        or not isinstance(repair["allowed_paths"], list)
        or not 1 <= len(repair["allowed_paths"]) <= 64
    ):
        raise RepairLoopError(
            "E_REPAIR_LOOP_SCHEMA",
            "repair must bind a candidate and 1-64 allowed paths",
        )
    allowed_paths = sorted(
        {_path(item, "repair.allowed_paths") for item in repair["allowed_paths"]}
    )
    ...
    return {
        ...
        "independent_recheck": {
            ...
            "positive_receipt": _bound_e2e_receipt(
                root,
                independent["positive_receipt"],
                "independent_recheck.positive_receipt",
            ),
            "negative_receipt": _bound_e2e_receipt(
                root,
                independent["negative_receipt"],
                "independent_recheck.negative_receipt",
            ),
        },
        "human_review": {
            "required": True,
            "reviewer": _id(human["reviewer"], "human_review.reviewer"),
        },
        "source_path": relative,
        "source_sha256": _file_sha(_inside(root, relative, required=True)),
    }


def assess_repair_loop(
    ...
    core = {
        ...
        "marker": "REPAIR_LOOP_READY",
        **manifest,
        "authority": dict(AUTHORITY),
        "scope_limits": [
            "The consequence list is a human-authored risk assessment, not a prediction or severity guarantee.",
            "This receipt never runs the candidate, repairs code, calls a model, merges a change, or approves a decision.",
        ],
        "created_at": _now(),
    }
    receipt = {**core, "receipt_sha256": _sha(core)}
    default = (
        RECEIPT_DIR / f"{manifest['id']}-{receipt['receipt_sha256'][:12]}.json"
    ).as_posix()
    relative = _path(str(out) if out else default, "output")
    if not relative.startswith(RECEIPT_DIR.as_posix() + "/"):
        raise RepairLoopError(
            "E_REPAIR_LOOP_PATH", "output must be beneath .factory/repair-loops"
        )
    target = _inside(workspace, relative)
    if target.exists():
        raise RepairLoopError(
            "E_REPAIR_LOOP_EXISTS",
            "repair-loop receipt is immutable; choose a new output",
        )
...
def repair_loop_projection(root: Path) -> dict[str, Any]:
    ...
    if directory.is_dir():
        for path in sorted(directory.glob("*.json"))[:500]:
            try:
                ...
                if value.get("schema") != RECEIPT_SCHEMA or value.get(
                    "receipt_sha256"
                ) != _sha(core):
                ...
                receipts.append(
                    {
                        ...
                        "failure_code": value["issue"]["failure_code"],
                        "consequence_count": len(value["consequences"]),
                        "highest_severity": max(
                            (item["severity"] for item in value["consequences"]),
                            key=_SEVERITY_RANK.index,
                        ),
                        "reviewer": value["human_review"]["reviewer"],
    ...
    return {
        ...
        "invalid": invalid[:100],
        "authority": dict(AUTHORITY),
        "claim_boundary": "Read-only local repair-loop facts. No repair, model, provider, approval, merge, release, or external communication action ran.",
    }
...
def repair_loop_template() -> dict[str, Any]:
    ...
    return {
        "schema": "factory.repair-loop-template.v1",
        "manifest_schema": MANIFEST_SCHEMA,
        "authority": dict(AUTHORITY),
        "claim_boundary": "Template only. It does not diagnose a real issue, run a repair, or approve a change.",
        "manifest": {
            "schema": MANIFEST_SCHEMA,
            "id": "replace-with-repair-loop-id",
            "oracle": {
                "contract_path": ".factory/oracles/contracts/current.json",
                "contract_sha256": "replace-with-lowercase-sha256",
            },
            "issue": {
                "failure_code": "E_REPLACE_WITH_EXACT_FAILURE",
                "summary": "replace-with-observed-symptom",
                "affected_obligations": ["replace-with-obligation-id"],
            },
            "consequences": [
                {
                    "kind": "user_experience",
                    "severity": "medium",
                    "rationale": "replace-with-human-reviewed-consequence",
                }
            ],
            "reproduction": {
                "path": ".factory/e2e/observed-failure.json",
                "sha256": "replace-with-lowercase-sha256",
            },
            "repair": {
                "candidate": {
                    "path": ".factory/repair-candidates/candidate.patch",
                    "sha256": "replace-with-lowercase-sha256",
                },
                "allowed_paths": ["src"],
            },
            "independent_recheck": {
                "challenge_plan": {
                    "path": ".factory/oracles/challenges/current.json",
                    "sha256": "replace-with-lowercase-sha256",
                },
                "positive_receipt": {
                    "path": ".factory/proofs/positive.json",
                    "sha256": "replace-with-lowercase-sha256",
                },
                "negative_receipt": {
                    "path": ".factory/proofs/negative.json",
                    "sha256": "replace-with-lowercase-sha256",
                },
            },
            "human_review": {
