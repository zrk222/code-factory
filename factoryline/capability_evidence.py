...
def _existing_evidence_file(
    ...
    try:
        ...
        raise CapabilityEvidenceError(
            "E_CAPABILITY_EVIDENCE_PATH", f"{field} escapes the workspace"
        ) from exc
    if not path.is_file():
        raise CapabilityEvidenceError(
...
def _read_evidence_file(path: Path, value: object, field: str) -> bytes:
    ...
    if len(content) > 10_000_000:
        raise CapabilityEvidenceError(
            "E_CAPABILITY_EVIDENCE_SIZE", f"{field} exceeds 10000000 bytes"
        )
    if not content:
        raise CapabilityEvidenceError(
...
def _argv(value: object, capability_id: str, tests: list[dict[str, Any]]) -> list[str]:
    if (
        ...
        or not value
        or not all(isinstance(arg, str) and arg for arg in value)
    ):
        raise CapabilityEvidenceError(
            "E_CAPABILITY_EVIDENCE_COMMAND",
            f"{capability_id}.verify.argv must be a non-empty string array",
        )
    expected = ["python", "-m", "pytest", "-q", *[item["path"] for item in tests]]
    if any(
        item["path"].startswith("-") or not item["path"].endswith(".py")
        for item in tests
    ):
        raise CapabilityEvidenceError(
            "E_CAPABILITY_EVIDENCE_COMMAND",
            "Test paths must be Python files, not pytest options",
        )
    if value != expected:
        raise CapabilityEvidenceError(
            "E_CAPABILITY_EVIDENCE_COMMAND",
            f"{capability_id}.verify.argv must run exactly its declared test files through python -m pytest -q",
        )
...
def _claim_identity_and_maturity(
    ...
    if not isinstance(capability_id, str) or not capability_id or capability_id in seen:
        raise CapabilityEvidenceError(
            "E_CAPABILITY_EVIDENCE_ID",
            f"capabilities[{index}].id must be unique and non-empty",
        )
...
def _claim_files(
    ...
    if (
        ...
        or not 1 <= len(implementation) <= 64
        or not isinstance(tests, list)
        or not 1 <= len(tests) <= 64
    ):
        raise CapabilityEvidenceError(
            "E_CAPABILITY_EVIDENCE_SCHEMA",
            f"{capability_id} requires implementation and tests",
        )
    files = {
        "implementation": [
            _file(workspace, path, f"{capability_id}.implementation")
...
def _claim_verification(
    ...
    if not isinstance(verify, dict):
        raise CapabilityEvidenceError(
            "E_CAPABILITY_EVIDENCE_COMMAND", f"{capability_id}.verify must be an object"
        )
...
def _execute(workspace: Path, claim: dict[str, Any]) -> dict[str, Any]:
    ...
    with tempfile.TemporaryDirectory(prefix="factory-capability-") as scratch:
        observed = run_bounded_command(
            claim["_argv"], workspace, claim["_timeout"], Path(scratch)
        )
    passed = (
...
def _evidence_unchanged(workspace: Path, claim: dict[str, Any]) -> bool:
    try:
        return all(
            _file(workspace, item["path"], claim["id"])["sha256"] == item["sha256"]
            for group in ("implementation", "tests")
            for item in claim[group]
        )
...
def _manifest(workspace: Path, source: dict[str, Any]) -> dict[str, Any]:
    try:
        ...
        if hashlib.sha256(data).hexdigest() != source["sha256"]:
            raise CapabilityEvidenceError(
                "E_CAPABILITY_EVIDENCE_CHANGED", "Manifest changed before validation"
            )
        raw = json.loads(data.decode("utf-8"))
    except (UnicodeError, json.JSONDecodeError) as exc:
        raise CapabilityEvidenceError(
            "E_CAPABILITY_EVIDENCE_SCHEMA", "manifest is not valid UTF-8 JSON"
        ) from exc
    if (
        not isinstance(raw, dict)
        or raw.get("schema") != SCHEMA
        or not isinstance(raw.get("capabilities"), list)
        or not 1 <= len(raw["capabilities"]) <= 64
    ):
        raise CapabilityEvidenceError(
            "E_CAPABILITY_EVIDENCE_SCHEMA",
            f"manifest must use {SCHEMA} with non-empty capabilities",
        )
    return raw
...
def _report(
    ...
    marker = (
        ...
        if passed
        else ("CAPABILITY_EVIDENCE_BLOCKED" if execute else "CAPABILITY_EVIDENCE_BOUND")
    )
    public_claims = [
    ...
    findings = [
        {"code": "E_CAPABILITY_EVIDENCE_CHECK_FAILED", "id": item["id"]}
        for item in executions
        if not item["passed"]
    ]
    return {
        "schema": "factory.capability-evidence-audit.v1",
        "marker": marker,
        "ok": passed if execute else True,
        "executed": execute,
        ...
        "claim_boundary": "Structural binding and local test execution are not independent battle-testing, production proof, certification, or customer validation. Execution runs reviewed repository code as the caller, not in a sandbox.",
    }


def audit_capability_evidence(
    root: Path, manifest_path: Path, *, execute: bool = False
) -> dict[str, Any]:
