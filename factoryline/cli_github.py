...
def run(a) -> int:
    ...
    try:
        if a.github_cmd == "policy-snapshot":
            ...
            payload = compile_github_plan_proof_review(Path(a.root), Path(a.plan), base=a.base, changed=a.changed or None, head_sha=a.head_sha)
        else:
            payload = compile_github_proof_review(Path(a.root), base=a.base, changed=a.changed or None, head_sha=a.head_sha)
    ...
    except (PlanProofReviewError, GitHubProofReviewError, GitHubPlanProofReviewError, GitHubAssuranceDossierError, OSError, UnicodeDecodeError, json.JSONDecodeError) as exc:
        assurance = a.github_cmd in {"policy-snapshot", "assurance-dossier"}
        fallback = "GITHUB_ASSURANCE_INPUT_INVALID" if assurance else "GITHUB_PLAN_PROOF_REVIEW_INPUT_INVALID" if a.github_cmd == "plan-proof-review" else "GITHUB_PROOF_REVIEW_INPUT_INVALID"
        error = {
