...
def _read_yaml(root: Path, path: Path) -> dict[str, Any]:
    ...
    if source.stat().st_size > MAX_INPUT_BYTES:
        raise RevenueForgeError(
            "REVENUEFORGE_INPUT_TOO_LARGE", "products input exceeds 1 MiB"
        )
    try:
        value = yaml.safe_load(source.read_text(encoding="utf-8-sig"))
    except (OSError, UnicodeDecodeError, yaml.YAMLError) as exc:
        raise RevenueForgeError(
            "REVENUEFORGE_INPUT_INVALID", "products input is not valid YAML"
        ) from exc
    if not isinstance(value, dict):
        raise RevenueForgeError(
            "REVENUEFORGE_INPUT_INVALID", "products input must be an object"
        )
    return value
...
def _product_type(raw: dict[str, Any], product_id: str) -> tuple[str, Any]:
    ...
    if product_type not in ALLOWED_PRODUCT_TYPES:
        raise RevenueForgeError(
            "REVENUEFORGE_MANIFEST_INVALID", f"unsupported product type: {product_type}"
        )
...
def _product_entitlements(raw: dict[str, Any], product_id: str) -> list[str]:
    ...
    if (
        ...
        or not entitlements
        or not all(isinstance(v, str) and v.strip() for v in entitlements)
    ):
        raise RevenueForgeError(
            "REVENUEFORGE_MANIFEST_INVALID",
...
def _product_offers(raw: dict[str, Any], product_id: str) -> list[dict[str, str]]:
    ...
    for offer in raw.get("offers", []):
        if (
            ...
            or offer.get("type") not in ALLOWED_OFFER_TYPES
            or offer.get("duration") not in ALLOWED_DURATIONS
        ):
            raise RevenueForgeError(
                "REVENUEFORGE_MANIFEST_INVALID", f"{product_id} has an invalid offer"
            )
        offers.append(
            {
                "id": _required_text(offer.get("id"), "offer.id"),
                "type": offer["type"],
...
def _normalize_product(raw: object, seen: set[str]) -> dict[str, Any]:
    ...
    return {
        ...
        "display_name": _required_text(
            raw.get("display_name"), f"{product_id}.display_name"
        ),
        "type": product_type,
...
def _sections(
    ...
    if not isinstance(products, list) or not products or len(products) > MAX_PRODUCTS:
        raise RevenueForgeError(
            "REVENUEFORGE_MANIFEST_INVALID",
            f"products must contain 1-{MAX_PRODUCTS} entries",
        )
    if not all(isinstance(value, dict) for value in (paywall, legal, privacy)):
        raise RevenueForgeError(
            "REVENUEFORGE_MANIFEST_INVALID",
            "paywall, legal, and privacy objects are required",
        )
...
) -> tuple[dict[str, bool], dict[str, bool], dict[str, bool], set[str]]:
    patterns = set(paywall.get("patterns", []))
    if patterns & FORBIDDEN_PATTERNS:
        raise RevenueForgeError(
            "REVENUEFORGE_DARK_PATTERN_REJECTED",
            "dark-pattern paywall controls are not permitted",
        )
    if any(
        raw.get(field)
        for field in ("ad_sdk", "crypto_payments", "alternative_payments")
    ):
        raise RevenueForgeError(
            "REVENUEFORGE_PAYMENT_LANE_REJECTED",
            "ads, crypto, and alternative payments are outside this safe lane",
        )
    ui = {
        "value_before_price": paywall.get("value_before_price") is True,
        "price_and_duration_before_cta": paywall.get("price_and_duration_before_cta")
        is True,
        "single_primary_cta": paywall.get("single_primary_cta") is True,
        "restore_purchases": paywall.get("restore_purchases") is True,
    }
    legal_gates = {
        "privacy_policy_url": bool(
            str(legal.get("privacy_policy_url", "")).startswith("https://")
        ),
        "terms_url": bool(str(legal.get("terms_url", "")).startswith("https://")),
    }
    privacy_gates = {
        "purchase_history_linked": privacy.get("purchase_history_linked") is True,
        "purpose_app_functionality": privacy.get("purpose") == "app_functionality",
    }
...
def validate_products(root: Path, products_path: Path) -> dict[str, Any]:
    ...
    required_ui, required_legal, required_privacy, patterns = _policy_gates(
        raw, ui, legal, privacy
    )
    seen: set[str] = set()
    products = [_normalize_product(item, seen) for item in products_raw]
    products.sort(key=lambda item: item["id"])
    gates = {
        "monetization_ready": all(required_ui.values())
        and all(required_legal.values())
        and all(required_privacy.values()),
        "subscription_disclosure": required_ui["value_before_price"]
        and required_ui["price_and_duration_before_cta"]
        and all(required_legal.values()),
    ...
    normalized = {
        ...
        "legal": {
            "privacy_policy_url": str(legal.get("privacy_policy_url", "")),
            "terms_url": str(legal.get("terms_url", "")),
        },
        "privacy": {
            "purchase_history_linked": privacy.get("purchase_history_linked") is True,
    ...
    return {
        "marker": "REVENUEFORGE_MANIFEST_VALIDATED",
        "ok": all(gates.values()),
        "manifest": normalized,
    }
...
def _evidence_html(receipt: dict[str, Any]) -> str:
    ...
    cards = "".join(
        f"<article><span>{escape(key.replace('_', ' ').title())}</span><strong>{'PASS' if value else 'BLOCKED'}</strong></article>"
...
def build_revenue_bundle(
    root: Path, products_path: Path, out_dir: Path
) -> dict[str, Any]:
    ...
    if not validated["ok"]:
        raise RevenueForgeError(
            "REVENUEFORGE_GATES_BLOCKED",
            "manifest does not pass all deterministic monetization gates",
        )
    ...
    dataflow = {
        "schema": "factory.dataflow.monetization.v1",
        "monetization": {
            "kind": "purchase_history",
            "linked_to_identity": True,
            "purpose": "app_functionality",
            "source": "StoreKit",
            "recipients": ["app_developer", "Apple"],
        },
    }
    receipt = {
        "schema": SCHEMA,
        "marker": "REVENUEFORGE_BUNDLE_WRITTEN",
        "markers": [
            "REVENUEFORGE_SERVER_SCAFFOLD_WRITTEN",
            "REVENUEFORGE_PAYWALL_SCAFFOLD_WRITTEN",
        ],
        "manifest": manifest,
        "artifacts": {
            key: path.relative_to(workspace).as_posix() for key, path in files.items()
        },
        "authority": AUTHORITY,
        "claim_boundary": "generated scaffold and deterministic local checks; not Apple approval, deployed entitlement-server proof, legal advice, or observed revenue",
    }
        ...
        json.dumps(
            {
                ...
                "requires_human_app_store_connect_confirmation": True,
            },
            indent=2,
            sort_keys=True,
        )
...
def plan_growth(root: Path, products_path: Path, growth_path: Path) -> dict[str, Any]:
    ...
    if not isinstance(experiments, list):
        raise RevenueForgeError(
            "REVENUEFORGE_GROWTH_INVALID", "experiments must be a list"
        )
    normalized_experiments = []
    for experiment in experiments:
        if not isinstance(experiment, dict):
            raise RevenueForgeError(
                "REVENUEFORGE_GROWTH_INVALID", "each experiment must be an object"
            )
        treatments = experiment.get("treatments", [])
        if (
            not isinstance(treatments, list)
            or not 1 <= len(treatments) <= MAX_EXPERIMENT_TREATMENTS
        ):
            raise RevenueForgeError(
                "REVENUEFORGE_EXPERIMENT_REJECTED",
                "each PPO experiment requires 1-3 treatments",
            )
        normalized_experiments.append(
            {
                "id": _required_text(experiment.get("id"), "experiment.id"),
    ...
    result = {
        ...
        "custom_product_pages": sorted(
            raw.get("custom_product_pages", []),
            key=lambda item: str(item.get("id", "")),
        )
        if isinstance(raw.get("custom_product_pages", []), list)
        else [],
        "offers": [
            {**offer, "send": "human_required"}
            for offer in offers
            if isinstance(offer, dict)
        ],
        "ratings_ops": {
            "prompt_after_success_only": True,
            "draft_response_only": True,
            "publication": "human_required",
        },
        "localization": {
            "storefront_price_schedule": "proposal_only",
            "human_confirmation": True,
        },
        "aso": {
            "keyword_refresh": "proposal_only",
            "no_rank_claim_without_observation": True,
        },
        "android_lane": {
            "status": "planned",
            "ruleset": "google-play-separate",
            "ios_policy_reuse_forbidden": True,
        },
        "authority": AUTHORITY,
    }
...
def benchmark_cell(records: list[dict[str, Any]]) -> dict[str, Any]:
    ...
    if not isinstance(records, list):
        raise RevenueForgeError(
            "REVENUEFORGE_BENCHMARK_INVALID", "records must be a list"
        )
    by_app: dict[str, float] = {}
    for record in records:
        if not isinstance(record, dict) or isinstance(record.get("value"), bool):
            raise RevenueForgeError(
                "REVENUEFORGE_BENCHMARK_INVALID",
                "each record needs app_id and numeric value",
            )
        app_id = _required_text(record.get("app_id"), "record.app_id")
        value = record.get("value")
        if not isinstance(value, (int, float)):
            raise RevenueForgeError(
                "REVENUEFORGE_BENCHMARK_INVALID", "record.value must be numeric"
            )
        by_app[app_id] = float(value)
    if len(by_app) < MIN_BENCHMARK_CELL:
        return {
            "schema": "factory.revenueforge.benchmark.v1",
            "marker": "REVENUEFORGE_BENCHMARK_WITHHELD",
            "published": False,
            "contributor_count": len(by_app),
            "minimum": MIN_BENCHMARK_CELL,
            "median": None,
        }
    ...
    return {
        "schema": "factory.revenueforge.benchmark.v1",
        "marker": "REVENUEFORGE_BENCHMARK_PUBLISHED",
        "published": True,
        "contributor_count": len(by_app),
        "minimum": MIN_BENCHMARK_CELL,
        "median": median,
...
def revenueforge_projection(root: Path) -> dict[str, Any]:
    ...
    workspace = Path(root).resolve()
    candidates = sorted(
        (workspace / ".factory" / "revenueforge").glob("*/revenueforge.json")
    )[:100]
    current = 0
    invalid = 0
    ...
    return {
        "marker": "GRAPH_OPS_REVENUEFORGE_READ_ONLY",
        "current_count": current,
        "invalid_count": invalid
        + evidence["invalid_count"]
        + integrity["invalid_count"],
        "latest": latest,
        "evidence": evidence,
        "integrity": integrity,
        "authority": AUTHORITY,
