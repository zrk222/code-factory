...
def _workspace_file(root: Path, value: object, field: str) -> Path:
    ...
    if not candidate.is_file():
        _reject(
            "E_TEAM_PILOT_EVIDENCE_PATH", f"{field} must name an existing regular file"
        )
    return candidate


def _load_json(
    root: Path, manifest_path: Path, *, code: str, label: str
...
def _text(
    ...
    *,
    ...
    if len(result) > limit or "\x00" in result:
        _reject(
            "E_TEAM_PILOT_SCHEMA",
            f"{field} must be at most {limit} characters and contain no NUL byte",
        )
...
def _validate_packaging(root: Path) -> dict[str, str | bool]:
    ...
    package, _, digest = _load_json(
        root,
        path,
        code="E_TEAM_PILOT_COMMERCIAL_BOUNDARY",
...
def _validate_manifest_header(source: dict[str, Any]) -> int:
    ...
    if set(source) != allowed:
        _reject(
            "E_TEAM_PILOT_SCHEMA",
            "manifest must contain exactly schema, pilot_id, owner, partner_count, governance, delivery_mode, and evidence",
        )
    ...
    if (
        isinstance(partner_count, bool)
        or not isinstance(partner_count, int)
        or not 1 <= partner_count <= 3
    ):
        _reject(
            "E_TEAM_PILOT_PARTNER_CAP",
            "partner_count must be an integer from 1 through 3",
        )
    ...
    if source.get("delivery_mode") != "customer_managed_reference":
        _reject(
            "E_TEAM_PILOT_DELIVERY_MODE",
            "delivery_mode must be customer_managed_reference; managed service delivery is not offered",
        )
...
def _validate_evidence_item(root: Path, item: object, index: int) -> dict[str, str]:
    if not isinstance(item, dict) or set(item) != {"kind", "path", "sha256"}:
        _reject(
            "E_TEAM_PILOT_SCHEMA",
            f"evidence[{index}] must contain exactly kind, path, and sha256",
        )
    kind = item.get("kind")
    if not isinstance(kind, str) or kind not in REQUIRED_EVIDENCE_KINDS:
        _reject(
            "E_TEAM_PILOT_EVIDENCE_KIND",
            f"evidence[{index}].kind must be a required readiness evidence kind",
        )
    evidence_path = _workspace_file(root, item.get("path"), f"evidence[{index}].path")
    digest = item.get("sha256")
    if not isinstance(digest, str) or not _SHA256.fullmatch(digest):
        _reject(
            "E_TEAM_PILOT_EVIDENCE_DIGEST",
    ...
    if actual != digest:
        _reject(
            "E_TEAM_PILOT_EVIDENCE_DIGEST",
            f"evidence[{index}] does not match the declared SHA-256 digest",
        )
...
def _validate_evidence(root: Path, evidence: object) -> list[dict[str, str]]:
    if not isinstance(evidence, list) or len(evidence) != len(REQUIRED_EVIDENCE_KINDS):
        _reject(
            "E_TEAM_PILOT_EVIDENCE_KIND",
            "evidence must contain exactly one item for every required readiness evidence kind",
        )
    ...
    if kinds != REQUIRED_EVIDENCE_KINDS:
        _reject(
            "E_TEAM_PILOT_EVIDENCE_KIND",
            "evidence does not cover every required readiness evidence kind",
        )
...
def validate_team_pilot_manifest(root: Path, manifest_path: Path) -> dict[str, Any]:
    ...
    source, path, manifest_sha256 = _load_json(
        workspace, manifest_path, code="E_TEAM_PILOT_SCHEMA", label="manifest"
    )
    partner_count = _validate_manifest_header(source)
    return {
        "schema": TEAM_PILOT_MANIFEST_SCHEMA,
        "pilot_id": _text(
            source.get("pilot_id"), "pilot_id", pattern=_PILOT_ID, limit=63
        ),
...
def _markdown(receipt: dict[str, Any]) -> str:
    ...
    return "\n".join(
        [
            ...
            "",
            *[
                f"- `{entry['kind']}`: `{entry['path']}` (`{entry['sha256']}`)"
...
def _validate_receipt_shape(value: object) -> dict[str, Any]:
    if not isinstance(value, dict) or value.get("schema") != TEAM_PILOT_RECEIPT_SCHEMA:
        _reject(
            "E_TEAM_PILOT_RECEIPT_INVALID",
            f"a {TEAM_PILOT_RECEIPT_SCHEMA} payload is required",
        )
    if set(value) != RECEIPT_FIELDS:
        _reject(
            "E_TEAM_PILOT_RECEIPT_INVALID", "receipt has unsupported or missing fields"
        )
    return value
...
def _validate_receipt_core(value: dict[str, Any]) -> None:
    if (
        value["marker"] != "TEAM_PILOT_READY_FOR_OWNER_REVIEW"
        or value["verdict"] != "READY_FOR_OWNER_REVIEW"
    ):
        _reject(
            "E_TEAM_PILOT_RECEIPT_INVALID",
            "receipt must preserve the owner-review-only terminal state",
        )
    ...
    core = {
        key: value[key]
        for key in RECEIPT_FIELDS - {"receipt_sha256", "mermaid", "receipt_markdown"}
    }
...
def _validate_receipt_views(value: dict[str, Any]) -> None:
    if value["evidence"] != value["manifest"].get("evidence"):
        _reject(
            "E_TEAM_PILOT_RECEIPT_INVALID",
            "receipt evidence must match the normalized manifest evidence",
        )
...
def write_team_pilot_artifacts(
