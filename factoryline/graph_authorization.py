...
from .reality_check import (
    RealityCheckError,
    run_reality_check,
    validate_reality_check_receipt,
...
def _text(value: object, field: str, *, identifier: bool = False) -> str:
    if not isinstance(value, str) or not value.strip() or len(value.strip()) > MAX_TEXT:
        _reject(
            ...
            f"{field} must be a non-empty string of at most {MAX_TEXT} characters",
        )
    text = value.strip()
...
def _relative_existing(root: Path, value: object, field: str) -> tuple[Path, str]:
    if not isinstance(value, str) or not value.strip():
        _reject(
            "GRAPH_AUTHORIZATION_INVALID", f"{field} must be a workspace-relative file"
        )
    raw = value.replace("\\", "/").strip()
    if raw.startswith("/") or re.match(r"^[A-Za-z]:/", raw) or ".." in raw.split("/"):
        _reject(
            "GRAPH_AUTHORIZATION_INVALID", f"{field} must stay inside the workspace"
        )
    ...
    try:
        ...
        _reject(
            "GRAPH_AUTHORIZATION_INVALID", f"{field} resolves outside the workspace"
        )
    if not path.is_file():
...
def _expiry(value: object, now: datetime) -> str:
    ...
    if not now < parsed <= now + timedelta(days=7):
        _reject(
            "GRAPH_AUTHORIZATION_INVALID",
            "expires_at must be after now and no more than seven days away",
        )
...
def _load(path: Path) -> dict[str, Any]:
    try:
        ...
        _reject(
            "GRAPH_AUTHORIZATION_INVALID",
            f"authorization cannot be read as JSON: {exc}",
        )
    if not isinstance(value, dict):
        _reject(
            "GRAPH_AUTHORIZATION_INVALID", "authorization must contain one JSON object"
        )
    return value
...
def _authorization_lock(path: Path):
    ...
    try:
        ...
        _reject(
            "GRAPH_AUTHORIZATION_IN_PROGRESS",
            "authorization is already executing; inspect its local lock before any manual recovery",
        )
    try:
        yield
    finally:
...
def _node_binding(root: Path, node_id: str, action: str) -> dict[str, Any]:
    ...
    node = next(
        (item for item in graph_ops_snapshot(root)["nodes"] if item["id"] == node_id),
        None,
    )
    if node is None:
        _reject(
            "GRAPH_AUTHORIZATION_TARGET_INVALID",
            "selected Graph Ops node is not present in the current local graph",
        )
    if action == "reality_check_execution":
        if node.get("kind") != "reality_check" or node.get("status") != "verified":
            _reject(
                "GRAPH_AUTHORIZATION_TARGET_INVALID",
                "only a verified Reality Check may authorize one re-run",
            )
        source_path, source = _relative_existing(
            root, node.get("source"), "selected Reality Check source"
        )
        try:
    ...
    if (
        node.get("kind") != "repair_candidate"
        or node.get("status") != "winner"
        or node.get("facts", {}).get("eligible") is not True
    ):
        _reject(
            "GRAPH_AUTHORIZATION_TARGET_INVALID",
            "only an eligible ProofSearch winner may receive a repair-plan review authorization",
        )
    source_path, source = _relative_existing(
        root, node.get("source"), "selected repair source"
    )
    return {
        "node_id": node_id,
...
def create_graph_authorization(
    root: Path, payload: dict[str, Any], *, now: datetime | None = None
) -> dict[str, Any]:
    ...
    if set(payload) != {
        ...
        _reject(
            "GRAPH_AUTHORIZATION_INVALID",
            "authorization request has unsupported or missing fields",
        )
    action = payload.get("action")
    if action not in _ACTIONS:
        _reject(
            "GRAPH_AUTHORIZATION_INVALID",
            "action must authorize one supported Graph Ops action",
        )
    authorization_id = _text(payload.get("id"), "id", identifier=True)
    if payload.get("confirmation") != f"AUTHORIZE {authorization_id}":
        _reject(
            "GRAPH_AUTHORIZATION_CONFIRMATION_REQUIRED",
            f"confirmation must exactly equal AUTHORIZE {authorization_id}",
        )
    ...
    core = {
        ...
        "expires_at": _expiry(payload.get("expires_at"), issued),
        "binding": binding,
    ...
    return {
        "schema": "factory.graph-ops-authorization-result.v1",
        "marker": "GRAPH_OPS_HUMAN_AUTHORIZATION_RECORDED",
        "path": str(path),
...
def _authorization_shape(value: object) -> tuple[dict[str, Any], set[str], set[str]]:
    if not isinstance(value, dict) or value.get("schema") != AUTHORIZATION_SCHEMA:
        _reject(
            "GRAPH_AUTHORIZATION_INVALID",
            f"a {AUTHORIZATION_SCHEMA} payload is required",
        )
    required = {
        "schema",
        "id",
    ...
    if (
        ...
        or value.get("state") not in {"approved", "consumed"}
    ):
        _reject(
            "GRAPH_AUTHORIZATION_INVALID",
            "authorization has unsupported fields or state",
        )
    return value, required, allowed


def _authorization_state_shape(
    value: dict[str, Any], required: set[str], allowed: set[str]
) -> None:
    if value["state"] == "approved" and set(value) != required:
        _reject(
            "GRAPH_AUTHORIZATION_INVALID",
            "an approved authorization cannot have execution fields",
        )
    if value["state"] == "consumed" and set(value) != allowed:
        _reject(
            "GRAPH_AUTHORIZATION_INVALID",
            "a consumed authorization must bind one execution receipt",
        )


def _authorization_integrity(value: dict[str, Any]) -> None:
    if not isinstance(value.get("authorization_sha256"), str) or not _SHA.fullmatch(
    ...
    if value["authority"] != _AUTHORITY[value["action"]]:
        _reject(
            "GRAPH_AUTHORIZATION_INVALID",
            "authorization authority does not match action",
        )


def _authorization_identity(value: dict[str, Any]) -> None:
    _text(value.get("id"), "id", identifier=True)
    _text(value.get("approved_by"), "approved_by")
    _text(value.get("rationale"), "rationale")
    if not isinstance(value.get("binding"), dict) or not isinstance(
        value["binding"].get("source"), str
    ):
...
def _authorization_timestamps(value: dict[str, Any]) -> None:
    try:
        ...
        _reject(
            "GRAPH_AUTHORIZATION_INVALID", "authorization timestamps must be ISO-8601"
        )
    if (
    ...
    if not issued < expires <= issued + timedelta(days=7):
        _reject(
            "GRAPH_AUTHORIZATION_INVALID",
            "authorization expiry must be after issuance and within seven days",
        )
...
def _authorization_path(root: Path, authorization_path: Path) -> Path:
    ...
    if candidate.is_absolute():
        ...
        try:
            ...
            _reject(
                "GRAPH_AUTHORIZATION_INVALID",
                "authorization must stay inside the workspace",
            )
        if not path.is_file():
            _reject(
                "GRAPH_AUTHORIZATION_INVALID", "authorization must name a readable file"
            )
        return path
...
def _active_reality_authorization(path: Path, now: datetime) -> dict[str, Any]:
    ...
    if (
        authorization["action"] != "reality_check_execution"
        or authorization["state"] != "approved"
    ):
        _reject(
            "GRAPH_AUTHORIZATION_NOT_EXECUTABLE",
            "authorization is not an unused Reality Check execution authorization",
        )
...
def _bound_reality_receipt(root: Path, authorization: dict[str, Any]) -> Path:
    ...
    source_path, _ = _relative_existing(
        root, binding.get("source"), "bound Reality Check source"
    )
    if sha256(source_path.read_bytes()).hexdigest() != binding.get("source_sha256"):
        _reject(
            "GRAPH_AUTHORIZATION_STALE",
            "bound Reality Check receipt bytes changed after approval",
        )
    try:
    ...
    if source_receipt["receipt_sha256"] != binding.get("receipt_sha256"):
        _reject(
            "GRAPH_AUTHORIZATION_STALE",
            "bound Reality Check receipt hash changed after approval",
        )
    manifest_path, _ = _relative_existing(
        root, binding.get("manifest_path"), "bound manifest"
    )
    if sha256(manifest_path.read_bytes()).hexdigest() != binding.get("manifest_sha256"):
        _reject(
            "GRAPH_AUTHORIZATION_STALE",
            "bound Reality Check manifest bytes changed after approval",
        )
...
def _consume(
    ...
    consumed_core["consumed_at"] = (
        current.replace(microsecond=0).isoformat().replace("+00:00", "Z")
    )
    consumed_core["execution_receipt"] = {
        "path": str(Path(outputs["receipt"]).resolve().relative_to(root).as_posix()),
...
def run_authorized_reality_check(
    root: Path, authorization_path: Path, *, now: datetime | None = None
) -> dict[str, Any]:
    ...
    with _authorization_lock(path):
        ...
        outputs = write_reality_check_artifacts(
            receipt, workspace / ".factory" / "reality"
        )
        _consume(path, authorization, current, receipt, outputs, workspace)
    return {
        "schema": "factory.graph-ops-execution-result.v1",
        "marker": "GRAPH_OPS_AUTHORIZED_REALITY_CHECK_EXECUTED",
