...
def record_override(
    root: Path, issue: str, *, reason: str, approved_by: str, expires: str | None = None
) -> dict:
    """Record a named, accountable override with its approver and optional expiry."""
    if not reason.strip() or not approved_by.strip():
        raise ValueError(
            "reason and approved_by are required for an auditable override"
        )
    created = datetime.now(timezone.utc).isoformat()
    payload = {
        "schema": "factory.override.v1",
        "id": uuid.uuid4().hex,
        "issue": issue,
        "reason": reason,
        ...
        "created_at": created,
        "scope_limits": [
            "Override records an exception; it does not turn a failed gate into a pass.",
            "Review expiry and ownership before release.",
        ],
    }
    ...
    receipt = Receipt(
        ...
        inputs={"issue": issue, "approved_by": approved_by},
    ).write(root)
