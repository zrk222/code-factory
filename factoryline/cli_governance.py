...
def add_parser(sub) -> None:
    ...
    judgment_reconsider = judgment_sub.add_parser(
        "reconsider", help="record a successor proposal while retaining an active Capsule"
    )
    ...
    if a.cmd == "intent":
        ...
        try:
            if a.intent_cmd == "capture":
                ...
                payload = inspect_intent_ledger(root, change_list=a.change_list, changed=a.changed or None, base=a.base)
        except (IntentLedgerError, ChangeReviewError, OSError) as exc:
            code = getattr(exc, "code", "INTENT_LEDGER_INPUT_UNAVAILABLE")
            error = {"schema": "factory.intent-ledger.error.v1", "marker": "INTENT_LEDGER_REFUSED", "code": code, "message": str(exc)}
