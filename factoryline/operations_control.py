...
def _canonical(value: object) -> bytes:
    try:
        ...
        raise OperationsControlError(
            "E_OPS_CONTROL_SCHEMA", "input must be canonical JSON"
        ) from exc
...
def _exact(value: object, fields: set[str], label: str) -> dict[str, Any]:
    if not isinstance(value, dict) or set(value) != fields:
        raise OperationsControlError(
            "E_OPS_CONTROL_SCHEMA", f"{label} fields must be exact"
        )
    return value
...
def _identifier(value: object, label: str) -> str:
    if not isinstance(value, str) or not _ID.fullmatch(value):
        raise OperationsControlError(
            "E_OPS_CONTROL_SCHEMA", f"{label} must be a safe identifier"
        )
    return value
...
def _sha256(value: object, label: str) -> str:
    if not isinstance(value, str) or not _SHA256.fullmatch(value):
        raise OperationsControlError(
            "E_OPS_CONTROL_SCHEMA", f"{label} must be a lowercase SHA-256"
        )
    return value
...
def _git_sha(value: object, label: str) -> str:
    if not isinstance(value, str) or not _GIT_SHA.fullmatch(value):
        raise OperationsControlError(
            ...
            f"{label} must be a 40-64 character lowercase Git hash",
        )
    return value
...
def _path(value: object, label: str) -> str:
    if not isinstance(value, str) or not value.strip() or len(value) > 512:
        raise OperationsControlError(
            "E_OPS_CONTROL_SCHEMA", f"{label} must be a workspace-relative path"
        )
    candidate = Path(value.replace("\\", "/"))
    if candidate.is_absolute() or ".." in candidate.parts:
        raise OperationsControlError(
            "E_OPS_CONTROL_PATH", f"{label} must remain beneath the workspace"
        )
...
def _paths(value: object, label: str, *, minimum: int = 0) -> list[str]:
    if not isinstance(value, list) or not minimum <= len(value) <= 128:
        raise OperationsControlError(
            "E_OPS_CONTROL_SCHEMA", f"{label} must contain {minimum}-128 paths"
        )
...
def _inside(
    root: Path, relative: str, *, file: bool = False, directory: bool = False
) -> Path:
    ...
    try:
        ...
        raise OperationsControlError(
            "E_OPS_CONTROL_PATH", "path escapes workspace"
        ) from exc
    if file and not target.is_file():
        raise OperationsControlError(
            "E_OPS_CONTROL_EVIDENCE", f"required file is unavailable: {relative}"
        )
    if directory and not target.is_dir():
        raise OperationsControlError(
            "E_OPS_CONTROL_COORDINATION",
            f"required repository directory is unavailable: {relative}",
        )
    return target
...
def _read_json(root: Path, relative: str, label: str) -> tuple[dict[str, Any], str]:
    ...
    try:
        ...
        raise OperationsControlError(
            "E_OPS_CONTROL_EVIDENCE", f"{label} is not readable JSON"
        ) from exc
    if not isinstance(value, dict):
        raise OperationsControlError(
            "E_OPS_CONTROL_EVIDENCE", f"{label} must be a JSON object"
        )
...
def _git(root: Path, *arguments: str) -> str:
    ...
    if completed.returncode:
        raise OperationsControlError(
            "E_OPS_CONTROL_GIT", completed.stderr.strip() or "git inspection failed"
        )
...
def _git_facts(root: Path, base: str) -> dict[str, Any]:
    if _git(root, "rev-parse", "--is-inside-work-tree") != "true":
        raise OperationsControlError(
            "E_OPS_CONTROL_GIT", "workspace must be a Git worktree"
        )
...
def _validate_manifest(root: Path, source: Path) -> tuple[dict[str, Any], str]:
    ...
    if entry["work_kind"] not in _WORK_KINDS:
        raise OperationsControlError(
            "E_OPS_CONTROL_SCHEMA", "work_kind must be bug_fix or feature"
        )
    ...
    if (
        not isinstance(isolation["expected_branch"], str)
        or not isolation["expected_branch"].strip()
        or not isinstance(isolation["require_clean"], bool)
    ):
        raise OperationsControlError(
    ...
    for name in ("max_attempts", "attempts_used", "token_budget", "observed_tokens"):
        if (
            not isinstance(reproduction[name], int)
            or isinstance(reproduction[name], bool)
            or reproduction[name] < 0
        ):
            raise OperationsControlError(
                "E_OPS_CONTROL_SCHEMA",
                f"reproduction.{name} must be a non-negative integer",
            )
    ...
    if not isinstance(envelope["purpose"], str) or not envelope["purpose"].strip():
        raise OperationsControlError(
            "E_OPS_CONTROL_SCHEMA", "change_envelope.purpose is required"
        )
    ...
    if any(_under(interface, item) for item in core) or any(
        _under(core, item) for item in interface
    ):
        raise OperationsControlError(
            "E_OPS_CONTROL_SCHEMA",
...
def _artifact_facts(root: Path, paths: list[str]) -> list[dict[str, str]]:
    return [
        {"path": path, "sha256": _file_sha(_inside(root, path, file=True))}
        for path in paths
    ]
...
def _reproduction(
    ...
    try:
        ...
        receipt_ok = (
            validated_receipt["marker"] == "E2E_POSITIVE_FAILED"
            and validated_receipt["ok"] is False
        )
    ...
    return {
        "failure_capsule": {
            "path": value["failure_capsule"],
            "sha256": capsule_sha,
            "bound": capsule_ok,
        },
        "execution_receipt": {
            "path": value["execution_receipt"],
            "sha256": receipt_sha,
            "reproduced": receipt_ok,
        },
        "budget": {
        ...
        "escalation": "human_shepherd" if blockers else None,
    }, blockers


def _tier_ok(task_kind: str, tier: str) -> bool:
    return {
        "logic": {"logs_metrics", "visual_pair", "interaction_video"},
        "visual": {"visual_pair", "interaction_video"},
        "interaction": {"interaction_video"},
...
def assess_operations_control(
    ...
    measured_lines = sum(
        item["lines"] for item in changed if isinstance(item["lines"], int)
    )
    ...
    core_changed = sorted(
        path for path in paths if _under(architecture["core_paths"], path)
    )
    interface_changed = sorted(
        path for path in paths if _under(architecture["interface_paths"], path)
    )
    unclassified = sorted(
        path
        for path in paths
        if path not in core_changed and path not in interface_changed
    )
    ...
    core = {
        ...
        "isolation": {
            ...
            "claim_boundary": "Git facts verify a local worktree state only; they do not prove a sandbox, network egress policy, VM, container, or remote runtime.",
        },
        "reproduction": reproduction,
        "change_envelope": {
            ...
            "limits": {
                ...
                "max_changed_lines": envelope["max_changed_lines"],
            },
            "changed": changed,
            "measured": {
    ...
    if not _under([RECEIPT_DIR.as_posix()], target_relative):
        raise OperationsControlError(
            "E_OPS_CONTROL_PATH", "output must remain under .factory/operations-control"
        )
...
def operations_control_projection(root: Path) -> dict[str, Any]:
        ...
        for path in sorted(directory.glob("*.json"))[:200]:
            try:
                ...
                core = {
                    key: value[key]
                    for key in value
                    if key not in {"receipt_sha256", "path"}
                }
                ...
                receipts.append(
                    {
                        ...
                        "marker": value["marker"],
                        "blocker_count": len(value.get("blockers", [])),
    ...
    return {
        ...
        "marker": "OPS_CONTROL_READ_ONLY",
        "receipt_count": len(receipts),
        "ready_count": sum(item["marker"] == "OPS_CONTROL_READY" for item in receipts),
        "blocked_count": sum(
            item["marker"] == "OPS_CONTROL_BLOCKED" for item in receipts
        ),
        ...
        "invalid": invalid[:100],
        "authority": dict(AUTHORITY),
        "claim_boundary": "Read-only local operations-control facts. The projection does not execute a reproduction, create a worktree, dispatch an agent, or authorize a merge or release.",
    }
...
def operations_control_template() -> dict[str, Any]:
    ...
    return {
        "schema": "factory.operations-control-template.v1",
        "manifest_schema": MANIFEST_SCHEMA,
        "authority": dict(AUTHORITY),
        "claim_boundary": "Template only. Fill every digest and receipt path from local reviewed evidence; it creates no worktree and performs no provider or agent action.",
        "manifest": {
            "schema": MANIFEST_SCHEMA,
            "id": "replace-with-control-id",
            "work_kind": "bug_fix",
            "base": "replace-with-base-revision",
            "scope_paths": ["src"],
            "isolation": {
                "expected_branch": "replace-with-branch",
                "expected_base_sha": "replace-with-40-or-64-char-lowercase-git-hash",
                "require_clean": True,
            },
            "reproduction": {
            ...
            "change_envelope": {
                "purpose": "replace-with-one-reviewable-purpose",
            ...
            "evidence": {
                "task_kind": "logic",
                "tier": "logs_metrics",
                "artifacts": ["replace-with-local-evidence-file"],
            },
            "architecture": {
                "core_paths": ["src"],
                "interface_paths": ["web"],
                "core_check_receipts": ["replace-with-local-core-check"],
            },
            "coordination": {
                "repositories": [
                    {
                        "id": "primary",
                        "path": ".",
                        "expected_head_sha": "replace-with-40-or-64-char-lowercase-git-hash",
                        "dependencies": [],
                    }
                ]
            },
