...
def _inside(root: Path, raw: str, label: str) -> tuple[Path, str]:
    ...
    try:
        ...
        raise SeniorAssuranceError(
            "E_REPLAY_PATH_ESCAPE", f"{label} escapes the workspace"
        ) from exc
    current = base
    for part in relative.parts:
        ...
        if current.is_symlink():
            raise SeniorAssuranceError(
                "E_REPLAY_SYMLINK",
                f"{label} traverses a symlink: {relative.as_posix()}",
            )
...
def _file_digest(path: Path, label: str) -> str:
    if path.is_symlink() or not path.is_file():
        raise SeniorAssuranceError(
            "E_REPLAY_INPUT", f"{label} is not a regular file: {path}"
        )
    raw = path.read_bytes()
    if len(raw) > MAX_ARTIFACT_BYTES * 16:
        raise SeniorAssuranceError(
            "E_REPLAY_INPUT_SIZE", f"{label} exceeds the bounded input size"
        )
...
def _tree_snapshot(source: Path) -> list[dict[str, Any]]:
    if source.is_symlink() or not source.is_dir():
        raise SeniorAssuranceError(
            "E_REPLAY_SOURCE", f"source_root is not a regular directory: {source}"
        )
    ...
    for candidate in sorted(source.rglob("*")):
        ...
        if candidate.is_symlink():
            raise SeniorAssuranceError(
                "E_REPLAY_SYMLINK", f"source tree contains a symlink: {candidate}"
            )
        ...
        if len(rows) >= MAX_FILES or total > MAX_TREE_BYTES:
            raise SeniorAssuranceError(
                "E_REPLAY_SOURCE_LIMIT",
                "source tree exceeds the bounded file or byte limit",
            )
        rows.append(
            {
                "path": candidate.relative_to(source).as_posix(),
                "sha256": _file_digest(candidate, "source file"),
                "size": int(size),
            }
        )
    if not rows:
        raise SeniorAssuranceError(
            "E_REPLAY_SOURCE_EMPTY",
            "source_root must contain at least one regular file",
        )
    return rows
...
def _descriptor_digest(root: Path, value: object, label: str) -> str:
    ...
    if not isinstance(value, dict):
        raise SeniorAssuranceError(
            "E_REPLAY_DIGEST", f"{label} must be a digest or descriptor"
        )
    try:
    ...
    if "path" in value:
        ...
        if observed != digest:
            raise SeniorAssuranceError(
                "E_REPLAY_DIGEST_MISMATCH", f"{label}.path digest does not match"
            )
    return digest
...
def _validate_argv(value: object) -> list[str]:
    ...
    if any(SECRET_NAME.search(item) for item in value):
        raise SeniorAssuranceError(
            "E_REPLAY_SECRET", "argv contains secret-shaped material"
        )
    return list(value)
...
def validate_replay_manifest(root: Path, value: dict[str, Any]) -> dict[str, Any]:
    ...
    try:
        ...
        if value.get("schema") != REPLAY_MANIFEST_SCHEMA:
            raise RuntimeAuditError(
                "E_REPLAY_SCHEMA", "schema must be factory.replay-manifest.v1"
            )
        ...
        dependencies_sha256 = _descriptor_digest(
            root, value["dependencies_sha256"], "dependencies_sha256"
        )
        policy_sha256 = _descriptor_digest(
            root, value["policy_sha256"], "policy_sha256"
        )
        input_files = value["input_files"]
        if not isinstance(input_files, list) or len(input_files) > MAX_FILES:
            raise RuntimeAuditError(
                "E_REPLAY_INPUT", "input_files must be a bounded list"
            )
        inputs: list[dict[str, Any]] = []
        for index, item in enumerate(input_files):
            if not isinstance(item, dict):
                raise RuntimeAuditError(
                    "E_REPLAY_INPUT", f"input_files[{index}] must be an object"
                )
            ...
            if observed != digest:
                raise RuntimeAuditError(
                    ...
                    f"input_files[{index}] digest does not match",
                )
            inputs.append({"path": relative, "sha256": digest})
        argv = _validate_argv(value["argv"])
        env = _validate_env(value.get("env"))
        expected_exit = require_int(
            value["expected_exit"], "expected_exit", minimum=-255, maximum=255
        )
        timeout_seconds = require_int(
            value["timeout_seconds"], "timeout_seconds", minimum=1, maximum=300
        )
        max_output_bytes = require_int(
            value["max_output_bytes"], "max_output_bytes", minimum=1, maximum=1_048_576
        )
    ...
    if observed_source_sha256 != source_sha256:
        raise SeniorAssuranceError(
            "E_REPLAY_SOURCE_CHANGED", "source_sha256 does not match source_root"
        )
    return {
...
def _capture_process(source: Path, normalized: dict[str, Any]) -> dict[str, Any]:
    ...
    if (
        ...
        and outcome["observed_exit"] != normalized["expected_exit"]
    ):
        outcome["failure_reason"] = (
            f"EXIT_MISMATCH:expected={normalized['expected_exit']}:observed={outcome['observed_exit']}"
        )
    outcome["state"] = (
...
def _verify_replay_inputs(root: Path, source: Path, normalized: dict[str, Any]) -> None:
    ...
    if source_after != normalized["source_sha256"]:
        raise SeniorAssuranceError(
            "E_REPLAY_SOURCE_CHANGED", "source changed during replay"
        )
    for item in normalized["input_files"]:
        path = root / item["path"]
        if _file_digest(path, "input") != item["sha256"]:
            raise SeniorAssuranceError(
                "E_REPLAY_INPUT_CHANGED", f"input changed during replay: {item['path']}"
            )
...
def _replay_base(normalized: dict[str, Any]) -> dict[str, Any]:
    return {
        ...
        "marker": "REPLAY_SOURCE_AND_POLICY_BOUND",
        "replay_id": normalized["replay_id"],
        "source_root": normalized["source_root"],
        "source_sha256": normalized["source_sha256"],
        "dependencies_sha256": normalized["dependencies_sha256"],
        "policy_sha256": normalized["policy_sha256"],
        "input_files": normalized["input_files"],
        "argv": normalized["argv"],
        "argv_sha256": _sha(normalized["argv"]),
        "contract_sha256": normalized["contract_sha256"],
        "expected_exit": normalized["expected_exit"],
        "isolation": "fresh_temporary_workspace_and_process",
        "sandbox_boundary": "process_boundary_only; no_kernel_or_container_claim",
        "authority": "none",
    ...
    *,
    ...
    receipt = {
        **base,
        ...
        "observed_exit": outcome["observed_exit"],
        "stdout_sha256": sha256_bytes(outcome["stdout"]),
        "stderr_sha256": sha256_bytes(outcome["stderr"]),
        "stdout_preview": outcome["stdout"].decode("utf-8", errors="replace")[
            :MAX_PREVIEW
        ],
        "stderr_preview": outcome["stderr"].decode("utf-8", errors="replace")[
            :MAX_PREVIEW
        ],
...
def _validate_repair_manifest(root: Path, value: dict[str, Any]) -> dict[str, Any]:
    try:
        ...
        exact_keys(
            ...
            {"expectations_changed", "review", "description", "authority"},
        )
        if value.get("schema") != REPAIR_SCHEMA:
            raise RuntimeAuditError(
                "E_REPAIR_SCHEMA", "schema must be factory.repair-comparison.v1"
            )
        comparison_id = require_str(
            value["comparison_id"], "comparison_id", maximum=128
        )
        contract_sha256 = require_digest(value["contract_sha256"], "contract_sha256")
        negative_controls = value["negative_controls"]
        if (
            not isinstance(negative_controls, list)
            or not 1 <= len(negative_controls) <= 32
        ):
            raise RuntimeAuditError(
                "E_REPAIR_CONTROLS", "negative_controls must contain 1..32 manifests"
            )
    except RuntimeAuditError as exc:
        raise SeniorAssuranceError(exc.code, exc.message) from exc
    manifests = {
    ...
    manifests["negative_controls"] = [
        validate_replay_manifest(root, item) for item in negative_controls
    ]
    if (
        manifests["buggy"]["contract_sha256"] != contract_sha256
        or manifests["fixed"]["contract_sha256"] != contract_sha256
        or any(
            item["contract_sha256"] != contract_sha256
            for item in manifests["negative_controls"]
        )
    ):
        raise SeniorAssuranceError(
            "E_REPAIR_CONTRACT",
            "all replay manifests must bind the same approved contract",
        )
    if manifests["buggy"]["source_sha256"] == manifests["fixed"]["source_sha256"]:
        raise SeniorAssuranceError(
            "E_REPAIR_NO_CHANGE", "fixed candidate must differ from the original source"
        )
    if (
        manifests["buggy"]["expected_exit"] == 0
        or manifests["fixed"]["expected_exit"] != 0
        or any(item["expected_exit"] == 0 for item in manifests["negative_controls"])
    ):
        raise SeniorAssuranceError(
    ...
    if not isinstance(changed, bool):
        raise SeniorAssuranceError(
            "E_REPAIR_REVIEW", "expectations_changed must be boolean"
        )
...
def compare_repair(
    ...
    *,
    ...
    ids = {
        key: item["replay_id"]
        for key, item in normalized["manifests"].items()
        if isinstance(item, dict) and "replay_id" in item
    }
    base = {
        "schema": REPAIR_SCHEMA,
        "marker": "REPAIR_COMPARISON_SEALED",
        "comparison_id": normalized["comparison_id"],
        "contract_sha256": normalized["contract_sha256"],
        "expectations_changed": normalized["expectations_changed"],
    ...
    if not execute:
        receipt = {
            **base,
            ...
            "executed": False,
            "replays": ids,
            "original_failure_required": True,
            "negative_controls_required": len(
                normalized["manifests"]["negative_controls"]
            ),
        }
        receipt["receipt_sha256"] = _sha(receipt)
        return _write_json(out, receipt)
    ...
    controls = [
        run_replay(root, item, execute=True)
        for item in normalized["manifests"]["negative_controls"]
    ]
    passed = (
        buggy["state"] == "PASS"
        and fixed["state"] == "PASS"
        and all(item["state"] == "PASS" for item in controls)
    )
    findings: list[dict[str, Any]] = []
    if buggy["observed_exit"] == buggy["expected_exit"] == 0:
        findings.append(
            {
                "id": "ORIGINAL_DID_NOT_FAIL",
                "summary": "original candidate did not reproduce the expected failure",
                "evidence": buggy["receipt_sha256"],
            }
        )
    if fixed["state"] != "PASS":
        findings.append(
            {
                "id": "REPAIR_DID_NOT_PASS",
                "summary": "proposed repair did not satisfy the approved expectation",
                "evidence": fixed["receipt_sha256"],
            }
        )
    for item in controls:
        if item["state"] != "PASS":
            findings.append(
                {
                    "id": "NEGATIVE_CONTROL_WEAKENED",
                    "summary": "a negative control no longer fails as expected",
                    "evidence": item["receipt_sha256"],
                }
            )
    receipt = {
        **base,
        ...
        "findings": findings,
        "regression_checks": {
            "original_fails": buggy["state"] == "PASS",
            "repaired_passes": fixed["state"] == "PASS",
            "negative_controls_fail": all(item["state"] == "PASS" for item in controls),
        },
    }
    receipt["receipt_sha256"] = _sha(receipt)
    return _write_json(out, receipt)
...
def _reuse_gate(
    ...
    if not isinstance(read_only, bool) or not isinstance(side_effects, bool):
        raise SeniorAssuranceError(
            "E_REUSE_GATE", f"{gate_id} must declare read_only and side_effects"
        )
...
def explain_evidence_reuse(
    root: Path, manifest: dict[str, Any], *, out: Path | None = None
) -> dict[str, Any]:
    ...
    try:
        ...
        exact_keys(
            manifest,
            {"schema", "request_id", "policy_sha256", "dependencies_sha256", "gates"},
            {"changed_paths", "toolchain", "environment", "authority"},
        )
        if manifest.get("schema") != REUSE_REQUEST_SCHEMA:
            raise RuntimeAuditError(
                "E_REUSE_SCHEMA", "schema must be factory.evidence-reuse-request.v1"
            )
        ...
        dependencies_sha256 = require_digest(
            manifest["dependencies_sha256"], "dependencies_sha256"
        )
    except RuntimeAuditError as exc:
        raise SeniorAssuranceError(exc.code, exc.message) from exc
    ...
    receipt = {
        ...
        "marker": "EVIDENCE_REUSE_EXPLAINED",
        ...
        "gates": explanations,
        "counts": {
            "RUN": sum(item["decision"] == "RUN" for item in explanations),
            "REUSE": sum(item["decision"] == "REUSE" for item in explanations),
            "BLOCK": sum(item["decision"] == "BLOCK" for item in explanations),
        },
        "authority": "none",
        "release_approval": False,
        "created_at": _now(),
...
def _brief_findings(
    receipt: dict[str, Any], evidence_ref: str | None
) -> list[dict[str, Any]]:
    findings = (
        receipt.get("findings") if isinstance(receipt.get("findings"), list) else []
    )
    if not findings and receipt.get("failure_reason"):
        findings = [
            {
                "id": "REPLAY_FAILURE",
                "summary": str(receipt["failure_reason"]),
                "evidence": receipt.get("receipt_sha256"),
            }
        ]
    if not findings and receipt.get("state", receipt.get("decision")) in {
        "FAIL",
        "BLOCKED",
        "MISMATCH",
        "INVALID",
        ...
        findings = [
            {
                "id": "FAILED_RECEIPT",
                "summary": f"receipt decision is {receipt.get('state', receipt.get('decision', 'UNKNOWN'))}",
                "evidence": receipt.get("receipt_sha256"),
            }
        ]
    rows = []
    for index, item in enumerate(findings):
        if isinstance(item, dict):
            rows.append(
                {
                    "id": item.get("id", f"finding-{index + 1}"),
                    "summary": item.get(
                        "summary", item.get("message", "unspecified finding")
                    ),
                    "evidence": item.get("evidence") or evidence_ref,
                }
            )
        else:
            rows.append(
...
def _brief_uncertainty(
    receipt: dict[str, Any], reproducer: dict[str, Any]
) -> list[str]:
    uncertainty: list[str] = []
    if not receipt.get("owners") and not receipt.get("owner"):
        uncertainty.append("owner was not supplied")
    if (
        not receipt.get("impact")
        and not receipt.get("affected_components")
        and not receipt.get("affected")
    ):
...
def _brief_evidence(
    receipt_path: str | None, evidence_ref: str | None
) -> list[dict[str, Any]]:
    if evidence_ref:
        return [
            {
                "receipt_sha256": evidence_ref,
...
def _brief_affected(receipt: dict[str, Any], source_root: str | None) -> Any:
    return (
        receipt.get("affected_components")
        or receipt.get("affected")
        or (
            [source_root]
            if source_root
            else ["unknown component; impact scope was not supplied"]
        )
    )
...
def failure_brief(
    ...
    *,
    ...
    failed = bool(
        decision in {"FAIL", "BLOCKED", "MISMATCH", "INVALID"}
    ...
    what_broke = findings or [
        {
            "id": "NONE",
            "summary": "no failure finding was declared",
            "evidence": evidence_ref,
        }
    ]
    source_root = receipt.get("source_root") or receipt.get("original", {}).get(
        "source_root"
    )
    affected = _brief_affected(receipt, source_root)
    reproducer = (
        receipt.get("reproducer")
        if isinstance(receipt.get("reproducer"), dict)
        else {"argv": receipt.get("argv"), "cwd": source_root}
    )
    next_fix = (
        receipt.get("next_fix")
        or receipt.get("next_action")
        or "Inspect the linked evidence, propose a bounded repair, then run the repair comparison with negative controls."
    )
    uncertainty = _brief_uncertainty(receipt, reproducer)
    brief = {
        "schema": BRIEF_SCHEMA,
        "marker": "FAILURE_BRIEF_EVIDENCE_LINKED",
        "state": "ACTION_REQUIRED" if failed else "NO_FAILURE_DECLARED",
