...
def _session_path(root: Path, session_id: str) -> Path:
    if not isinstance(session_id, str) or not _ID.fullmatch(session_id):
        raise ValueError(
            "session_id must use lowercase letters, digits, hyphens, or underscores"
        )
    ...
    return [
        {
            "id": "intent",
            "kind": "decision",
            "prompt": "What exact user or operator outcome must this change create?",
            "required": True,
        },
        {
            "id": "actors",
            "kind": "fact",
            "prompt": "Who is the primary actor and who owns the consequential decision?",
            "required": True,
        },
        {
            "id": "scope",
            "kind": "decision",
            "prompt": "What is explicitly in scope, and what is out of scope for this slice?",
            "required": True,
        },
        {
            "id": "acceptance",
            "kind": "decision",
            "prompt": "What observable evidence proves the outcome, and where will it be checked?",
            "required": True,
        },
        {
            "id": "forbidden",
            "kind": "decision",
            "prompt": "What must never happen even if the happy path succeeds?",
            "required": True,
        },
        {
            "id": "negative_case",
            "kind": "decision",
            "prompt": "Which counterexample must fail or be refused to prove the boundary?",
            "required": True,
        },
        {
            "id": "reconsideration",
            "kind": "decision",
            "prompt": "What fact would force this decision to be revisited?",
            "required": False,
        },
    ]
...
def start_grill(
    root: Path, session_id: str, source: Path | None = None
) -> dict[str, Any]:
    ...
    if path.exists():
        ...
        raise ValueError(
            "session already exists with different source; choose a new session_id"
        )
    session = {
        **core,
        "session_sha256": _sha(core),
        "created_at": _now(),
        "updated_at": _now(),
        "authority": {"execute": False, "approve": False, "publish": False},
    }
    _write(path, session)
    return {
        **session,
...
def next_question(root: Path, session_id: str) -> dict[str, Any]:
    ...
    for question in session.get("questions", []):
        ...
        if question["id"] not in answers and question.get("required", False):
            return {
                "schema": "factory.grill-next.v1",
                "session_id": session_id,
                "status": "QUESTION_REQUIRED",
                "question": question,
                "answered": len(answers),
                "total": len(session["questions"]),
                "claim_boundary": "One question only; no implementation or approval authority.",
            }
    if session.get("status") != "confirmed":
        return {
            ...
            "answered": len(answers),
            "total": len(session["questions"]),
            "claim_boundary": "Shared understanding is complete but still requires named human confirmation.",
        }
    return {
        "schema": "factory.grill-next.v1",
        "session_id": session_id,
        ...
        "answered": len(answers),
        "total": len(session["questions"]),
        "claim_boundary": "Confirmed intent worksheet only; no execution or release authority.",
    }


def answer_question(
    root: Path, session_id: str, question_id: str, answer: str, *, answered_by: str
) -> dict[str, Any]:
    ...
    question = next(
        (item for item in session["questions"] if item["id"] == question_id), None
    )
    if question is None:
...
def confirm_grill(root: Path, session_id: str, *, approved_by: str) -> dict[str, Any]:
    ...
    missing = [
        ...
        if item.get("required") and item["id"] not in session["answers"]
    ]
    if missing:
    ...
    session["confirmation_sha256"] = _sha(
        {
            key: value
            for key, value in session.items()
            if key not in {"confirmation_sha256", "updated_at"}
        }
    )
    _write(_session_path(root, session_id), session)
    return {**session, "next": next_question(root, session_id)}
