...
def add_parser(sub: Any) -> None:
    ...
    status = commands.add_parser("status", help="read local blueprint receipt inventory")
    ...
    reflect = commands.add_parser("reflect", help="aggregate recalled memory without an LLM")
    reflect.add_argument("observations")
    reflect.add_argument("--json", action="store_true")
    librarian = commands.add_parser("librarian", help="promote one observation through human review")
    librarian.add_argument("observation")
    librarian.add_argument("--reviewer", required=True)
    librarian.add_argument("--stage", required=True, choices=["silver", "gold", "contested"])
    ...
    signal = commands.add_parser("signal-intent", help="create a human-reviewable intent proposal from a normalized signal")
    ...
    access = commands.add_parser("access", help="seal a declaration-only runtime access profile")
    access.add_argument("payload")
    access.add_argument("--out", required=True)
    access.add_argument("--json", action="store_true")
    team = commands.add_parser("team-plan", help="compile a typed, non-dispatching team plan")
...
def run(args: Any) -> int:
    ...
    try:
        if args.blueprint_cmd == "status":
            ...
            result = recall_observations(observations, project=args.project, query=args.query, tags=args.tag, limit=args.limit)
            ...
            result = librarian_promotion(_read(args.observation), reviewer=args.reviewer, stage=args.stage, source_digest=args.source_digest, feedback=args.feedback)
            result["artifact"] = _write(args.out, result)
            ...
            result["artifact"] = _write(args.out, result)
    except (BlueprintError, OSError, json.JSONDecodeError, TypeError, ValueError) as exc:
        print(json.dumps({"schema": "factory.blueprint.error.v1", "marker": "BLUEPRINT_REFUSED", "code": getattr(exc, "code", "E_INPUT"), "message": str(exc)}, indent=2, sort_keys=True))
        return 1
    print(json.dumps(result, indent=2, sort_keys=True) if args.json else result.get("marker", "BLUEPRINT_OK"))
    return 0
