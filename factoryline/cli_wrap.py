...
def run(args: Any) -> int:
    ...
    if command and command[0] == "--":
        command = command[1:]
    try:
        payload = run_observed_session(workspace, workspace / args.admission, workspace / args.validators, command, args.run_id)
        ...
        print(json.dumps(error, indent=2, sort_keys=True) if args.json else f"wrap failed: {code}: {exc}", file=sys.stderr)
        return 2
    if args.json:
