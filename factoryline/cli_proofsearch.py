...
def add_parser(sub: Any) -> None:
    ...
    draft = worklog_sub.add_parser("draft", help="write one immutable local proof worklog draft")
    draft.add_argument("--root", default=".")
    draft.add_argument("--contract", required=True, help="workspace-relative current sealed Oracle Contract")
    draft.add_argument("--out", help="workspace-relative immutable local draft path")
    draft.add_argument("--json", action="store_true")
    verify = worklog_sub.add_parser("verify", help="verify one local proof worklog draft and current Oracle binding")
    verify.add_argument("draft")
    verify.add_argument("--root", default=".")
    verify.add_argument("--json", action="store_true")
    status = worklog_sub.add_parser("status", help="read bounded local proof worklog facts without posting anything")
    status.add_argument("--root", default=".")
    status.add_argument("--json", action="store_true")

    proof = sub.add_parser("proofsearch", help="compare hash-bound repair candidates without applying them")
    proof_sub = proof.add_subparsers(required=True, dest="proofsearch_cmd")
    plan = proof_sub.add_parser("plan", help="seal one graph divergence and its exact proof-impact slice")
    ...
    evaluate = proof_sub.add_parser("evaluate", help="verify, reject, and rank supplied candidate evidence")
    evaluate.add_argument("request")
    evaluate.add_argument("--root", default=".")
    evaluate.add_argument("--out", required=True)
    evaluate.add_argument("--json", action="store_true")
    verify = proof_sub.add_parser("verify", help="verify one sealed ProofSearch evaluation and its evidence")
    verify.add_argument("evaluation")
    verify.add_argument("--root", default=".")
    verify.add_argument("--json", action="store_true")
    frontier = proof_sub.add_parser("frontier", help="rank or verify the next evidence experiment")
    frontier_sub = frontier.add_subparsers(required=True, dest="frontier_cmd")
    frontier_plan = frontier_sub.add_parser("plan", help="rank the next evidence experiment without executing it")
    ...
    frontier_verify = frontier_sub.add_parser("verify", help="verify one sealed evidence frontier")
...
def _run_worklog(args: Any) -> int:
    ...
    try:
        if args.worklog_cmd == "draft":
            result = create_proof_worklog(root, Path(args.contract), Path(args.out) if args.out else None)
...
def _run_proofsearch(args: Any) -> int:
    ...
    try:
        if args.proofsearch_cmd == "plan":
            payload = create_proofsearch_plan(Path(args.root), Path(args.baseline), Path(args.candidate), args.changed, Path(args.out))
        elif args.proofsearch_cmd == "evaluate":
            payload = evaluate_proofsearch(Path(args.root), Path(args.request), Path(args.out))
        elif args.proofsearch_cmd == "frontier" and args.frontier_cmd == "plan":
            payload = plan_evidence_frontier(Path(args.root), Path(args.request), Path(args.out))
        elif args.proofsearch_cmd == "frontier":
            payload = verify_evidence_frontier(Path(args.root), Path(args.frontier))
        else:
            payload = verify_proofsearch_evaluation(Path(args.root), Path(args.evaluation))
    ...
    if args.json:
        ...
        print("apply  : locked")
    if args.proofsearch_cmd == "verify" or (args.proofsearch_cmd == "frontier" and args.frontier_cmd == "verify"):
