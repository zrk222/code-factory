...
EVENT_TYPES = frozenset({"RUN_STARTED", "STATE_SNAPSHOT", "REVIEW_CARD", "INTERRUPT", "RUN_FINISHED"})
_FORBIDDEN_KEYS = frozenset({"credential", "credentials", "secret", "password", "pat", "token", "raw_prompt", "transcript"})
...
def _run_id(value: object) -> str:
    if not isinstance(value, str) or not _RUN_ID.fullmatch(value):
        raise AguiError("E_AGUI_RUN_ID", "run_id must be lowercase and contain at most 96 safe characters")
    return value
...
def _safe_value(value: Any, *, depth: int = 0) -> Any:
    ...
    if isinstance(value, dict):
        ...
        for key, item in value.items():
            if not isinstance(key, str) or key.lower() in _FORBIDDEN_KEYS:
                raise AguiError("E_AGUI_SENSITIVE", "event payload contains a forbidden sensitive field")
    ...
    if isinstance(value, (str, int, float, bool)) or value is None:
        if isinstance(value, str) and len(value) > 4096:
            raise AguiError("E_AGUI_BOUNDS", "event strings may contain at most 4096 characters")
        return value
    raise AguiError("E_AGUI_PAYLOAD", "event payload must be JSON-compatible")


def _event(run_id: str, sequence: int, event_type: str, payload: dict[str, Any]) -> dict[str, Any]:
    safe_payload = _safe_value(payload)
    core = {"schema": SCHEMA, "runId": run_id, "sequence": sequence, "type": event_type, "payload": safe_payload}
    return {**core, "eventId": "agui:" + _sha(core)[:32], "payloadSha256": _sha(safe_payload)}
...
def _projection_payload(status: dict[str, Any]) -> dict[str, Any]:
    ...
    allowed = ("state", "marker", "receipt_path", "receipt_sha256", "stale_paths", "next_actions", "claim_boundary")
    result = {key: status[key] for key in allowed if key in status}
    result["authority"] = status.get("authority", "none")
    return result


def build_review_events(status: dict[str, Any], *, run_id: str = "local-review", surface: str = "mission_control") -> list[dict[str, Any]]:
    ...
    events = [
        _event(rid, 0, "RUN_STARTED", {"surface": surface, "mode": "controlled_review", "authority": {"execution": False, "approval": False, "release": False}}),
        _event(rid, 1, "STATE_SNAPSHOT", projection),
        _event(rid, 2, "REVIEW_CARD", {
        ...
        _event(rid, 3, "RUN_FINISHED", {"state": state, "authority": {"execution": False, "approval": False, "release": False}}),
    ]
    return validate_agui_events(events)


def build_release_interrupt_event(envelope: dict[str, Any], *, run_id: str = "release-gate") -> dict[str, Any]:
    ...
    if not isinstance(envelope, dict) or envelope.get("type") != "input_required":
        raise AguiError("E_AGUI_INTERRUPT", "an MCP2 input_required envelope is required")
    context = envelope.get("context")
    schema = envelope.get("inputSchema")
    if not isinstance(context, dict) or not isinstance(schema, dict):
        raise AguiError("E_AGUI_INTERRUPT", "input_required must include context and inputSchema")
    payload = {
    ...
    finish = _event(rid, 2, "RUN_FINISHED", {"state": "WAITING_FOR_HUMAN", "authority": {"execution": False, "approval": False, "release": False}})
    return validate_agui_events([_event(rid, 0, "RUN_STARTED", {"surface": "release_gate", "mode": "controlled_review"}), event, finish])[1]


def build_release_completed_event(envelope: dict[str, Any], *, run_id: str = "release-gate") -> dict[str, Any]:
...
def validate_agui_events(events: Iterable[dict[str, Any]]) -> list[dict[str, Any]]:
    ...
    if not 1 <= len(rows) <= MAX_EVENTS:
        raise AguiError("E_AGUI_BOUNDS", f"event stream must contain 1-{MAX_EVENTS} events")
    for index, row in enumerate(rows):
        if not isinstance(row, dict) or row.get("schema") != SCHEMA:
            raise AguiError("E_AGUI_SCHEMA", "every event must use the Code Factory AGUI schema")
        if row.get("sequence") != index or row.get("type") not in EVENT_TYPES:
            raise AguiError("E_AGUI_SEQUENCE", "event sequences must be contiguous and use known types")
        ...
        core = {"schema": SCHEMA, "runId": rid, "sequence": index, "type": row["type"], "payload": payload}
    ...
    if rows[0]["type"] != "RUN_STARTED" or rows[-1]["type"] != "RUN_FINISHED":
        raise AguiError("E_AGUI_SEQUENCE", "event stream must start with RUN_STARTED and end with RUN_FINISHED")
    if sum(row["type"] == "RUN_STARTED" for row in rows) != 1 or sum(row["type"] == "RUN_FINISHED" for row in rows) != 1:
        raise AguiError("E_AGUI_SEQUENCE", "event stream must contain exactly one start and finish")
    return rows
