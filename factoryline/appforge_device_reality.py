...
def _text(value: object, field: str, *, limit: int = 500) -> str:
    ...
    if not result or len(result) > limit:
        raise RevenueForgeError(
            "APPFORGE_DEVICE_REALITY_INVALID",
...
def _digest(value: object, field: str) -> str:
    ...
    if len(result) != 64 or any(char not in "0123456789abcdef" for char in result):
        raise RevenueForgeError(
            "APPFORGE_DEVICE_REALITY_INVALID", f"{field} must be a SHA-256 hex digest"
        )
    return result
...
def _candidate(value: object, field: str) -> dict[str, str]:
    if not isinstance(value, dict):
        raise RevenueForgeError(
            "APPFORGE_DEVICE_REALITY_CANDIDATE_INVALID", f"{field} must be an object"
        )
    return {
        key: _text(value.get(key), f"{field}.{key}", limit=200)
...
def _local(root: Path, path: Path, *, exists: bool = True) -> Path:
    ...
    try:
        ...
        raise RevenueForgeError(
            "APPFORGE_DEVICE_REALITY_PATH_REJECTED",
...
def _read(root: Path, path: Path, schema: str) -> tuple[dict[str, Any], Path]:
    ...
    if source.stat().st_size > MAX_BYTES:
        raise RevenueForgeError(
            "APPFORGE_DEVICE_REALITY_INPUT_TOO_LARGE", "JSON input exceeds 1 MiB"
        )
    try:
        value = json.loads(source.read_text(encoding="utf-8-sig"))
    except (OSError, UnicodeDecodeError, json.JSONDecodeError) as exc:
        raise RevenueForgeError(
            "APPFORGE_DEVICE_REALITY_INPUT_INVALID", "input must be valid JSON"
        ) from exc
    if not isinstance(value, dict) or value.get("schema") != schema:
        raise RevenueForgeError(
            "APPFORGE_DEVICE_REALITY_SCHEMA_REJECTED", f"expected {schema}"
        )
    return value, source
...
def _write_new(root: Path, path: Path, payload: dict[str, Any]) -> Path:
    ...
    if target.exists():
        raise RevenueForgeError(
            "APPFORGE_DEVICE_REALITY_OUTPUT_EXISTS",
            "destination already exists; sealed artifacts are immutable",
        )
...
def _valid_hash_seal(value: object, schema: str, field: str) -> bool:
    ...
    return (
        ...
        and len(supplied) == 64
        and _sha({key: item for key, item in value.items() if key != field}) == supplied
    )
...
def _rules(contract: dict[str, Any]) -> list[dict[str, str]]:
    ...
    for group in (
        ...
        for item in contract.get("rules", {}).get(group, []):
            if (
                item.get("effect") in {"blocking", "release"}
                and item.get("origin") in AUTHORITY_ORIGINS
            ):
                selected.append(
                    {
                        "group": group,
    ...
    if not selected:
        raise RevenueForgeError(
            "APPFORGE_DEVICE_REALITY_ORACLE_UNAUTHORIZED",
            "sealed Oracle contract has no human-confirmed or trusted blocking obligations",
        )
    return selected
...
def _journeys(value: object) -> list[dict[str, str]]:
    if not isinstance(value, list) or not 1 <= len(value) <= 30:
        raise RevenueForgeError(
            "APPFORGE_DEVICE_REALITY_JOURNEYS_INVALID",
            "required_journeys must contain 1 through 30 approved journeys",
        )
    ...
    for index, item in enumerate(value):
        if not isinstance(item, dict):
            raise RevenueForgeError(
                "APPFORGE_DEVICE_REALITY_JOURNEYS_INVALID",
                f"required_journeys[{index}] must be an object",
            )
        result.append(
            {
                ...
                "expected_outcome": _text(
                    ...
                    f"required_journeys[{index}].expected_outcome",
                ),
                "forbidden_outcome": _text(
                    item.get("forbidden_outcome"),
                    f"required_journeys[{index}].forbidden_outcome",
                ),
            }
        )
    if len({item["id"] for item in result}) != len(result):
        raise RevenueForgeError(
            "APPFORGE_DEVICE_REALITY_JOURNEYS_INVALID",
            "required journey ids must be unique",
        )
    return result
...
def create_device_reality_intent_envelope(
    ...
    workspace = Path(root).resolve()
    authority_raw, authority_source = _read(
        workspace, oracle_authority_path, ORACLE_AUTHORITY_SCHEMA
    )
    candidate = _candidate(authority_raw.get("candidate"), "authority.candidate")
    derived = verify_appforge_oracle_authority(
        workspace, oracle_authority_path, candidate=candidate
    )
    if not derived.get("ok"):
        raise RevenueForgeError(
            "APPFORGE_DEVICE_REALITY_ORACLE_BLOCKED",
            "Oracle authority must be current before sealing device intent",
        )
    contract_path = authority_raw.get("contract_path")
    if not isinstance(contract_path, str) or not contract_path.strip():
        raise RevenueForgeError(
            "APPFORGE_DEVICE_REALITY_ORACLE_INVALID",
            "authority.contract_path is required",
        )
    contract_result = verify_oracle_contract(workspace, Path(contract_path))
    if not contract_result.get("ok"):
        raise RevenueForgeError(
            "APPFORGE_DEVICE_REALITY_ORACLE_BLOCKED",
            "sealed Oracle contract is not current",
        )
    design_source = _local(workspace, design_input_path)
    if design_source.stat().st_size > MAX_BYTES:
        raise RevenueForgeError(
            "APPFORGE_DEVICE_REALITY_INPUT_TOO_LARGE", "design input exceeds 1 MiB"
        )
    journeys = _journeys(required_journeys)
    if not isinstance(allowed_transports, list) or not allowed_transports:
        raise RevenueForgeError(
            "APPFORGE_DEVICE_REALITY_TRANSPORT_INVALID",
            "allowed_transports must contain an approved capture transport",
        )
    transports = sorted(
        {_text(item, "allowed_transports", limit=64) for item in allowed_transports}
    )
    if any(item not in ALLOWED_TRANSPORTS for item in transports):
        raise RevenueForgeError(
            "APPFORGE_DEVICE_REALITY_TRANSPORT_INVALID",
            "allowed_transports contains an unsupported capture transport",
        )
    core: dict[str, Any] = {
        ...
        "user_design_input": {
            ...
            "sha256": _file_sha(design_source),
        },
        "oracle_authority": {
            "path": authority_source.relative_to(workspace).as_posix(),
            "sha256": _file_sha(authority_source),
        ...
        "approved_by": _text(
            authority_raw.get("human_reviewer"), "authority.human_reviewer", limit=160
        ),
...
def _capture_file(
    ...
    try:
        ...
        if path.stat().st_size > MAX_CAPTURE_BYTES:
            raise RevenueForgeError(
                "APPFORGE_DEVICE_REALITY_CAPTURE_TOO_LARGE", "capture exceeds 25 MiB"
            )
...
def _capture_record(
    ...
    try:
        return {
            ...
            "forbidden_outcome": expected["forbidden_outcome"],
        }, None
    except ValueError:
        return None, {
            "code": "APPFORGE_DEVICE_REALITY_PATH_REJECTED",
            "detail": f"captures[{index}] escapes workspace",
        }


def _binding_findings(
    envelope: dict[str, Any], evidence: dict[str, Any], candidate: dict[str, str]
) -> list[dict[str, str]]:
    checks = (
        (
            _candidate(evidence.get("candidate"), "evidence.candidate") == candidate,
            "APPFORGE_DEVICE_REALITY_CANDIDATE_MISMATCH",
            "device evidence is not bound to the sealed candidate",
        ),
        (
            evidence.get("intent_envelope_sha256") == envelope["envelope_sha256"],
            "APPFORGE_DEVICE_REALITY_INTENT_MISMATCH",
            "device evidence does not name the sealed intent envelope",
        ),
        (
            evidence.get("user_design_input_sha256")
            == envelope["user_design_input"]["sha256"],
            "APPFORGE_DEVICE_REALITY_DESIGN_MISMATCH",
            "device evidence is not bound to the reviewed user design input",
        ),
    )
    return [
...
def _current_intent_binding_findings(
    workspace: Path, envelope: dict[str, Any], candidate: dict[str, str]
) -> list[dict[str, str]]:
    ...
    try:
        design = _local(
            ...
            Path(
                _text(
                    envelope["user_design_input"].get("path"),
                    "envelope.user_design_input.path",
                    limit=512,
                )
            ),
        )
        if _file_sha(design) != _digest(
            envelope["user_design_input"].get("sha256"),
            "envelope.user_design_input.sha256",
        ):
            findings.append(
                {
                    "code": "APPFORGE_DEVICE_REALITY_DESIGN_SOURCE_STALE",
                    "detail": "reviewed user design input changed after the intent envelope was sealed",
                }
            )
    except (KeyError, TypeError, RevenueForgeError, OSError):
        findings.append(
            {
                "code": "APPFORGE_DEVICE_REALITY_DESIGN_SOURCE_INVALID",
                "detail": "reviewed user design input cannot be revalidated",
            }
        )
    try:
        authority = envelope["oracle_authority"]
        authority_path = _local(
            workspace,
            Path(
                _text(
                    authority.get("path"), "envelope.oracle_authority.path", limit=512
                )
            ),
        )
        if _file_sha(authority_path) != _digest(
            authority.get("sha256"), "envelope.oracle_authority.sha256"
        ):
            findings.append(
                {
                    "code": "APPFORGE_DEVICE_REALITY_ORACLE_AUTHORITY_STALE",
                    "detail": "Oracle authority changed after the intent envelope was sealed",
                }
            )
        derived = verify_appforge_oracle_authority(
            workspace, authority_path, candidate=candidate
        )
        if not derived.get("ok") or derived.get("receipt_sha256") != authority.get(
            "receipt_sha256"
        ):
            findings.append(
                {
                    "code": "APPFORGE_DEVICE_REALITY_ORACLE_AUTHORITY_STALE",
                    "detail": "current Oracle authority no longer matches the sealed authority receipt",
                }
            )
    except (KeyError, TypeError, RevenueForgeError, OSError, ValueError):
        findings.append(
            {
                "code": "APPFORGE_DEVICE_REALITY_ORACLE_AUTHORITY_INVALID",
                "detail": "Oracle authority cannot be revalidated",
            }
        )
    try:
        contract = envelope["oracle_contract"]
        result = verify_oracle_contract(
            workspace,
            Path(
                _text(contract.get("path"), "envelope.oracle_contract.path", limit=512)
            ),
        )
        if not result.get("ok") or result.get("contract", {}).get(
            "contract_sha256"
        ) != contract.get("contract_sha256"):
            findings.append(
                {
                    "code": "APPFORGE_DEVICE_REALITY_ORACLE_CONTRACT_STALE",
                    "detail": "Oracle contract no longer matches the sealed intent envelope",
                }
            )
    except (KeyError, TypeError, RevenueForgeError, OSError, ValueError):
        findings.append(
            {
                "code": "APPFORGE_DEVICE_REALITY_ORACLE_CONTRACT_INVALID",
                "detail": "Oracle contract cannot be revalidated",
            }
        )
    return findings
...
def _transport_findings(
    ...
    authorized = (
        ...
        and kind in envelope["allowed_transports"]
...
def _capture_evidence(
    workspace: Path, envelope: dict[str, Any], evidence: dict[str, Any], transport: str
    ...
    if not isinstance(raw_captures, list):
        return [], [
            {
                "code": "APPFORGE_DEVICE_REALITY_CAPTURE_INVALID",
                "detail": "captures must be an array",
            }
        ]
    ...
    if set(ids) != set(allowed) or len(ids) != len(set(ids)):
        findings.append(
            {
                "code": "APPFORGE_DEVICE_REALITY_JOURNEYS_INCOMPLETE",
                "detail": "every sealed journey must have exactly one passing, hash-valid capture",
            }
        )
    return captures, findings


def verify_device_reality(
    root: Path, envelope_path: Path, evidence_path: Path, out_path: Path
) -> dict[str, Any]:
    ...
    if not _valid_hash_seal(envelope, ENVELOPE_SCHEMA, "envelope_sha256"):
        raise RevenueForgeError(
            "APPFORGE_DEVICE_REALITY_ENVELOPE_TAMPERED",
            "intent envelope hash is invalid",
        )
    ...
    supervision, transport_kind, authority_findings = _supervision_and_transport(
        envelope, evidence
    )
    captures, capture_findings = _capture_evidence(
        workspace, envelope, evidence, transport_kind
    )
    ...
    core: dict[str, Any] = {
        ...
        "marker": "APPFORGE_DEVICE_REALITY_READY"
        if ready
        else "APPFORGE_DEVICE_REALITY_BLOCKED",
        "ok": ready,
        ...
        "candidate": candidate,
        "intent_envelope": {
            "path": envelope_source.relative_to(workspace).as_posix(),
    ...
    if base.exists():
        for path in sorted(base.rglob("*device-reality*.json"))[:100]:
            try:
                ...
                invalid.append(path.relative_to(workspace).as_posix())
    return {
        "schema": "factory.appforge.device-reality-projection.v1",
        "marker": "APPFORGE_DEVICE_REALITY_READ_ONLY",
        ...
        "claim_boundary": "Read-only local receipt status; not device control, a device test, Apple submission, or Apple approval.",
    }
