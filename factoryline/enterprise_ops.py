...
SAFE_RESULT_SET = frozenset(
    {"success", "failure", "deployed", "rolled_back", "incident", "aborted"}
)
...
def _canonical(value: Any) -> bytes:
    try:
        ...
        raise EnterpriseOpsError(
            "E_OPS_NON_CANONICAL", f"value is not canonical JSON: {exc}"
        ) from exc
...
def _ensure_initialized(root: Path) -> dict[str, Any]:
    ...
    if (
        ...
        or not _db_path(root).exists()
    ):
        raise EnterpriseOpsError(
            "E_OPS_NOT_INITIALIZED",
            "run factory ops init before using the operations workspace",
        )
    config = _load_json(config_path, code="E_OPS_CONFIG_INVALID")
    if not isinstance(config, dict) or config.get("schema") != OPS_SCHEMA:
        raise EnterpriseOpsError(
            "E_OPS_CONFIG_INVALID", "operations config has an unsupported schema"
        )
...
def _identities(root: Path) -> list[dict[str, Any]]:
    ...
    if (
        ...
        raise EnterpriseOpsError(
            "E_IDENTITY_REGISTRY_INVALID", "identity registry has an unsupported schema"
        )
...
def _identity(root: Path, tenant_id: str, subject: str) -> dict[str, Any]:
    matches = [
        item
        for item in _identities(root)
        if item.get("tenant_id") == tenant_id and item.get("subject") == subject
    ]
    if not matches:
        raise EnterpriseOpsError(
            "E_IDENTITY_UNKNOWN", "identity is not provisioned in this tenant"
        )
    item = matches[0]
    if item.get("status") != "active":
        raise EnterpriseOpsError(
            "E_IDENTITY_INACTIVE", "identity is suspended or revoked"
        )
    return item


def _authorize(
    root: Path, tenant_id: str, subject: str, roles: Iterable[str]
) -> dict[str, Any]:
    identity = _identity(root, tenant_id, subject)
    allowed = set(roles)
    if not allowed.intersection(
        {str(identity.get("role")), *identity.get("roles", [])}
    ):
        raise EnterpriseOpsError(
            "E_ACTION_DENIED", "identity role does not authorize this operation"
        )
    return identity
...
def initialize_workspace(
    ...
    *,
    ...
    if (
        ...
        raise EnterpriseOpsError(
            "E_RETENTION_INVALID", "retention_days must be between 1 and 3650"
        )
    config_path = _config_path(root)
    if config_path.exists() and not force:
        raise EnterpriseOpsError(
            "E_OPS_ALREADY_INITIALIZED",
            "operations workspace already exists; use --force only to replace it",
        )
    _ops_dir(root)
    config = {
        "schema": OPS_SCHEMA,
        ...
        "authority": {
            ...
            "deploy": False,
            "billing": False,
            "sso_enrollment": False,
            "sla_activation": False,
        },
    }
...
def provision_identity(
    ...
    *,
    ...
    if config["tenant_id"] != tenant_id:
        raise EnterpriseOpsError(
            "E_TENANT_BOUNDARY",
            "identity tenant differs from the initialized operations tenant",
        )
    role = _require(role, "role").lower()
    status = _require(status, "status").lower()
    if role not in ROLE_SET:
        raise EnterpriseOpsError(
            "E_ROLE_INVALID", f"role must be one of {sorted(ROLE_SET)}"
        )
    if status not in STATUS_SET:
        raise EnterpriseOpsError(
            "E_IDENTITY_STATUS_INVALID", f"status must be one of {sorted(STATUS_SET)}"
        )
    ...
    if existing:
        existing.update(
            {"role": role, "roles": [role], "status": status, "updated_at": now}
        )
    ...
    return {
        "schema": IDENTITY_SCHEMA,
        "marker": "IDENTITY_LIFECYCLE_RECORDED",
        "eops_marker": "EOPS_IDENTITY_READY",
        "identity": existing,
        "audit": audit,
    }
...
def put_evidence(
    ...
    *,
    ...
    if config["tenant_id"] != tenant_id:
        raise EnterpriseOpsError(
            "E_TENANT_BOUNDARY",
            "evidence tenant differs from the initialized operations tenant",
        )
    ...
    if not isinstance(payload, dict) or payload.get("tenant_id") != tenant_id:
        raise EnterpriseOpsError(
            "E_EVIDENCE_TENANT",
            "evidence payload must be an object bound to the selected tenant",
        )
    supplied_id = evidence_id or "ev-" + _sha(payload)[:32]
    if not re.fullmatch(r"[A-Za-z0-9._:-]{1,128}", supplied_id):
        raise EnterpriseOpsError(
            "E_EVIDENCE_ID_INVALID", "evidence_id has an unsupported format"
        )
    digest = _sha(payload)
    ...
    with _connect(root) as db:
        existing = db.execute(
            "SELECT * FROM evidence WHERE evidence_id = ?", (supplied_id,)
        ).fetchone()
        if existing:
            if (
                ...
                raise EnterpriseOpsError(
                    "E_EVIDENCE_IMMUTABLE",
                    "evidence id is already bound to different content",
                )
            return {
                "schema": EVIDENCE_SCHEMA,
                "marker": "EVIDENCE_REUSED",
                "eops_marker": "EOPS_EVIDENCE_READY",
    ...
    audit = _audit(
        ...
        subject,
        supplied_id,
        {"payload_sha256": digest, "verdict": str(payload.get("verdict", "UNKNOWN"))},
    )
    return {
        "schema": EVIDENCE_SCHEMA,
        "marker": "EVIDENCE_RECORDED",
        "eops_marker": "EOPS_EVIDENCE_READY",
        "evidence_id": supplied_id,
        "payload_sha256": digest,
        "tenant_id": tenant_id,
...
def verify_workspace(root: Path, tenant_id: str | None = None) -> dict[str, Any]:
    ...
    if tenant_id != config["tenant_id"]:
        raise EnterpriseOpsError(
            "E_TENANT_BOUNDARY",
            "workspace verification cannot cross the initialized tenant",
        )
    with _connect(root) as db:
    ...
    return {
        "schema": OPS_SCHEMA,
        "marker": "OPS_WORKSPACE_VERIFIED" if not errors else "OPS_WORKSPACE_TAMPERED",
        "eops_marker": "EOPS_EVIDENCE_READY" if not errors else "EOPS_FAIL_CLOSED",
        "tenant_id": tenant_id,
...
def export_evidence(root: Path, out: Path) -> dict[str, Any]:
    ...
    with _connect(root) as db:
        rows = db.execute(
            "SELECT evidence_id,payload_sha256,verdict,created_at,expires_at,inserted_by FROM evidence WHERE tenant_id = ? ORDER BY created_at,evidence_id",
            (config["tenant_id"],),
        ).fetchall()
    payload = {
        "schema": "factory.enterprise-ops.export.v1",
        "marker": "OPS_EXPORT_WRITTEN",
        ...
        "records": [
            {
                ...
                "inserted_by": hashlib.sha256(
                    str(row["inserted_by"]).encode()
                ).hexdigest()[:16],
            }
            for row in rows
        ],
        "disclosure": "metadata-only; payloads, prompts, source paths, and credentials omitted",
    }
...
def _validate_command(command: Any) -> list[str]:
    if (
        ...
        raise EnterpriseOpsError(
            "E_RUNNER_COMMAND_INVALID",
            "command must be a non-empty argv list; shell strings are rejected",
        )
    if any(part in {"&&", ";", "|", ">", "<", "`"} for part in command):
        raise EnterpriseOpsError(
            "E_RUNNER_COMMAND_INVALID", "shell operators are not allowed in proof argv"
        )
    return command


def _validate_runner_limits(
    backend: str, timeout_seconds: int, output_limit: int
) -> None:
    if backend not in {"docker", "process"}:
        raise EnterpriseOpsError(
            "E_RUNNER_BACKEND_INVALID", "backend must be docker or process"
        )
    if (
        ...
        raise EnterpriseOpsError(
            "E_RUNNER_TIMEOUT_INVALID", "timeout_seconds must be between 1 and 3600"
        )
    if (
        not isinstance(output_limit, int)
        or isinstance(output_limit, bool)
        or not 1024 <= output_limit <= 4 * 1024 * 1024
    ):
        raise EnterpriseOpsError(
            "E_RUNNER_OUTPUT_INVALID", "output_limit must be between 1024 and 4194304"
        )
...
def _runner_argv(
    ...
    timeout_seconds: int,
    image: str,
    allow_process_boundary: bool,
) -> tuple[list[str], dict[str, Any]]:
    if backend == "docker":
        if shutil.which("docker") is None:
            raise EnterpriseOpsError(
                "E_RUNNER_BACKEND_UNAVAILABLE",
                "Docker is not installed or not on PATH; process fallback is never implicit",
            )
        if not re.fullmatch(r"[A-Za-z0-9._/@:-]{1,200}", image):
            raise EnterpriseOpsError(
                "E_RUNNER_IMAGE_INVALID", "Docker image has an unsupported format"
            )
        argv = [
            "docker",
            "run",
        ...
        posture = {
            ...
            "resource_limits": {"cpus": 1, "memory_mb": 512, "pids": 256},
        }
        return argv, posture
    if not allow_process_boundary:
        raise EnterpriseOpsError(
            "E_RUNNER_ISOLATION_REQUIRED",
            "process execution requires --allow-process-boundary and is never labelled isolated",
        )
    return command, {
        "backend": "process",
        "isolation": "not-isolated",
        "network": "not-enforced",
        "filesystem": "workspace-process",
        "resource_limits": {"timeout_seconds": timeout_seconds},
    }


def _execute_runner(
    argv: list[str], workspace: Path, timeout_seconds: int
) -> tuple[int | None, str, str, bool]:
    result, captures = _run_command(
    ...
    if result["status"] == "spawn_error":
        ...
        raise EnterpriseOpsError(
            "E_RUNNER_EXECUTION", f"proof command could not start: {error}"
        )
...
def run_proof(
    ...
    *,
    ...
    allow_process_boundary: bool = False,
) -> dict[str, Any]:
    ...
    argv, posture = _runner_argv(
        workspace, command, backend, timeout_seconds, image, allow_process_boundary
    )
    started = time.perf_counter()
    exit_code, stdout, stderr, timed_out = _execute_runner(
        argv, workspace, timeout_seconds
    )
    combined = stdout + ("\n" if stdout and stderr else "") + stderr
    result = {
        "schema": RUNNER_SCHEMA,
        "marker": "RUNNER_ISOLATION_ENFORCED"
        if posture["isolation"] == "container"
        else "RUNNER_NOT_ISOLATED",
        "eops_marker": "EOPS_RUNNER_READY",
        "tenant_id": config["tenant_id"],
        "command": command,
        "posture": posture,
        ...
        "output_truncated": len(combined) > output_limit,
        "authority": {
            "source_write": False,
            "merge": False,
            "deploy": False,
...
def _normalize_changed_paths(
    workspace: Path, changed_paths: Iterable[str]
) -> list[str]:
    normalized: list[str] = []
    for path in sorted({_require(path, "changed_path") for path in changed_paths}):
        ...
        try:
            ...
            raise EnterpriseOpsError(
                "E_PATH_ESCAPE", "changed path must stay inside the workspace"
            ) from exc
    return normalized


def _read_proof_receipts(
    workspace: Path, proof_receipts: Iterable[str]
) -> list[dict[str, Any]]:
    ...
    for item in proof_receipts:
        ...
        try:
            ...
            raise EnterpriseOpsError(
                "E_PROOF_RECEIPT_INVALID", f"proof receipt cannot be read: {exc}"
            ) from exc
        if isinstance(value, dict) and (
            value.get("verified") is True
            or value.get("status") in {"verified", "passed"}
        ):
...
def evaluate_required_checks(
    root: Path, changed_paths: Iterable[str], *, proof_receipts: Iterable[str] = ()
) -> dict[str, Any]:
    ...
    for path in normalized:
        ...
        if changed.exists() and any(
            item.get("created_at")
            and _parse_timestamp(item["created_at"]) < changed.stat().st_mtime
            for item in indexed.get(path, [])
        ):
            stale.append(path)
    decision = (
        "READY_FOR_HUMAN_REVIEW" if not missing and not stale else "REVIEW_REQUIRED"
    )
    return {
        "schema": CHECK_SCHEMA,
        "marker": "REQUIRED_CHECKS_READY"
        if decision == "READY_FOR_HUMAN_REVIEW"
        else "REQUIRED_CHECKS_REVIEW_REQUIRED",
        "eops_marker": "EOPS_CHECK_READY",
        "tenant_id": config["tenant_id"],
        "changed_paths": normalized,
        "proof_receipts": len(receipts),
        "missing": sorted(set(missing)),
        "stale": sorted(set(stale)),
        "decision": decision,
        "authority": {"merge": False, "deploy": False, "release": False},
    }
...
def record_outcome(
    ...
    *,
    ...
    if tenant_id != config["tenant_id"]:
        raise EnterpriseOpsError(
            "E_TENANT_BOUNDARY",
            "outcome tenant differs from the initialized operations tenant",
        )
    _authorize(root, tenant_id, subject, {"operator", "admin", "service_owner"})
    service, environment, result = (
        _require(service, "service"),
        _require(environment, "environment"),
        _require(result, "result").lower(),
    )
    if result not in SAFE_RESULT_SET:
        raise EnterpriseOpsError(
            "E_OUTCOME_RESULT_INVALID",
            f"result must be one of {sorted(SAFE_RESULT_SET)}",
        )
    if (
        ...
        raise EnterpriseOpsError(
            "E_OUTCOME_DURATION_INVALID", "duration_ms must be a non-negative integer"
        )
    ...
    event = {
        ...
        "marker": "OUTCOME_RECORDED",
        "eops_marker": "EOPS_OUTCOME_READY",
        "tenant_id": tenant_id,
        "actor": subject,
        "service": service,
...
def outcome_summary(root: Path) -> dict[str, Any]:
    ...
    return {
        "schema": OUTCOME_SCHEMA,
        "marker": "OUTCOME_SUMMARY_RECONCILED"
        if not errors
        else "OUTCOME_SUMMARY_TAMPERED",
        "events": len(rows),
        "deployments": deployments,
        "incidents": incidents,
        "rollbacks": rollbacks,
        "failed": sum(item.get("result") == "failure" for item in rows),
        "change_failure_rate": (incidents + rollbacks) / deployments
        if deployments
        else None,
        "integrity": {"valid": not errors, "errors": errors},
        "claim_boundary": "aggregate observations only; not a DORA benchmark or causal productivity claim",
    }
...
def export_otel(root: Path, out: Path) -> dict[str, Any]:
    ...
    payload = {
        ...
        "resourceSpans": [
            {
                "resource": {
                    "attributes": [
                        {
                            "key": "service.name",
                            "value": {"stringValue": row["service"]},
                        },
                        {
                            "key": "deployment.environment",
                            "value": {"stringValue": row["environment"]},
                        },
                    ]
                },
                "scopeSpans": [
                    {
                        "spans": [
                            {
                                "name": "factory.outcome",
                                "startTimeUnixNano": int(
                                    _parse_timestamp(row["created_at"]) * 1_000_000_000
                                ),
                                "durationNano": int(row["duration_ms"]) * 1_000_000,
                                "status": {
                                    "code": "STATUS_CODE_OK"
                                    if row["result"] in {"success", "deployed"}
                                    else "STATUS_CODE_ERROR"
                                },
                                "attributes": [
                                    {
                                        "key": "factory.result",
                                        "value": {"stringValue": row["result"]},
                                    },
                                    {
                                        "key": "factory.deployed",
                                        "value": {"boolValue": bool(row["deployed"])},
                                    },
                                    {
                                        "key": "factory.incident",
                                        "value": {"boolValue": bool(row["incident"])},
                                    },
                                    {
                                        "key": "factory.rollback",
                                        "value": {"boolValue": bool(row["rollback"])},
                                    },
                                ],
                            }
                        ]
        ...
        "events": len(rows),
        "disclosure": "allowlisted outcome metadata only; no prompts, paths, source, or credentials",
    }
    path = _atomic_json(Path(out).resolve(), payload)
    return {**payload, "path": str(path)}
...
def _sla_manifest(manifest: Path | None) -> dict[str, Any]:
    ...
    if not isinstance(supplied, dict):
        raise EnterpriseOpsError(
            "E_SLA_MANIFEST_INVALID", "SLA manifest must be a JSON object"
        )
    gates = supplied.get("gates", supplied)
    if not isinstance(gates, dict):
        raise EnterpriseOpsError(
            "E_SLA_MANIFEST_INVALID", "SLA manifest gates must be an object"
        )
    return gates
...
def _sla_gate_result(gate: str, value: Any) -> dict[str, Any]:
    ...
    if gate == "signed_acceptance":
        verified = (
            ...
            and bool(
                re.fullmatch(r"[0-9a-f]{64}", str(value.get("signature_sha256", "")))
            )
        )
    return {
        "verified": verified,
...
def evaluate_sla(
    root: Path, manifest: Path | None = None, *, out: Path | None = None
) -> dict[str, Any]:
    """Evaluate seven SLA activation gates without activating a contract."""
    _ensure_initialized(root)
    gates = _sla_manifest(manifest)
    results = {
        gate: _sla_gate_result(gate, gates.get(gate)) for gate in KNOWN_SLA_GATES
    }
    missing = [name for name, item in results.items() if not item["verified"]]
    payload = {
        "schema": SLA_SCHEMA,
        "marker": "SLA_READINESS_RECORDED",
        "eops_marker": "EOPS_SLA_READY",
        "status": "READY_FOR_CONTRACT" if not missing else "PROPOSED",
        "active": False,
        "gates": results,
        "missing_gates": missing,
        "authority": {"activate": False, "contract": False, "billing": False},
        "claim_boundary": "readiness evidence is not a live SLA or response-time guarantee",
    }
    if out is not None:
...
def workspace_status(root: Path) -> dict[str, Any]:
    ...
    try:
        ...
        return {
            "schema": OPS_SCHEMA,
            "marker": "OPS_NOT_INITIALIZED",
            "eops_marker": "EOPS_GOLDEN_READY",
            "markers": ["EOPS_GOLDEN_READY"],
            "initialized": False,
            "next": "factory ops init --tenant <tenant> --owner <subject>",
            "authority": {
                "merge": False,
                "deploy": False,
                "billing": False,
    ...
    if not verification["valid"]:
        ...
        next_action = (
            "factory ops identity provision <subject> --role operator --actor <admin>"
        )
    ...
    return {
        "schema": OPS_SCHEMA,
        "marker": "OPS_STATUS_READY",
        "eops_marker": status_marker,
        "markers": [
            "EOPS_EVIDENCE_READY" if verification["valid"] else "EOPS_FAIL_CLOSED",
            "EOPS_IDENTITY_READY",
            "EOPS_RUNNER_READY",
            "EOPS_CHECK_READY",
            "EOPS_OUTCOME_READY",
        ...
        "audit": {
            "events": verification["audit_events"],
            "valid": verification["valid"],
        },
        "identities": {
            "total": len(identities),
            "active": active,
            "suspended": sum(item.get("status") == "suspended" for item in identities),
            "revoked": sum(item.get("status") == "revoked" for item in identities),
        },
        "runner": {
            "docker_available": docker_available,
            "isolated_backend": "docker" if docker_available else None,
            "process_backend": "not-isolated",
            "network_enforcement": "none only in Docker backend",
        },
        "checks": {
            "eops_marker": "EOPS_CHECK_READY",
