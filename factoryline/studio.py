...
<div class="modes" role="tablist"><button class="mode active" id="dashboard-mode" type="button">Dashboard</button><button class="mode" id="assembly-mode" type="button">Assembly</button><button class="mode" id="savings-mode" type="button">Savings</button><button class="mode" id="starter-mode" type="button">Starter</button><button class="mode" id="mission-mode" type="button">Product mission</button></div>
<section id="dashboard" class="dashboard"><div class="stats"><div class="stat"><small>Stages measured</small><strong id="stat-stages">not available</strong></div><div class="stat"><small>First-pass gates</small><strong id="stat-first-pass">not available</strong></div><div class="stat"><small>Runs observed</small><strong id="stat-runs">not available</strong></div><div class="stat"><small>Flow efficiency</small><strong id="stat-flow">not available</strong></div></div><div class="dash-grid"><div class="panel"><h2><span class="live-dot"></span>Live factory activity</h2><div id="activity-list" class="data-list"></div></div><div class="panel"><h2>Paired savings</h2><div id="savings-list" class="data-list"></div></div><div class="panel"><h2>Human approval inbox</h2><div id="approval-list" class="data-list"></div></div><div class="panel wide-panel"><h2>Product Graph and journey map</h2><div id="product-list" class="data-list"></div></div><div class="panel"><h2>Dependency-aware slice queue</h2><div id="slice-list" class="data-list"></div></div><div class="panel"><h2>Mission board</h2><div id="mission-list" class="data-list"></div></div><div class="panel wide-panel"><h2>Proof timeline: requirement to receipt</h2><div id="proof-list" class="data-list"></div></div><div class="panel"><h2>Current vs previous run</h2><div id="comparison-list" class="data-list"></div></div><div class="panel"><h2>Capability Pack trust</h2><div id="pack-list" class="data-list"></div></div><div class="panel"><h2>Deployment routes</h2><div id="deployment-list" class="data-list"></div></div><div class="panel"><h2>Authority boundary</h2><div id="authority-list" class="data-list"></div></div></div></section>
...
function setMode(next){{const mission=next==='mission';const dash=next==='dashboard';const assemblyMode=next==='assembly';const savingsMode=next==='savings';mode=next;document.getElementById('dashboard-mode').classList.toggle('active',dash);document.getElementById('assembly-mode').classList.toggle('active',assemblyMode);document.getElementById('savings-mode').classList.toggle('active',savingsMode);document.getElementById('starter-mode').classList.toggle('active',next==='starter');document.getElementById('mission-mode').classList.toggle('active',mission);dashboard.classList.toggle('hidden',!dash);assembly.classList.toggle('hidden',!assemblyMode);savings.classList.toggle('hidden',!savingsMode);form.classList.toggle('hidden',dash||assemblyMode||savingsMode);document.getElementById('target-fieldset').classList.toggle('hidden',mission);document.querySelectorAll('.starter-only').forEach(el=>el.classList.toggle('hidden',mission));document.querySelectorAll('.mission-only').forEach(el=>el.classList.toggle('hidden',!mission));document.getElementById('surface-label').textContent=dash?'Live local control plane':assemblyMode?'State-aware local assembly':savingsMode?'Exact paired measurement':mission?'Local product compiler':'Local target compiler';document.getElementById('workflow-label').textContent=dash?'MEASURED LOCAL FACTORY TELEMETRY':assemblyMode?'SAFE STAGES TO EXPLICIT HUMAN BOUNDARY':savingsMode?'BASELINE TO HASH-BOUND SAVINGS RECEIPT':mission?'PRD TO SUPERVISED VALUE MISSION':'INTENT TO PROOF-CARRYING STARTER';document.getElementById('workflow-title').textContent=dash?'Factory control dashboard.':assemblyMode?'Resume without reconstructing state.':savingsMode?'Measure savings without inventing them.':mission?'Compile the next reviewable value slice.':'Choose what you are building.';button.textContent=mission?'Compile product mission':'Compile starter';if(dash)refreshDashboard();}}
document.getElementById('dashboard-mode').onclick=()=>setMode('dashboard');document.getElementById('assembly-mode').onclick=()=>setMode('assembly');document.getElementById('savings-mode').onclick=()=>setMode('savings');document.getElementById('starter-mode').onclick=()=>setMode('starter');document.getElementById('mission-mode').onclick=()=>setMode('mission');const requestedMode=new URLSearchParams(location.search).get('mode');if(requestedMode==='product')setMode('mission');if(requestedMode==='assembly')setMode('assembly');if(requestedMode==='savings')setMode('savings');
