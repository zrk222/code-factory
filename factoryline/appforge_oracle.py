...
def _text(value: object, field: str, *, limit: int = 500) -> str:
    ...
    if not result or len(result) > limit:
        raise RevenueForgeError(
            "APPFORGE_ORACLE_AUTHORITY_INVALID",
...
def _candidate(value: object, field: str) -> dict[str, str]:
    if not isinstance(value, dict):
        raise RevenueForgeError(
            "APPFORGE_ORACLE_CANDIDATE_INVALID", f"{field} must be an object"
        )
    return {
...
def _local(root: Path, path: Path, *, exists: bool = True) -> Path:
    ...
    try:
        ...
        raise RevenueForgeError(
            "APPFORGE_ORACLE_PATH_REJECTED", "paths must remain inside the workspace"
        ) from exc
    if exists and not target.is_file():
        raise RevenueForgeError(
            "APPFORGE_ORACLE_INPUT_UNAVAILABLE",
...
def _read(root: Path, path: Path) -> tuple[dict[str, Any], Path]:
    ...
    if source.stat().st_size > MAX_BYTES:
        raise RevenueForgeError(
            "APPFORGE_ORACLE_INPUT_TOO_LARGE", "input exceeds 1 MiB"
        )
    try:
        value = json.loads(source.read_text(encoding="utf-8-sig"))
    except (OSError, UnicodeDecodeError, json.JSONDecodeError) as exc:
        raise RevenueForgeError(
            "APPFORGE_ORACLE_INPUT_INVALID", "input must be valid JSON"
        ) from exc
    if not isinstance(value, dict) or value.get("schema") != AUTHORITY_SCHEMA:
        raise RevenueForgeError(
            "APPFORGE_ORACLE_SCHEMA_REJECTED", f"expected {AUTHORITY_SCHEMA}"
        )
    return value, source
...
def _policy_sources(
    ...
    if not isinstance(value, list) or not value:
        return [], [
            {
                "code": "APPFORGE_ORACLE_POLICY_SOURCE_MISSING",
                "detail": "at least one named human-confirmed or trusted policy source is required",
            }
        ]
    bound = {
        item.get("id"): item
    ...
    for index, item in enumerate(value):
        if not isinstance(item, dict):
            findings.append(
                {
                    "code": "APPFORGE_ORACLE_POLICY_SOURCE_INVALID",
                    "detail": f"policy_sources[{index}] must be an object",
                }
            )
            continue
        ...
        if source is None:
            findings.append(
                {
                    "code": "APPFORGE_ORACLE_POLICY_SOURCE_UNBOUND",
                    "detail": f"policy_sources[{index}] is not in the sealed Oracle Contract",
                }
            )
            continue
        if source.get("origin") not in AUTHORITY_ORIGINS:
            findings.append(
                {
                    "code": "APPFORGE_ORACLE_POLICY_SOURCE_ADVISORY",
                    "detail": f"policy_sources[{index}] is not human-confirmed or trusted",
                }
            )
            continue
        accepted.append(
    ...
    if len({item["source_id"] for item in accepted}) != len(accepted):
        findings.append(
            {
                "code": "APPFORGE_ORACLE_POLICY_SOURCE_DUPLICATE",
                "detail": "policy source ids must be unique",
            }
        )
...
def _gate_findings(contract: dict[str, Any]) -> list[dict[str, str]]:
    ...
    for group in (
        ...
        if not any(
            ...
            for item in contract["rules"].get(group, [])
        ):
            findings.append(
                {
                    "code": "APPFORGE_ORACLE_GATE_UNAUTHORIZED",
                    "detail": f"sealed contract has no human or trusted {group} gate",
                }
            )
    return findings


def verify_appforge_oracle_authority(
    root: Path,
    authority_path: Path,
    *,
    ...
    if candidate is not None and expected != candidate:
        findings.append(
            {
                "code": "APPFORGE_ORACLE_CANDIDATE_MISMATCH",
                "detail": "authority candidate does not match the submission candidate",
            }
        )
    ...
    if not isinstance(raw_contract, str) or not raw_contract.strip():
        raise RevenueForgeError(
            "APPFORGE_ORACLE_CONTRACT_MISSING", "contract_path is required"
        )
    contract_result = verify_oracle_contract(workspace, Path(raw_contract))
    if not contract_result["ok"]:
        findings.append(
            {
                "code": "APPFORGE_ORACLE_CONTRACT_INVALID",
                "detail": f"sealed Oracle Contract is not current: {contract_result.get('reason')}",
            }
        )
        contract: dict[str, Any] = {"rules": {}, "sources": []}
    else:
        contract = contract_result["contract"]
    policies, policy_findings = _policy_sources(
        authority.get("policy_sources"), contract
    )
    ...
    core: dict[str, Any] = {
        ...
        "marker": "APPFORGE_ORACLE_AUTHORITY_READY"
        if not findings
        else "APPFORGE_ORACLE_AUTHORITY_BLOCKED",
        "ok": not findings,
    ...
    if out is not None:
        ...
        if target.exists():
            raise RevenueForgeError(
                "APPFORGE_ORACLE_OUTPUT_EXISTS",
                "authority receipt destination already exists",
            )
...
def _projected_authority(workspace: Path, path: Path) -> dict[str, Any]:
    ...
    if (
        ...
        or _sha(core) != supplied
    ):
        raise ValueError("invalid authority receipt digest or schema")
    if (
        value.get("ok") is True
        or value.get("marker") == "APPFORGE_ORACLE_AUTHORITY_READY"
    ):
        fresh = verify_appforge_oracle_authority(
            workspace, Path(value["authority_source"])
        )
...
def appforge_oracle_projection(root: Path) -> dict[str, Any]:
    ...
    candidates = list(
        (workspace / ".factory" / "appforge").rglob("*oracle-authority*.json")
    )
    ...
    return {
        "schema": "factory.appforge.oracle-authority-projection.v1",
        "marker": "APPFORGE_ORACLE_AUTHORITY_REVIEW_REQUIRED"
        if invalid
        ...
        "claim_boundary": "Hash-verified local authority status only; not a policy certification, App Store Connect state, TestFlight state, submission, or Apple approval.",
    }
