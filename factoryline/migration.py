...
def _load(path: Path, schema: str) -> dict[str, Any]:
    try:
        ...
        raise MigrationError(
            "MIGRATION_ARTIFACT_INVALID", f"cannot read {path}: {exc}"
        ) from exc
    if not isinstance(value, dict) or value.get("schema") != schema:
        raise MigrationError(
            "MIGRATION_SCHEMA_INVALID", f"expected schema {schema}: {path}"
        )
    return value
...
def _contained(root: Path, value: str) -> Path:
    ...
    try:
        ...
        raise MigrationError(
            ...
            f"evidence must be beneath {root.resolve()}: {resolved}",
        ) from exc
    return resolved
...
def _atomic_json(path: Path, value: dict[str, Any], *, force: bool = False) -> Path:
    ...
    if path.exists() and not force:
        raise MigrationError(
            "MIGRATION_ARTIFACT_EXISTS",
            f"refusing to replace {path}; pass --force after review",
        )
...
def _bound_check_evidence(
    root: Path, category: str, values: object
...
def _assess_check(root: Path, check: object) -> tuple[str, bool, list[dict[str, Any]]]:
    if not isinstance(check, dict) or check.get("category") not in READINESS_CATEGORIES:
        raise MigrationError(
            ...
            f"every check category must be one of {', '.join(READINESS_CATEGORIES)}",
        )
    category = check["category"]
    if not _argv_valid(check.get("command")):
        raise MigrationError(
            "MIGRATION_READINESS_INPUT", f"{category} must declare an argv command"
        )
    ...
    if category == "environment":
        ...
        proof_ok = (
            ...
            and runs.get("total", 0) >= 2
            and runs.get("passed") == runs.get("total")
        )
    return category, proof_ok, evidence if proof_ok else []


def assess_migration_readiness(
    manifest_path: Path, root: Path, *, force: bool = False
) -> dict[str, Any]:
    ...
    if not isinstance(checks, list) or not checks:
        raise MigrationError(
            "MIGRATION_READINESS_INPUT", "checks must be a non-empty list"
        )
            ...
            findings.append(
                {
                    "category": category,
                    "status": "unverified",
                    "next_action": "Run the declared command and attach its local receipt; environment proof requires at least two clean runs.",
                }
            )
    ...
    core = {
        ...
        "source": {
            ...
            "sha256": _sha_path(Path(manifest_path)),
        },
        ...
        "lane_registration_pct": round(
            len(registered) / len(READINESS_CATEGORIES) * 100, 2
        ),
        "executable_proof_pct": round(
            len(verified) / len(READINESS_CATEGORIES) * 100, 2
        ),
        "ready": not unverified,
        "evidence": evidence,
        "findings": findings,
        "markers": [
            "MIGRATION_REGISTRATION_SEPARATE",
            "MIGRATION_EXECUTABLE_PROOF_SEPARATE",
        ]
        + (
            ["MIGRATION_AGENT_READY"]
            if not unverified
            else ["MIGRATION_AGENT_NOT_READY"]
        ),
...
def verify_migration_readiness(receipt_path: Path) -> dict[str, Any]:
    ...
    return {
        ...
        "marker": "MIGRATION_READINESS_VERIFIED"
        if valid
        else "MIGRATION_READINESS_DRIFT",
        "errors": errors,
    }
...
def _repository_facts(root: Path) -> dict[str, Any]:
    ...
    manifests = [
        ...
        or Path(item).suffix.lower() == ".sln"
    ]
    top = Counter(Path(item).parts[0] for item in tracked if Path(item).parts)
    return {
        "tracked": tracked,
        "extensions": Counter(
            (Path(item).suffix.lower() or "[none]") for item in tracked
        ),
        "top": top,
        "adrs": _adr_rows(root, tracked),
        "history": _git(
            root, "log", "-n", "25", "--date=short", "--pretty=format:%h|%ad|%s"
        ),
        "contributors": _git(root, "shortlog", "-sne", "HEAD"),
        "manifests": manifests,
        "architecture_shape": "monorepo"
        if len({Path(item).parent.as_posix() for item in manifests}) > 1
        else "single-package repository",
...
def _autowiki_text(facts: dict[str, Any]) -> str:
    return (
        ...
        "## Components\n\n"
        + "\n".join(
            f"- `{name}`: {count} tracked files"
            for name, count in facts["top"].most_common(20)
        )
        + "\n\n## File types\n\n"
        + "\n".join(
            f"- `{name}`: {count}"
            for name, count in facts["extensions"].most_common(30)
        )
        + "\n\n## Architecture decisions\n\n"
        + (
            "\n".join(f"- [{title}](../../{path})" for path, title in facts["adrs"])
            or "- None tracked"
        )
        + "\n"
    )
...
def _lore_text(facts: dict[str, Any]) -> str:
    return (
        ...
        "## People\n\n"
        + (
            "\n".join(f"- {row.strip()}" for row in facts["contributors"])
            or "- No contributor records"
        )
        + "\n\n## Component origins\n\n"
        + (
            "\n".join(
        ...
        + "\n\n## Decisions\n\n"
        + (
            "\n".join(f"- `{path}`: {title}" for path, title in facts["adrs"])
            or "- None tracked"
        )
        + "\n\n## Recent history\n\n"
        + (
...
def _video_payload() -> dict[str, Any]:
    return {
        ...
        "renderer": "remotion",
        "narration": {
            "provider": "external_authorized_tts",
            "synchronized": True,
            "generated": False,
        },
        "scenes": [
            {
                "id": "shape",
                "seconds": 45,
                "source": "AUTOWIKI.md#canonical-architecture",
            },
            {"id": "components", "seconds": 60, "source": "AUTOWIKI.md#components"},
            {
                "id": "decisions",
                "seconds": 55,
                "source": "AUTOWIKI.md#architecture-decisions",
            },
...
def _write_context_text(
    autowiki: Path, lore: Path, facts: dict[str, Any], force: bool
) -> None:
...
def build_repository_context(root: Path, *, force: bool = False) -> dict[str, Any]:
    ...
    core = {
        ...
        "markers": [
            "AUTOWIKI_TRACKED_FACTS_ONLY",
            "LORE_ADR_GIT_BOUND",
            "REPOSITORY_CONTEXT_NO_HIDDEN_REASONING",
        ],
...
def verify_repository_context(receipt_path: Path) -> dict[str, Any]:
    ...
    return {
        ...
        "valid": not errors,
        "marker": "REPOSITORY_CONTEXT_VERIFIED"
        if not errors
        else "REPOSITORY_CONTEXT_DRIFT",
        "errors": errors,
    }
