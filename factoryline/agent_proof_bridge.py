...
def _canonical(value: object) -> bytes:
    try:
        ...
        raise AgentProofBridgeError(
            "E_AGENT_BRIDGE_SCHEMA", "input must be canonical JSON"
        ) from exc
...
def _identifier(value: object, field: str) -> str:
    if not isinstance(value, str) or not _IDENTIFIER.fullmatch(value):
        raise AgentProofBridgeError(
            "E_AGENT_BRIDGE_SCHEMA", f"{field} must use 1-96 safe identifier characters"
        )
    return value
...
def _path(value: object, field: str) -> str:
    ...
    if candidate.is_absolute() or ".." in candidate.parts:
        raise AgentProofBridgeError(
            "E_AGENT_BRIDGE_PRIVATE_FIELD", f"{field} must remain workspace-relative"
        )
...
def _inside(root: Path, value: object, field: str, *, exists: bool) -> Path:
    ...
    try:
        ...
        raise AgentProofBridgeError(
            "E_AGENT_BRIDGE_PRIVATE_FIELD", f"{field} must remain beneath the workspace"
        ) from exc
...
def _exact(
    value: object, allowed: set[str], field: str, *, required: set[str] | None = None
) -> dict[str, Any]:
    if not isinstance(value, dict):
        raise AgentProofBridgeError(
            "E_AGENT_BRIDGE_SCHEMA", f"{field} must be an object"
        )
...
def _paths(value: object, field: str) -> list[str]:
    if not isinstance(value, list) or not 1 <= len(value) <= MAX_ITEMS:
        raise AgentProofBridgeError(
            "E_AGENT_BRIDGE_SCHEMA", f"{field} must contain 1-{MAX_ITEMS} paths"
        )
...
def _scope_allows(scope: list[str], candidate: str) -> bool:
    return any(
        item == "." or candidate == item or candidate.startswith(item.rstrip("/") + "/")
        for item in scope
    )


def _source_preconditions(
    root: Path, value: object, scope: list[str]
) -> list[dict[str, str]]:
    if not isinstance(value, list) or not 1 <= len(value) <= MAX_ITEMS:
        raise AgentProofBridgeError(
            "E_AGENT_BRIDGE_EVIDENCE_UNVERIFIED",
            "source_preconditions must contain 1-64 entries",
        )
    ...
    for index, raw in enumerate(value):
        ...
        if not _scope_allows(scope, path):
            raise AgentProofBridgeError(
                ...
                "source precondition is outside the sealed Oracle scope",
            )
        ...
        if hashlib.sha256(source.read_bytes()).hexdigest() != digest:
            raise AgentProofBridgeError(
                "E_AGENT_BRIDGE_EVIDENCE_UNVERIFIED",
                "source precondition does not match current workspace bytes",
            )
        result.append({"path": path, "sha256": digest})
    if len({item["path"] for item in result}) != len(result):
        raise AgentProofBridgeError(
            "E_AGENT_BRIDGE_EVIDENCE_UNVERIFIED",
            "source precondition paths must be unique",
        )
...
def _workflow(value: object) -> dict[str, Any]:
    ...
    for index, raw in enumerate(entry["nodes"]):
        ...
        if node["kind"] not in _NODE_KINDS:
            raise AgentProofBridgeError(
                "E_AGENT_BRIDGE_EVIDENCE_UNVERIFIED",
                "workflow node kind is unsupported",
            )
        nodes.append(
            {
                "id": _identifier(node["id"], f"workflow.nodes[{index}].id"),
                "kind": node["kind"],
            }
        )
    ...
    if _digest(entry["topology_sha256"], "workflow.topology_sha256") != topology:
        raise AgentProofBridgeError(
            "E_AGENT_BRIDGE_EVIDENCE_UNVERIFIED",
            "workflow topology digest does not bind the declared DAG",
        )
    return {
        "id": _identifier(entry["id"], "workflow.id"),
        "definition_sha256": _digest(
            entry["definition_sha256"], "workflow.definition_sha256"
        ),
...
def _evidence(root: Path, value: object, visual: bool) -> list[dict[str, str]]:
    ...
    if len({item["id"] for item in result}) != len(result):
        raise AgentProofBridgeError(
            "E_AGENT_BRIDGE_EVIDENCE_UNVERIFIED",
            "evidence pair identities must be unique",
        )
    if visual and not any(item["kind"] == "visual" for item in result):
        raise AgentProofBridgeError(
            "E_AGENT_BRIDGE_EVIDENCE_UNVERIFIED",
            "a declared visual surface requires a before/after visual evidence pair",
        )
...
def _provider_receipt(provider: str, value: object) -> dict[str, Any]:
    ...
    profiles = {
        ...
        "junie": common
        | {"mission_sha256", "action_policy_sha256", "change_list_sha256"},
...
def _read_receipt(root: Path, path: Path) -> tuple[dict[str, Any], Path]:
    ...
    if target.stat().st_size > MAX_BYTES:
        raise AgentProofBridgeError(
            "E_AGENT_BRIDGE_EVIDENCE_UNVERIFIED", "receipt exceeds 1 MiB"
        )
    try:
        value = json.loads(target.read_text(encoding="utf-8-sig"))
    except (OSError, UnicodeDecodeError, json.JSONDecodeError) as exc:
        raise AgentProofBridgeError(
            "E_AGENT_BRIDGE_SCHEMA", "receipt must be valid JSON"
        ) from exc
    if not isinstance(value, dict) or value.get("schema") != RECEIPT_SCHEMA:
        raise AgentProofBridgeError(
            "E_AGENT_BRIDGE_SCHEMA", "receipt schema is unsupported"
        )
    ...
    if not isinstance(digest, str) or digest != _sha(core):
        raise AgentProofBridgeError(
            "E_AGENT_BRIDGE_EVIDENCE_UNVERIFIED",
            "receipt digest does not match its canonical body",
        )
    return value, target
...
def _resume(
    ...
    if (
        ...
        != workflow["topology_sha256"]
        or prior.get("oracle", {}).get("contract_sha256") != contract_sha256
    ):
        raise AgentProofBridgeError(
            "E_AGENT_BRIDGE_RESUME_DIVERGENCE",
    ...
    if provider == "eve" and (
        ...
        raise AgentProofBridgeError(
            "E_AGENT_BRIDGE_RESUME_DIVERGENCE",
            "Eve checkpoint differs from the declared current checkpoint",
        )
    if provider == "eve" and (
        checkpoint != prior_profile.get("checkpoint_id")
        or checkpoint_sha != prior_profile.get("checkpoint_sha256")
    ):
        raise AgentProofBridgeError(
            "E_AGENT_BRIDGE_RESUME_DIVERGENCE",
...
def import_agent_proof(
    ...
    if source.stat().st_size > MAX_BYTES:
        raise AgentProofBridgeError(
            "E_AGENT_BRIDGE_PRIVATE_FIELD", "envelope exceeds 1 MiB"
        )
    try:
    ...
    entry = _exact(
        ...
        allowed,
        "envelope",
        required=allowed - {"resume", "semantic_authority", "intake_parameters"},
    )
    if (
        entry.get("schema") != ENVELOPE_SCHEMA
        or entry.get("provider") not in _PROVIDERS
        or entry.get("status") not in _STATUS
    ):
        raise AgentProofBridgeError(
            "E_AGENT_BRIDGE_SCHEMA",
    ...
    if (
        ...
        raise AgentProofBridgeError(
            "E_AGENT_BRIDGE_SCHEMA", "autonomy or isolation is unsupported"
        )
    if entry["autonomy"] == "autonomous" and entry["isolation"] == "unverified":
        raise AgentProofBridgeError(
            "E_AGENT_BRIDGE_EVIDENCE_UNVERIFIED",
            "autonomous evidence requires a declared isolated boundary",
        )
    ...
    if (
        not checked_contract.get("ok")
        or checked_contract.get("contract", {}).get("contract_sha256") != contract_sha
    ):
        raise AgentProofBridgeError(
            "E_AGENT_BRIDGE_UNBOUND_INTENT",
            "agent proof must bind one current sealed Oracle Contract",
        )
    ...
    try:
        ...
        code = (
            "E_AGENT_BRIDGE_SCOPE_ESCAPE"
            if exc.code == "ORACLE_SCOPE_ESCAPE"
    ...
    core = {
        ...
        "imported_at": _now(),
        "envelope": {
            "id": _identifier(entry["envelope_id"], "envelope_id"),
    ...
    destination = (
        ...
        / "agent-bridges"
        / f"{provider}-{run_id}-{receipt['receipt_sha256'][:12]}.json"
    )
    target = _inside(workspace, destination.as_posix(), "out", exists=False)
    if target.exists():
        raise AgentProofBridgeError(
            "E_AGENT_BRIDGE_EVIDENCE_UNVERIFIED",
            "output receipt already exists; use a new immutable path",
        )
...
def agent_proof_projection(root: Path) -> dict[str, Any]:
        ...
        receipts.append(
            {
                ...
                "contract_sha256": receipt["oracle"]["contract_sha256"],
                "workflow_id": receipt["workflow"]["id"],
                "stage_count": len(receipt["workflow"]["nodes"]),
                "evidence_pair_count": len(receipt["evidence_pairs"]),
                "visual": receipt["surface"] == "visual",
                "resumed": receipt.get("resume") is not None,
                "semantic_authority_bound": bool(
                    receipt.get("semantic_authority", {}).get("bound")
                )
                if isinstance(receipt.get("semantic_authority"), dict)
                else False,
                "intake_parameters_bound": isinstance(
                    receipt.get("intake_parameters"), dict
                ),
    ...
    provider_counts = {
        provider: sum(item["provider"] == provider for item in receipts)
    ...
    return {
        ...
        "visual_evidence_count": sum(int(item["visual"]) for item in receipts),
        "semantic_authority_bound_count": sum(
            int(item["semantic_authority_bound"]) for item in receipts
        ),
...
def agent_handoff_brief(root: Path, contract_path: Path) -> dict[str, Any]:
    ...
    if not checked.get("ok"):
        raise AgentProofBridgeError(
            "E_AGENT_BRIDGE_UNBOUND_INTENT",
            "handoff brief requires one current sealed Oracle Contract",
        )
    ...
    if not isinstance(contract, dict) or not isinstance(
        ...
        raise AgentProofBridgeError(
            "E_AGENT_BRIDGE_UNBOUND_INTENT", "sealed Oracle Contract is incomplete"
        )
...
def provider_template(provider: str) -> dict[str, Any]:
    ...
    if provider not in _PROVIDERS:
        raise AgentProofBridgeError(
            "E_AGENT_BRIDGE_PROVIDER_PROFILE",
            "provider must be eve, junie, grok_build, coderabbit, devin, or generic",
        )
    common = {
        "session_id": "replace-with-safe-run-id",
        "runtime_sha256": "replace-with-sha256",
        "tool_manifest_sha256": "replace-with-sha256",
    }
    if provider == "eve":
        common.update(
            {
                ...
                "deployment_commit_sha": "replace-with-lowercase-git-sha",
            }
        )
    elif provider == "junie":
        common.update(
            {
                "mission_sha256": "replace-with-sha256",
                "action_policy_sha256": "replace-with-sha256",
                "change_list_sha256": "replace-with-sha256",
            }
        )
    elif provider == "grok_build":
        common.update(
            {
                "stream_sha256": "replace-with-sha256",
                "mode": "headless",
                "permission_policy_sha256": "replace-with-sha256",
            }
        )
    elif provider == "coderabbit":
        common.update(
            {
                "review_output_sha256": "replace-with-sha256-of-local-jsonl",
                "review_mode": "agent",
                "base_commit_sha": "replace-with-lowercase-git-sha",
                "head_commit_sha": "replace-with-lowercase-git-sha",
                "finding_count": 0,
            }
        )
    elif provider == "devin":
        common.update(
            {
                "task_sha256": "replace-with-sha256",
                "result_sha256": "replace-with-sha256",
                "base_commit_sha": "replace-with-lowercase-git-sha",
                "head_commit_sha": "replace-with-lowercase-git-sha",
                "permission_profile_sha256": "replace-with-sha256",
            }
        )
    return {
        "schema": ENVELOPE_SCHEMA,
        "provider": provider,
        "provider_receipt": common,
        "authority": dict(AUTHORITY),
        "claim_boundary": "Template only. Replace every placeholder with a reviewed local digest; never put prompts, source bodies, URLs, credentials, or provider tokens in an envelope.",
    }
