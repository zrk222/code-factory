...
from .change_review import review_change
...
def _load_plan(value: Path | dict[str, Any]) -> dict[str, Any]:
    if isinstance(value, Path):
        try:
            ...
            raise PlanProofReviewError(
                "PLAN_TO_PROOF_PLAN_INVALID",
                f"agent plan cannot be read as JSON: {exc}",
            ) from exc
    else:
        loaded = value
...
def _item_for_path(plan: dict[str, Any], path: str) -> dict[str, Any] | None:
    matches = [
        ...
        if path in item["paths"] or path in item["test_paths"]
    ]
    if len(matches) > 1:
        raise PlanProofReviewError(
            "PLAN_TO_PROOF_PLAN_INVALID", f"path {path} maps to multiple plan items"
        )
...
def _changed_path_alignment(
    ...
    declared_unmodified = [
        item["id"] for item in plan["items"] if item["id"] not in changed_item_ids
    ]
    findings: list[dict[str, Any]] = []
    if unplanned:
        findings.append(
            _finding(
                "unplanned_changed_path",
                "blocking",
...
def _next_action(
    plan_findings: list[dict[str, Any]], source_review: dict[str, Any]
) -> dict[str, Any]:
    for finding in plan_findings:
        if finding["kind"] == "unplanned_changed_path":
            return {
                "action": "reconcile_unplanned_change",
                "reason": finding["message"],
                "path": finding["facts"]["path"],
            }
    for finding in plan_findings:
        if finding["kind"] == "declared_test_path_missing":
            return {
                "action": "provide_declared_test_change",
                "reason": finding["message"],
                "item_id": finding["facts"]["item_id"],
            }
    for finding in plan_findings:
        if finding["kind"] == "named_human_review_required":
            return {
                "action": "route_to_named_reviewer",
                "reason": finding["message"],
                "review_owner": finding["facts"]["review_owner"],
            }
    if source_review["next_action"]["action"] != "review_packet":
        return dict(source_review["next_action"])
    return {
        "action": "review_packet",
        "reason": "Plan scope and existing Diff-to-Proof facts have no higher-priority action.",
    }
...
def _mermaid(review: dict[str, Any]) -> str:
    lines = [
        "flowchart LR",
        '  PLAN["Approved agent plan"]',
        '  DIFF["Changed paths"]',
        '  PROOF["Diff-to-Proof facts"]',
    ]
...
def review_plan_proof(
    root: Path, plan_path: Path, *, base: str = "main", changed: list[str] | None = None
) -> dict[str, Any]:
...
def validate_plan_proof_review(value: object) -> dict[str, Any]:
    ...
    if not required.issubset(value) or set(value) - (required | {"artifacts"}):
        raise PlanProofReviewError(
            "PLAN_TO_PROOF_REVIEW_INVALID",
            "the Plan-to-Proof review has unsupported or missing fields",
        )
    plan = validate_agent_plan(value["plan"])
    if value["plan_sha256"] != _sha(plan):
        raise PlanProofReviewError(
            "PLAN_TO_PROOF_REVIEW_INVALID",
            "the canonical agent-plan SHA-256 does not match",
        )
    if value["authority"] != AUTHORITY:
        raise PlanProofReviewError(
            "PLAN_TO_PROOF_REVIEW_INVALID",
            "the Plan-to-Proof authority boundary changed",
        )
    core = {
        key: value[key]
        for key in required - {"review_sha256", "mermaid", "review_markdown"}
    }
    if value["review_sha256"] != _sha(core):
        raise PlanProofReviewError(
            "PLAN_TO_PROOF_REVIEW_INVALID",
            "the Plan-to-Proof review SHA-256 does not match",
        )
    return value
...
def write_plan_proof_review_artifacts(
    review: dict[str, Any], out_dir: Path
) -> dict[str, Any]:
    ...
    payload = {
        ...
        if key not in {"artifacts", "review_markdown", "mermaid"}
    }
    digests = {
        "json": _atomic_text(
            paths["json"],
