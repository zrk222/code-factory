    ...
    try:
        ...
        if candidate is not None:
            if (
                ...
                or set(candidate)
                - {"source_version", "source_commit", "artifact_versions"}
            ):
                raise ValueError("release contract candidate binding is invalid")
            if (
                not isinstance(candidate.get("source_version"), str)
                or not candidate["source_version"].strip()
            ):
                raise ValueError("release contract candidate source_version is invalid")
            if not isinstance(candidate.get("source_commit"), str) or not re.fullmatch(
                r"[0-9a-f]{40,64}", candidate["source_commit"]
            ):
            ...
            if artifact_versions is not None:
                if not isinstance(artifact_versions, dict) or set(artifact_versions) - {
                    "python",
                    "vscode",
                    "intellij",
                }:
                    raise ValueError(
                        "release contract candidate artifact_versions is invalid"
                    )
                if not artifact_versions or not all(
                    isinstance(value, str)
                    and re.fullmatch(r"\d+\.\d+\.\d+", value.strip())
                    for value in artifact_versions.values()
                ):
                    raise ValueError(
        ...
        if "appforge:mobile-evidence" in declared_set:
            ...
            if not mobile.get("ok"):
                return {
                    ...
                    "reason": str(
                        mobile.get("reason", "mobile evidence is not current")
                    ),
                }
        elif evidence:
            return {
