...
def _timestamp(value: object, field: str) -> datetime:
    if not isinstance(value, str):
        raise AgentLicenseError(
            "E_LICENSE_INPUT_INVALID", f"{field} must be RFC3339 text"
        )
    try:
    ...
    if parsed.tzinfo is None:
        raise AgentLicenseError(
            "E_LICENSE_INPUT_INVALID", f"{field} must include a timezone"
        )
...
def _relative(root: Path, value: object, field: str) -> tuple[Path, str]:
    ...
    if not text.strip() or len(text.strip()) > 2048:
        raise AgentLicenseError(
            "E_LICENSE_INPUT_INVALID", f"{field} must be a non-empty path"
        )
    text = text.replace("\\", "/")
    candidate = Path(text)
    ...
    try:
        ...
        raise AgentLicenseError(
            "E_LICENSE_PATH_OUT_OF_SCOPE", f"{field} must remain inside the workspace"
        ) from exc
    return resolved, relative


def _load_json(
    path: Path, *, code: str = "E_LICENSE_INPUT_UNREADABLE"
) -> dict[str, Any]:
    try:
...
def _file_binding(root: Path, value: object, field: str) -> dict[str, str]:
    ...
    if not path.is_file():
        raise AgentLicenseError(
            ...
            f"{field} must name an existing workspace file",
        )
...
def normalize_agent_identity(value: object, field: str = "agent") -> dict[str, str]:
    ...
    if (
        ...
        or not {"schema", "subject", "provider", "model"}.issubset(value)
        or set(value) - allowed
    ):
        raise AgentLicenseError(
            "E_LICENSE_IDENTITY_INVALID",
            f"{field} must contain schema, subject, provider, and model",
        )
    if value.get("schema") != AGENT_IDENTITY_SCHEMA:
        raise AgentLicenseError(
            "E_LICENSE_IDENTITY_INVALID",
            f"{field}.schema must equal {AGENT_IDENTITY_SCHEMA}",
        )
    identity = {
    ...
    if "identity_sha256" in value and value["identity_sha256"] != digest:
        raise AgentLicenseError(
            "E_LICENSE_IDENTITY_INVALID",
            f"{field}.identity_sha256 does not match identity fields",
        )
...
def _normalized_paths(value: object, field: str) -> list[str]:
    if not isinstance(value, list) or not value or len(value) > 64:
        raise AgentLicenseError(
            "E_LICENSE_INPUT_INVALID",
            f"{field} must contain 1 through 64 workspace paths",
        )
    paths: list[str] = []
    for item in value:
        ...
        if path.is_absolute() or ".." in path.parts:
            raise AgentLicenseError(
                ...
                f"{field} paths must be workspace relative",
            )
        paths.append(path.as_posix())
    if len(set(paths)) != len(paths):
        raise AgentLicenseError(
            "E_LICENSE_INPUT_INVALID", f"{field} paths must be unique"
        )
    return sorted(paths)
...
def _event_core(value: dict[str, Any]) -> dict[str, Any]:
    ...
    if set(value) != fields or value.get("schema") != AGENT_RUN_SCHEMA:
        raise AgentLicenseError(
            ...
            "agent run event has an unsupported schema or fields",
        )
    core = {key: value[key] for key in fields - {"event_sha256"}}
    if value.get("event_sha256") != _sha(core):
        raise AgentLicenseError(
            "E_LICENSE_EVENT_INVALID", "agent run event hash mismatch"
        )
    return core
...
def _validate_event_identity(
    ...
    if not _TASK_IDENTIFIER.fullmatch(event_id):
        raise AgentLicenseError(
            "E_LICENSE_EVENT_INVALID",
            "event_id must use lowercase letters, digits, and hyphens",
        )
    ...
    if task_id is not None and (
        not isinstance(task_id, str) or not _TASK_IDENTIFIER.fullmatch(task_id)
    ):
        raise AgentLicenseError(
            "E_LICENSE_EVENT_INVALID",
...
def _validate_event_bindings(
    ...
    if not isinstance(admission, dict) or set(admission) != {"path", "packet_sha256"}:
        raise AgentLicenseError(
            "E_LICENSE_EVENT_INVALID", "admission must contain path and packet_sha256"
        )
    if not isinstance(result, dict) or set(result) != {"path", "sha256"}:
        raise AgentLicenseError(
            "E_LICENSE_EVENT_INVALID", "result_receipt must contain path and sha256"
        )
...
def _validate_event_verifier(
    ...
    if verifier_subject == agent["subject"]:
        raise AgentLicenseError(
            "E_LICENSE_VERIFIER_NOT_INDEPENDENT",
            "verifier subject must differ from the agent subject",
        )
    verifier_receipt = verification.get("receipt")
    if not isinstance(verifier_receipt, dict) or set(verifier_receipt) != {
        "path",
        "sha256",
    }:
        raise AgentLicenseError(
...
def _verified_admission_context(
    ...
    if not admission_path.is_file():
        raise AgentLicenseError(
            "E_LICENSE_ADMISSION_INVALID", "admission packet is missing"
        )
...
def _run_input_verifier(
    ...
    if not isinstance(verification, dict) or set(verification) != {
        ...
        raise AgentLicenseError(
            "E_LICENSE_INPUT_INVALID", "verification must contain subject and receipt"
        )
    verifier_subject = _text(verification.get("subject"), "verification.subject")
    if verifier_subject == agent["subject"]:
        raise AgentLicenseError(
            "E_LICENSE_VERIFIER_NOT_INDEPENDENT",
            "verifier subject must differ from agent subject",
        )
...
def _incident(event: dict[str, Any]) -> dict[str, Any] | None:
    ...
    core = {
        ...
        "effect": {
            ...
            "requalification_clean_runs": POST_INCIDENT_REQUALIFICATION_RUNS,
        },
        "scope_limits": [
            "Incident capsules classify a governed result; they do not diagnose a person or external identity."
        ],
    }
    return {**core, "incident_sha256": _sha(core)}


def _store_governed_event(
    workspace: Path, event: dict[str, Any], out_dir: Path | None
) -> dict[str, Any]:
    ...
    try:
        ...
        raise AgentLicenseError(
            "E_LICENSE_PATH_OUT_OF_SCOPE", "out_dir must remain inside workspace"
        ) from exc
    ...
    if output.exists():
        ...
        if existing.get("event_sha256") == final["event_sha256"]:
            return {
                "marker": "AGENT_LICENSE_EVENT_ALREADY_RECORDED",
                "event": existing,
                "path": str(output.resolve()),
                "incident_path": None,
            }
        raise AgentLicenseError(
            "E_LICENSE_EVENT_EXISTS",
            "event id is already bound to different immutable content",
        )
    ...
    return {
        "marker": "AGENT_LICENSE_EVENT_RECORDED",
        "event": final,
        "path": str(output.resolve()),
        "incident_path": str(incident_path.resolve()) if incident_path else None,
    }


def record_bound_governed_event(
    root: Path, event: dict[str, Any], *, out_dir: Path | None = None
) -> dict[str, Any]:
    ...
    admission_path, _ = _relative(
        workspace, validated["admission"]["path"], "admission"
    )
    ...
    if packet.get("packet_sha256") != validated["admission"]["packet_sha256"]:
        raise AgentLicenseError(
            "E_LICENSE_ADMISSION_INVALID",
            "admission packet digest does not match the pre-run binding",
        )
    return _store_governed_event(workspace, validated, out_dir)


def record_governed_run(
    root: Path, event_path: Path, *, out_dir: Path | None = None
) -> dict[str, Any]:
...
def load_governed_runs(
    root: Path, *, agent: dict[str, Any] | None = None
) -> list[dict[str, Any]]:
    ...
    for path in sorted((workspace / EVENT_DIR).glob("*.json")):
        try:
            value = _validate_ledger_event(
                workspace, _load_json(path), require_evidence_files=True
            )
        ...
        if (
            identity is None
            or event["agent"]["identity_sha256"] == identity["identity_sha256"]
        ):
            events.append(event)
...
def _license_evidence(
    root: Path, identity: dict[str, Any], instant: datetime
) -> dict[str, Any]:
    ...
    current = [
        ...
        if _timestamp(event["recorded_at"], "recorded_at") >= expiry_cutoff
    ]
    stale = [event for event in evidence if event not in current]
    current.sort(key=lambda item: (item["recorded_at"], item["event_id"]))
    incidents = [
        event for event in current if set(event["failure_classes"]) & SEVERE_FAILURES
    ]
    try:
    ...
    event_incident_at = (
        _timestamp(latest_incident["recorded_at"], "recorded_at")
        if latest_incident
        else None
    )
    oracle_incident_at = max(
        (_timestamp(item["recorded_at"], "recorded_at") for item in oracle_incidents),
        default=None,
    )
    latest_incident_at = max(
        (item for item in (event_incident_at, oracle_incident_at) if item is not None),
        default=None,
    )
    after_incident = [
        event
        for event in current
        if latest_incident_at is None
        or _timestamp(event["recorded_at"], "recorded_at") > latest_incident_at
    ]
    clean_after_incident = [
        event
        for event in after_incident
        if event["passed"] and not event["failure_classes"]
    ]
    independent = [
        event
        for event in clean_after_incident
        if event["verification"]["subject"] != identity["subject"]
    ]
    ...
    return {
        ...
        "stale": stale,
        "incidents": incidents,
        "oracle_incidents": oracle_incidents,
        "latest_incident": latest_incident,
        "latest_incident_at": latest_incident_at,
        "clean": clean_after_incident,
...
def _license_tier(evidence: dict[str, Any]) -> tuple[str, str]:
    ...
    if not current:
        return (
            ("supervised", "EVIDENCE_EXPIRED")
            if all_events
            else ("human_controlled", "INSUFFICIENT_GOVERNED_EVIDENCE")
        )
    if (
        evidence["latest_incident"] is not None
        and len(evidence["clean"]) < POST_INCIDENT_REQUALIFICATION_RUNS
    ):
        return "human_controlled", "SEVERE_FAILURE_DEMOTION"
    if (
        len(evidence["clean"]) >= AUTONOMOUS_MIN_CLEAN_RUNS
        and len(evidence["independent"]) >= AUTONOMOUS_MIN_INDEPENDENT_VERIFICATIONS
        and evidence["scopes"]
    ):
...
def derive_license(
    root: Path, agent: dict[str, Any], *, now: datetime | None = None
) -> dict[str, Any]:
    ...
    expires_at = (
        _iso(
            _timestamp(latest["recorded_at"], "recorded_at")
            + timedelta(days=EVIDENCE_TTL_DAYS)
        )
        if latest
        else None
    )
    core = {
        ...
        "evidence": {
            ...
            "independent_verification_count_since_latest_incident": len(
        ...
        "incidents": [
            {
                ...
                "failure_classes": sorted(
                    set(event["failure_classes"]) & SEVERE_FAILURES
                ),
                "event_sha256": event["event_sha256"],
            }
        ...
        + [
            {
                ...
                "failure_classes": [FailureClass.ORACLE_WEAKENING.value],
                "incident_sha256": item["incident_sha256"],
            }
...
def issue_license(
    root: Path, agent: dict[str, Any], *, out: Path | None = None
) -> dict[str, Any]:
    ...
    target = (
        ...
        / LICENSE_DIR
        / f"{license_value['agent']['identity_sha256'][:24]}.json"
    )
    target = target if target.is_absolute() else workspace / target
    try:
        target.resolve().relative_to(workspace)
        ...
        raise AgentLicenseError(
            "E_LICENSE_PATH_OUT_OF_SCOPE", "license output must remain inside workspace"
        ) from exc
    _atomic_json(target, license_value)
    return {
        "marker": "AGENT_LICENSE_ISSUED",
...
def verify_license(path: Path) -> dict[str, Any]:
    ...
    try:
        ...
        fields = set(value)
        if (
            value.get("schema") != AGENT_LICENSE_SCHEMA
            or "license_sha256" not in fields
        ):
        ...
        return {
            "schema": AGENT_LICENSE_SCHEMA,
            "marker": "AGENT_LICENSE_INVALID",
            "ok": False,
            "reason": exc.code,
        }
    return {
        "schema": AGENT_LICENSE_SCHEMA,
        "marker": "AGENT_LICENSE_VERIFIED",
        "ok": True,
...
def seal_license(
    ...
    *,
    ...
    if not verified["ok"]:
        raise AgentLicenseError(
            "E_LICENSE_SIGNING_FAILED", "license must verify before sealing"
        )
    value = _load_json(Path(path))
    try:
        ...
        payload = {
            ...
            "module": "agent-license",
            "stage": "license",
            "feature": value["agent"]["identity_sha256"][:32],
            "ok": True,
            "tenant_id": _text(tenant_id, "tenant_id"),
            "run_id": value["license_sha256"][:32],
            "ts": _iso(_now()),
            "subject_sha256": value["license_sha256"],
        }
    ...
    return {
        "schema": AGENT_LICENSE_SCHEMA,
        "marker": "AGENT_LICENSE_SEALED",
        "license_sha256": value["license_sha256"],
        "path": str(Path(out).resolve()),
        "payload_sha256": envelope["payload_sha256"],
        "authority": {**_AUTHORITY, "signing": True},
    }


def admission_license_decision(
    root: Path, passport: dict[str, Any], request: dict[str, Any]
) -> dict[str, Any] | None:
    ...
    if requested not in AUTONOMY_RANK:
        raise AgentLicenseError(
            "E_LICENSE_EXCEEDED", "Loop Passport declares an unsupported autonomy level"
        )
    if AUTONOMY_RANK[requested] > AUTONOMY_RANK[license_value["tier"]]:
        raise AgentLicenseError(
            "E_LICENSE_EXCEEDED",
            f"requested {requested} autonomy exceeds current {license_value['tier']} license",
        )
    if requested == "autonomous":
        ...
        if not grants or any(
            ...
            raise AgentLicenseError(
                "E_LICENSE_EXCEEDED",
                "requested paths exceed the current autonomous license scope",
            )
...
def license_projection(root: Path) -> dict[str, Any]:
    ...
    return {
        ...
        "authority": dict(_AUTHORITY),
        "scope_limits": [
            "Projection excludes agent prompts, source bodies, and raw verifier transcripts."
        ],
    }
