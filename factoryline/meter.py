...
class MeterLog:
    ...
    def record(self, t: StageTiming) -> None:
        ...
        if any(
            (value := getattr(t, field)) is not None and value < 0
...
def summarize(
    root: Path, *, baseline_tokens_per_run: int = 4000, runs_projected: int = 1000
) -> dict:
    ...
    if not stages:
        return {
            ...
            "markers": [
                "METER_V2_BACKWARD_COMPATIBLE",
                "FLOW_EFFICIENCY_MEASURED",
                "METER_UNKNOWN_VALUES_PRESERVED",
            ],
            "stages_measured": 0,
    ...
    pct_saved = (
        (tokens_saved / baseline_total_tokens * 100) if baseline_total_tokens else 0.0
    )
    ...
    return {
        ...
        "markers": [
            "METER_V2_BACKWARD_COMPATIBLE",
            "FLOW_EFFICIENCY_MEASURED",
            "METER_TIME_CLASSES_SEPARATE",
            "METER_TOKEN_COST_QUALITY_SEPARATE",
            "METER_PRODUCTIVITY_GUARDED",
            "METER_QUALITY_OUTCOMES_GUARDED",
        ],
        ...
        "flow": flow,
        "note": (
            "All wall_ms are measured. "
            + (
                "Token counts are reported by one or more modules."
                if tokens_reported
                else "NOTE: no module reported token usage on these runs, so the token-savings "
...
def _known_rate(stages: list[StageTiming], field: str) -> dict:
    ...
    known = [value for value in values if value is not None]
    return {
        "value": round(sum(bool(value) for value in known) / len(known), 4)
        if known
        else None,
        "known": len(known),
        "unknown": len(values) - len(known),
...
def _guarded_ratio(
    numerator: dict, denominator: dict, *, scale: float = 1.0
) -> float | None:
        ...
        gates.append(
            {
                ...
                "stage": stage,
                "runs": len(rows),
                "total_wall_ms": sum(values),
                "avg_wall_ms": round(sum(values) / len(values), 1),
                "max_wall_ms": max(values),
                "failed_runs": sum(not row.ok for row in rows),
            }
        )
    return {
        "schema": "factory.overhead.v1",
        "gates": gates,
        "total_wall_ms": sum(item["total_wall_ms"] for item in gates),
        "scope_limits": [
            "Wall-clock measurements are local run observations, not a machine-independent performance claim.",
            "No gate is skipped by this report; use project policy to decide which gates are required.",
        ],
    }
...
def live_snapshot(
    ...
    return {
        ...
        "activity": {
            ...
            "stage_success_rate": round(successful / len(stages), 4)
            if stages
            else None,
            "latest_stage": (
                {
...
def summary_table(summary: dict) -> str:
    ...
    note = summary["note"]
    tok_line = (
        f"one-time build tokens  : {summary['build_tokens']:,}"
    ...
    lines.extend(
        [
            ...
            + (
                ...
                if activity["stage_success_rate"] is not None
                else "no stages yet"
            ),
            "features observed       : "
            + (", ".join(activity["features_observed"]) or "none yet"),
            "latest stage            : "
            + (
