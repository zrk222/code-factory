...
def _parse_time(value: object, field: str) -> datetime:
    if not isinstance(value, str):
        raise SemanticAuthorityError(
            "SEMANTIC_INPUT_INVALID", f"{field} must be an RFC3339 UTC timestamp"
        )
    try:
    ...
    if parsed.tzinfo is None:
        raise SemanticAuthorityError(
            "SEMANTIC_INPUT_INVALID", f"{field} must include a timezone"
        )
...
def _identifier(value: object, field: str) -> str:
    ...
    if not IDENTIFIER.fullmatch(result):
        raise SemanticAuthorityError(
            "SEMANTIC_INPUT_INVALID", f"{field} must match {IDENTIFIER.pattern}"
        )
    return result
...
def _context(value: object) -> str:
    ...
    if not CONTEXT.fullmatch(result):
        raise SemanticAuthorityError(
            ...
            "context_urn must be a versioned urn:factory:<context>:vN identifier",
        )
    return result
...
def _inside(root: Path, path: Path, *, exists: bool = True) -> Path:
    ...
    try:
        ...
        raise SemanticAuthorityError(
            "SEMANTIC_PATH_OUT_OF_SCOPE", "all paths must remain inside the workspace"
        ) from exc
    if exists and not target.is_file():
        raise SemanticAuthorityError(
            "SEMANTIC_INPUT_UNAVAILABLE", "input must be a regular workspace file"
        )
    return target
...
def _relative_paths(value: object, field: str) -> list[str]:
    if not isinstance(value, list) or not value or len(value) > MAX_ITEMS:
        raise SemanticAuthorityError(
            ...
            f"{field} must contain 1 through {MAX_ITEMS} workspace-relative paths",
        )
    paths: list[str] = []
    for raw in value:
        if not isinstance(raw, str) or not raw.strip():
            raise SemanticAuthorityError(
                "SEMANTIC_SCOPE_INVALID", f"{field} contains an empty path"
            )
        path = Path(raw.replace("\\", "/"))
        if path.is_absolute() or ".." in path.parts:
            raise SemanticAuthorityError(
                "SEMANTIC_SCOPE_INVALID",
                f"{field} paths must remain workspace-relative",
            )
...
def _actions(value: object, field: str) -> list[str]:
    if not isinstance(value, list) or not value or len(value) > MAX_ITEMS:
        raise SemanticAuthorityError(
            ...
            f"{field} must contain 1 through {MAX_ITEMS} actions",
        )
    actions = sorted({_identifier(item, field) for item in value})
    if any(action.lower() in FORBIDDEN_ACTIONS for action in actions):
        raise SemanticAuthorityError(
            "SEMANTIC_ACTION_FORBIDDEN",
            "local semantic authority never grants an external or consequential action",
        )
    return actions


def _epistemic_entry(
    value: object, field: str, *, source_ids: set[str], require_source: bool
) -> dict[str, Any]:
    if not isinstance(value, dict):
        raise SemanticAuthorityError(
            "SEMANTIC_EPISTEMIC_INVALID", f"{field} entries must be objects"
        )
    ...
    if require_source:
        ...
        if source_id not in source_ids:
            raise SemanticAuthorityError(
                "SEMANTIC_EPISTEMIC_INVALID",
                f"{field}.source_id must be bound by the sealed Oracle Contract",
            )
        result.update(
            {
                "statement": _text(
                    value.get("statement"), f"{field}.statement", limit=320
                ),
                "source_id": source_id,
            }
        )
        ...
        result.update(
            {
                ...
                "impact": _text(value.get("impact"), f"{field}.impact", limit=320),
            }
        )
        if field.startswith("unknown"):
            ...
            if not isinstance(blocking, bool):
                raise SemanticAuthorityError(
                    "SEMANTIC_EPISTEMIC_INVALID", f"{field}.blocking must be boolean"
                )
...
def _epistemic(value: object, sources: dict[str, dict[str, Any]]) -> dict[str, Any]:
    if not isinstance(value, dict):
        raise SemanticAuthorityError(
            "SEMANTIC_EPISTEMIC_INVALID",
            "epistemic must declare known, unknown, uncertain, and capability_limits",
        )
    ...
    for key, (require_source, field) in groups.items():
        ...
        if not isinstance(items, list) or not items or len(items) > 16:
            raise SemanticAuthorityError(
                "SEMANTIC_EPISTEMIC_INVALID",
                f"epistemic.{key} must contain 1 through 16 explicit declarations",
            )
        parsed = [
            _epistemic_entry(
                item, field, source_ids=source_ids, require_source=require_source
            )
            for item in items
        ]
        if len({item["id"] for item in parsed}) != len(parsed):
            raise SemanticAuthorityError(
                "SEMANTIC_EPISTEMIC_INVALID",
                f"epistemic.{key} identifiers must be unique",
            )
        result[key] = parsed
    ...
    if not isinstance(limits, list) or not limits or len(limits) > 16:
        raise SemanticAuthorityError(
            "SEMANTIC_EPISTEMIC_INVALID",
            "epistemic.capability_limits must contain 1 through 16 declared limits",
        )
    result["capability_limits"] = sorted(
        {_text(item, "epistemic.capability_limits", limit=240) for item in limits}
    )
    return result
...
def _read_json(root: Path, path: Path, schema: str) -> tuple[dict[str, Any], Path]:
    ...
    if source.stat().st_size > MAX_BYTES:
        raise SemanticAuthorityError(
            "SEMANTIC_INPUT_TOO_LARGE", "JSON input exceeds 1 MiB"
        )
    try:
        value = json.loads(source.read_text(encoding="utf-8-sig"))
    except (OSError, UnicodeDecodeError, json.JSONDecodeError) as exc:
        raise SemanticAuthorityError(
            "SEMANTIC_INPUT_INVALID", "input must be valid UTF-8 JSON"
        ) from exc
...
def _valid(value: object, schema: str, field: str) -> bool:
    if (
        ...
        return False
    return (
        _sha({key: item for key, item in value.items() if key != field}) == value[field]
    )
...
def _atomic_json(path: Path, payload: dict[str, Any]) -> None:
    ...
    try:
        ...
        raise SemanticAuthorityError(
            "SEMANTIC_OUTPUT_EXISTS",
            "destination already exists; receipts are immutable",
        ) from exc
    try:
...
def _contract(root: Path, path: Path) -> tuple[dict[str, Any], Path]:
    ...
    if not checked.get("ok") or not isinstance(checked.get("contract"), dict):
        raise SemanticAuthorityError(
            "E_SEMANTIC_AUTHORIZATION",
            "the referenced Oracle contract is not hash-valid and current",
        )
...
def seal_semantic_handoff(root: Path, input_path: Path, out: Path) -> dict[str, Any]:
    ...
    if not isinstance(contract_raw, str):
        raise SemanticAuthorityError(
            "SEMANTIC_INPUT_INVALID", "oracle_contract is required"
        )
    ...
    if performative not in PERFORMATIVES:
        raise SemanticAuthorityError(
            "SEMANTIC_INPUT_INVALID", "performative is unsupported"
        )
    ...
    if (
        ...
        or bound_source.get("origin") not in AUTHORITY_ORIGINS
    ):
        raise SemanticAuthorityError(
            "E_SEMANTIC_AUTHORIZATION",
            "context source must be a human-confirmed or trusted-source Oracle input",
        )
    ...
    if not _within_scope(scope_paths, contract_scope):
        raise SemanticAuthorityError(
            "E_SEMANTIC_AUTHORIZATION",
            "handoff scope exceeds the sealed Oracle contract",
        )
    sensitivity = value.get("sensitivities", [])
    if (
        not isinstance(sensitivity, list)
        or len(sensitivity) > 16
        or any(not isinstance(item, dict) for item in sensitivity)
    ):
        raise SemanticAuthorityError(
            "SEMANTIC_INPUT_INVALID",
            "sensitivities must be up to 16 structured, non-executable observations",
        )
    cleaned_sensitivity = [
        {
            "id": _identifier(item.get("id"), "sensitivity.id"),
            "when": _text(item.get("when"), "sensitivity.when", limit=240),
            "impact": _text(item.get("impact"), "sensitivity.impact", limit=240),
        }
    ...
    core = {
        ...
        "marker": "SEMANTIC_HANDOFF_SEALED",
        "handoff_id": _identifier(value.get("id"), "id"),
        "sealed_at": _stamp(),
        "performative": performative,
        "goal": _text(value.get("goal"), "goal", limit=320),
        "sender": sender,
        "receiver": receiver,
        "context": {
            "urn": context_urn,
            "source_id": context_source_id,
            "source_sha256": bound_source.get("sha256"),
        },
        "oracle": {
        ...
        "allowed_actions": _actions(value.get("allowed_actions"), "allowed_actions"),
        "sensitivities": cleaned_sensitivity,
        "epistemic": epistemic,
        "authority": dict(AUTHORITY),
        "claim_boundary": "Hash-sealed local message envelope only. It validates declared context, sourced facts, explicit unknowns/uncertainties, stated limits, and scope. It does not prove semantic truth, external identity, private reasoning, tool execution, approval, or network authorization.",
    }
...
def verify_semantic_handoff(root: Path, path: Path) -> dict[str, Any]:
    ...
    try:
        ...
        contract, _ = _contract(
            workspace, Path(str(value.get("oracle", {}).get("path", "")))
        )
        if contract.get("contract_sha256") != value.get("oracle", {}).get(
            "contract_sha256"
        ):
...
def seal_authority_lease(root: Path, input_path: Path, out: Path) -> dict[str, Any]:
    ...
    if not verified.get("ok"):
        raise SemanticAuthorityError(
            "E_SEMANTIC_AUTHORIZATION",
            "the semantic handoff is not current and hash-valid",
        )
    handoff = verified["handoff"]
    if _blocking_unknowns(handoff):
        raise SemanticAuthorityError(
            "E_SEMANTIC_AUTHORIZATION",
            "a blocking epistemic unknown must be resolved in a newly sealed handoff before a lease can be issued",
        )
    delegatee = normalize_agent_identity(value.get("delegatee"), "delegatee")
    if not _identity_matches(delegatee, handoff.get("receiver")):
        raise SemanticAuthorityError(
            "E_SEMANTIC_AUTHORIZATION",
            "lease delegatee must exactly match the handoff receiver",
        )
    scope_paths = _relative_paths(value.get("scope_paths"), "scope_paths")
    actions = _actions(value.get("allowed_actions"), "allowed_actions")
    if not _within_scope(scope_paths, handoff["scope_paths"]) or not set(
        actions
    ).issubset(set(handoff["allowed_actions"])):
        raise SemanticAuthorityError(
            "E_SEMANTIC_AUTHORIZATION",
            "lease cannot widen the sealed handoff scope or actions",
        )
    expires_at = _parse_time(value.get("expires_at"), "expires_at")
    if expires_at <= _now() or expires_at - _now() > MAX_LEASE:
        raise SemanticAuthorityError(
            "E_SEMANTIC_AUTHORIZATION", "lease must expire in the next 24 hours"
        )
    approval_origin = _text(value.get("approval_origin"), "approval_origin", limit=64)
    if approval_origin not in AUTHORITY_ORIGINS:
        raise SemanticAuthorityError(
            "E_SEMANTIC_AUTHORIZATION",
            "only human-confirmed or trusted-source approval may issue a lease",
        )
    core = {
        "schema": LEASE_SCHEMA,
        "marker": "AUTHORITY_LEASE_SEALED",
        "lease_id": _identifier(value.get("id"), "id"),
        "issued_at": _stamp(),
        "expires_at": _stamp(expires_at),
        "handoff": {
            "path": verified["path"],
            "handoff_sha256": handoff["handoff_sha256"],
            "oracle_contract_sha256": handoff["oracle"]["contract_sha256"],
            "context_urn": handoff["context"]["urn"],
        },
        "delegatee": delegatee,
        "scope_paths": scope_paths,
        ...
        "approval": {
            ...
            "approved_by": _text(value.get("approved_by"), "approved_by", limit=160),
            "rationale": _text(value.get("rationale"), "rationale"),
        },
        "authority": dict(AUTHORITY),
        "claim_boundary": "Hash-sealed local, expiring scope receipt only. A runner must independently enforce it; this artifact never calls a tool, changes code, approves work, or grants external authority.",
    }
...
def verify_authority_lease(root: Path, path: Path) -> dict[str, Any]:
    ...
    try:
        ...
        handoff_result = verify_semantic_handoff(
            workspace, Path(str(lease.get("handoff", {}).get("path", "")))
        )
        if not handoff_result.get("ok") or handoff_result["handoff"].get(
            "handoff_sha256"
        ) != lease.get("handoff", {}).get("handoff_sha256"):
            return {"ok": False, "code": "E_SEMANTIC_AUTHORIZATION"}
...
def authorize_semantic_action(
    root: Path, lease_path: Path, request: object
) -> dict[str, Any]:
    ...
    if not checked.get("ok"):
        return {
            "marker": "E_SEMANTIC_AUTHORIZATION",
            "allowed": False,
            "reason": checked.get("code"),
            "authority": dict(AUTHORITY),
        }
    lease = checked["lease"]
    ...
    try:
        ...
        return {
            "marker": "E_SEMANTIC_AUTHORIZATION",
            "allowed": False,
            "reason": getattr(exc, "code", "request_invalid"),
            "authority": dict(AUTHORITY),
        }
    reason = None
    ...
    return {
        "marker": "SEMANTIC_ACTION_CONSTRAINED",
        "allowed": True,
        "reason": "deterministic_constraints_satisfied",
        "action_id": action_id,
        "action": action,
        "scope_paths": scope_paths,
        "lease_sha256": lease["lease_sha256"],
        "authority": dict(AUTHORITY),
        "claim_boundary": "This is an admission decision for a separate runner, not execution, approval, tool authorization, or proof that the requested action is semantically correct.",
    }


def verify_semantic_binding(
    root: Path, binding: object, actor: object, scope_paths: object
) -> dict[str, Any]:
    ...
    if not isinstance(binding, dict) or set(binding) != {
        ...
        raise SemanticAuthorityError(
            "E_SEMANTIC_AUTHORIZATION",
            "semantic_authority must contain only the exact lease and action binding fields",
        )
    lease_path = _inside(
        workspace,
        Path(_text(binding["lease_path"], "semantic_authority.lease_path", limit=512)),
    )
    supplied = _text(
        binding["lease_sha256"], "semantic_authority.lease_sha256", limit=64
    )
    request = {
        "actor": actor,
        "action_id": binding["action_id"],
        "action": binding["action"],
        "context_urn": binding["context_urn"],
        "scope_paths": scope_paths,
    }
    ...
    if not decision.get("allowed") or decision.get("lease_sha256") != supplied:
        raise SemanticAuthorityError(
            "E_SEMANTIC_AUTHORIZATION",
            str(decision.get("reason", "lease binding rejected")),
        )
    return {
        "lease_path": lease_path.relative_to(workspace).as_posix(),
        "lease_sha256": supplied,
        "action_id": decision["action_id"],
        "action": decision["action"],
        "context_urn": binding["context_urn"],
        "claim_boundary": "Current local lease and envelope binding only; not external provider enforcement, tool execution, semantic correctness, approval, or release authority.",
    }


def record_semantic_action_decision(
    root: Path, lease_path: Path, request: object, out: Path
) -> dict[str, Any]:
    ...
    if not decision.get("allowed"):
        raise SemanticAuthorityError(
            "E_SEMANTIC_AUTHORIZATION", str(decision.get("reason"))
        )
    ...
    if requested != destination:
        raise SemanticAuthorityError(
            ...
            "decision output must use the deterministic lease-and-action destination",
        )
    if destination.exists():
        raise SemanticAuthorityError(
            "E_SEMANTIC_REPLAY", "action_id was already recorded for this lease"
        )
    core = {
        "schema": DECISION_SCHEMA,
        **decision,
        "marker": "SEMANTIC_ACTION_DECISION_RECORDED",
        "recorded_at": _stamp(),
    }
    try:
...
def _handoff_or_lease_identity(kind: str, item: dict[str, Any]) -> dict[str, Any]:
    if kind == "handoffs":
        return {
            "sha256": item.get("handoff_sha256"),
            "id": item.get("handoff_id"),
            "context_urn": item.get("context", {}).get("urn"),
            "contract_sha256": item.get("oracle", {}).get("contract_sha256"),
        }
    return {
        "sha256": item.get("lease_sha256"),
        "id": item.get("lease_id"),
        "context_urn": item.get("handoff", {}).get("context_urn"),
        "contract_sha256": item.get("handoff", {}).get("oracle_contract_sha256"),
    }


def _epistemic_counts(handoff: dict[str, Any]) -> dict[str, int]:
    epistemic = (
        handoff.get("epistemic", {})
        if isinstance(handoff.get("epistemic"), dict)
        else {}
    )
    unknown = epistemic.get("unknown", [])
    return {
        "known_count": len(epistemic.get("known", [])),
        "unknown_count": len(unknown),
        "uncertain_count": len(epistemic.get("uncertain", [])),
        "blocking_unknown_count": sum(
            1
            for entry in unknown
            if isinstance(entry, dict) and entry.get("blocking") is True
        ),
        "capability_limit_count": len(epistemic.get("capability_limits", [])),
    }


def _project_handoffs_and_leases(
    workspace: Path, base: Path, result: dict[str, Any]
) -> None:
    ...
    for kind, verifier in kinds:
        ...
        for path in paths:
            _project_handoff_or_lease(
                result, kind, verifier(workspace, path), path, workspace
            )
    ...
    for path in paths:
        try:
            ...
            if not _valid(item, DECISION_SCHEMA, "decision_sha256"):
                raise SemanticAuthorityError(
                    "SEMANTIC_DECISION_HASH_INVALID", "invalid decision"
                )
            result["decision_count"] += 1
            result["decisions"].append(
                {
                    "path": path.relative_to(workspace).as_posix(),
                    "sha256": item["decision_sha256"],
                    "lease_sha256": item.get("lease_sha256"),
...
def semantic_authority_projection(root: Path) -> dict[str, Any]:
    ...
    result: dict[str, Any] = {
        ...
        "marker": "SEMANTIC_AUTHORITY_READ_ONLY",
        "handoff_count": 0,
        ...
        "decisions": [],
        "authority": dict(AUTHORITY),
        "claim_boundary": "Read-only local hash-valid receipt projection. It does not authenticate a real-world identity, inspect semantic truth, run a sandbox, execute a tool, or grant any authority.",
    }
