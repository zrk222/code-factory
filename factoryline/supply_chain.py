...
from .runtime_audit_common import canonical_bytes, require_bool, require_digest, require_int, require_str
...
COLLECTOR_ROLES = frozenset({"local_evaluator", "independent_builder", "independent_security_scanner", "release_attestor"})
...
def _inside(root: Path, supplied: str | Path, label: str) -> tuple[Path, str]:
    ...
    if not raw.strip() or "\x00" in raw:
        raise SupplyChainError("E_PATH_BOUNDARY", f"{label} must be a relative workspace path")
    ...
    try:
        ...
        raise SupplyChainError("E_PATH_BOUNDARY", f"{label} escapes the workspace") from exc
...
def _descriptor(root: Path, value: Any, label: str, *, suffixes: tuple[str, ...] = ()) -> dict[str, Any]:
    if not isinstance(value, dict) or set(value) != {"path", "sha256", "bytes"}:
        raise SupplyChainError("E_DESCRIPTOR_FIELDS", f"{label} must contain exactly path, sha256 and bytes")
    path, relative = _inside(root, value["path"], label)
    if path.is_symlink() or not path.is_file():
        raise SupplyChainError("E_FILE_BOUNDARY", f"{label} is missing or a symlink: {relative}")
    actual_digest, actual_bytes = _file_digest(path)
    supplied_digest = require_digest(value["sha256"], f"{label}.sha256")
    supplied_bytes = require_int(value["bytes"], f"{label}.bytes", minimum=0, maximum=MAX_ARCHIVE_BYTES)
    if actual_digest != supplied_digest:
        raise SupplyChainError("E_FILE_DIGEST", f"{relative} digest does not match its descriptor")
    if actual_bytes != supplied_bytes:
        raise SupplyChainError("E_FILE_SIZE", f"{relative} byte count does not match its descriptor")
    if suffixes and not relative.lower().endswith(suffixes):
        raise SupplyChainError("E_ARTIFACT_SUFFIX", f"{relative} is not an allowed release artifact")
    return {"path": relative, "sha256": actual_digest, "bytes": actual_bytes}


def _list_descriptors(root: Path, value: Any, label: str, *, minimum: int, suffixes: tuple[str, ...] = ()) -> list[dict[str, Any]]:
    if not isinstance(value, list) or not minimum <= len(value) <= MAX_FILES:
        raise SupplyChainError("E_DESCRIPTOR_LIST", f"{label} must contain {minimum}..{MAX_FILES} descriptors")
    rows = [_descriptor(root, item, f"{label}[{index}]", suffixes=suffixes) for index, item in enumerate(value)]
...
def _read_json(root: Path, descriptor: dict[str, Any], label: str) -> dict[str, Any]:
    ...
    try:
        ...
        raise SupplyChainError("E_JSON_ARTIFACT", f"{label} is not readable JSON: {exc}") from exc
...
def _assert_exact(value: Any, required: set[str], label: str, optional: set[str] | None = None) -> dict[str, Any]:
    if not isinstance(value, dict):
...
def _scan_archive(path: Path) -> list[tuple[str, bytes]]:
    ...
    try:
            ...
            with zipfile.ZipFile(path) as archive:
                ...
                for info in archive.infolist():
                    ...
                    if total > MAX_ARCHIVE_BYTES:
                        raise SupplyChainError("E_ARCHIVE_SIZE", f"archive contents exceed {MAX_ARCHIVE_BYTES} bytes")
        ...
        raise SupplyChainError("E_ARCHIVE_INVALID", f"{path.name} cannot be inspected: {exc}") from exc
    return members
...
def _scan_artifact_secrets(root: Path, artifacts: list[dict[str, Any]]) -> dict[str, Any]:
    ...
    for descriptor in artifacts:
        ...
        for pattern in _SECRET_PATTERNS:
            if pattern.search(raw):
                findings.append({"path": descriptor["path"], "location": "archive-bytes", "pattern": pattern.pattern.decode("ascii", "ignore")})
        for member, content in _scan_archive(path):
            for pattern in _SECRET_PATTERNS:
                if pattern.search(content):
                    findings.append({"path": descriptor["path"], "location": member, "pattern": pattern.pattern.decode("ascii", "ignore")})
    if findings:
        raise SupplyChainError("E_SECRET_IN_ARTIFACT", f"secret-shaped material found in {findings[0]['path']}")
...
def _validate_source_manifest(root: Path, value: Any) -> tuple[dict[str, Any], str]:
    ...
    if declared != calculated:
        raise SupplyChainError("E_SOURCE_MANIFEST_DIGEST", "source manifest digest does not match sorted file descriptors")
...
def _validate_sbom(root: Path, descriptor: dict[str, Any]) -> dict[str, Any]:
    ...
    if value.get("schema") != "factory.sbom.cyclonedx.v1":
        raise SupplyChainError("E_SBOM_SCHEMA", "SBOM is not a Factory CycloneDX artifact")
    ...
    if not isinstance(components, list) or len(components) > MAX_COMPONENTS:
        raise SupplyChainError("E_SBOM_COMPONENTS", "SBOM components are missing or too large")
...
def _validate_vex(root: Path, descriptor: dict[str, Any], now: datetime) -> tuple[dict[str, Any], dict[str, int], list[tuple[str, str, str]]]:
    ...
    for index, entry in enumerate(entries):
        ...
        vulnerability = require_str(entry.get("vulnerability"), f"vex.entries[{index}].vulnerability")
...
def _validate_licenses(value: Any, now: datetime) -> dict[str, Any]:
    policy = _assert_exact(value, {"policy_sha256", "entries", "exceptions"}, "licenses")
    ...
    for index, item in enumerate(exceptions):
        row = _assert_exact(item, {"id", "component", "license", "expires_at", "reason"}, f"licenses.exceptions[{index}]")
        active.add((require_str(row["component"], "license exception component"), require_str(row["license"], "license exception license")))
        if _timestamp(row["expires_at"], f"license exception[{index}].expires_at") <= now:
            raise SupplyChainError("E_EXCEPTION_EXPIRED", f"license exception {row['id']} is expired")
        require_str(row["reason"], "license exception reason", maximum=1024)
    denied = 0
    for index, item in enumerate(entries):
        row = _assert_exact(item, {"component", "license", "status"}, f"licenses.entries[{index}]")
        ...
        if status not in LICENSE_STATUSES:
            raise SupplyChainError("E_LICENSE_POLICY", f"unsupported license status {status}")
        if status in {"denied", "unknown"}:
            denied += 1
            if (component, license_name) not in active:
                raise SupplyChainError("E_LICENSE_POLICY", f"{component} has unapproved {status} license")
    return {"policy_sha256": policy_digest, "entry_count": len(entries), "exception_count": len(exceptions), "unapproved_count": denied}


def _validate_build(root: Path, value: Any, now: datetime) -> dict[str, Any]:
    build = _assert_exact(value, {"builder_id", "builder_version", "toolchain_sha256", "command_sha256", "clean_environment", "reproducible", "rebuilds"}, "build")
    ...
    if not require_bool(build["clean_environment"], "build.clean_environment") or not require_bool(build["reproducible"], "build.reproducible"):
        raise SupplyChainError("E_REPRO_BUILD_UNPROVEN", "clean_environment and reproducible must both be true")
    rebuilds = build["rebuilds"]
    if not isinstance(rebuilds, list) or not 2 <= len(rebuilds) <= 4:
        raise SupplyChainError("E_REPRO_BUILD_COUNT", "two to four independent rebuilds are required")
    ...
    for index, rebuild in enumerate(rebuilds):
        ...
        artifacts = _list_descriptors(root, row["artifacts"], f"build.rebuilds[{index}].artifacts", minimum=1, suffixes=ARTIFACT_SUFFIXES)
    ...
    for item in normalized[1:]:
        current = {(artifact["path"], artifact["sha256"]) for artifact in item["artifacts"]}
        if current != baseline:
            raise SupplyChainError("E_REPRO_BUILD_DRIFT", f"rebuild {item['id']} differs from the first build")
    return {"builder_id": build["builder_id"], "builder_version": build["builder_version"], "rebuild_count": len(normalized), "artifact_count": len(baseline), "artifacts": [dict(path=path, sha256=digest) for path, digest in sorted(baseline)]}
...
def _validate_collector(value: Any, *, require_external: bool) -> dict[str, str]:
    ...
    if require_external and (role == "local_evaluator" or backend == "local"):
        raise SupplyChainError("E_COLLECTOR_INDEPENDENCE", "signed verification requires a non-local collector")
...
def _validate_attestation_header(value: dict[str, Any], now: datetime) -> None:
    ...
    if issued > now or expires <= now or expires <= issued or (expires - issued).total_seconds() > 86_400:
        raise SupplyChainError("E_ATTESTATION_WINDOW", "attestation must be current and valid for at most 24 hours")
    if value["authority"] != "none" or value["release_approval"] is not False:
        raise SupplyChainError("E_AUTHORITY", "supply-chain evidence cannot grant release authority")

def validate_attestation(root: Path, manifest: dict[str, Any], *, candidate_sha256: str | None = None, now: datetime | None = None, require_external: bool = False) -> dict[str, Any]:
    """Validate a manifest and every referenced file without executing builds."""
    workspace = Path(root).resolve()
    required = {"schema", "attestation_id", "candidate_sha256", "source_manifest", "lockfiles", "sbom", "vex", "vex_policy", "licenses", "build", "collector", "issued_at", "expires_at", "authority", "release_approval"}
    value = _assert_exact(manifest, required, "attestation")
    current = (now or datetime.now(timezone.utc)).astimezone(timezone.utc)
    _validate_attestation_header(value, current)
    source, source_digest = _validate_source_manifest(workspace, value["source_manifest"])
    candidate = require_digest(value["candidate_sha256"], "candidate_sha256")
    if candidate != source_digest or (candidate_sha256 is not None and candidate_sha256 != candidate):
        raise SupplyChainError("E_CANDIDATE_BINDING", "candidate digest is not bound to the source manifest")
...
def _receipt(core: dict[str, Any]) -> dict[str, Any]:
    body = {"schema": RECEIPT_SCHEMA, "marker": "SUPPLY_CHAIN_RECEIPT", **core, "authority": "none", "release_approval": False, "claim_boundary": "Evidence-only supply-chain checks; no signing, release, publication, deployment, or approval authority."}
    body["receipt_sha256"] = _digest(body)
    return body


def evaluate_supply_chain(root: Path, manifest: dict[str, Any], *, candidate_sha256: str | None = None, now: datetime | None = None) -> dict[str, Any]:
    """Return a self-hashed PASS/BLOCKED receipt and never raise for bad input."""
    try:
        normalized = validate_attestation(Path(root), manifest, candidate_sha256=candidate_sha256, now=now)
        checks = [
            {"id": "SOURCE_MANIFEST_BOUND", "passed": True, "evidence": normalized["candidate_sha256"]},
            {"id": "LOCKFILES_BOUND", "passed": True, "evidence": f"{len(normalized['lockfiles'])} lockfile(s)"},
            {"id": "SBOM_BOUND", "passed": True, "evidence": normalized["sbom"]["bom_sha256"]},
            {"id": "VEX_POLICY_BOUND", "passed": True, "evidence": normalized["vex"]["vex_sha256"]},
            {"id": "LICENSE_POLICY_BOUND", "passed": True, "evidence": normalized["licenses"]["policy_sha256"]},
            {"id": "REPRODUCIBLE_BUILDS", "passed": True, "evidence": f"{normalized['build']['rebuild_count']} identical rebuilds"},
            {"id": "ARTIFACT_SECRET_SCAN", "passed": True, "evidence": "no secret-shaped material"},
        ]
        return _receipt({"decision": "PASS", "candidate_sha256": normalized["candidate_sha256"], "manifest_sha256": _digest(manifest), "checks": checks, "blockers": [], "collector": normalized["collector"], "facts": normalized["facts"]})
    except SupplyChainError as exc:
        return _receipt({"decision": "BLOCKED", "candidate_sha256": candidate_sha256, "manifest_sha256": _digest(manifest) if isinstance(manifest, dict) else None, "checks": [], "blockers": [{"code": exc.code, "detail": exc.message}], "collector": manifest.get("collector") if isinstance(manifest, dict) else None, "facts": {}})


def write_supply_chain_receipt(root: Path, manifest: dict[str, Any] | Path, out: Path, *, candidate_sha256: str | None = None, now: datetime | None = None) -> dict[str, Any]:
    """Evaluate and atomically persist a local supply-chain receipt."""
    workspace = Path(root).resolve()
    value = json.loads(Path(manifest).read_text(encoding="utf-8")) if isinstance(manifest, Path) else manifest
    result = evaluate_supply_chain(workspace, value, candidate_sha256=candidate_sha256, now=now)
    ...
    payload["receipt_sha256"] = _digest({key: item for key, item in payload.items() if key != "receipt_sha256"})
...
def verify_signed_supply_chain_attestation(path: Path, trust_root_path: Path, root: Path, *, candidate_sha256: str | None = None, now: datetime | None = None) -> dict[str, Any]:
    """Verify an externally collected DSSE attestation and keep release authority false."""
    try:
        result = verify_signed_document(Path(path), payload_type=PAYLOAD_TYPE, schema=ATTESTATION_SCHEMA, trust_root_path=Path(trust_root_path))
        normalized = validate_attestation(Path(root), result["payload"], candidate_sha256=candidate_sha256, now=now, require_external=True)
    ...
    body = {"schema": VERIFICATION_SCHEMA, "state": "VERIFIED", "attestation_id": normalized["attestation_id"], "candidate_sha256": normalized["candidate_sha256"], "payload_sha256": result["payload_sha256"], "verification": result["verification"], "collector": normalized["collector"], "facts": normalized["facts"], "authority": "none", "release_approval": False, "claim_boundary": "Signature verifies evidence only; it does not grant release authority."}
...
def supply_chain_status(root: Path) -> dict[str, Any]:
    ...
    if not path.is_file() or path.is_symlink():
        return {"schema": RECEIPT_SCHEMA, "state": "MISSING", "decision": "NOT_REQUESTED", "receipt_sha256": None, "claim_boundary": "No local supply-chain receipt was present; no conclusion about release readiness."}
    try:
        value = json.loads(path.read_text(encoding="utf-8"))
        ...
        if supplied != expected or value.get("schema") != RECEIPT_SCHEMA:
            return {"schema": RECEIPT_SCHEMA, "state": "INCOMPLETE", "decision": value.get("decision"), "receipt_sha256": supplied, "error": "SELF_HASH_MISMATCH", "claim_boundary": "Receipt integrity is invalid; no readiness conclusion."}
        return {"schema": RECEIPT_SCHEMA, "state": "PASS" if value.get("decision") == "PASS" else "BLOCKED", "decision": value.get("decision"), "receipt_sha256": supplied, "blockers": value.get("blockers", []), "facts": value.get("facts", {}), "claim_boundary": value.get("claim_boundary")}
    except (OSError, UnicodeDecodeError, json.JSONDecodeError):
        return {"schema": RECEIPT_SCHEMA, "state": "INCOMPLETE", "decision": None, "receipt_sha256": None, "error": "SOURCE_UNREADABLE", "claim_boundary": "Receipt could not be read; no readiness conclusion."}


__all__ = ["ATTESTATION_SCHEMA", "RECEIPT_SCHEMA", "VERIFICATION_SCHEMA", "PAYLOAD_TYPE", "SupplyChainError", "validate_attestation", "evaluate_supply_chain", "write_supply_chain_receipt", "verify_signed_supply_chain_attestation", "supply_chain_status"]
