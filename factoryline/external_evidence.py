...
_FAILURE_KINDS = frozenset(
    {"none", "assertion", "timeout", "environment", "network", "unknown"}
)
...
def _canonical(value: object) -> bytes:
    try:
        ...
        raise ExternalEvidenceError(
            "EXTERNAL_EVIDENCE_BUNDLE_INVALID", f"value is not canonical JSON: {exc}"
        ) from exc
...
def _workspace_file(
    root: Path, value: object, field: str, *, exists: bool = True
) -> tuple[Path, str]:
    if not isinstance(value, (str, Path)) or not str(value).strip():
        _reject(
            "EXTERNAL_EVIDENCE_PATH_INVALID",
    ...
    try:
        ...
        _reject(
            "EXTERNAL_EVIDENCE_PATH_OUTSIDE_ROOT",
...
def _load_json(
    path: Path, field: str, *, max_bytes: int = MAX_SOURCE_BYTES
    ...
    try:
        ...
        _reject(
            "EXTERNAL_EVIDENCE_BUNDLE_INVALID", f"{field} must be UTF-8 JSON: {exc}"
        )
    if not isinstance(value, dict):
        _reject(
            "EXTERNAL_EVIDENCE_BUNDLE_INVALID", f"{field} must contain one JSON object"
        )
    return value, raw


def _text(
    value: object, field: str, *, identifier: bool = False, allow_empty: bool = False
) -> str:
    ...
    if len(result) > MAX_TEXT:
        _reject(
            "EXTERNAL_EVIDENCE_BUNDLE_INVALID", f"{field} exceeds {MAX_TEXT} characters"
        )
    if identifier and not _ID.fullmatch(result):
        _reject(
            "EXTERNAL_EVIDENCE_BUNDLE_INVALID", f"{field} has an unsupported identifier"
        )
    return result
...
def _hash(value: object, field: str) -> str:
    if not isinstance(value, str) or not _SHA.fullmatch(value):
        _reject(
            "EXTERNAL_EVIDENCE_ARTIFACT_INVALID", f"{field} must be a lowercase SHA-256"
        )
    return value
...
def _normalize_step(value: object) -> dict[str, object]:
    if not isinstance(value, dict) or set(value) != _STEP_FIELDS:
        _reject(
            "EXTERNAL_EVIDENCE_BUNDLE_INVALID",
            "first_failed_step must contain index and label",
        )
    ...
    if index is not None and (
        ...
        _reject(
            "EXTERNAL_EVIDENCE_BUNDLE_INVALID",
            "first_failed_step.index must be a non-negative integer or null",
        )
...
def _normalize_artifacts(root: Path, value: object) -> list[dict[str, str]]:
    if not isinstance(value, list) or len(value) > MAX_ARTIFACTS:
        _reject(
            "EXTERNAL_EVIDENCE_ARTIFACT_INVALID",
    ...
    for index, item in enumerate(value):
        if not isinstance(item, dict) or set(item) != _ARTIFACT_FIELDS:
            _reject(
                "EXTERNAL_EVIDENCE_ARTIFACT_INVALID",
                f"artifacts[{index}] must contain path, sha256, and kind",
            )
        path, relative = _workspace_file(
            root, item.get("path"), f"artifacts[{index}].path"
        )
        if path.stat().st_size > MAX_ARTIFACT_BYTES:
            _reject(
                "EXTERNAL_EVIDENCE_SIZE_LIMIT",
                f"artifact {relative} exceeds {MAX_ARTIFACT_BYTES} bytes",
            )
        ...
        if actual != digest:
            _reject(
                ...
                f"artifact {relative} bytes do not match its SHA-256",
            )
        kind = _text(item.get("kind"), f"artifacts[{index}].kind", identifier=True)
        if relative in seen:
            _reject(
                "EXTERNAL_EVIDENCE_ARTIFACT_INVALID",
                f"artifact path {relative} is repeated",
            )
        seen.add(relative)
...
def _normalize_bundle(root: Path, value: object) -> dict[str, Any]:
    if not isinstance(value, dict) or set(value) != _BUNDLE_FIELDS:
        _reject(
            "EXTERNAL_EVIDENCE_BUNDLE_INVALID", "bundle has an unsupported field set"
        )
    ...
    if not _PROVIDER.fullmatch(provider):
        _reject(
            "EXTERNAL_EVIDENCE_BUNDLE_INVALID", "provider has an unsupported identifier"
        )
    ...
    if not isinstance(environment, dict) or set(environment) != _ENVIRONMENT_FIELDS:
        _reject(
            "EXTERNAL_EVIDENCE_BUNDLE_INVALID",
            "environment must contain fingerprint and label",
        )
    environment = {
        "fingerprint": _text(
            environment.get("fingerprint"), "environment.fingerprint", identifier=True
        ),
    ...
    failure_kind = _text(
        value.get("failure_kind"), "failure_kind", identifier=True
    ).lower()
    if failure_kind not in _FAILURE_KINDS:
        _reject(
            "EXTERNAL_EVIDENCE_BUNDLE_INVALID",
            f"failure_kind must be one of {sorted(_FAILURE_KINDS)}",
        )
    return {
        ...
        "code_version": _text(
            value.get("code_version"), "code_version", identifier=True
        ),
        ...
        "recommended_fix": _text(
            value.get("recommended_fix"), "recommended_fix", allow_empty=True
        ),
...
def _receipt_from_bundle(
    bundle: dict[str, Any], source_path: str, source_digest: str
) -> dict[str, Any]:
    core: dict[str, Any] = {
...
def _load_and_verify_receipt(
    ...
    if value.get("schema") != RECEIPT_SCHEMA:
        _reject(
            "EXTERNAL_EVIDENCE_RECEIPT_INVALID",
    ...
    if not isinstance(expected, str) or _sha(_receipt_core(value)) != expected:
        _reject(
            ...
            "receipt canonical bytes do not match receipt_sha256",
        )
    source = value.get("source_bundle")
    if not isinstance(source, dict) or set(source) != {"path", "sha256"}:
        _reject(
            "EXTERNAL_EVIDENCE_RECEIPT_INVALID",
            "source_bundle must contain path and sha256",
        )
    source_path, _source_relative = _workspace_file(
        root, source.get("path"), "source_bundle.path"
    )
    ...
    if (
        _receipt_from_bundle(
            normalized, str(source["path"]).replace("\\", "/"), source_digest
        )
        != value
    ):
        _reject(
            "EXTERNAL_EVIDENCE_RECEIPT_STALE",
            "receipt no longer matches its source bundle",
        )
    # Re-run artifact checks against current bytes to catch a changed fixture.
    _normalize_artifacts(root, normalized["artifacts"])
    if (
        value.get("authority") != _AUTHORITY
        or value.get("trust") != "observed_external"
    ):
        _reject(
            "EXTERNAL_EVIDENCE_AUTHORITY_INVALID",
            "external evidence cannot carry execution or release authority",
        )
    return value, relative


def import_external_runtime_bundle(
    root: Path, bundle: Path, provider: str, out: Path | None = None
) -> dict[str, Any]:
    ...
    if normalized["provider"] != requested_provider:
        _reject(
            "EXTERNAL_EVIDENCE_PROVIDER_MISMATCH",
            "bundle provider does not match --provider",
        )
    ...
    if not out_relative.startswith(".factory/external-evidence/"):
        _reject(
            "EXTERNAL_EVIDENCE_OUTPUT_INVALID",
            "receipt output must be below .factory/external-evidence/",
        )
    ...
    if out_path.exists():
        try:
            ...
            _reject(
                "EXTERNAL_EVIDENCE_OUTPUT_INVALID",
                f"existing output cannot be read: {exc}",
            )
        if existing != encoded:
            _reject(
                "EXTERNAL_EVIDENCE_OUTPUT_EXISTS",
                "output exists with different evidence",
            )
        return {
            "status": "idempotent",
            "marker": "EXTERNAL_EVIDENCE_IMPORTED",
...
def diff_external_runtime_receipts(
    ...
    if not same_identity:
        core = {
            ...
            "left": {
                "path": left_relative,
                "receipt_sha256": left_value["receipt_sha256"],
            },
            "right": {
                "path": right_relative,
                "receipt_sha256": right_value["receipt_sha256"],
            },
            "identity": identity,
            "deltas": {},
            "authority": dict(_AUTHORITY),
    ...
    right_artifacts = {
        item["path"]: item["sha256"] for item in right_value["artifacts"]
    }
    ...
    changed = sorted(
        path
        for path in left_artifacts
        if path in right_artifacts and left_artifacts[path] != right_artifacts[path]
    )
    deltas = {
        "verdict": {
            "from": left_value["verdict"],
            "to": right_value["verdict"],
            "changed": left_value["verdict"] != right_value["verdict"],
        },
        "failure_kind": {
            "from": left_value["failure_kind"],
            "to": right_value["failure_kind"],
            "changed": left_value["failure_kind"] != right_value["failure_kind"],
        },
        "first_failed_step": {
            "from": left_value["first_failed_step"],
            "to": right_value["first_failed_step"],
            "changed": left_value["first_failed_step"]
            != right_value["first_failed_step"],
        },
        "code_version": {
            "from": left_value["code_version"],
            "to": right_value["code_version"],
            "changed": left_value["code_version"] != right_value["code_version"],
        },
        "environment": {
            "from": left_value["environment"],
            "to": right_value["environment"],
            "changed": left_value["environment"] != right_value["environment"],
        },
