...
_KNOWN_SECTIONS = frozenset(
    {"schema", "risk", "quality", "tokens", "design", "release"}
)
_KNOWN_RISK_KEYS = frozenset({"default", "require_human_approval_for"})
_KNOWN_KEYS = {
    "risk": _KNOWN_RISK_KEYS,
    "quality": frozenset(
        {key for section, key in (*_BOOL_RULES, *_NUMBER_RULES) if section == "quality"}
    ),
    "tokens": frozenset(
        {key for section, key in (*_BOOL_RULES, *_NUMBER_RULES) if section == "tokens"}
    ),
...
def _canonical(value: Any) -> bytes:
    try:
        ...
        raise PolicyCompileError(
            "E_POLICY_NOT_CANONICAL", f"policy is not canonical JSON: {exc}"
        ) from exc
...
def _load_policy(path: Path) -> dict[str, Any]:
    try:
        ...
        raise PolicyCompileError(
            "E_POLICY_INPUT_INVALID", f"unable to read policy: {exc}"
        ) from exc
    if not isinstance(value, dict) or value.get("schema") != POLICY_SCHEMA:
        raise PolicyCompileError(
            "E_POLICY_SCHEMA_UNSUPPORTED", f"policy schema must be {POLICY_SCHEMA}"
        )
    return value
...
def _workspace_path(root: Path, supplied: Path) -> Path:
    ...
    try:
        ...
        raise PolicyCompileError(
            "E_POLICY_PATH_ESCAPE", "policy path must remain inside the workspace"
        ) from exc
    return resolved


def _section(
    policy: dict[str, Any], name: str, review: list[dict[str, Any]]
) -> dict[str, Any]:
...
def _sections(
    policy: dict[str, Any], review: list[dict[str, Any]]
) -> dict[str, dict[str, Any]]:
    """Return typed known sections and retain malformed-section findings."""
    sections = {
        name: _section(policy, name, review)
        for name in ("risk", "quality", "tokens", "design", "release")
    }
    ...
    for section_name, section in sections.items():
        for key in sorted(set(section) - _KNOWN_KEYS[section_name]):
            review.append(
                {"path": f"{section_name}.{key}", "reason": "unsupported policy rule"}
            )
    return sections


def _compile_boolean_checks(
    sections: dict[str, dict[str, Any]], review: list[dict[str, Any]]
) -> list[dict[str, Any]]:
    ...
    for (section_name, key), (check_name, kind) in sorted(_BOOL_RULES.items()):
        ...
        if not isinstance(value, bool):
            review.append(
                {"path": f"{section_name}.{key}", "reason": "rule must be boolean"}
            )
            continue
        checks.append(
            {
...
def _compile_numeric_checks(
    sections: dict[str, dict[str, Any]], review: list[dict[str, Any]]
) -> list[dict[str, Any]]:
    checks: list[dict[str, Any]] = []
    for (section_name, key), (check_name, operator, kind) in sorted(
        _NUMBER_RULES.items()
    ):
        section = sections[section_name]
        if key not in section:
            ...
            review.append(
                {
                    ...
                    "reason": "rule must be a non-negative number",
                }
            )
            continue
        checks.append(
...
def _compile_risk_gates(
    risk: dict[str, Any], review: list[dict[str, Any]]
) -> list[dict[str, Any]]:
    ...
    if default is not None:
        if not isinstance(default, str) or default not in _KNOWN_ENUMS["risk.default"]:
            review.append(
                {
                    "path": "risk.default",
                    "reason": "must be human, supervised, or autonomous",
                }
            )
        else:
            human_gates.append(
                {"id": "policy.risk.default", "mode": default, "authority": "none"}
            )
    ...
    if requested is not None:
        if not isinstance(requested, list) or not all(
            ...
            review.append(
                {
                    "path": "risk.require_human_approval_for",
                    "reason": "rule must be a list of non-empty strings",
                }
            )
        else:
...
def _compile_purpose_gate(
    design: dict[str, Any], review: list[dict[str, Any]]
) -> list[dict[str, Any]]:
    human_gates: list[dict[str, Any]] = []
    ...
    if purpose is not None:
        if not isinstance(purpose, str) or not purpose.strip():
            review.append(
                {
                    "path": "design.purpose_profile",
                    "reason": "purpose profile must be a non-empty string",
                }
            )
        else:
            human_gates.append(
                {
                    "id": "policy.design.purpose-profile",
...
def compile_policy(root: Path, policy: Path) -> dict[str, Any]:
    ...
    checks = _compile_boolean_checks(sections, review) + _compile_numeric_checks(
        sections, review
    )
    human_gates = _compile_risk_gates(sections["risk"], review) + _compile_purpose_gate(
    ...
    body["manifest_sha256"] = _sha(
        {
            key: value
            for key, value in body.items()
            if key not in {"manifest_sha256", "source_path"}
        }
    )
    return body


def write_compiled_policy(
    root: Path, policy: Path, out: Path | None = None
) -> dict[str, Any]:
    ...
    if out is not None:
        ...
        final = {
            **result,
            "marker": "POLICY_CLI_WRITTEN",
            "markers": [*result["markers"], "POLICY_CLI_WRITTEN"],
        ...
        try:
            ...
            try:
                ...
                pass
            raise PolicyCompileError(
                "E_POLICY_OUTPUT", f"unable to write compiled policy: {exc}"
            ) from exc
        result = final
    return result
