...
RULES: tuple[tuple[str, str, str, str, str], ...] = (
    (
        ...
        "always",
        "Bind all evidence to the exact signed candidate.",
        GUIDELINES,
    ),
    (
        "launch_and_core_paths_stable",
        "2.1 App Completeness",
        "always",
        "Exercise launch and every advertised core path without crashes, placeholders, or dead ends.",
        GUIDELINES,
    ),
    (
        "review_notes_complete",
        "2.1 App Completeness",
        "always",
        "Provide accurate review notes, contact information, setup steps, and non-obvious feature guidance.",
        GUIDELINES,
    ),
    (
        "reviewer_access_ready",
        "2.1 App Completeness",
        "always",
        "Provide a working review path and production-safe reviewer access when required.",
        GUIDELINES,
    ),
    (
        ...
        "always",
        "Reconcile claims, features, prices, and screenshots with the selected build.",
        GUIDELINES,
    ),
    (
        "authentic_screenshots",
        "2.3 Accurate Metadata",
        "always",
        "Bind authentic store screenshots to the selected build and supported devices.",
        GUIDELINES,
    ),
    (
        "age_rating_complete",
        "App information",
        "always",
        "Complete the age-rating questionnaire against actual content and capabilities.",
        APP_INFO,
    ),
    (
        "privacy_policy_reachable",
        "5.1 Privacy",
        "always",
        "Publish a reachable privacy-policy URL and make the policy available inside the app where required.",
        GUIDELINES,
    ),
    (
        "privacy_attestation_complete",
        "5.1 Privacy",
        "always",
        "Reconcile runtime collection, SDKs, tracking, retention, deletion, and processors with App Store privacy disclosures.",
        GUIDELINES,
    ),
    (
        ...
        "always",
        "Request only permissions required for the feature and provide a usable alternative when practical.",
        GUIDELINES,
    ),
    (
        ...
        "1.6 Data Security",
        "always",
        "Verify appropriate transport, storage, authorization, and sensitive-data handling controls.",
        GUIDELINES,
    ),
    (
        "accessibility_tasks_verified",
        "Accessibility",
        "always",
        "Exercise core tasks with VoiceOver, Dynamic Type, contrast, non-color cues, and Reduce Motion as applicable.",
        ACCESSIBILITY,
    ),
    (
        "minimum_functionality_verified",
        "4.2 Minimum Functionality",
        "always",
        "Demonstrate durable app-specific value beyond a repackaged website or marketing surface.",
        GUIDELINES,
    ),
    (
        "processor_terms_verified",
        "5.2.2 Third-Party Services",
        "always",
        "Record authority to use material third-party services and content.",
        GUIDELINES,
    ),
    (
        "export_compliance_determined",
        "Export Compliance",
        "always",
        "Complete the encryption/export determination and attach required documentation to the build.",
        EXPORT,
    ),
    (
        "physical_iphone_authentication",
        "2.1 App Completeness",
        "conditional",
        "Exercise authentication on the current build and a physical iPhone.",
        GUIDELINES,
    ),
    (
        "physical_ipad_navigation",
        "4 Design",
        "conditional",
        "Exercise the reviewer iPad path, including explicit accessible return actions and layout checks.",
        GUIDELINES,
    ),
    (
        "physical_iphone_purchase",
        "2.1(b) In-App Purchases",
        "conditional",
        "Complete a Sandbox purchase on the current build and a physical iPhone.",
        GUIDELINES,
    ),
    (
        "physical_iphone_restore",
        "2.1(b) In-App Purchases",
        "conditional",
        "Restore the purchase on the current build and a physical iPhone.",
        GUIDELINES,
    ),
    (
        "sandbox_products_verified",
        "2.1(b) In-App Purchases",
        "conditional",
        "Verify every reviewed product is available in Apple's Sandbox catalog.",
        GUIDELINES,
    ),
    (
        "restore_available",
        "3.1.2 Subscriptions",
        "conditional",
        "Expose and exercise a restore path for restorable purchases.",
        GUIDELINES,
    ),
    (
        "subscription_disclosures_complete",
        "3.1.2 Subscriptions",
        "conditional",
        "Show price, duration, renewal, cancellation, and required terms without misleading hierarchy.",
        GUIDELINES,
    ),
    (
        "account_deletion_available",
        "5.1.1 Account Sign-In",
        "conditional",
        "Provide in-app account deletion when the app supports account creation.",
        GUIDELINES,
    ),
    (
        "login_services_compliant",
        "4.8 Login Services",
        "conditional",
        "When third-party login creates the primary account, provide an equivalent privacy-preserving option or record an allowed exception.",
        GUIDELINES,
    ),
    (
        "ugc_moderation_controls",
        "1.2 User-Generated Content",
        "conditional",
        "Provide filtering, reporting, blocking, and reachable support for user-generated content.",
        GUIDELINES,
    ),
    (
        "kids_controls_verified",
        "1.3 Kids Category",
        "conditional",
        "Verify parental gates, data restrictions, ads, and age-appropriate content when targeting children.",
        GUIDELINES,
    ),
    (
        "health_claims_supported",
        "1.4 Physical Harm",
        "conditional",
        "Bind health or medical claims to appropriate evidence, disclaimers, and regulatory authority.",
        GUIDELINES,
    ),
    (
        "financial_services_authorized",
        "3.2.1 Financial Services",
        "conditional",
        "Record required licensing and entity authority for regulated financial functionality.",
        GUIDELINES,
    ),
    (
        "location_background_use_justified",
        "5.1 Privacy",
        "conditional",
        "Demonstrate that location and background modes are directly relevant and accurately disclosed.",
        GUIDELINES,
    ),
    (
        "ai_content_controls_verified",
        "1 Safety and 5.1 Privacy",
        "conditional",
        "Verify AI disclosures, unsafe-content handling, user control, and data-use boundaries for generated content.",
        GUIDELINES,
    ),
)
...
def _candidate(value: object, label: str) -> dict[str, str]:
    if not isinstance(value, dict):
        raise RevenueForgeError(
            "APP_REVIEW_CANDIDATE_INVALID", f"{label} must be an object"
        )
...
def _review_inputs(
    ...
    if not isinstance(checks, dict):
        raise RevenueForgeError(
            "APP_REVIEW_EVIDENCE_INVALID", "evidence.checks must be an object"
        )
...
def _readiness_receipt(
    ...
    return {
        ...
        "policy_registry": {
            ...
            "official_sources": sorted({source for _, _, _, _, source in RULES}),
        },
        "findings": findings,
        "provenance": {
            "basis": "sanitized regression classes derived from prior real App Review findings and release-readiness failures",
            "private_app_data_copied": False,
        },
        "release_authority": {
...
def app_review_gate_projection(root: Path) -> dict[str, Any]:
    ...
    for path in sorted((workspace / ".factory" / "appforge").rglob("app-review*.json"))[
        :100
    ]:
        try:
