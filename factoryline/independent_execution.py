...
def _validate_identity(value: dict[str, Any]) -> tuple[str, str, str, str, str]:
    if not isinstance(value, dict):
        raise ExecutionAttestationError(
            "E_ATTESTATION_RUNNER", "runner must be an object"
        )
    _exact(
        value,
        {"id", "version", "platform", "backend", "executable_sha256"},
        "E_ATTESTATION_RUNNER",
    )
    ...
    if backend not in BACKENDS:
        raise ExecutionAttestationError(
            "E_ATTESTATION_RUNNER", "unsupported runner backend"
        )
    executable = require_digest(
        value.get("executable_sha256"), "runner.executable_sha256"
    )
...
def _validate_observations(value: dict[str, Any]) -> dict[str, Any]:
    if not isinstance(value, dict):
        raise ExecutionAttestationError(
            "E_ATTESTATION_INCOMPLETE", "observations must be an object"
        )
    _exact(
        value,
        {
            "target_artifact_sha256",
            "known_bad_artifact_sha256",
        ...
        "E_ATTESTATION_INCOMPLETE",
    )
    target = _digest_or_null(
        value.get("target_artifact_sha256"), "observations.target_artifact_sha256"
    )
    known_bad = _digest_or_null(
        value.get("known_bad_artifact_sha256"), "observations.known_bad_artifact_sha256"
    )
    if target is None or known_bad is None:
        raise ExecutionAttestationError(
            "E_ATTESTATION_INCOMPLETE",
            "target and known-bad artifact digests are required",
        )
    ...
    if not cleanup:
        raise ExecutionAttestationError(
            "E_ATTESTATION_INCOMPLETE", "cleanup must be confirmed"
        )
    ...
    return {
        ...
        "known_bad_artifact_sha256": known_bad,
        "target_stdout_sha256": _digest_or_null(
            value.get("target_stdout_sha256"), "observations.target_stdout_sha256"
        ),
        "target_stderr_sha256": _digest_or_null(
            value.get("target_stderr_sha256"), "observations.target_stderr_sha256"
        ),
...
def _validate_binding(
    ...
    expected_candidate = (
        require_digest(candidate_sha256, "candidate_sha256 expectation")
        if candidate_sha256 is not None
        else None
    )
    expected_plan = (
        require_digest(plan_sha256, "plan_sha256 expectation")
...
def _validate_assurance(value: dict[str, Any], require_independent: bool) -> str:
    ...
    if (
        ...
        or require_independent
        and assurance not in INDEPENDENT_LEVELS
    ):
        raise ExecutionAttestationError(
...
def validate_execution_attestation(
    ...
    *,
    ...
    attestation_id, candidate, plan, nonce, issued, expires, assurance = (
        _validate_header(
            value,
    ...
    runner_id, version, platform, backend, executable = _validate_identity(
        value["runner"]
    )
    allowed_backends = COMPATIBLE_BACKENDS[assurance]
    if backend not in allowed_backends:
        raise ExecutionAttestationError(
            "E_ATTESTATION_BACKEND",
            "runner backend is incompatible with assurance level",
        )
...
def verify_signed_execution_attestation(
    ...
    *,
    ...
    if signature.get("keyid") in set(revoked_keyids) or signature.get(
        "identity"
    ) in set(revoked_identities):
        raise ExecutionAttestationError(
            "E_ATTESTATION_REVOKED", "attestation signer is revoked"
        )
    payload = validate_execution_attestation(
        verified["payload"],
        candidate_sha256=candidate_sha256,
        plan_sha256=plan_sha256,
        now=now,
    ...
    return {
        "schema": "factory.execution-attestation-verification.v1",
        "state": "VERIFIED",
