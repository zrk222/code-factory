...
_QUESTION_SPECS = {
    ...
    "JOURNEYS_MISSING": {
        "id": "Q-JOURNEY",
        "order": 40,
        "section": "Journeys and business rules",
    ...
    "DATA_OWNERSHIP_MISSING": {
        "id": "Q-DATA-OWNERSHIP",
        "order": 60,
        "section": "Data ownership and trust boundaries",
    ...
    "APPROVAL_REQUIREMENTS_MISSING": {
        "id": "Q-APPROVAL",
        "order": 80,
        "section": "External effects and approvals",
...
def _repository_facts(root: Path) -> list[dict[str, str]]:
    ...
    for name in ("pyproject.toml", "package.json", "README.md", "Cargo.toml", "go.mod"):
        ...
        if candidate.is_file():
            facts.append(
                {
                    "kind": "allowlisted_workspace_file",
                    "path": name,
...
def _question(code: str, gap: dict[str, str]) -> dict[str, Any]:
    if code.startswith("UX_") and code.endswith("_MISSING"):
        ...
        return {
            "id": f"Q-UX-{state.upper()}",
            "order": 100,
            "section": "Experience states",
    ...
    if not spec:
        raise ProductMissionError(
            "PRD_GRILL_GAP_UNSUPPORTED",
            f"no deterministic question for observed gap {code}",
        )
    return {
        **spec,
        "gap_code": code,
...
def _frontier(
    gaps: list[dict[str, str]], mode: str
) -> tuple[list[dict[str, Any]], list[dict[str, Any]]]:
    candidates = [
        _question(item["code"], item)
        for item in gaps
        if mode == "deep" or item["code"] in _QUICK_CODES
    ]
    ...
    deferred = [
        {
            **item,
            "status": "deferred",
            "deferred_by": sorted(set(item["depends_on"]) & missing_ids),
        }
        for item in candidates
        if set(item["depends_on"]) & missing_ids
    ]
    ready.sort(key=lambda item: (item["order"], item["id"]))
    selected, overflow = ready[: MAX_QUESTIONS[mode]], ready[MAX_QUESTIONS[mode] :]
    deferred.extend(
        {**item, "status": "deferred", "deferred_by": ["round_limit"]}
...
def _markdown(receipt: dict[str, Any]) -> str:
        ...
        for item in receipt["questions"]:
            lines.extend(
                [
                    ...
                    "",
                    f"**Observed evidence:** {item['evidence']}",
                    "",
                    "**Answer:**",
                    ">",
                    "",
    ...
    if receipt["deferred_questions"]:
        ...
        for item in receipt["deferred_questions"]:
            lines.append(
                f"- `{item['id']}` waits for: {', '.join(item['deferred_by'])}."
            )
        lines.append("")
    if not receipt["questions"] and not receipt["deferred_questions"]:
        lines.extend(
            [
                ...
                "",
                "No observed PRD gaps require a clarification question. A human still confirms the product contract before implementation.",
                "",
            ]
        )
    lines.extend(
        [
            "## Next step",
            "",
            "Update the PRD deliberately, then rerun PRD Grill and `specline optimize-prd`. Do not treat this sheet or its recommendations as implementation approval.",
            "",
        ]
    )
    return "\n".join(lines)
...
def _receipt_for_core(
    receipt_path: Path, core: dict[str, Any], *, force: bool
    ...
    try:
        ...
        raise ProductMissionError(
            "ARTIFACT_INVALID", f"cannot read PRD Grill receipt: {receipt_path}"
        ) from exc
    if existing.get("grill_sha256") == core_sha:
        return existing, True
    raise ProductMissionError(
        "ARTIFACT_EXISTS", f"refusing to replace {receipt_path}; use --force"
    )


def _write_grill_artifacts(
    source_path: Path,
    receipt_path: Path,
    markdown_path: Path,
    source: bytes,
...
def grill_prd(
    ...
    if mode not in MAX_QUESTIONS:
        raise ProductMissionError(
            "PRD_GRILL_MODE_INVALID", "mode must be quick or deep"
        )
    source = _read_prd(Path(prd_path))
    analysis = analyze_product_text(
        source.decode("utf-8"), source_name=Path(prd_path).name, project=project
    )
    questions, deferred = _frontier(analysis["gaps"], mode)
    if confirm and analysis["gaps"]:
        raise ProductMissionError(
            "PRD_GRILL_UNRESOLVED",
            "cannot confirm shared understanding while clarification questions remain",
        )
    project_id, source_sha = analysis["project"], _sha(source)
    status = (
        "confirmed"
        if confirm
        else ("needs_input" if analysis["gaps"] else "ready_for_confirmation")
    )
    core = {
        "schema": PRD_GRILL_SCHEMA,
        "project": project_id,
        "mode": mode,
        "status": status,
        "source": {
            "name": Path(prd_path).name,
            "sha256": source_sha,
            "bytes": len(source),
        },
        "repository_facts": _repository_facts(Path(root)),
        ...
        "markers": [
            ...
            "PRD_GRILL_QUESTIONS_BOUND",
            "PRD_GRILL_DRAFT_REVIEW_REQUIRED",
            *(["PRD_GRILL_SHARED_UNDERSTANDING_CONFIRMED"] if confirm else []),
        ],
        "authority": {
    ...
    _write_grill_artifacts(
        source_path, receipt_path, markdown_path, source, receipt, force=force
    )
    return {
        **receipt,
...
def verify_prd_grill(receipt_path: Path) -> dict[str, Any]:
    ...
    if not isinstance(receipt, dict) or receipt.get("schema") != PRD_GRILL_SCHEMA:
        raise ProductMissionError(
            "SCHEMA_UNSUPPORTED", f"expected {PRD_GRILL_SCHEMA}: {receipt_path}"
        )
    core = {
        key: value
        for key, value in receipt.items()
        if key not in {"grill_sha256", "generated_at"}
    }
    errors: list[str] = []
    if receipt.get("grill_sha256") != _sha(_canonical(core)):
        errors.append("receipt hash mismatch")
    source_path = Path(receipt_path).with_name(
        f"{receipt['source']['sha256']}.source.md"
    )
    ...
    return {
        "valid": not errors,
        "marker": "PRD_GRILL_VERIFIED" if not errors else "PRD_GRILL_DRIFT",
        "errors": errors,
