...
def _relative_file(root: Path, value: object, field: str) -> tuple[Path, str]:
    ...
    try:
        ...
        _reject(
            "REALITY_CHECK_MANIFEST_INVALID", f"{field} resolves outside the workspace"
        )
    if not path.is_file():
        _reject(
            "REALITY_CHECK_MANIFEST_INVALID", f"{field} must name a readable JSON file"
        )
...
def _load(root: Path, manifest_path: Path) -> tuple[dict[str, Any], str, str]:
    ...
    try:
        ...
        _reject(
            "REALITY_CHECK_MANIFEST_INVALID", "manifest must stay inside the workspace"
        )
    if not path.is_file():
        _reject(
            "REALITY_CHECK_MANIFEST_INVALID", "manifest must name a readable JSON file"
        )
    raw = path.read_bytes()
    try:
        ...
        _reject(
            "REALITY_CHECK_MANIFEST_INVALID",
            f"manifest cannot be decoded as JSON: {exc}",
        )
    if not isinstance(source, dict):
        _reject(
            "REALITY_CHECK_MANIFEST_INVALID", "manifest must contain one JSON object"
        )
...
def _intent_assertions(value: object) -> list[dict[str, str]]:
    if not isinstance(value, list) or not 2 <= len(value) <= MAX_ASSERTIONS:
        _reject(
            "REALITY_CHECK_MANIFEST_INVALID",
            f"intent_assertions must contain 2 through {MAX_ASSERTIONS} assertions",
        )
    ...
    for index, item in enumerate(value):
        if not isinstance(item, dict) or set(item) != {"id", "statement", "evidence"}:
            _reject(
                "REALITY_CHECK_MANIFEST_INVALID",
                f"intent_assertions[{index}] must contain exactly id, statement, and evidence",
            )
        evidence = item.get("evidence")
        if evidence not in {"positive", "negative"}:
            _reject(
                "REALITY_CHECK_MANIFEST_INVALID",
                f"intent_assertions[{index}].evidence must be positive or negative",
            )
        assertions.append(
            {
                "id": _text(
                    item.get("id"), f"intent_assertions[{index}].id", identifier=True
                ),
                "statement": _text(
                    item.get("statement"), f"intent_assertions[{index}].statement"
                ),
                "evidence": evidence,
            }
        )
    if len({item["id"] for item in assertions}) != len(assertions) or {
        item["evidence"] for item in assertions
    } != {"positive", "negative"}:
        _reject(
            "REALITY_CHECK_MANIFEST_INVALID",
            "intent assertions require unique ids and at least one positive plus one negative assertion",
        )
    return assertions
...
def validate_reality_check_manifest(root: Path, manifest_path: Path) -> dict[str, Any]:
    ...
    if set(source) != required or source.get("schema") != REALITY_CHECK_MANIFEST_SCHEMA:
        _reject(
            "REALITY_CHECK_MANIFEST_INVALID",
            "manifest must contain exactly schema, id, approval, behavior, intent_assertions, and e2e_manifest",
        )
    ...
    if approval.get("state") != "approved":
        _reject(
            "REALITY_CHECK_UNAPPROVED",
            "approval.state must be approved before the declared checks can run",
        )
    ...
    if not isinstance(behavior, dict) or set(behavior) != {
        ...
        _reject(
            "REALITY_CHECK_MANIFEST_INVALID",
            "behavior must contain exactly promise, happy_path, and failure_case",
        )
    e2e_path, e2e_relative = _relative_file(
        workspace, source.get("e2e_manifest"), "e2e_manifest"
    )
    ...
    for assertion in validated_assertions:
        _clear_intent(
            assertion["statement"],
            f"intent_assertions[{assertion['id']}].statement",
            observable=True,
        )
    return {
        ...
        "behavior": {
            name: _text(behavior.get(name), f"behavior.{name}")
            for name in ("promise", "happy_path", "failure_case")
        },
        "intent_assertions": validated_assertions,
        "e2e_manifest": {
            "path": e2e_relative,
            "sha256": sha256(e2e_path.read_bytes()).hexdigest(),
        },
        "manifest_path": path,
...
def inspect_reality_intent(root: Path, manifest_path: Path) -> dict[str, Any]:
    ...
    return {
        "schema": "factory.reality-intent-inspection.v1",
        "marker": "REALITY_INTENT_CONTRACT_READY",
        "manifest": manifest,
        ...
        "execution": False,
        "scope_limits": [
            "Inspection validates declared intent-to-evidence coverage only; it does not run commands or establish behavior."
        ],
    }
...
def _assertion_results(
    manifest: dict[str, Any], e2e: dict[str, Any]
) -> list[dict[str, Any]]:
    verified = e2e["marker"] == "E2E_PROOF_PASS"
    return [
        {**item, "verified": verified, "e2e_marker": e2e["marker"]}
        for item in manifest["intent_assertions"]
    ]
...
def _mermaid(receipt: dict[str, Any]) -> str:
    return "\n".join(
        [
            "flowchart LR",
            '  P["Declared product promise"] --> H["Approved happy path"]',
            '  F["Declared failure case"] --> N["Negative check"]',
            '  H --> G["Reality Check"]',
            "  N --> G",
            f'  G --> R["{receipt["marker"]}"]',
            '  R --> X["Human release decision remains external"]',
            "",
        ]
    )
...
def _markdown(receipt: dict[str, Any]) -> str:
    ...
    return "\n".join(
        [
            ...
            "",
            f"- Check ID: `{receipt['manifest']['id']}`",
            f"- Promise: {behavior['promise']}",
            f"- Happy path: {behavior['happy_path']}",
            f"- Failure case: {behavior['failure_case']}",
...
def run_reality_check(root: Path, manifest_path: Path) -> dict[str, Any]:
    ...
    try:
        e2e_private = verify_e2e_proof(
            workspace, workspace / manifest["e2e_manifest"]["path"]
        )
    ...
    core = {
        "schema": REALITY_CHECK_RECEIPT_SCHEMA,
        "marker": marker,
        "ok": ok,
        "run_state": state,
        "manifest": manifest,
        "e2e_receipt": e2e,
        "intent_verification": _assertion_results(manifest, e2e),
        "authority": AUTHORITY,
        "scope_limits": [
...
def _receipt_shape(value: object) -> tuple[dict[str, Any], set[str]]:
    if (
        ...
        or value.get("schema") != REALITY_CHECK_RECEIPT_SCHEMA
    ):
        _reject(
            "REALITY_CHECK_RECEIPT_INVALID",
            f"a {REALITY_CHECK_RECEIPT_SCHEMA} payload is required",
        )
    required = {
        "schema",
        "marker",
    ...
    if set(value) != required or value.get("authority") != AUTHORITY:
        _reject(
            "REALITY_CHECK_RECEIPT_INVALID",
            "receipt has unsupported fields or a changed authority boundary",
        )
...
def _validate_manifest_behavior(behavior: object) -> None:
    ...
    _clear_intent(
        behavior["happy_path"], "receipt manifest behavior.happy_path", observable=True
    )
    _clear_intent(
        behavior["failure_case"],
...
def _receipt_result(value: dict[str, Any]) -> None:
    ...
    if (value["run_state"], value["marker"], value["ok"]) != (state, marker, ok):
        _reject(
            "REALITY_CHECK_RECEIPT_INVALID",
            "marker, state, and E2E result are inconsistent",
        )
    if value["intent_verification"] != _assertion_results(
        _receipt_manifest(value["manifest"]), e2e
    ):
        _reject(
            "REALITY_CHECK_RECEIPT_INVALID",
            "intent assertion verification does not match the declared E2E result",
        )
...
def _receipt_hash_and_views(value: dict[str, Any], required: set[str]) -> None:
    ...
    if (
        ...
        or not _SHA.fullmatch(value["receipt_sha256"])
        or value["receipt_sha256"] != _sha(core)
    ):
    ...
    if value["mermaid"] != _mermaid(value) or value["receipt_markdown"] != _markdown(
        ...
        _reject(
            "REALITY_CHECK_RECEIPT_INVALID", "receipt views do not match receipt facts"
        )
...
def write_reality_check_artifacts(
    ...
    outputs = {
        "receipt": destination / f"{stem}.reality.json",
    ...
    for name, path in outputs.items():
        value = (
            receipt
            if name == "receipt"
            else receipt["receipt_markdown"]
            if name == "markdown"
            else receipt["mermaid"]
        )
        path.write_bytes(
            _canonical(value) if name == "receipt" else value.encode("utf-8")
        )
