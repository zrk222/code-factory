    ...
    try:
        if not source.is_file() or source.stat().st_size > MAX_SOURCE_BYTES:
            raise ProofSearchError(
                "PROOFSEARCH_SOURCE_UNREADABLE",
                "source is missing or exceeds 2097152 bytes",
            )
        ...
        raise ProofSearchError(
            "PROOFSEARCH_SOURCE_UNREADABLE", "source must be readable JSON"
        ) from exc
    if not isinstance(value, dict):
        raise ProofSearchError(
            "PROOFSEARCH_SOURCE_INVALID", "source root must be one object"
        )
    return value
...
def _relative(value: object, field: str) -> str:
    ...
    if (
        ...
        raise ProofSearchError(
            "PROOFSEARCH_PATH_INVALID", f"{field} must be workspace-relative"
        )
    return path
...
def _number_or_none(value: object, field: str) -> float | int | None:
    ...
    if (
        ...
        raise ProofSearchError(
            "PROOFSEARCH_SOURCE_INVALID",
            f"{field} must be null or a finite non-negative number",
        )
    return value
...
def _sha(value: object, field: str) -> str:
    if not isinstance(value, str) or not _SHA.fullmatch(value):
        raise ProofSearchError(
            "PROOFSEARCH_SOURCE_INVALID", f"{field} must be a lowercase SHA-256 digest"
        )
    return value
...
def _path_sha(root: Path, relative: str, expected: str, field: str) -> None:
    ...
    try:
        ...
        raise ProofSearchError(
            "PROOFSEARCH_PATH_INVALID", f"{field} escapes the workspace"
        ) from exc
    if not path.is_file() or path.stat().st_size > MAX_SOURCE_BYTES:
        raise ProofSearchError(
            "PROOFSEARCH_EVIDENCE_UNREADABLE", f"{field} is missing or too large"
        )
    actual = hashlib.sha256(path.read_bytes()).hexdigest()
    if actual != expected:
        raise ProofSearchError(
            "PROOFSEARCH_EVIDENCE_HASH_MISMATCH", f"{field} SHA-256 does not match"
        )


def create_proofsearch_plan(
    root: Path,
    baseline_path: Path,
    candidate_path: Path,
    ...
    if not changed or len(changed) > MAX_PATHS:
        raise ProofSearchError(
            "PROOFSEARCH_CHANGED_PATHS_INVALID",
            f"changed_paths must contain 1 through {MAX_PATHS} items",
        )
    try:
    ...
    if forensic["divergence"] is None:
        raise ProofSearchError(
            "PROOFSEARCH_DIVERGENCE_REQUIRED",
            "verified lineage receipts do not diverge",
        )
    ...
    payload = {
        **core,
        ...
        "marker": "PROOFSEARCH_PLAN_SEALED",
        "markers": [
            "PROOFSEARCH_PLAN_SEALED",
            "PROOFSEARCH_PROOF_SLICE_EXACT",
...
def _verify_plan(value: dict[str, Any]) -> dict[str, Any]:
    if value.get("schema") != PLAN_SCHEMA or value.get("authority") != _AUTHORITY:
        raise ProofSearchError(
            "PROOFSEARCH_PLAN_INVALID", "plan schema or authority boundary is invalid"
        )
    core = {
        key: value.get(key)
        for key in (
    ...
    if value.get("plan_sha256") != _digest(core):
        raise ProofSearchError(
            "PROOFSEARCH_PLAN_INVALID",
            "plan_sha256 does not match canonical plan content",
        )
    return value
...
def _proof(root: Path, raw: object, offset: int) -> dict[str, Any]:
    if not isinstance(raw, dict):
        raise ProofSearchError(
            "PROOFSEARCH_CANDIDATE_INVALID", f"proofs[{offset}] must be an object"
        )
    receipt = raw.get("receipt")
    if not isinstance(receipt, dict):
        raise ProofSearchError(
            "PROOFSEARCH_CANDIDATE_INVALID",
            f"proofs[{offset}].receipt must be an object",
        )
    ...
    if status not in {"passed", "failed"}:
        raise ProofSearchError(
            "PROOFSEARCH_CANDIDATE_INVALID",
            f"proofs[{offset}].status must be passed or failed",
        )
    return {
...
def _candidate(
    ...
    if not isinstance(patch, dict):
        raise ProofSearchError(
            "PROOFSEARCH_CANDIDATE_INVALID", f"{candidate_id}.patch must be an object"
        )
    ...
    paths = sorted(
        {
            ...
            for item in raw.get("changed_paths", [])
        }
    )
    ...
    if not isinstance(mutation, dict):
        raise ProofSearchError(
            "PROOFSEARCH_CANDIDATE_INVALID",
            f"{candidate_id}.mutation must be an object",
        )
    ...
    if (
        ...
        or set(guardrails) != {"weakens_tests", "suppresses_errors", "expands_scope"}
        or any(not isinstance(value, bool) for value in guardrails.values())
    ):
        raise ProofSearchError(
            "PROOFSEARCH_CANDIDATE_INVALID",
    ...
    if any(
        item["receipt_passed"] is not None
        and item["receipt_passed"] != (item["status"] == "passed")
        for item in proofs
    ):
    ...
    if guardrails["weakens_tests"]:
        reasons.append("TEST_WEAKENING_DECLARED")
    if guardrails["suppresses_errors"]:
...
def _metrics(candidate: dict[str, Any]) -> dict[str, Any]:
    ...
    return {
        ...
        "cost_usd": round(sum(float(item) for item in costs), 8)
...
def _rank(candidate: dict[str, Any]) -> tuple[object, ...]:
    ...
    return (
        ...
        metrics["proof_elapsed_ms"],
        metrics["tokens"] is None,
        metrics["tokens"] or 0,
        metrics["cost_usd"] is None,
...
def _savings(request: dict[str, Any], winner: dict[str, Any] | None) -> dict[str, Any]:
    ...
    if not isinstance(baseline, dict) or set(baseline) != {
        ...
        raise ProofSearchError(
            "PROOFSEARCH_BASELINE_INVALID",
            "paired_baseline must contain exactly elapsed_ms, tokens, and cost_usd",
        )
    ...
    return {
        ...
        "cost_usd": round(float(cost) - metrics["cost_usd"], 8)
        if cost is not None and metrics["cost_usd"] is not None
        else None,
        "productivity": None,
...
def _compare_candidates(
    ...
    seen: set[str] = set()
    candidates = [
        _candidate(workspace, raw, authorized, seen) for raw in raw_candidates
    ]
...
def evaluate_proofsearch(root: Path, request_path: Path, out: Path) -> dict[str, Any]:
    ...
    if request.get("schema") != REQUEST_SCHEMA:
        raise ProofSearchError(
            "PROOFSEARCH_REQUEST_INVALID", f"schema must be {REQUEST_SCHEMA}"
        )
    ...
    if (
        ...
        raise ProofSearchError(
            "PROOFSEARCH_CANDIDATE_COUNT",
            f"candidates must contain 2 through {MAX_CANDIDATES} items",
        )
    ...
    marker = (
        "PROOFSEARCH_WINNER_VERIFIED" if winner else "PROOFSEARCH_NO_ELIGIBLE_CANDIDATE"
    )
    markers = _evaluation_markers(marker, candidates, request.get("paired_baseline"))
    payload = {
        **core,
        "evaluation_sha256": _digest(core),
        "marker": marker,
        "markers": markers,
    }
    _write(payload, out)
...
def verify_proofsearch_evaluation(root: Path, evaluation_path: Path) -> dict[str, Any]:
    ...
    if value.get("authority") != _AUTHORITY or value.get("apply") is not False:
    ...
    if value.get("evaluation_sha256") != calculated:
        errors.append("evaluation_sha256 does not match canonical evaluation content")
    marker = (
        "PROOFSEARCH_EVALUATION_VERIFIED"
        if not errors
        else "PROOFSEARCH_EVALUATION_INVALID"
    )
    return {
        "schema": "factory.proofsearch-verification.v1",
        "valid": not errors,
        "errors": sorted(set(errors)),
        "evaluation_sha256": calculated,
        "winner": value.get("winner"),
        "marker": marker,
        "markers": [marker, "PROOFSEARCH_AUTHORITY_RETAINED"],
    }
