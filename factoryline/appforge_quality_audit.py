...
def _text(value: object, field: str, *, limit: int = 500) -> str:
    ...
    if not result or len(result) > limit:
        raise RevenueForgeError(
            "APPFORGE_QUALITY_AUDIT_INVALID",
...
def _digest(value: object, field: str) -> str:
    ...
    if len(result) != 64 or any(
        ...
        raise RevenueForgeError(
            "APPFORGE_QUALITY_AUDIT_INVALID", f"{field} must be a SHA-256 hex digest"
        )
    return result
...
def _candidate(value: object, field: str) -> dict[str, str]:
    if not isinstance(value, dict):
        raise RevenueForgeError(
            "APPFORGE_QUALITY_CANDIDATE_INVALID", f"{field} must be an object"
        )
    return {
...
def _timestamp(value: object, field: str) -> str:
    ...
    try:
        ...
        raise RevenueForgeError(
            "APPFORGE_QUALITY_AUDIT_INVALID", f"{field} must be RFC3339"
        ) from exc
    return result
...
def _local(root: Path, path: Path, *, exists: bool = True) -> Path:
    ...
    try:
        ...
        raise RevenueForgeError(
            "APPFORGE_QUALITY_PATH_REJECTED", "paths must remain inside the workspace"
        ) from exc
    if exists and not resolved.is_file():
        raise RevenueForgeError(
            "APPFORGE_QUALITY_INPUT_UNAVAILABLE",
...
def _read_json(root: Path, path: Path, schema: str) -> tuple[dict[str, Any], Path]:
    ...
    if source.stat().st_size > MAX_BYTES:
        raise RevenueForgeError(
            "APPFORGE_QUALITY_INPUT_TOO_LARGE", "input exceeds 1 MiB"
        )
    try:
        value = json.loads(source.read_text(encoding="utf-8-sig"))
    except (OSError, UnicodeDecodeError, json.JSONDecodeError) as exc:
        raise RevenueForgeError(
            "APPFORGE_QUALITY_INPUT_INVALID", "input must be valid JSON"
        ) from exc
    if not isinstance(value, dict) or value.get("schema") != schema:
        raise RevenueForgeError(
            "APPFORGE_QUALITY_SCHEMA_REJECTED", f"expected {schema}"
        )
    return value, source
...
def _requirements(
    ...
    if not isinstance(classifications, dict):
        raise RevenueForgeError(
            "APPFORGE_QUALITY_CONDITIONAL_UNREVIEWED",
            "conditional must classify each conditional check",
        )
    ...
    for item in CONDITIONAL_CHECKS:
        ...
        if not isinstance(classification, dict) or classification.get("status") not in {
            ...
            raise RevenueForgeError(
                "APPFORGE_QUALITY_CONDITIONAL_UNREVIEWED",
                f"conditional.{item} must be reviewed",
            )
        reviewer = _text(
            classification.get("reviewed_by"), f"conditional.{item}.reviewed_by"
        )
        rationale = _text(
            classification.get("rationale"), f"conditional.{item}.rationale"
        )
        if len(rationale) < 20:
            raise RevenueForgeError(
                "APPFORGE_QUALITY_CONDITIONAL_UNREVIEWED",
                f"conditional.{item}.rationale must contain at least 20 characters",
            )
        if classification["status"] == "required":
            required[item] = {"kind": "conditional"}
        else:
            skipped.append(
                {"id": item, "reviewed_by": reviewer, "rationale": rationale}
            )
...
def verify_quality_audit(
    ...
    if observed != expected:
        findings.append(
            {
                "code": "APPFORGE_QUALITY_CANDIDATE_MISMATCH",
    ...
    if (
        ...
        or review.get("user_design_input_considered") is not True
    ):
        findings.append(
            {
                "code": "APPFORGE_QUALITY_USER_DESIGN_UNCONFIRMED",
    ...
    if not isinstance(supplied, list) or len(supplied) > 100:
        raise RevenueForgeError(
            ...
            "checks must contain at most 100 objects",
        )
    ...
    for check_id in required:
        if check_id not in seen:
            findings.append(
                {
                    "code": "APPFORGE_QUALITY_CHECK_MISSING",
    ...
    core: dict[str, Any] = {
        ...
        if not findings
        else "APPFORGE_QUALITY_AUDIT_BLOCKED",
        "ok": not findings,
...
def quality_audit_projection(root: Path) -> dict[str, Any]:
    ...
    invalid: list[str] = []
    for path in sorted(
        (workspace / ".factory" / "appforge").rglob("quality-audit*.json")
    )[:100]:
        try:
            ...
            invalid.append(path.relative_to(workspace).as_posix())
    return {
        "schema": "factory.appforge.quality-audit-projection.v1",
        "marker": "APPFORGE_QUALITY_AUDIT_READ_ONLY",
        "current_count": len(current),
        "invalid_count": len(invalid),
        "latest": current[-1] if current else None,
        "invalid": invalid,
        "authority": AUTHORITY,
        "claim_boundary": "hash-verified local quality audit status only; not a visual certification, physical-device result, or App Review approval.",
    }
