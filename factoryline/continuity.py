...
_FORBIDDEN_PAYLOAD_KEYS = frozenset(
    {"content", "payload", "embedding", "text", "messages", "vector"}
)
_ROLE_ACTIONS: dict[str, frozenset[str]] = {
    ...
    "promoter": frozenset(
        {"continuity.promote", "continuity.read", "continuity.prove"}
    ),
    "admin": frozenset(
        {
            "continuity.write",
    ...
    def __post_init__(self) -> None:
        if not self.subject.strip():
            raise ContinuityError(
                "E_IDENTITY_REQUIRED", "principal subject is required"
            )
        if not self.tenant_id.strip():
            raise ContinuityError(
                "E_TENANT_REQUIRED", "principal tenant_id is required"
            )
        if not self.purposes:
            raise ContinuityError(
                "E_PURPOSE_REQUIRED",
                "principal must declare at least one purpose reference",
            )
...
def _canonical(value: Any) -> bytes:
    try:
        ...
        raise ContinuityError(
            "E_NON_CANONICAL_DATA", f"value is not canonical JSON: {exc}"
        ) from exc
...
def _parse_future_utc(value: object) -> str:
    if not isinstance(value, str) or not value.strip():
        raise ContinuityError(
            "E_EXPIRY_REQUIRED", "expires_at is required as an RFC3339 UTC timestamp"
        )
    try:
    ...
    if parsed.tzinfo is None or parsed.utcoffset() is None:
        raise ContinuityError(
            "E_EXPIRY_INVALID", "expires_at must include a UTC offset"
        )
...
def _purpose_ref(payload: dict[str, Any]) -> str:
    ...
    if not isinstance(purpose, dict):
        raise ContinuityError(
            "E_PURPOSE_REQUIRED", "purpose must be an object with id and version"
        )
...
def _evidence_refs(payload: dict[str, Any]) -> list[str]:
    ...
    if not isinstance(value, list) or not value or len(value) > 24:
        raise ContinuityError(
            "E_EVIDENCE_REQUIRED",
            "evidence_refs must contain 1 through 24 opaque references",
        )
    refs: list[str] = []
    for item in value:
        if not isinstance(item, str) or not item.strip() or len(item.strip()) > 320:
            raise ContinuityError(
                "E_EVIDENCE_REQUIRED",
                "every evidence reference must be a bounded non-empty string",
            )
...
def _ensure_metadata_only(payload: dict[str, Any]) -> None:
    ...
    if forbidden:
        raise ContinuityError(
            ...
            "Factory Continuity records memory references and evidence metadata, not memory content: "
            + ", ".join(forbidden),
        )
...
def _validate_record(payload: dict[str, Any]) -> dict[str, Any]:
    ...
    if payload.get("schema") != CONTINUITY_RECORD_SCHEMA:
        raise ContinuityError(
            "E_SCHEMA_INVALID", f"record schema must be {CONTINUITY_RECORD_SCHEMA}"
        )
    record_type = _required_string(payload, "record_type", maximum=40).lower()
    if record_type not in _ALLOWED_TYPES:
        raise ContinuityError(
            "E_RECORD_INVALID",
            f"record_type must be one of {', '.join(sorted(_ALLOWED_TYPES))}",
        )
    ...
    if summary is not None and (
        not isinstance(summary, str) or len(summary.strip()) > 280
    ):
        raise ContinuityError(
            "E_RECORD_INVALID", "summary must be a string of at most 280 characters"
        )
    return {
...
def _authorize(
    principal: ContinuityPrincipal, action: str, tenant_id: str, purpose_ref: str
) -> None:
    ...
    if action not in allowed:
        raise ContinuityError(
            "E_ACTION_DENIED",
            f"action {action!r} is not granted to {principal.subject!r}",
        )
    if principal.tenant_id != tenant_id:
        raise ContinuityError(
            "E_TENANT_BOUNDARY", "principal cannot access another tenant"
        )
    if "*" not in principal.purposes and purpose_ref not in principal.purposes:
        raise ContinuityError(
            "E_PURPOSE_DENIED",
            f"purpose {purpose_ref!r} is not granted to {principal.subject!r}",
        )
    ...
    *,
    ...
    if not candidate.is_file():
        raise ContinuityError(
            ...
            "continuity database must already exist for read-only recall",
        )
    try:
        ...
        raise ContinuityError(
            "E_CONTINUITY_UNREADABLE",
            "continuity metadata cannot be read through the read-only boundary",
        ) from exc
    current = [
        {
            "schema": CONTINUITY_RECORD_SCHEMA,
...
class ContinuityStore:
    ...
    def _audit(
        ...
        *,
        ...
        previous = db.execute(
            "SELECT event_hash FROM continuity_audit_events WHERE tenant_id = ? ORDER BY sequence DESC LIMIT 1",
            (tenant_id,),
        ).fetchone()
        ...
        event = {
            "schema": CONTINUITY_AUDIT_SCHEMA,
            "sequence": sequence,
            "tenant_id": tenant_id,
            "action": action,
            "actor": actor,
        ...
        db.execute(
            "UPDATE continuity_audit_events SET event_hash = ? WHERE sequence = ?",
            (_sha(event), sequence),
        )
    ...
    def record(
        ...
        *,
        ...
        _authorize(
            ...
            normalized["purpose_ref"],
        )
        if (
            not isinstance(idempotency_key, str)
            or not idempotency_key.strip()
            or len(idempotency_key.strip()) > 160
        ):
            raise ContinuityError(
                "E_IDEMPOTENCY_REQUIRED",
                "idempotency_key is required and must be at most 160 characters",
            )
        ...
        with self._session() as db:
            existing = db.execute(
                "SELECT * FROM continuity_records WHERE tenant_id = ? AND idempotency_key = ?",
                (normalized["tenant_id"], idempotency_key.strip()),
            ).fetchone()
            if existing:
            ...
            try:
                db.execute(
                    ...
                    (
                        ...
                        normalized["memory_ref"],
                        normalized["memory_ref_sha256"],
                        normalized["purpose_ref"],
                        normalized["scope_ref"],
                        normalized["scope_ref_sha256"],
                        _canonical(normalized["evidence_refs"]).decode("utf-8"),
                ...
                raise ContinuityError(
                    "E_RECORD_IMMUTABLE",
                    "record id is already bound to immutable content",
                ) from exc
            self._audit(
                db,
            ...
            row = db.execute(
                "SELECT * FROM continuity_records WHERE record_id = ?", (record_id,)
            ).fetchone()
    ...
    def promote(
        ...
        *,
        ...
        if (
            ...
            raise ContinuityError(
                "E_PROMOTION_REASON_REQUIRED",
                "promotion reason is required and must be at most 280 characters",
            )
        with self._session() as db:
            row = db.execute(
                "SELECT * FROM continuity_records WHERE record_id = ? AND tenant_id = ?",
                (record_id, tenant_id),
            ).fetchone()
            if not row:
            ...
            if row["created_by"] == principal.subject:
                raise ContinuityError(
                    "E_SELF_PROMOTION",
                    "record author cannot promote their own continuity record",
                )
            if row["status"] != "draft":
                raise ContinuityError(
                    "E_ALREADY_PROMOTED",
                    "only a draft continuity record can be promoted",
                )
            if _is_expired(row["expires_at"]):
                raise ContinuityError(
                    "E_RECORD_EXPIRED", "an expired record cannot be promoted"
                )
            if not json.loads(row["evidence_json"]):
                raise ContinuityError(
                    "E_EVIDENCE_REQUIRED",
                    "a promoted record requires evidence references",
                )
            promoted_at = _now()
            db.execute(
    ...
    def withdraw(
        ...
        *,
        ...
        if (
            status not in {"superseded", "contradicted", "revoked"}
            or not isinstance(reason, str)
            or not 1 <= len(reason.strip()) <= 280
        ):
            raise ContinuityError(
                "E_WITHDRAW_INVALID", "withdrawal status and bounded reason required"
            )
        with self._session() as db:
            row = db.execute(
                "SELECT * FROM continuity_records WHERE tenant_id=? AND record_id=?",
                (tenant_id, record_id),
            ).fetchone()
            if not row:
                raise ContinuityError("E_NOT_FOUND", "continuity record not found")
            ...
            if principal.subject == row["created_by"]:
                raise ContinuityError(
                    "E_SELF_WITHDRAWAL",
                    "record creator cannot withdraw its own reviewed record",
                )
            if row["status"] != "verified":
                raise ContinuityError(
                    "E_WITHDRAW_STATE", "only verified records can be withdrawn"
                )
            self._validate_replacement(db, row, status, replacement_id)
            db.execute(
                "UPDATE continuity_records SET status=? WHERE record_id=? AND tenant_id=?",
    ...
    def _validate_replacement(db, row, status, replacement_id):
        if status != "superseded":
            if replacement_id is not None:
                raise ContinuityError(
                    "E_REPLACEMENT_INVALID", "replacement only applies to supersession"
                )
            return
        replacement = db.execute(
            "SELECT * FROM continuity_records WHERE record_id=? AND tenant_id=?",
            (replacement_id, row["tenant_id"]),
        ).fetchone()
        if not replacement or replacement_id == row["record_id"]:
            raise ContinuityError(
                "E_REPLACEMENT_INVALID", "distinct verified replacement required"
            )
        if replacement["status"] != "verified" or _is_expired(
            replacement["expires_at"]
        ):
            raise ContinuityError(
                "E_REPLACEMENT_INVALID", "replacement must be current"
            )
        if any(replacement[key] != row[key] for key in ("scope_ref", "purpose_ref")):
            raise ContinuityError(
                "E_REPLACEMENT_INVALID", "replacement scope or purpose differs"
            )
    ...
    def prove(
        self, principal: ContinuityPrincipal, tenant_id: str, record_id: str
    ) -> dict[str, Any]:
        ...
        with self._session() as db:
            ...
            events = db.execute(
                "SELECT * FROM continuity_audit_events WHERE tenant_id = ? AND record_id = ? ORDER BY sequence",
                (tenant_id, record_id),
            ).fetchall()
        ...
        return {
            ...
            "limitations": [
                ...
                "CLI principal arguments are not authenticated identity.",
                "This reference store does not hold memory contents or provide erasure, KMS, or external anchoring.",
            ],
        }

    def verify_audit(
        self, principal: ContinuityPrincipal, tenant_id: str, *, purpose_ref: str
    ) -> dict[str, Any]:
        ...
        with self._session() as db:
            rows = db.execute(
                "SELECT * FROM continuity_audit_events WHERE tenant_id = ? ORDER BY sequence",
                (tenant_id,),
            ).fetchall()
        previous_hash = ""
        errors: list[str] = []
    ...
    def status(self) -> dict[str, Any]:
        ...
        with self._session() as db:
            rows = db.execute(
                "SELECT tenant_id, status, expires_at FROM continuity_records ORDER BY tenant_id, record_id"
            ).fetchall()
        facts = {
...
def continuity_projection(root: Path, *, limit: int = 100) -> dict[str, Any]:
    ...
    try:
        ...
        try:
            rows = connection.execute(
                "SELECT * FROM continuity_records ORDER BY created_at DESC, record_id ASC LIMIT ?",
                (limit + 1,),
            ).fetchall()
            all_rows = connection.execute(
                "SELECT status, expires_at FROM continuity_records"
            ).fetchall()
        finally:
            connection.close()
        if len(rows) > limit:
        ...
        for row in rows:
            ...
            record["effective_status"] = (
                "expired" if _is_expired(record["expires_at"]) else record["status"]
            )
...
def principal_from_args(
    subject: str, tenant_id: str, roles: Iterable[str], purposes: Iterable[str]
) -> ContinuityPrincipal:
...
def recall_continuity(
    ...
    return store.recall(
        principal, tenant_id, purpose_ref=purpose_ref, scope_ref=scope_ref
    )


def promote_continuity(
    store: ContinuityStore,
    principal: ContinuityPrincipal,
    tenant_id: str,
    record_id: str,
