...
_RISK_TAGS = frozenset(
    {"boundary", "authorization", "idempotency", "temporal", "state", "validation"}
)
_MUTATIONS = {
...
def _timestamp(value: object, field: str) -> datetime:
    if not isinstance(value, str):
        raise GauntletError(
            "GAUNTLET_ADMISSION_INVALID", f"{field} must be RFC3339 text"
        )
    try:
    ...
    if parsed.tzinfo is None:
        raise GauntletError(
            "GAUNTLET_ADMISSION_INVALID", f"{field} must include a timezone"
        )
    return parsed.astimezone(timezone.utc)


def _inside(
    root: Path, value: Path, field: str, *, exists: bool = True
) -> tuple[Path, str]:
    ...
    try:
        ...
        raise GauntletError(
            "GAUNTLET_PATH_INVALID", f"{field} must remain inside the workspace"
        ) from exc
    if exists and not resolved.is_file():
        raise GauntletError(
            "GAUNTLET_INPUT_UNREADABLE", f"{field} must name a readable workspace file"
        )
    return resolved, relative


def _text(
    value: object, field: str, *, identifier: bool = False, limit: int = 600
) -> str:
    ...
    if identifier and not _ID.fullmatch(result):
        raise GauntletError(
            "GAUNTLET_SOURCE_INVALID", f"{field} has an unsupported identifier"
        )
    return result
...
def _clear_intent(value: str, field: str) -> str:
    ...
    try:
        ...
        raise GauntletError(
            "GAUNTLET_INTENT_UNCLEAR", f"{field}: {exc.message}"
        ) from exc
    return value
...
def _compound_identifier(value: object, field: str) -> str:
    ...
    if not _COMPOUND_ID.fullmatch(result):
        raise GauntletError(
            "GAUNTLET_SOURCE_INVALID", f"{field} has an unsupported compound identifier"
        )
    return result


def _load_json(
    root: Path, path: Path, field: str
    ...
    try:
        ...
        raise GauntletError(
            "GAUNTLET_INPUT_INVALID", f"{field} must be UTF-8 JSON: {exc}"
        ) from exc
    if not isinstance(value, dict):
        raise GauntletError(
            "GAUNTLET_INPUT_INVALID", f"{field} must contain one JSON object"
        )
...
def _relative_text(root: Path, value: object, field: str) -> Path:
    ...
    if supplied.is_absolute() or ".." in supplied.parts:
        raise GauntletError(
            "GAUNTLET_PATH_INVALID", f"{field} must remain workspace relative"
        )
...
def _risk_tags(value: object, field: str) -> list[str]:
    if not isinstance(value, list) or not 1 <= len(value) <= len(_RISK_TAGS):
        raise GauntletError(
            "GAUNTLET_SOURCE_INVALID",
            f"{field} must contain 1 through {len(_RISK_TAGS)} taxonomy tags",
        )
    if any(not isinstance(tag, str) or tag not in _RISK_TAGS for tag in value) or len(
        set(value)
    ) != len(value):
        raise GauntletError(
            "GAUNTLET_SOURCE_INVALID",
            f"{field} must contain unique supported taxonomy tags",
        )
    return sorted(value)


def _string_list(
    value: object, field: str, *, maximum: int, identifier: bool = False
) -> list[str]:
    ...
    result = [
        _text(item, f"{field} entry", identifier=identifier, limit=240)
        for item in value
    ]
    if len(set(result)) != len(result):
        raise GauntletError(
            "GAUNTLET_SOURCE_INVALID", f"{field} entries must be unique"
        )
...
def _continuity_binding(root: Path, value: object) -> dict[str, Any] | None:
    ...
    if not isinstance(value, dict) or set(value) != required:
        raise GauntletError(
            "GAUNTLET_CONTINUITY_INVALID",
            "continuity must contain exactly db, tenant_id, purpose_ref, scope_ref, principal, and record_ids",
        )
    ...
    if not isinstance(principal_raw, dict) or set(principal_raw) != {
        "subject",
        "roles",
        "purposes",
    }:
        raise GauntletError(
            "GAUNTLET_CONTINUITY_INVALID",
            "continuity.principal must contain subject, roles, and purposes",
        )
    record_ids = _string_list(
        value.get("record_ids"),
        "continuity.record_ids",
        maximum=MAX_CONTINUITY_RECORDS,
        identifier=True,
    )
    try:
        principal = ContinuityPrincipal(
            subject=_text(
                principal_raw.get("subject"), "continuity.principal.subject", limit=160
            ),
            tenant_id=tenant_id,
            roles=tuple(
                _string_list(
                    principal_raw.get("roles"),
                    "continuity.principal.roles",
                    maximum=8,
                    identifier=True,
                )
            ),
            purposes=tuple(
                _string_list(
                    principal_raw.get("purposes"),
                    "continuity.principal.purposes",
                    maximum=12,
                )
            ),
        )
        ...
        raise GauntletError(
            "GAUNTLET_CONTINUITY_UNAVAILABLE",
            f"verified continuity metadata is unavailable: {exc.code}",
        ) from exc
    ...
    if any(record_id not in records_by_id for record_id in record_ids):
        raise GauntletError(
            "GAUNTLET_CONTINUITY_STALE",
            "every selected continuity record must be verified, unexpired, and exact-scope authorized",
        )
    records = [
        {
...
def _validate_continuity_binding(value: object, label: str) -> None:
    ...
    if (
        ...
        or set(value) != fields
        or value.get("marker") != "GAUNTLET_CONTINUITY_METADATA_BOUND"
    ):
        raise GauntletError(
    ...
    if value["binding_sha256"] != _sha(core):
        raise GauntletError(
            ...
            f"{label} continuity binding SHA-256 does not match",
        )
...
def _source(root: Path, source_path: Path) -> tuple[dict[str, Any], Path, str, str]:
    ...
    if (
        ...
        or set(source) - (required | {"continuity"})
        or source.get("schema") != GAUNTLET_SOURCE_SCHEMA
    ):
        raise GauntletError(
            "GAUNTLET_SOURCE_INVALID",
...
def _proposal_core(root: Path, source_path: Path) -> dict[str, Any]:
    ...
    for promise in source["promises"]:
        try:
            reality = validate_reality_check_manifest(
                root, root / promise["reality_manifest"]
            )
            ...
            proposals.append(
                {
                    ...
                    "reality": {
                        "path": promise["reality_manifest"],
                        "sha256": reality["manifest_sha256"],
                        "promise": reality["behavior"]["promise"],
                        "failure_case": reality["behavior"]["failure_case"],
                    },
                    "sabotage": {
                        "id": case["id"],
                        "risk_tag": case["risk_tag"],
                        "mutation": _MUTATIONS[case["risk_tag"]],
                        "summary": case["summary"],
                    },
                    "e2e": {
                        "path": case["e2e_manifest"],
                        "sha256": e2e["manifest_sha256"],
                        "id": e2e["id"],
                        "approved_by": e2e["approval"]["approved_by"],
                        "timeout_seconds": e2e["timeout_seconds"],
                        "positive_argv": e2e["positive"]["argv"],
                        "negative_argv": e2e["negative"]["argv"],
                    },
                }
            )
    if total_timeout > MAX_TOTAL_TIMEOUT_SECONDS:
        raise GauntletError(
            "GAUNTLET_BUDGET_EXCEEDED",
            f"declared E2E timeouts exceed {MAX_TOTAL_TIMEOUT_SECONDS} seconds",
        )
    return {
        ...
        "facts": {
            "promise_count": len(source["promises"]),
            "case_count": len(proposals),
            "declared_timeout_seconds": total_timeout,
            "risk_tags": sorted({entry["sabotage"]["risk_tag"] for entry in proposals}),
        },
...
def _proposal(root: Path, proposal_path: Path) -> tuple[dict[str, Any], Path, str]:
    ...
    if (
        set(value) != required
        or value.get("schema") != GAUNTLET_PROPOSAL_SCHEMA
        or value.get("marker") != "GAUNTLET_PROPOSAL_COMPILED"
    ):
        raise GauntletError(
    ...
    if (
        ...
        or value["proposal_sha256"] != _sha(core)
    ):
        raise GauntletError(
...
def verify_gauntlet_proposal(root: Path, proposal_path: Path) -> dict[str, Any]:
    ...
    try:
        current = compile_gauntlet_proposal(
            workspace, workspace / value["source"]["path"]
        )
    except GauntletError as exc:
        return {
            "schema": GAUNTLET_PROPOSAL_SCHEMA,
            "marker": "GAUNTLET_PROPOSAL_STALE",
            "ok": False,
    ...
    if current != expected:
        return {
            ...
            "ok": False,
            "proposal_path": relative,
            "reason": "source_or_e2e_binding_changed",
        }
    return {
        "schema": GAUNTLET_PROPOSAL_SCHEMA,
        "marker": "GAUNTLET_PROPOSAL_VERIFIED",
        "ok": True,
        "proposal_path": relative,
...
def _atomic(path: Path, payload: dict[str, Any]) -> Path:
    ...
    if path.exists():
        if path.read_bytes() == content:
            return path
        raise GauntletError(
            "GAUNTLET_ARTIFACT_EXISTS", f"a different artifact already exists at {path}"
        )
...
def write_gauntlet_proposal(
    root: Path, proposal: dict[str, Any], out: Path | None = None
) -> Path:
    """Write a proposal under an explicit or canonical local artifact path."""
    workspace = Path(root).resolve()
    if (
        not isinstance(proposal, dict)
        or proposal.get("schema") != GAUNTLET_PROPOSAL_SCHEMA
    ):
        raise GauntletError(
            "GAUNTLET_PROPOSAL_INVALID", "a compiled Gauntlet proposal is required"
        )
    ...
    if (
        ...
        raise GauntletError(
            "GAUNTLET_PROPOSAL_INVALID", "proposal source or digest is invalid"
        )
    target = (
        Path(out)
        if out is not None
        ...
        / "gauntlets"
        / source["id"]
        / f"{proposal['proposal_sha256']}.proposal.json"
    )
    resolved, _relative_path = _inside(
        workspace, target, "proposal output", exists=False
    )
...
def admit_gauntlet(
    ...
    *,
    ...
    if not verification["ok"]:
        raise GauntletError(
            "GAUNTLET_PROPOSAL_STALE",
            "current source and E2E bindings must verify before admission",
        )
    ...
    if confirmation != f"ADMIT {proposal['source']['id']}":
        raise GauntletError(
            "GAUNTLET_CONFIRMATION_REQUIRED",
            f"confirmation must be exactly ADMIT {proposal['source']['id']}",
        )
    if (
        isinstance(valid_for_minutes, bool)
        or not isinstance(valid_for_minutes, int)
        or not 1 <= valid_for_minutes <= 60
    ):
        raise GauntletError(
            "GAUNTLET_ADMISSION_INVALID",
            "valid_for_minutes must be an integer from 1 through 60",
        )
    core = _admission_core(
        proposal, relative, owner, reason, _now() + timedelta(minutes=valid_for_minutes)
    )
    ...
    target = (
        ...
        / "gauntlets"
        / proposal["source"]["id"]
        / f"{admission['admission_sha256']}.admission.json"
    )
...
def _admission(root: Path, admission_path: Path) -> tuple[dict[str, Any], Path, str]:
    ...
    if (
        set(value) != required
        or value.get("schema") != GAUNTLET_ADMISSION_SCHEMA
        or value.get("marker") != "GAUNTLET_ADMISSION_SEALED"
    ):
        raise GauntletError(
            "GAUNTLET_ADMISSION_INVALID",
    ...
    if (
        ...
        or value["admission_sha256"] != _sha(core)
    ):
        raise GauntletError(
    ...
    if (
        ...
        or not _SHA.fullmatch(proposal["sha256"])
    ):
        raise GauntletError(
    ...
    issued_at, expires_at = (
        ...
        _timestamp(value.get("expires_at"), "expires_at"),
    )
...
def verify_gauntlet_admission(
    root: Path, admission_path: Path, proposal_path: Path | None = None
) -> dict[str, Any]:
    ...
    if proposal_path is not None:
        ...
        if requested_relative != admission["proposal"]["path"]:
            return {
                "schema": GAUNTLET_ADMISSION_SCHEMA,
                "marker": "GAUNTLET_ADMISSION_MISMATCH",
                "ok": False,
                "admission_path": relative,
            }
        bound_proposal = requested
    verification = verify_gauntlet_proposal(workspace, bound_proposal)
    if (
        not verification["ok"]
        or verification.get("proposal_sha256") != admission["proposal"]["sha256"]
    ):
        return {
            "schema": GAUNTLET_ADMISSION_SCHEMA,
            "marker": "GAUNTLET_ADMISSION_STALE",
            "ok": False,
            "admission_path": relative,
        }
    if _timestamp(admission["expires_at"], "expires_at") <= _now():
        return {
            "schema": GAUNTLET_ADMISSION_SCHEMA,
            "marker": "GAUNTLET_ADMISSION_EXPIRED",
            "ok": False,
            "admission_path": relative,
        }
    return {
        "schema": GAUNTLET_ADMISSION_SCHEMA,
        "marker": "GAUNTLET_ADMISSION_VERIFIED",
        "ok": True,
        "admission_path": relative,
        "admission_sha256": admission["admission_sha256"],
        "proposal_sha256": admission["proposal"]["sha256"],
...
def _case_outcome(proposal: dict[str, Any], receipt: dict[str, Any]) -> dict[str, Any]:
    ...
    return {
        ...
        "reality": proposal["reality"],
        "sabotage": proposal["sabotage"],
        "e2e": {
            "path": proposal["e2e"]["path"],
            "sha256": proposal["e2e"]["sha256"],
            "id": proposal["e2e"]["id"],
        },
        "status": status,
        "e2e_receipt": public,
...
def _card_markdown(card: dict[str, Any]) -> str:
    ...
    return "\n".join(
        [
            "# Code Factory Survival Card",
            "",
            f"- Card: `{card['card_id']}`",
            f"- Result: `{card['marker']}`",
            f"- Survived: `{summary['survived_count']}/{summary['case_count']}`",
            f"- Hollow: `{summary['hollow_count']}`",
            f"- Blocked: `{summary['blocked_count']}`",
            f"- Unproven promises: `{summary['unproven_promise_count']}`",
            f"- Card SHA-256: `{card['card_sha256']}`",
            "",
            "**Do not trust the badge alone — verify the card.**",
            "",
...
def _card_svg(card: dict[str, Any]) -> str:
    ...
    lines = [
        title,
        f"{summary['survived_count']}/{summary['case_count']} declared sabotages survived",
        f"{summary['unproven_promise_count']} promise(s) unproven",
        "verify: factory gauntlet card verify <card>",
    ]
    text = "".join(
        f'<text x="36" y="{74 + index * 31}" fill="#eff8f1" font-family="Arial, sans-serif" font-size="{28 if index == 0 else 18}" font-weight="{700 if index == 0 else 400}">{html.escape(line)}</text>'
...
def _card_core(
    ...
    return {
        ...
        "summary": {
            ...
            "survived_count": sum(item["status"] == "survived" for item in outcomes),
            "hollow_count": sum(item["status"] == "hollow" for item in outcomes),
...
def _admission_already_consumed(root: Path, admission_sha256: str) -> bool:
    for path in (root / ".factory" / "gauntlets").glob("*/*.card.json"):
        ...
        if (
            ...
            and isinstance(value.get("admission"), dict)
            and value["admission"].get("sha256") == admission_sha256
        ):
            return True
    return False
...
def _claim_admission(root: Path, source_id: str, admission_sha256: str) -> Path:
    ...
    try:
        ...
        raise GauntletError(
            "GAUNTLET_ADMISSION_CONSUMED",
            "this admission has already reserved or completed its one allowed run",
        ) from exc
    return lock
...
def run_gauntlet(
    ...
    if not proposal_verification["ok"]:
        raise GauntletError(
            "GAUNTLET_PROPOSAL_STALE",
            "the source and declared E2E bindings must verify before execution",
        )
    if admission_path is None:
        raise GauntletError(
            "GAUNTLET_ADMISSION_REQUIRED",
            "a current named admission receipt is required before execution",
        )
    admission_verification = verify_gauntlet_admission(
        workspace, admission_path, proposal_path
    )
    if not admission_verification["ok"]:
        raise GauntletError(
            admission_verification["marker"],
            "a current, unexpired admission receipt is required before execution",
        )
    proposal, _proposal_resolved, proposal_relative = _proposal(
        workspace, proposal_path
    )
    admission, _admission_resolved, admission_relative = _admission(
        workspace, admission_path
    )
    if _admission_already_consumed(workspace, admission["admission_sha256"]):
        raise GauntletError(
            "GAUNTLET_ADMISSION_CONSUMED",
            "this admission has already completed its one allowed run",
        )
    lock = _claim_admission(
        workspace, proposal["source"]["id"], admission["admission_sha256"]
    )
    try:
        outcomes: list[dict[str, Any]] = []
...
def _load_card(card_path: Path) -> dict[str, Any]:
    try:
        ...
        raise GauntletError(
            "SURVIVAL_CARD_INVALID", f"card must be readable UTF-8 JSON: {exc}"
        ) from exc
...
def _validate_card_bindings(card: dict[str, Any]) -> None:
    ...
    admission = _hash_binding(
        card.get("admission"), {"path", "sha256", "approved_by"}, "admission"
    )
...
def _validate_commit_binding(value: object) -> None:
    ...
    if value.get("state") == "unavailable" and set(value) == {"state"}:
        return
    if (
        value.get("state") == "bound"
        and set(value) == {"state", "commit"}
        and isinstance(value.get("commit"), str)
        and _COMMIT.fullmatch(value["commit"])
    ):
        return
    raise GauntletError("SURVIVAL_CARD_INVALID", "card commit binding is invalid")
...
def _validate_card_envelope(card: object) -> dict[str, Any]:
    if (
        ...
        or card.get("schema") != SURVIVAL_CARD_SCHEMA
    ):
        raise GauntletError(
            "SURVIVAL_CARD_INVALID", "card has an unsupported schema or fields"
        )
    core = {
        key: card[key]
        for key in _CARD_FIELDS - {"card_sha256", "card_markdown", "card_svg"}
    }
    if (
        not isinstance(card.get("card_sha256"), str)
        or not _SHA.fullmatch(card["card_sha256"])
        or card["card_sha256"] != _sha(core)
    ):
...
def _validate_outcome(outcome: object) -> tuple[str, str]:
    ...
    if not isinstance(outcome, dict) or set(outcome) != fields:
        raise GauntletError(
            "SURVIVAL_CARD_INVALID", "card outcome has unsupported fields"
        )
    ...
    promise_statement = _clear_intent(
        _text(promise.get("statement"), "outcome promise statement"),
        "outcome promise statement",
    )
    if not proposal_id.startswith(f"{promise_id}--"):
        raise GauntletError(
            "SURVIVAL_CARD_INVALID",
            "card outcome promise id is not bound to its proposal id",
        )
    reality = _hash_binding(
        outcome.get("reality"),
        {"path", "sha256", "promise", "failure_case"},
        "outcome Reality Check",
    )
    if reality["promise"] != promise_statement:
        raise GauntletError(
            "SURVIVAL_CARD_INVALID",
            "card outcome promise differs from its Reality Check promise",
        )
    ...
    if (
        ...
        or set(sabotage) != {"id", "risk_tag", "mutation", "summary"}
        or sabotage.get("risk_tag") not in _RISK_TAGS
        or sabotage.get("mutation") != _MUTATIONS[sabotage["risk_tag"]]
    ):
    ...
    status = (
        "survived"
        if receipt["marker"] == "E2E_PROOF_PASS"
        else "hollow"
        if receipt["marker"] == "HOLLOW_E2E_TEST"
        else "blocked"
    )
    if (
        outcome.get("status") != status
        or receipt["manifest"]["manifest_sha256"] != e2e["sha256"]
        or receipt["manifest"]["id"] != e2e["id"]
    ):
        raise GauntletError(
...
def _validated_outcomes(card: dict[str, Any]) -> list[str]:
    ...
    if not isinstance(outcomes, list) or not outcomes or len(outcomes) > MAX_CASES:
        raise GauntletError(
            "SURVIVAL_CARD_INVALID",
            "card outcomes must contain 1 through the case limit",
        )
    ...
    if len({proposal_id for proposal_id, _status in validated}) != len(validated):
        raise GauntletError(
            "SURVIVAL_CARD_INVALID", "card outcome proposal ids must be unique"
        )
...
def _validate_card_summary(card: dict[str, Any], statuses: list[str]) -> None:
    ...
    marker, ok = _card_marker(outcomes)
    unproven = sorted(
        {entry["promise"]["id"] for entry in outcomes if entry["status"] != "survived"}
    )
    summary = {
        "case_count": len(outcomes),
        "survived_count": statuses.count("survived"),
        "hollow_count": statuses.count("hollow"),
    ...
    if (
        ...
        card.get("ok"),
        card.get("summary"),
        card.get("unproven_promises"),
    ) != (marker, ok, summary, unproven):
        raise GauntletError(
            "SURVIVAL_CARD_INVALID", "card result summary does not match outcomes"
        )


def _validate_card_views(card: dict[str, Any]) -> None:
    if card.get("card_markdown") != _card_markdown(card) or card.get(
        "card_svg"
    ) != _card_svg(card):
        raise GauntletError(
            "SURVIVAL_CARD_INVALID", "card views do not match card facts"
        )
...
def write_survival_card_artifacts(
    card: dict[str, Any], out_dir: Path
) -> dict[str, str]:
    ...
    for name, path in paths.items():
        content = (
            _canonical(card) + b"\n"
            if name == "json"
            else card["card_markdown"].encode("utf-8")
            if name == "markdown"
            else card["card_svg"].encode("utf-8")
        )
        if path.exists() and path.read_bytes() != content:
            raise GauntletError(
                "GAUNTLET_ARTIFACT_EXISTS",
                f"a different card artifact already exists at {path}",
            )
        if not path.exists():
            temporary = path.with_name(f".{path.name}.tmp")
            temporary.write_bytes(content)
...
def verify_survival_card(
    ...
    *,
    ...
    result: dict[str, Any] = {
        "schema": SURVIVAL_CARD_SCHEMA,
        "marker": "SURVIVAL_CARD_VERIFIED",
        "ok": True,
        "card_sha256": card["card_sha256"],
        "signature": "not_supplied",
    }
    if (envelope_path is None) != (trust_root_path is None):
        raise GauntletError(
            "SURVIVAL_CARD_SIGNATURE_INPUT_INVALID",
            "supply both envelope_path and trust_root_path or neither",
        )
    if envelope_path is not None and trust_root_path is not None:
        ...
        payload_bytes = base64.urlsafe_b64decode(
            (envelope["payload"] + "=" * (-len(envelope["payload"]) % 4)).encode(
                "ascii"
            )
        )
        ...
        if payload.get("subject_sha256") != card["card_sha256"]:
            raise GauntletError(
                "SURVIVAL_CARD_SIGNATURE_INVALID",
                "Receipt v2 subject_sha256 does not bind this card",
            )
        result["signature"] = {
    ...
    *,
    ...
    try:
        ...
        payload = {
            ...
            "ok": card["ok"],
            "tenant_id": _text(tenant_id, "tenant_id", identifier=True),
            "run_id": card["card_sha256"][:32],
            "ts": _iso(_now()),
            "subject_sha256": card["card_sha256"],
        }
        envelope = seal_receipt_v2(
            payload,
            Path(private_key_path),
            _text(keyid, "keyid", identifier=True),
...
def challenge_survival_card(card_path: Path) -> dict[str, Any]:
    ...
    try:
        ...
        return {
            "schema": SURVIVAL_CARD_SCHEMA,
            "marker": "GAUNTLET_CARD_MUTATION_REJECTED",
            "ok": True,
            "card_sha256": card["card_sha256"],
            "mutations": 1,
        }
    return {
        "schema": SURVIVAL_CARD_SCHEMA,
        "marker": "HOLLOW_SURVIVAL_CARD_VERIFIER",
        "ok": False,
        "card_sha256": card["card_sha256"],
        "mutations": 1,
    }
...
def _continuity_status(binding: dict[str, Any] | None) -> dict[str, Any]:
    if binding is None:
        return {
            "bound": False,
            "marker": "GAUNTLET_CONTINUITY_NOT_BOUND",
            "record_count": 0,
        }
    return {
        "bound": True,
        ...
        entries.append(
            {
                ...
                "card_sha256": card["card_sha256"],
                "summary": card["summary"],
                "continuity": _continuity_status(card["continuity"]),
                "commit": card["commit"],
            }
        )
    return {
        "schema": SURVIVAL_CARD_SCHEMA,
        "marker": "GAUNTLET_STATUS_READ_ONLY",
        "source_id": source_id,
        "entries": entries,
        "authority": dict(_PLANNING_AUTHORITY),
        "scope_limits": [
            "Status is a local projection. It does not execute, admit, sign, repair, or promote a Gauntlet result."
        ],
    }
