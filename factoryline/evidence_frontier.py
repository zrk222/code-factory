    ...
    try:
        if not source.is_file() or source.stat().st_size > MAX_SOURCE_BYTES:
            raise EvidenceFrontierError(
                "EVIDENCE_FRONTIER_SOURCE_UNREADABLE",
        ...
        raise EvidenceFrontierError(
            "EVIDENCE_FRONTIER_SOURCE_UNREADABLE", "source must be readable JSON"
        ) from exc
    if not isinstance(value, dict):
        raise EvidenceFrontierError(
            "EVIDENCE_FRONTIER_SOURCE_INVALID", "source root must be one object"
        )
    return value
...
def _text(value: object, field: str, maximum: int = 320) -> str:
    if not isinstance(value, str) or not value.strip() or len(value) > maximum:
        raise EvidenceFrontierError(
            "EVIDENCE_FRONTIER_SOURCE_INVALID",
...
def _relative(value: object, field: str) -> str:
    ...
    if (
        ...
        raise EvidenceFrontierError(
            "EVIDENCE_FRONTIER_PATH_INVALID", f"{field} must be workspace-relative"
        )
    return path
...
def _path_sha(root: Path, relative: str, expected: str, field: str) -> Path:
    ...
    try:
        ...
        raise EvidenceFrontierError(
            "EVIDENCE_FRONTIER_PATH_INVALID", f"{field} escapes the workspace"
        ) from exc
    if not path.is_file() or path.stat().st_size > MAX_SOURCE_BYTES:
        raise EvidenceFrontierError(
            "EVIDENCE_FRONTIER_EVIDENCE_UNREADABLE", f"{field} is missing or too large"
        )
    if hashlib.sha256(path.read_bytes()).hexdigest() != expected:
        raise EvidenceFrontierError(
            "EVIDENCE_FRONTIER_EVIDENCE_HASH_MISMATCH",
            f"{field} SHA-256 does not match",
        )
    return path
...
def _evaluation(
    ...
    if not isinstance(raw, dict) or set(raw) != {"path", "sha256"}:
        raise EvidenceFrontierError(
            "EVIDENCE_FRONTIER_REQUEST_INVALID",
            "evaluation must contain exactly path and sha256",
        )
    ...
    if verification.get("valid") is not True:
        raise EvidenceFrontierError(
            "EVIDENCE_FRONTIER_EVALUATION_INVALID",
            "evaluation must be a current verified ProofSearch receipt",
        )
    value = _load(source)
    candidates = value.get("candidates")
    if not isinstance(candidates, list):
        raise EvidenceFrontierError(
            "EVIDENCE_FRONTIER_EVALUATION_INVALID", "evaluation candidates are invalid"
        )
    eligible = sorted(
        _text(item.get("candidate_id"), "evaluation.candidate_id", 120)
        for item in candidates
        if isinstance(item, dict) and item.get("eligible") is True
    )
    if not 2 <= len(eligible) <= 12 or len(set(eligible)) != len(eligible):
        raise EvidenceFrontierError(
            "EVIDENCE_FRONTIER_ELIGIBLE_CANDIDATES_REQUIRED",
            "evaluation must contain 2 through 12 unique eligible candidates",
        )
...
def _measurement(root: Path, raw: object, experiment_id: str) -> dict[str, Any] | None:
    ...
    if not isinstance(raw, dict) or set(raw) != {"elapsed_ms", "receipt"}:
        raise EvidenceFrontierError(
            "EVIDENCE_FRONTIER_EXPERIMENT_INVALID",
            f"{experiment_id}.measurement must contain exactly elapsed_ms and receipt",
        )
    receipt = raw.get("receipt")
    if not isinstance(receipt, dict) or set(receipt) != {"path", "sha256"}:
        raise EvidenceFrontierError(
            "EVIDENCE_FRONTIER_EXPERIMENT_INVALID",
            f"{experiment_id}.measurement.receipt must contain exactly path and sha256",
        )
    ...
    return {
        "elapsed_ms": _integer(
            raw.get("elapsed_ms"),
            f"{experiment_id}.measurement.elapsed_ms",
            MAX_ELAPSED_MS,
        ),
...
def _experiment(
    root: Path, raw: object, eligible: list[str], seen: set[str]
) -> dict[str, Any]:
    if not isinstance(raw, dict) or set(raw) != {
        ...
        raise EvidenceFrontierError(
            "EVIDENCE_FRONTIER_EXPERIMENT_INVALID",
            "experiment must contain exactly experiment_id, kind, description, predictions, and measurement",
        )
    experiment_id = _text(raw.get("experiment_id"), "experiment_id", 120)
    if experiment_id in seen:
        raise EvidenceFrontierError(
            "EVIDENCE_FRONTIER_EXPERIMENT_DUPLICATE",
    ...
    if kind not in _KINDS:
        raise EvidenceFrontierError(
            "EVIDENCE_FRONTIER_EXPERIMENT_INVALID",
            f"{experiment_id}.kind must be one of {', '.join(sorted(_KINDS))}",
        )
    predictions = raw.get("predictions")
    if not isinstance(predictions, dict) or set(predictions) != set(eligible):
        raise EvidenceFrontierError(
            "EVIDENCE_FRONTIER_PREDICTION_INVALID",
            f"{experiment_id}.predictions must contain exactly the eligible candidate identifiers",
        )
    ...
    if any(value not in _OUTCOMES for value in normalized.values()):
        raise EvidenceFrontierError(
            "EVIDENCE_FRONTIER_PREDICTION_INVALID",
            f"{experiment_id}.predictions values must be pass, fail, or unknown",
        )
    pairs = list(combinations(eligible, 2))
    separated = sum(
        normalized[left] != "unknown"
        and normalized[right] != "unknown"
...
def _rank(experiment: dict[str, Any]) -> tuple[object, ...]:
    ...
    return (
        -experiment["separation_count"],
        measurement is None,
        measurement["elapsed_ms"] if measurement else 0,
...
def _core(
    ...
    return {
        ...
        "decision": "next_experiment_selected"
        if chosen
        else "no_discriminating_experiment",
        "savings": {
            "elapsed_ms": None,
            "tokens": None,
            "cost_usd": None,
...
def plan_evidence_frontier(root: Path, request_path: Path, out: Path) -> dict[str, Any]:
    ...
    if request.get("schema") != REQUEST_SCHEMA:
        raise EvidenceFrontierError(
            "EVIDENCE_FRONTIER_REQUEST_INVALID", f"schema must be {REQUEST_SCHEMA}"
        )
    evaluation_value, evaluation, eligible = _evaluation(
        workspace, request.get("evaluation")
    )
    maximum = _integer(
        request.get("max_experiments"), "max_experiments", MAX_EXPERIMENTS
    )
    if maximum < 1:
        raise EvidenceFrontierError(
            "EVIDENCE_FRONTIER_REQUEST_INVALID",
            "max_experiments must be from 1 through 64",
        )
    raw_experiments = request.get("experiments")
    if (
        not isinstance(raw_experiments, list)
        or not 1 <= len(raw_experiments) <= maximum
    ):
        raise EvidenceFrontierError(
            "EVIDENCE_FRONTIER_EXPERIMENT_COUNT",
            "experiments must contain 1 through max_experiments items",
        )
    seen: set[str] = set()
    experiments = [
        _experiment(workspace, item, eligible, seen) for item in raw_experiments
    ]
    ...
    marker = (
        ...
        if chosen
        else "EVIDENCE_FRONTIER_NO_DISCRIMINATING_EXPERIMENT"
    )
    payload = {
        **core,
        ...
        "markers": [
            marker,
            "EVIDENCE_FRONTIER_PREDICTIONS_UNVERIFIED",
            "EVIDENCE_FRONTIER_AUTHORITY_RETAINED",
            "EVIDENCE_FRONTIER_SAVINGS_UNMEASURED",
        ],
    }
    _write(payload, out)
...
def verify_evidence_frontier(root: Path, frontier_path: Path) -> dict[str, Any]:
    ...
    try:
        ...
        maximum = _integer(
            value.get("max_experiments"), "max_experiments", MAX_EXPERIMENTS
        )
        ...
        if (
            len(rebuilt) != len(experiments)
            or value.get("eligible_candidate_ids") != eligible
        ):
            raise EvidenceFrontierError(
                "EVIDENCE_FRONTIER_SOURCE_INVALID",
                "frontier candidates or experiments are invalid",
            )
