...
def _text(value: object, field: str, *, limit: int = 500) -> str:
    ...
    if not result or len(result) > limit:
        raise RevenueForgeError(
            "APPFORGE_NATIVE_SURFACE_INVALID",
...
def _digest(value: object, field: str) -> str:
    ...
    if not re.fullmatch(r"[0-9a-f]{64}", result):
        raise RevenueForgeError(
            "APPFORGE_NATIVE_SURFACE_INVALID", f"{field} must be a SHA-256 hex digest"
        )
    return result
...
def _timestamp(value: object, field: str) -> str:
    ...
    try:
        ...
        raise RevenueForgeError(
            "APPFORGE_NATIVE_SURFACE_INVALID", f"{field} must be RFC3339"
        ) from exc
    return result
...
def _local(root: Path, path: Path, *, exists: bool = True) -> Path:
    ...
    try:
        ...
        raise RevenueForgeError(
            "APPFORGE_NATIVE_SURFACE_PATH_REJECTED",
            "paths must remain inside the workspace",
        ) from exc
    if exists and not target.is_file():
        raise RevenueForgeError(
            "APPFORGE_NATIVE_SURFACE_INPUT_UNAVAILABLE",
...
def _read_json(root: Path, path: Path, schema: str) -> tuple[dict[str, Any], Path]:
    ...
    if source.stat().st_size > MAX_BYTES:
        raise RevenueForgeError(
            "APPFORGE_NATIVE_SURFACE_INPUT_TOO_LARGE", "JSON input exceeds 1 MiB"
        )
    try:
        value = json.loads(source.read_text(encoding="utf-8-sig"))
    except (OSError, UnicodeDecodeError, json.JSONDecodeError) as exc:
        raise RevenueForgeError(
            "APPFORGE_NATIVE_SURFACE_INPUT_INVALID", "input must be valid JSON"
        ) from exc
    if not isinstance(value, dict) or value.get("schema") != schema:
        raise RevenueForgeError(
            "APPFORGE_NATIVE_SURFACE_SCHEMA_REJECTED", f"expected {schema}"
        )
    return value, source
...
def _contract_design_and_platforms(
    ...
    if set(value) != required or value.get("candidate") != candidate:
        raise RevenueForgeError(
            ...
            "contract must contain only the native-surface fields and match the exact candidate",
        )
    design_hash = _digest(
        value.get("user_design_input_sha256"), "user_design_input_sha256"
    )
...
def _contract_sources(root: Path, source_files: object) -> list[Path]:
    if not isinstance(source_files, list) or not 1 <= len(source_files) <= 100:
        raise RevenueForgeError(
            "APPFORGE_NATIVE_SURFACE_CONTRACT_INVALID",
            "source_files must contain 1-100 Swift source paths",
        )
    sources: list[Path] = []
    for item in source_files:
        ...
        if path.suffix.lower() != ".swift" or path in sources:
            raise RevenueForgeError(
                "APPFORGE_NATIVE_SURFACE_CONTRACT_INVALID",
                "source_files must be unique Swift files",
            )
...
def _adaptive_contract(adaptive: object, platforms: list[str]) -> dict[str, Any]:
    if not isinstance(adaptive, dict) or set(adaptive) != {
        "iphone_navigation",
        "ipad_navigation",
        "independent_destination_paths",
        "hardcoded_screen_geometry_allowed",
    }:
        raise RevenueForgeError(
            "APPFORGE_NATIVE_SURFACE_CONTRACT_INVALID",
            "adaptive must have the fixed navigation fields",
        )
    if adaptive.get("iphone_navigation") not in {
        "tabs_or_stack",
        "stack",
    } or adaptive.get("ipad_navigation") not in {"split_or_sidebar", "not_supported"}:
        raise RevenueForgeError(
            "APPFORGE_NATIVE_SURFACE_CONTRACT_INVALID",
            "adaptive navigation values are unsupported",
        )
    if (
        adaptive.get("independent_destination_paths") is not True
        or adaptive.get("hardcoded_screen_geometry_allowed") is not False
    ):
        raise RevenueForgeError(
            "APPFORGE_NATIVE_SURFACE_CONTRACT_INVALID",
            "adaptive navigation must retain destination paths and reject hardcoded screen geometry",
        )
...
def _accessibility_contract(accessibility: object) -> dict[str, Any]:
    ...
    if (
        ...
        or set(accessibility) != expected_accessibility
        or any(accessibility.get(item) is not True for item in expected_accessibility)
    ):
        raise RevenueForgeError(
            "APPFORGE_NATIVE_SURFACE_CONTRACT_INVALID",
...
def _materials_contract(materials: object) -> dict[str, Any]:
    if not isinstance(materials, dict) or set(materials) != {
        ...
        "max_custom_glass_controls",
    }:
        raise RevenueForgeError(
            "APPFORGE_NATIVE_SURFACE_CONTRACT_INVALID",
            "materials must have the fixed usage fields",
        )
    if (
        materials.get("system_components_preferred") is not True
        or materials.get("content_layer_glass_allowed") is not False
        or not isinstance(materials.get("max_custom_glass_controls"), int)
        or not 0 <= materials["max_custom_glass_controls"] <= 3
    ):
        raise RevenueForgeError(
            "APPFORGE_NATIVE_SURFACE_CONTRACT_INVALID",
...
def _storyboard_contract(
    ...
    if not isinstance(storyboard, list) or not storyboard or len(storyboard) > 20:
        raise RevenueForgeError(
            "APPFORGE_NATIVE_SURFACE_CONTRACT_INVALID",
            "storyboard must contain 1-20 platform-bound scenes",
        )
    seen: set[str] = set()
    ...
    for scene in storyboard:
        if not isinstance(scene, dict) or set(scene) != {
            ...
            raise RevenueForgeError(
                "APPFORGE_NATIVE_SURFACE_CONTRACT_INVALID",
                "each storyboard scene must use the fixed fields",
            )
        ...
        if identifier in seen or platform not in platforms:
            raise RevenueForgeError(
                "APPFORGE_NATIVE_SURFACE_CONTRACT_INVALID",
                "storyboard ids must be unique and bind declared platforms",
            )
        seen.add(identifier)
        represented.add(platform)
...
def _contract(
    ...
    return {
        ...
        "source_files": [item.relative_to(root).as_posix() for item in sources],
        "adaptive": adaptive,
...
def _evidence(
    ...
    if (
        ...
        or _digest(value.get("contract_sha256"), "evidence.contract_sha256")
        != contract_sha
    ):
        raise RevenueForgeError(
            "APPFORGE_NATIVE_SURFACE_EVIDENCE_INVALID",
            "evidence must match the candidate and exact native-surface contract",
        )
    ...
    if not isinstance(review, dict) or set(review) != {
        ...
        raise RevenueForgeError(
            "APPFORGE_NATIVE_SURFACE_EVIDENCE_INVALID",
            "review must contain the fixed named confirmations",
        )
    if any(
        ...
        raise RevenueForgeError(
            "APPFORGE_NATIVE_SURFACE_REVIEW_REQUIRED",
            "all named native-surface review confirmations are required",
        )
    return {
        "reviewed_by": _text(
            review.get("reviewed_by"), "review.reviewed_by", limit=120
        ),
        "confirmed_at": _timestamp(review.get("confirmed_at"), "review.confirmed_at"),
    }
...
def _accessibility_findings(
    ...
    for name, observed in signals.items():
        if contract["accessibility"][name] and not observed:
            findings.append(
                {
                    "code": "APPFORGE_NATIVE_ACCESSIBILITY_SIGNAL_MISSING",
...
def verify_native_surface(
    ...
    contract_raw, contract_source = _read_json(
        workspace, contract_path, CONTRACT_SCHEMA
    )
    ...
    result: dict[str, Any] = {
        ...
        "marker": "APPFORGE_NATIVE_SURFACE_READY"
        if not findings
        else "APPFORGE_NATIVE_SURFACE_BLOCKED",
        "ok": not findings,
    ...
    if destination.exists():
        raise RevenueForgeError(
            "APPFORGE_NATIVE_SURFACE_OUTPUT_COLLISION",
            "receipt output path already exists",
        )
...
def native_surface_projection(root: Path) -> dict[str, Any]:
    ...
    invalid: list[str] = []
    for path in sorted(
        (workspace / ".factory" / "appforge").rglob("*native-surface*.json")
    )[:100]:
        try:
        ...
        if _sealed(value):
            ...
            invalid.append(path.relative_to(workspace).as_posix())
    return {
        "schema": "factory.appforge.native-surface-projection.v1",
        "marker": "APPFORGE_NATIVE_SURFACE_READ_ONLY",
        ...
        "claim_boundary": "Read-only local static preflight status; not a device, accessibility, screenshot, App Store Connect, TestFlight, App Review, or Apple approval result.",
    }
