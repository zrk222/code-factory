...
def _relative(root: Path, path: Path) -> str:
    try:
        ...
        raise ResilienceError(
            "RESILIENCE_PATH_INVALID", "lineage path must stay inside the workspace"
        ) from exc
...
def _lineage(root: Path, path: Path) -> tuple[dict[str, Any], str]:
    ...
    if not verification.get("valid"):
        raise ResilienceError(
            "RESILIENCE_LINEAGE_INVALID",
            "lineage must pass hash and bounds verification",
        )
    try:
        payload = json.loads(candidate.read_text(encoding="utf-8"))
    except (OSError, UnicodeDecodeError, json.JSONDecodeError) as exc:
        raise ResilienceError(
            "RESILIENCE_LINEAGE_UNREADABLE", f"lineage cannot be read: {exc}"
        ) from exc
    ...
    *,
    ...
    return {
        ...
        "expected_marker": {
            ...
            "parallel_write": "PARALLEL_WRITE_CONFLICT",
            "duplicate_effect": "DUPLICATE_SIDE_EFFECT",
            "retry_replay": "UNSAFE_RETRY",
            "checkpoint_replay": "UNSAFE_CHECKPOINT_REPLAY",
        }[kind],
    }
...
def _schedules(lineage: dict[str, Any]) -> list[dict[str, Any]]:
    ...
    for step in lineage["steps"]:
        ...
        for effect in step.get("side_effects", []):
            ...
            plans.append(
                _schedule(
                    f"duplicate-{sequence}-{effect_id}",
                    "duplicate_effect",
                    node_id=node_id,
                    sequence=sequence,
                    effect_id=effect_id,
        ...
        for item in step.get("writes", []):
            writes.setdefault((int(step["superstep"]), str(item["key"])), []).append(
                {"node_id": node_id, "sequence": sequence, "item": item}
            )
            plans.append(
                _schedule(
                    f"checkpoint-{sequence}-{item['key']}",
                    "checkpoint_replay",
                    node_id=node_id,
                    sequence=sequence,
                    state_key=item["key"],
    ...
    for (_, key), members in writes.items():
        if len(members) > 1:
            for member in members:
                plans.append(
                    _schedule(
                        f"parallel-{member['sequence']}-{key}",
                        "parallel_write",
    ...
    ordered = sorted(
        {item["id"]: item for item in plans}.values(), key=lambda item: item["id"]
    )
...
def compile_temporal_resilience_plan(root: Path, lineage_path: Path) -> dict[str, Any]:
    ...
    core = {
        "schema": RESILIENCE_PLAN_SCHEMA,
        "marker": "TEMPORAL_RESILIENCE_PLAN_COMPILED",
        "source": {
            "path": relative,
        ...
        "facts": {
            ...
            "kinds": sorted({item["kind"] for item in schedules}),
        },
        "authority": {
            "execution": False,
        ...
        "scope_limits": [
            "Schedules are fault hypotheses derived from declared lineage only.",
            "The planner cannot invoke a graph, replay a checkpoint, or establish production resilience.",
        ],
    }
...
def _read_resilience_plan(root: Path, plan_path: Path) -> dict[str, Any]:
    ...
    try:
        ...
        raise ResilienceError(
            "RESILIENCE_PLAN_UNREADABLE", f"plan cannot be read: {exc}"
        ) from exc
    if not isinstance(plan, dict) or plan.get("schema") != RESILIENCE_PLAN_SCHEMA:
        raise ResilienceError(
            "RESILIENCE_PLAN_INVALID", f"plan must use {RESILIENCE_PLAN_SCHEMA}"
        )
    return plan
...
def _resilience_comparison(
    plan: dict[str, Any], expected: dict[str, Any], supplied: str
) -> dict[str, Any]:
    if expected["source"] != plan["source"]:
        return {
            "schema": RESILIENCE_PLAN_SCHEMA,
            "marker": "TEMPORAL_RESILIENCE_SOURCE_STALE",
            "ok": False,
        }
    if expected["schedules"] != plan.get("schedules"):
        count = (
            len(plan.get("schedules", []))
            if isinstance(plan.get("schedules"), list)
            else None
        )
        return {
            "schema": RESILIENCE_PLAN_SCHEMA,
            "marker": "TEMPORAL_RESILIENCE_PLAN_INCOMPLETE",
            "ok": False,
            "expected_schedule_count": len(expected["schedules"]),
            "actual_schedule_count": count,
        }
    return {
        "schema": RESILIENCE_PLAN_SCHEMA,
        "marker": "TEMPORAL_RESILIENCE_PLAN_VERIFIED",
        "ok": True,
        "schedule_count": len(expected["schedules"]),
...
def verify_temporal_resilience_plan(root: Path, plan_path: Path) -> dict[str, Any]:
    ...
    if supplied is None:
        return {
            "schema": RESILIENCE_PLAN_SCHEMA,
            "marker": "TEMPORAL_RESILIENCE_PLAN_TAMPERED",
            "ok": False,
        }
