...
def _workspace(root: Path | str) -> Path:
    ...
    if not workspace.is_dir():
        raise McpSetupError(
            "workspace root must be an existing directory", "MCP_ROOT_REJECTED"
        )
    return workspace
...
def _json_config(root: Path) -> dict[str, object]:
    return {
        "mcpServers": {
            "code-factory": {"command": "factory", "args": _command(root)[1:]}
        }
    }
...
def mcp_connection_config(
    root: Path | str, client: str = "generic"
) -> dict[str, object]:
    """Return one copy-only MCP configuration without touching a client or workspace."""
    workspace = _workspace(root)
    normalized = (
        client.strip().lower().replace("-", "_") if isinstance(client, str) else ""
    )
    if normalized not in _CLIENTS:
        raise McpSetupError(
            f"client must be one of: {', '.join(_CLIENTS)}", "MCP_CLIENT_REJECTED"
        )
    ...
    if normalized in {"generic", "cursor", "junie"}:
        ...
        payload["target"] = (
            ...
            else ".cursor/mcp.json"
            if normalized == "cursor"
            else ".junie/mcp/mcp.json"
        )
...
def install_project_mcp_config(
    ...
    if target.exists():
        try:
            ...
            raise McpSetupError(
                "existing Junie MCP config is not valid UTF-8 JSON",
                "MCP_SETUP_CONFLICT",
            ) from exc
        if (
            ...
            raise McpSetupError(
                "existing Junie MCP config has an unsupported shape; copy the rendered entry manually",
                "MCP_SETUP_CONFLICT",
            )
        current = loaded
