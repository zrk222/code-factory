...
def _canonical(value: object) -> bytes:
    try:
        ...
        raise AtomicProofAdapterError(
            "E_ATOMIC_ENVELOPE_SCHEMA", "input must be canonical JSON"
        ) from exc
...
def _identifier(value: object, field: str) -> str:
    if not isinstance(value, str) or not _IDENTIFIER.fullmatch(value):
        raise AtomicProofAdapterError(
            "E_ATOMIC_ENVELOPE_SCHEMA",
            f"{field} must use 1-96 safe identifier characters",
        )
    return value
...
def _inside(root: Path, value: object, field: str, *, exists: bool) -> Path:
    ...
    if supplied.is_absolute() or ".." in supplied.parts:
        raise AtomicProofAdapterError(
            "E_ATOMIC_SCOPE_ESCAPE", f"{field} must remain beneath the workspace"
        )
...
def _path(value: object, field: str) -> str:
    ...
    if path.is_absolute() or ".." in path.parts:
        raise AtomicProofAdapterError(
            ...
            f"{field} must be workspace-relative without parent traversal",
        )
...
def _exact_keys(
    ...
    if not isinstance(value, dict):
        raise AtomicProofAdapterError(
            "E_ATOMIC_ENVELOPE_SCHEMA", f"{field} must be an object"
        )
    missing = (required or allowed) - set(value)
    unknown = set(value) - allowed
    if missing or unknown:
        raise AtomicProofAdapterError(
            "E_ATOMIC_PRIVATE_FIELD" if unknown else "E_ATOMIC_ENVELOPE_SCHEMA",
            f"{field} has unsupported or missing fields",
        )
    return value
...
def _paths(value: object, field: str) -> list[str]:
    if not isinstance(value, list) or not 1 <= len(value) <= 64:
        raise AtomicProofAdapterError(
            "E_ATOMIC_ENVELOPE_SCHEMA", f"{field} must contain 1-64 paths"
        )
    result = sorted({_path(item, field) for item in value})
    if len(result) != len(value):
        raise AtomicProofAdapterError(
            "E_ATOMIC_ENVELOPE_SCHEMA", f"{field} must be unique"
        )
    return result
...
def _source_preconditions(
    root: Path, value: object, field: str, scope: list[str]
) -> list[dict[str, str]]:
    if not isinstance(value, list) or not 1 <= len(value) <= 64:
        raise AtomicProofAdapterError(
            "E_ATOMIC_EVIDENCE_UNVERIFIED",
            f"{field} must contain 1-64 source preconditions",
        )
    ...
    for index, item in enumerate(value):
        ...
        if not _scope_allows(scope, path):
            raise AtomicProofAdapterError(
                "E_ATOMIC_SCOPE_ESCAPE",
                f"{field}[{index}].path is outside the sealed Oracle scope",
            )
        ...
        if hashlib.sha256(source.read_bytes()).hexdigest() != declared_sha:
            raise AtomicProofAdapterError(
                "E_ATOMIC_EVIDENCE_UNVERIFIED",
                f"{field}[{index}] no longer matches the workspace source bytes",
            )
        result.append({"path": path, "sha256": declared_sha})
    if len({item["path"] for item in result}) != len(result):
        raise AtomicProofAdapterError(
            "E_ATOMIC_EVIDENCE_UNVERIFIED", f"{field} paths must be unique"
        )
...
def _topology(value: object) -> tuple[dict[str, Any], set[tuple[str, str]]]:
    ...
    if not isinstance(nodes, list) or not 1 <= len(nodes) <= MAX_ITEMS:
        raise AtomicProofAdapterError(
            "E_ATOMIC_EVIDENCE_UNVERIFIED",
            "workflow.nodes must contain 1-128 typed stages",
        )
    ...
    for index, item in enumerate(nodes):
        ...
        if kind not in _STAGE_KINDS:
            raise AtomicProofAdapterError(
                "E_ATOMIC_EVIDENCE_UNVERIFIED",
                f"workflow.nodes[{index}].kind is unsupported",
            )
        normalized_nodes.append(
            {
                "id": _identifier(entry["id"], f"workflow.nodes[{index}].id"),
                "kind": kind,
            }
        )
    ...
    for index, item in enumerate(edges):
        ...
        if source not in node_ids or target not in node_ids or source == target:
            raise AtomicProofAdapterError(
                "E_ATOMIC_EVIDENCE_UNVERIFIED",
                "workflow edge must join distinct declared stages",
            )
    ...
    normalized = {
        ...
        "definition_sha256": _hash(
            workflow["definition_sha256"], "workflow.definition_sha256"
        ),
...
def _stages(
    root: Path, value: object, workflow: dict[str, Any], contract_scope: list[str]
    ...
    if stage_ids != set(node_kinds) or len(stage_ids) != len(normalized):
        raise AtomicProofAdapterError(
            "E_ATOMIC_STAGE_IDENTITY_UNPROVEN",
            "stages must contain every declared workflow node exactly once",
        )
    checkpoints = [item["checkpoint"]["id"] for item in normalized]
    if len(set(checkpoints)) != len(checkpoints):
        raise AtomicProofAdapterError(
            "E_ATOMIC_EVIDENCE_UNVERIFIED",
            "checkpoint identities must be unique within one run",
        )
...
def _receipt_valid(value: object) -> bool:
    ...
    return (
        ...
        and _sha({key: item for key, item in value.items() if key != "receipt_sha256"})
        == digest
    )
...
def _read_receipt(root: Path, path: Path) -> tuple[dict[str, Any], Path]:
    ...
    if target.stat().st_size > MAX_BYTES:
        raise AtomicProofAdapterError(
            "E_ATOMIC_EVIDENCE_UNVERIFIED", "receipt exceeds 1 MiB"
        )
    try:
        value = json.loads(target.read_text(encoding="utf-8-sig"))
    except (OSError, UnicodeDecodeError, json.JSONDecodeError) as exc:
        raise AtomicProofAdapterError(
            "E_ATOMIC_EVIDENCE_UNVERIFIED", "receipt must be valid UTF-8 JSON"
        ) from exc
    if not _receipt_valid(value):
        raise AtomicProofAdapterError(
            "E_ATOMIC_EVIDENCE_UNVERIFIED", "receipt hash is invalid"
        )
    return value, target
...
def _resume(
    ...
    if receipt.get("run", {}).get("id") != _identifier(
        ...
        raise AtomicProofAdapterError(
            ...
            "resume prior_run_id does not match the bound receipt",
        )
    if receipt["run"]["id"] == run_id:
        raise AtomicProofAdapterError(
            "E_ATOMIC_RESUME_DIVERGENCE", "resume must use a new run identifier"
        )
    if receipt.get("oracle", {}).get("contract_sha256") != contract_sha256:
        raise AtomicProofAdapterError(
            "E_ATOMIC_RESUME_DIVERGENCE", "resume Oracle Contract digest diverges"
        )
    prior_workflow = receipt.get("workflow", {})
    if not isinstance(prior_workflow, dict) or any(
        prior_workflow.get(field) != workflow.get(field)
        for field in ("id", "definition_sha256", "topology_sha256")
    ):
        raise AtomicProofAdapterError(
    ...
    prior_stage = next(
        (
            ...
            and item.get("checkpoint", {}).get("id") == checkpoint_id
        ),
        None,
    )
    current_stage = next(
        (item for item in stages.values() if item["checkpoint"]["id"] == checkpoint_id),
        None,
    )
    ...
    if (
        ...
        raise AtomicProofAdapterError(
            "E_ATOMIC_RESUME_DIVERGENCE", "resume checkpoint hash diverges"
        )
    if prior_stage.get("id") != current_stage["id"]:
        raise AtomicProofAdapterError(
            "E_ATOMIC_RESUME_DIVERGENCE", "resume checkpoint stage identity diverges"
        )
    if (
        prior_stage.get("tool_manifest_sha256") != current_stage["tool_manifest_sha256"]
        or prior_stage.get("source_preconditions")
        != current_stage["source_preconditions"]
    ):
        raise AtomicProofAdapterError(
            "E_ATOMIC_RESUME_DIVERGENCE",
...
def _handoff_history(
    root: Path, workflow_id: str, handoffs: list[dict[str, Any]]
) -> None:
...
def _atomic_json(path: Path, payload: dict[str, Any]) -> None:
    ...
    try:
        ...
        raise AtomicProofAdapterError(
            "E_ATOMIC_EVIDENCE_UNVERIFIED",
            "output receipt already exists; choose a new immutable path",
        ) from exc
    try:
...
def import_atomic_run(
    ...
    if source.stat().st_size > MAX_BYTES:
        raise AtomicProofAdapterError(
            "E_ATOMIC_PRIVATE_FIELD", "envelope exceeds 1 MiB"
        )
    try:
        ...
        raise AtomicProofAdapterError(
            "E_ATOMIC_ENVELOPE_SCHEMA", "envelope must be valid canonical UTF-8 JSON"
        ) from exc
    allowed = {
        "schema",
        "envelope_id",
    ...
    if entry.get("status") not in _STAGE_STATUS:
        raise AtomicProofAdapterError(
            "E_ATOMIC_ENVELOPE_SCHEMA", "status is unsupported"
        )
    autonomy = entry.get("autonomy")
    isolation = entry.get("isolation")
    if autonomy not in _AUTONOMY or isolation not in _ISOLATION:
        raise AtomicProofAdapterError(
            "E_ATOMIC_ENVELOPE_SCHEMA", "autonomy or isolation is unsupported"
        )
    if autonomy == "autonomous" and isolation == "unverified":
        raise AtomicProofAdapterError(
            "E_ATOMIC_ISOLATION_UNPROVEN",
            "autonomous Atomic evidence requires a declared isolated host boundary",
        )
    try:
    ...
    if (
        ...
        or checked_contract.get("contract", {}).get("contract_sha256")
        != contract_sha256
    ):
        raise AtomicProofAdapterError(
            "E_ATOMIC_UNBOUND_INTENT",
    ...
    all_paths = sorted(
        {
            ...
            for path in [
                *stage["scope_paths"],
                *(item["path"] for item in stage["source_preconditions"]),
            ]
        }
    )
    try:
        admission = admission_oracle_decision(
            workspace, Path(contract_path), autonomy, all_paths
        )
    except OracleFirewallError as exc:
        code = (
            "E_ATOMIC_SCOPE_ESCAPE"
            if exc.code == "ORACLE_SCOPE_ESCAPE"
            else "E_ATOMIC_UNBOUND_INTENT"
        )
    ...
    core = {
        ...
        "isolation": {
            ...
            "verified": False,
            "claim_boundary": "A declared host boundary is not proof of a sandbox, container, worktree, VM, or remote policy.",
        },
        "oracle": {
    ...
    destination = (
        ...
        / "atomic"
        / f"{run_id}-{receipt['receipt_sha256'][:12]}.json"
    )
...
def verify_atomic_receipt(root: Path, receipt_path: Path) -> dict[str, Any]:
    ...
    try:
        ...
        if not isinstance(oracle, dict) or not isinstance(oracle.get("path"), str):
            return {
                "ok": False,
                "marker": "ATOMIC_RECEIPT_INVALID",
                "reason": "oracle_missing",
                "authority": dict(AUTHORITY),
            }
        contract = verify_oracle_contract(workspace, Path(oracle["path"]))
        if not contract.get("ok") or contract.get("contract", {}).get(
            "contract_sha256"
            ...
            "stages": [
                {
                    ...
                    "checkpoint": {
                        "id": "replace-with-checkpoint-id",
                        "sha256": digest,
                    },
                    "source_preconditions": [
                        {"path": "replace-with-existing-source-path", "sha256": digest}
                    ],
                },
                {
                    "id": "build",
                    ...
                    "tool_manifest_sha256": digest,
                    "checkpoint": {
                        "id": "replace-with-build-checkpoint-id",
                        "sha256": digest,
                    },
                    "source_preconditions": [
                        {"path": "replace-with-existing-source-path", "sha256": digest}
                ...
                },
                {
                    ...
                    "tool_manifest_sha256": digest,
                    "checkpoint": {
                        "id": "replace-with-verify-checkpoint-id",
                        "sha256": digest,
                    },
                    "source_preconditions": [
                        {"path": "replace-with-existing-source-path", "sha256": digest}
