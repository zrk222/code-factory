...
def _text(value: object, field: str, *, limit: int = 160) -> str:
    if not isinstance(value, str) or not value.strip() or len(value.strip()) > limit:
        raise EnterpriseRunnerAdmissionError(
            "E_RUNNER_ADMISSION_INPUT",
...
def _paths(value: object, field: str) -> list[str]:
    if not isinstance(value, list) or not value or len(value) > 64:
        raise EnterpriseRunnerAdmissionError(
            "E_RUNNER_ADMISSION_INPUT",
            f"{field} must contain 1 through 64 relative paths",
        )
    result: list[str] = []
    for raw in value:
        ...
        if path.is_absolute() or ".." in path.parts:
            raise EnterpriseRunnerAdmissionError(
                "E_RUNNER_ADMISSION_INPUT", f"{field} cannot escape a workspace"
            )
...
def _load(root: Path, path: Path, schema: str) -> tuple[dict[str, Any], Path]:
    ...
    try:
        ...
        raise EnterpriseRunnerAdmissionError(
            "E_RUNNER_ADMISSION_PATH", "input must remain in the workspace"
        ) from exc
    try:
        value = json.loads(candidate.read_text(encoding="utf-8"))
    except (OSError, json.JSONDecodeError) as exc:
        raise EnterpriseRunnerAdmissionError(
            "E_RUNNER_ADMISSION_INPUT", str(exc)
        ) from exc
    if not isinstance(value, dict) or value.get("schema") != schema:
        raise EnterpriseRunnerAdmissionError(
            "E_RUNNER_ADMISSION_INPUT", f"schema must be {schema}"
        )
...
def _argv(value: object) -> list[str]:
    if (
        ...
        raise EnterpriseRunnerAdmissionError(
            ...
            "argv must contain 1 through 128 non-empty strings",
        )
    argv = [item.strip() for item in value]
    if any(item in FORBIDDEN_ARGV_TOKENS for item in argv):
        raise EnterpriseRunnerAdmissionError(
            "E_RUNNER_COMMAND_INVALID", "shell operators are forbidden; use argv only"
        )
    return argv
...
def _stamp(value: object, field: str) -> str:
    ...
    if not isinstance(value, str) or not value.strip():
        raise EnterpriseRunnerAdmissionError(
            "E_RUNNER_FRESHNESS_MISSING", f"{field} must be present"
        )
    try:
        parsed = datetime.fromisoformat(value.strip().replace("Z", "+00:00"))
    except ValueError as exc:
        raise EnterpriseRunnerAdmissionError(
            "E_RUNNER_FRESHNESS_INVALID", f"{field} must be an RFC3339 timestamp"
        ) from exc
    if parsed.tzinfo is None:
        raise EnterpriseRunnerAdmissionError(
            "E_RUNNER_FRESHNESS_INVALID", f"{field} must include an offset"
        )
...
def _admission_expiry(decision: dict[str, Any], *, now: datetime | None = None) -> str:
    ...
    identity = (
        decision.get("workload_identity")
        if isinstance(decision.get("workload_identity"), dict)
        else {}
    )
    expires_at = _stamp(
        identity.get("expires_at"), "decision.workload_identity.expires_at"
    )
    if datetime.fromisoformat(expires_at.replace("Z", "+00:00")) <= (now or _now()):
        raise EnterpriseRunnerAdmissionError(
            "E_RUNNER_ADMISSION_EXPIRED",
            "the bound workload identity has expired; record a new decision",
        )
    return expires_at


def prepare_runner_admission(
    root: Path, input_path: Path, out: Path, *, now: datetime | None = None
) -> dict[str, Any]:
    ...
    if missing:
        raise EnterpriseRunnerAdmissionError(
            "E_RUNNER_ADMISSION_INPUT", f"missing fields: {missing}"
        )
    ...
    if action_class != request.get("action_class"):
        raise EnterpriseRunnerAdmissionError(
            "E_RUNNER_ACTION_MISMATCH",
            "runner action does not match the admitted decision",
        )
    if scope_paths != request.get("scope_paths"):
        raise EnterpriseRunnerAdmissionError(
            "E_RUNNER_SCOPE_MISMATCH",
            "runner scope does not match the admitted decision",
        )
    target = (
        (workspace / out).resolve()
        if not Path(out).is_absolute()
        else Path(out).resolve()
    )
    try:
        target.relative_to(workspace)
    except ValueError as exc:
        raise EnterpriseRunnerAdmissionError(
            "E_RUNNER_ADMISSION_PATH", "output must remain in the workspace"
        ) from exc
    if not target.is_relative_to((workspace / PACKET_DIR).resolve()):
        raise EnterpriseRunnerAdmissionError(
            "E_RUNNER_ADMISSION_PATH", f"output must stay under {PACKET_DIR.as_posix()}"
        )
    if target.exists():
        raise EnterpriseRunnerAdmissionError(
            "E_RUNNER_ADMISSION_IMMUTABLE", "refusing to overwrite a runner packet"
        )
    packet = {
        "schema": PACKET_SCHEMA,
        "marker": "RUNNER_ADMISSION_PACKET_SEALED",
        "packet_id": _text(value["id"], "id"),
...
def _packet_path(workspace: Path, path: Path) -> Path:
    ...
    try:
        ...
        raise EnterpriseRunnerAdmissionError(
            "E_RUNNER_ADMISSION_PATH", f"packet must stay under {PACKET_DIR.as_posix()}"
        ) from exc
    return candidate
...
def _packet_value(candidate: Path) -> tuple[dict[str, Any], str]:
    ...
    if (
        ...
        or value.get("marker") != "RUNNER_ADMISSION_PACKET_SEALED"
    ):
        raise EnterpriseRunnerAdmissionError(
            "E_RUNNER_PACKET_INVALID", "packet schema or marker is invalid"
        )
    ...
    if (
        ...
        or hashlib.sha256(canonical_json(unsigned)).hexdigest() != supplied
    ):
        raise EnterpriseRunnerAdmissionError(
            "E_RUNNER_PACKET_HASH_INVALID", "packet hash is invalid"
        )
    if not isinstance(value.get("authority"), dict) or any(
        item is not False for item in value["authority"].values()
    ):
        raise EnterpriseRunnerAdmissionError(
            "E_RUNNER_PACKET_AUTHORITY", "packet must grant zero authority"
        )
    return value, supplied


def _packet_binding(
    workspace: Path, value: dict[str, Any], *, now: datetime | None = None
) -> tuple[str, str]:
    ...
    if decision_ref.get("decision_sha256") != decision["decision_sha256"]:
        raise EnterpriseRunnerAdmissionError(
            "E_RUNNER_DECISION_MISMATCH",
            "packet decision digest does not match its verified receipt",
        )
    request = (
        decision["decision"].get("request")
        if isinstance(decision["decision"].get("request"), dict)
        else {}
    )
    ...
    if value.get("argv_sha256") != hashlib.sha256(canonical_json(argv)).hexdigest():
        raise EnterpriseRunnerAdmissionError(
            "E_RUNNER_COMMAND_HASH_INVALID", "argv hash is invalid"
        )
    if action_class != request.get("action_class"):
        raise EnterpriseRunnerAdmissionError(
            "E_RUNNER_ACTION_MISMATCH", "packet action does not match its decision"
        )
    if scope_paths != request.get("scope_paths"):
        raise EnterpriseRunnerAdmissionError(
            "E_RUNNER_SCOPE_MISMATCH", "packet scope does not match its decision"
        )
    admission_expires_at = _admission_expiry(decision["decision"], now=now)
    if (
        _stamp(value.get("admission_expires_at"), "admission_expires_at")
        != admission_expires_at
    ):
        raise EnterpriseRunnerAdmissionError(
            "E_RUNNER_FRESHNESS_MISMATCH",
            "packet expiry does not match its bound signed identity evidence",
        )
    return decision["decision_sha256"], admission_expires_at


def verify_runner_admission_packet(
    root: Path, path: Path, *, now: datetime | None = None
) -> dict[str, Any]:
...
def runner_admission_projection(root: Path) -> dict[str, Any]:
    ...
    result: dict[str, Any] = {
        ...
        "marker": "RUNNER_ADMISSION_READ_ONLY",
        "packet_count": 0,
        "verified_count": 0,
        "fresh_count": 0,
        "expired_count": 0,
        ...
        "claim_boundary": "Read-only local runner-packet projection. Freshness is derived from sealed identity expiry; it does not execute argv, authenticate a workload, re-check live revocations, prove runner topology, enforce isolation, or grant authority.",
    }
            ...
            result["packets"].append(
                {
                    ...
                    "scope_count": len(packet.get("scope_paths", [])),
                    "argv_sha256": packet.get("argv_sha256"),
                    "admission_expires_at": checked["admission_expires_at"],
                }
            )
