...
def _relative(root: Path, path: Path) -> str:
    try:
        ...
        raise CounterexampleError(
            "COUNTEREXAMPLE_PATH_INVALID", "path must stay inside the workspace"
        ) from exc
...
def _load(root: Path, path: Path) -> tuple[dict[str, Any], str, str]:
    ...
    if not candidate.is_file():
        raise CounterexampleError(
            "COUNTEREXAMPLE_SOURCE_UNREADABLE",
            "source must name a readable workspace JSON file",
        )
    ...
    try:
        ...
        raise CounterexampleError(
            "COUNTEREXAMPLE_SOURCE_INVALID", f"source is not UTF-8 JSON: {exc}"
        ) from exc
    if not isinstance(source, dict):
        raise CounterexampleError(
            "COUNTEREXAMPLE_SOURCE_INVALID", "source must contain one JSON object"
        )
    return source, relative, sha256(raw).hexdigest()


def _text(
    value: object, field: str, *, limit: int = 480, identifier: bool = False
) -> str:
    if not isinstance(value, str) or not value.strip() or len(value.strip()) > limit:
        raise CounterexampleError(
            "COUNTEREXAMPLE_SOURCE_INVALID",
            f"{field} must be a non-empty string of at most {limit} characters",
        )
    result = value.strip()
    if identifier and not _IDENTIFIER.fullmatch(result):
        raise CounterexampleError(
            "COUNTEREXAMPLE_SOURCE_INVALID", f"{field} has an unsupported identifier"
        )
    return result
...
def _clear_requirement(value: str, field: str) -> str:
    ...
    try:
        ...
        raise CounterexampleError(
            "COUNTEREXAMPLE_INTENT_UNCLEAR", f"{field}: {exc.message}"
        ) from exc
    return value
...
def _validated_requirement(item: object, index: int, seen: set[str]) -> dict[str, Any]:
    if not isinstance(item, dict) or set(item) != {"id", "statement", "risk_tags"}:
        raise CounterexampleError(
            "COUNTEREXAMPLE_SOURCE_INVALID",
            f"requirements[{index}] must contain exactly id, statement, and risk_tags",
        )
    requirement_id = _text(
        item.get("id"), f"requirements[{index}].id", limit=96, identifier=True
    )
    if requirement_id in seen:
        raise CounterexampleError(
            "COUNTEREXAMPLE_SOURCE_INVALID", "requirement ids must be unique"
        )
    ...
    if not isinstance(tags, list) or not 1 <= len(tags) <= 6:
        raise CounterexampleError(
            "COUNTEREXAMPLE_SOURCE_INVALID",
            f"requirements[{index}].risk_tags must contain 1 through 6 supported tags",
        )
    if any(not isinstance(tag, str) or tag not in _TAGS for tag in tags) or len(
        set(tags)
    ) != len(tags):
        raise CounterexampleError(
            "COUNTEREXAMPLE_SOURCE_INVALID",
            f"requirements[{index}].risk_tags must be unique supported tags",
        )
    statement = _text(item.get("statement"), f"requirements[{index}].statement")
    return {
        "id": requirement_id,
        "statement": _clear_requirement(statement, f"requirements[{index}].statement"),
...
def validate_counterexample_source(value: object) -> dict[str, Any]:
    ...
    if not isinstance(value, dict) or set(value) != {"schema", "id", "requirements"}:
        raise CounterexampleError(
            "COUNTEREXAMPLE_SOURCE_INVALID",
            "source must contain exactly schema, id, and requirements",
        )
    if value.get("schema") != COUNTEREXAMPLE_SOURCE_SCHEMA:
        raise CounterexampleError(
            "COUNTEREXAMPLE_SOURCE_INVALID",
            f"schema must be {COUNTEREXAMPLE_SOURCE_SCHEMA}",
        )
    requirements = value.get("requirements")
    if not isinstance(requirements, list) or not 1 <= len(requirements) <= 128:
        raise CounterexampleError(
            "COUNTEREXAMPLE_SOURCE_INVALID",
            "requirements must contain 1 through 128 entries",
        )
    ...
    return {
        "schema": COUNTEREXAMPLE_SOURCE_SCHEMA,
        "id": _text(value.get("id"), "id", limit=96, identifier=True),
        "requirements": sorted(normalized, key=lambda item: item["id"]),
    }
...
def _cases(source: dict[str, Any]) -> list[dict[str, str]]:
    return [
        {
            "id": f"{requirement['id']}--{tag}",
            "requirement_id": requirement["id"],
            "risk_tag": tag,
            "mutation": _MUTATIONS[tag],
            "expected": "negative_proof_must_reject",
        }
        for requirement in source["requirements"]
        for tag in requirement["risk_tags"]
    ]
...
def compile_counterexample_plan(root: Path, source_path: Path) -> dict[str, Any]:
    ...
    core = {
        ...
        "facts": {
            ...
            "risk_tags": sorted({case["risk_tag"] for case in cases}),
        },
        "authority": {
            "execution": False,
            "source_write": False,
        ...
        "scope_limits": [
            "A case is a declared negative proof obligation, not an executed test.",
            "The planner never invents commands, edits source, or authorizes a repair.",
        ],
    }
    return {**core, "plan_sha256": _sha(core)}


def _counterexample_plan_core(
    plan: dict[str, Any], relative: str
    ...
    if not isinstance(supplied_hash, str) or supplied_hash != _sha(core):
        return {
            "schema": COUNTEREXAMPLE_PLAN_SCHEMA,
            "marker": "COUNTEREXAMPLE_PLAN_TAMPERED",
            "ok": False,
            "plan_path": relative,
        }
...
def _counterexample_source(plan: dict[str, Any]) -> dict[str, Any]:
    ...
    if not isinstance(source, dict) or set(source) != {"path", "sha256", "id"}:
        raise CounterexampleError(
            "COUNTEREXAMPLE_PLAN_INVALID",
            "plan source must contain path, sha256, and id",
        )
    return source


def _counterexample_comparison(
    plan: dict[str, Any],
    source: dict[str, Any],
    current_sha: str,
    ...
    if source.get("sha256") != current_sha or source.get("id") != normalized["id"]:
        return {
            "schema": COUNTEREXAMPLE_PLAN_SCHEMA,
            "marker": "COUNTEREXAMPLE_SOURCE_STALE",
            "ok": False,
            "plan_path": relative,
        }
    ...
    if actual != expected:
        return {
            "schema": COUNTEREXAMPLE_PLAN_SCHEMA,
            "marker": "HOLLOW_COUNTEREXAMPLE",
            "ok": False,
            "plan_path": relative,
            "expected_case_count": len(expected),
            "actual_case_count": len(actual) if isinstance(actual, list) else None,
        }
    return {
        "schema": COUNTEREXAMPLE_PLAN_SCHEMA,
        "marker": "COUNTEREXAMPLE_PLAN_VERIFIED",
        "ok": True,
        "plan_path": relative,
...
def verify_counterexample_plan(root: Path, plan_path: Path) -> dict[str, Any]:
    ...
    if not isinstance(plan, dict) or plan.get("schema") != COUNTEREXAMPLE_PLAN_SCHEMA:
        raise CounterexampleError(
            "COUNTEREXAMPLE_PLAN_INVALID", f"plan must use {COUNTEREXAMPLE_PLAN_SCHEMA}"
        )
    ...
    return _counterexample_comparison(
        plan, source, current_sha, normalized, plan_relative
    )
