...
_ALLOWED = {
    "created": {"isolated", "stopped"},
    "isolated": {"context_ready", "stopped"},
...
def _canonical(value: object) -> bytes:
    try:
        ...
        raise LifecycleLedgerError(
            "E_LIFECYCLE_SCHEMA", "input must be canonical JSON"
        ) from exc
...
def _identifier(value: object, label: str) -> str:
    if not isinstance(value, str) or not _ID.fullmatch(value):
        raise LifecycleLedgerError(
            "E_LIFECYCLE_SCHEMA", f"{label} must be a safe identifier"
        )
    return value
...
def _hash(value: object, label: str) -> str:
    if not isinstance(value, str) or not _SHA256.fullmatch(value):
        raise LifecycleLedgerError(
            "E_LIFECYCLE_SCHEMA", f"{label} must be a lowercase SHA-256"
        )
    return value
...
def _relative(value: object, label: str) -> str:
    if not isinstance(value, str) or not value.strip() or len(value) > 512:
        raise LifecycleLedgerError(
            "E_LIFECYCLE_SCHEMA", f"{label} must be a workspace-relative path"
        )
...
def _inside(root: Path, relative: str, *, required: bool = False) -> Path:
    ...
    try:
        ...
        raise LifecycleLedgerError(
            "E_LIFECYCLE_PATH", "path escapes the workspace"
        ) from exc
    if required and not target.is_file():
        raise LifecycleLedgerError(
            "E_LIFECYCLE_EVIDENCE", f"evidence file is unavailable: {relative}"
        )
    return target
...
def _read(root: Path, relative: str) -> tuple[dict[str, Any], str]:
    ...
    if path.stat().st_size > MAX_BYTES:
        raise LifecycleLedgerError(
            "E_LIFECYCLE_SCHEMA", "event exceeds the 1 MiB limit"
        )
    try:
        value = json.loads(path.read_text(encoding="utf-8"))
    except (OSError, UnicodeDecodeError, json.JSONDecodeError) as exc:
        raise LifecycleLedgerError(
            "E_LIFECYCLE_SCHEMA", "event must be UTF-8 JSON"
        ) from exc
...
def _event(root: Path, source: Path) -> tuple[dict[str, Any], str]:
    ...
    if not isinstance(sequence, int) or isinstance(sequence, bool) or sequence < 1:
        raise LifecycleLedgerError(
            "E_LIFECYCLE_SCHEMA", "sequence must be a positive integer"
        )
    event = value["event"]
    ...
    if (
        ...
        or actor["kind"] not in {"human", "agent", "system"}
    ):
        raise LifecycleLedgerError(
    ...
    if (
        not contract.get("ok")
        or contract.get("contract", {}).get("contract_sha256") != contract_sha
    ):
        raise LifecycleLedgerError(
    ...
    if not isinstance(session_trace, dict) or set(session_trace) != {
        ...
        raise LifecycleLedgerError(
            "E_LIFECYCLE_SCHEMA", "session_trace fields must be exact"
        )
    ...
    normalized_trace = {
        ...
        "input_sha256": _hash(
            session_trace["input_sha256"], "session_trace.input_sha256"
        ),
        "output_sha256": _hash(
    ...
    for index, item in enumerate(evidence):
        if not isinstance(item, dict) or set(item) != {"path", "sha256"}:
            raise LifecycleLedgerError(
                "E_LIFECYCLE_SCHEMA", f"evidence[{index}] fields must be exact"
            )
    ...
    for path in sorted(directory.glob(f"{run_id}-*.json")):
        try:
            ...
            if (
                ...
                and value.get("receipt_sha256") == _sha(core)
            ):
                result.append(value)
...
def record_lifecycle_event(
    ...
    if prior:
        ...
        if (
            ...
            or event["previous_receipt_sha256"] != last["receipt_sha256"]
        ):
            raise LifecycleLedgerError(
                "E_LIFECYCLE_CONTINUITY",
                "sequence and previous receipt must continue the exact local run",
            )
        if event["event"] not in _ALLOWED.get(last["event"], set()):
            raise LifecycleLedgerError(
                "E_LIFECYCLE_TRANSITION", "event transition is not allowed"
            )
    elif (
        event["sequence"] != 1
        or event["event"] != "created"
        or event["previous_receipt_sha256"] is not None
    ):
        raise LifecycleLedgerError(
            "E_LIFECYCLE_CONTINUITY",
    ...
    if not target_relative.startswith(RECEIPT_DIR.as_posix() + "/"):
        raise LifecycleLedgerError(
            "E_LIFECYCLE_PATH", "output must be beneath .factory/lifecycle"
        )
...
def lifecycle_projection(root: Path) -> dict[str, Any]:
    ...
    for run_id, events in sorted(runs.items()):
        ...
        latest = ordered[-1]
        trace = (
            latest.get("session_trace")
            if isinstance(latest.get("session_trace"), dict)
            else {}
        )
        summaries.append(
            {
                ...
                "latest_session_trace": {
                    ...
                    "trace_sha256": latest.get("session_trace_sha256"),
                },
                "requires_human": latest["event"] in {"review_required", "escalated"},
            }
        )
    return {
        "schema": PROJECTION_SCHEMA,
        "marker": "LIFECYCLE_READ_ONLY",
        "run_count": len(summaries),
        "review_required_count": sum(item["requires_human"] for item in summaries),
        "invalid_count": len(invalid),
        "latest": summaries[-1] if summaries else None,
        "runs": summaries[-50:],
        "invalid": invalid[:100],
        "authority": dict(AUTHORITY),
        "claim_boundary": "Read-only local lifecycle facts. The projection does not dispatch a task, broadcast to a provider, start or resume an agent, or grant execution or approval authority.",
    }
...
def lifecycle_template() -> dict[str, Any]:
    ...
    return {
        "schema": "factory.lifecycle-template.v1",
        "event_schema": EVENT_SCHEMA,
        "authority": dict(AUTHORITY),
        "claim_boundary": "Template only. It creates no event, session, provider connection, webhook, or approval.",
        "event": {
            "schema": EVENT_SCHEMA,
            ...
            "sequence": 1,
            "event": "created",
            "actor": {"kind": "agent", "id": "replace-with-declared-agent-id"},
            "oracle": {
            ...
            "session_trace": {
                ...
                "harness": "replace-with-harness",
                "stage": "intake",
                "input_sha256": "replace-with-lowercase-sha256",
                "output_sha256": "replace-with-lowercase-sha256",
            },
            "evidence": [],
