...
def _workspace(root: Path) -> Path:
    ...
    if not workspace.is_dir():
        raise JetBrainsHandshakeError(
            "JETBRAINS_HANDSHAKE_ROOT_INVALID", "root must be an existing directory"
        )
    return workspace
...
def _artifact(workspace: Path, value: Path, label: str) -> Path:
    ...
    try:
        ...
        raise JetBrainsHandshakeError(
            "JETBRAINS_HANDSHAKE_PATH_REJECTED",
            f"{label} must stay inside the workspace",
        ) from exc
    if not candidate.is_file() or candidate.stat().st_size > MAX_ARTIFACT_BYTES:
        raise JetBrainsHandshakeError(
            "JETBRAINS_HANDSHAKE_ARTIFACT_INVALID",
            f"{label} must be a regular file no larger than {MAX_ARTIFACT_BYTES} bytes",
        )
    return candidate


def _load_json(
    workspace: Path, value: Path, label: str
    ...
    try:
        ...
        raise JetBrainsHandshakeError(
            "JETBRAINS_HANDSHAKE_JSON_INVALID", f"{label} must contain UTF-8 JSON"
        ) from exc
    if not isinstance(payload, dict):
        raise JetBrainsHandshakeError(
            "JETBRAINS_HANDSHAKE_JSON_INVALID", f"{label} must contain a JSON object"
        )
...
def _changed_paths(values: list[str]) -> list[str]:
    if not values or len(values) > 200:
        raise JetBrainsHandshakeError(
            "JETBRAINS_HANDSHAKE_CHANGED_INVALID",
            "one through 200 returned changed paths are required",
        )
    normalized: set[str] = set()
    for value in values:
        if not isinstance(value, str):
            raise JetBrainsHandshakeError(
                "JETBRAINS_HANDSHAKE_CHANGED_INVALID", "changed paths must be strings"
            )
        ...
        if (
            ...
            raise JetBrainsHandshakeError(
                "JETBRAINS_HANDSHAKE_CHANGED_INVALID",
                "changed paths must be workspace-relative without traversal",
            )
        normalized.add(item)
...
def _scope(
    workspace: Path, scope_path: Path
...
def _intent(workspace: Path, change_list: str, changed: list[str]) -> dict[str, Any]:
    try:
        return inspect_intent_ledger(
            workspace, change_list=change_list, changed=changed
        )
...
def build_agent_proof_mission(
    root: Path, scope_path: Path, changed_paths: list[str] | None = None
) -> dict[str, Any]:
    ...
    promise = (
        ...
        else "Unknown: capture an Intent Ledger before treating completion as aligned."
    )
    non_goal = contract.get("non_goal") if isinstance(contract, dict) else "Unknown"
    failure_case = (
        contract.get("failure_case")
        if isinstance(contract, dict)
        else "Unknown: a negative case is required before approval."
    )
    ...
    core = {
        ...
        "scope": {
            ...
            "change_list": scope["change_list"],
            "sealed_paths": sealed,
        },
        "intent": {
            "state": intent["state"],
            "inspection_sha256": intent.get("inspection_sha256"),
            "promise": promise,
            "non_goal": non_goal,
            "negative_case": failure_case,
        },
        ...
        "scope_limits": [
            "This mission is local context for Junie or another agent; FactoryLine did not start, configure, or contact that agent."
        ],
    }
    ...
    text = (
        "\n".join(
            [
                ...
                "Sealed paths:",
                *[f"- {item}" for item in sealed],
                "Working contract:",
                *[f"- {item}" for item in core["working_contract"]],
...
def _e2e(workspace: Path, receipt_path: Path | None) -> dict[str, Any]:
    ...
    try:
        ...
        raise JetBrainsHandshakeError(exc.code, str(exc)) from exc
    return {
        "state": "passed" if receipt["ok"] else "failed",
...
def _handshake_receipt(
    ...
    core = {
        ...
        "verdict": verdict,
        "scope": {
            "path": scope_file.relative_to(workspace).as_posix(),
            "file_sha256": scope_file_sha,
            "scope_sha256": scope["scope_sha256"],
            "sealed_paths": sealed,
            "returned_paths": changed,
        ...
        "analysis": {
            **analysis,
            "policy": {
                ...
                "max_new_warnings": max_new_warnings,
            },
            "observed_new_errors": new_errors,
            "observed_new_warnings": new_warnings,
        },
        "e2e": e2e,
        "blockers": blockers,
        "unknowns": unknowns,
        ...
        "authority": dict(_AUTHORITY),
        "scope_limits": [
            "The handshake validates supplied local artifacts only; it does not execute an agent, analyzer, test, approval, or release."
        ],
    }
...
def validate_jetbrains_handshake(value: object) -> dict[str, Any]:
    ...
    if not isinstance(value, dict) or value.get("schema") != HANDSHAKE_SCHEMA:
        raise JetBrainsHandshakeError(
            "JETBRAINS_HANDSHAKE_RECEIPT_INVALID",
            f"a {HANDSHAKE_SCHEMA} object is required",
        )
    supplied = value.get("handshake_sha256")
    if not isinstance(supplied, str) or not _SHA256.fullmatch(supplied):
        raise JetBrainsHandshakeError(
            "JETBRAINS_HANDSHAKE_RECEIPT_INVALID",
            "handshake_sha256 must be a lowercase SHA-256 digest",
        )
    core = {key: item for key, item in value.items() if key != "handshake_sha256"}
    if _sha(core) != supplied:
        raise JetBrainsHandshakeError(
            "JETBRAINS_HANDSHAKE_RECEIPT_INVALID",
            "handshake_sha256 does not match the receipt facts",
        )
    return value


def write_jetbrains_handshake(
    root: Path, value: dict[str, Any], out: Path
) -> dict[str, Any]:
    ...
    target = Path(out)
    target = (
        target.resolve() if target.is_absolute() else (workspace / target).resolve()
    )
    try:
        ...
        raise JetBrainsHandshakeError(
            "JETBRAINS_HANDSHAKE_PATH_REJECTED",
            "handshake output must stay inside the workspace",
        ) from exc
    if target.suffix.lower() != ".json" or not relative.as_posix().startswith(
        ".factory/jetbrains-handshake/"
    ):
        raise JetBrainsHandshakeError(
            "JETBRAINS_HANDSHAKE_PATH_REJECTED",
            "handshake output must be JSON below .factory/jetbrains-handshake",
        )
    target.parent.mkdir(parents=True, exist_ok=True)
    encoded = (
        json.dumps(receipt, indent=2, sort_keys=True, ensure_ascii=False).encode(
            "utf-8"
        )
        + b"\n"
    )
    ...
    return {
        "marker": "JETBRAINS_PROOF_HANDSHAKE_WRITTEN",
        "path": relative.as_posix(),
        "file_sha256": sha256(encoded).hexdigest(),
        "handshake_sha256": receipt["handshake_sha256"],
    }
...
def jetbrains_handshake_projection(root: Path) -> dict[str, Any]:
    ...
    if not path.is_file():
        return {
            ...
            "marker": "JETBRAINS_PROOF_HANDSHAKE_EMPTY",
            "state": "empty",
            "receipt": None,
        }
    try:
        ...
        return {
            "schema": "factory.jetbrains-proof-handshake.status.v1",
            "marker": "JETBRAINS_PROOF_HANDSHAKE_INVALID",
            "state": "invalid",
            "code": exc.code,
            "receipt": None,
        }
