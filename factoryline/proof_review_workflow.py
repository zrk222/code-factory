...
def _workspace(root: Path) -> Path:
    ...
    if not value.is_dir():
        raise ProofReviewError(
            "PROOF_REVIEW_ROOT_INVALID", "root must be an existing directory"
        )
    return value


def _path(
    workspace: Path, value: Path, field: str, *, must_exist: bool = True
) -> tuple[Path, str]:
    ...
    try:
        ...
        raise ProofReviewError(
            "PROOF_REVIEW_PATH_REJECTED", f"{field} must stay inside the workspace"
        ) from exc
    if must_exist and (not resolved.is_file() or resolved.stat().st_size > MAX_BYTES):
        raise ProofReviewError(
            "PROOF_REVIEW_INPUT_INVALID",
            f"{field} must be a file no larger than {MAX_BYTES} bytes",
        )
...
def _load(workspace: Path, value: Path, field: str) -> tuple[dict[str, Any], Path, str]:
    ...
    try:
        ...
        raise ProofReviewError(
            "PROOF_REVIEW_JSON_INVALID", f"{field} is not valid JSON"
        ) from exc
    if not isinstance(data, dict):
        raise ProofReviewError(
            "PROOF_REVIEW_JSON_INVALID", f"{field} must be a JSON object"
        )
...
def _strings(value: Any, field: str, *, minimum: int = 1) -> list[str]:
    if (
        ...
        raise ProofReviewError(
            ...
            f"{field} must contain at least {minimum} non-empty string",
        )
...
def create_intent_contract(
    root: Path, contract_id: str, draft_path: Path, confirmed_by: str
) -> dict[str, Any]:
    """Seal a complete workspace-contained intent draft after named human confirmation."""
    workspace = _workspace(root)
    if not _ID.fullmatch(contract_id):
        raise ProofReviewError(
            "INTENT_CONTRACT_ID_INVALID", "contract id must be a lowercase identifier"
        )
    if not isinstance(confirmed_by, str) or not confirmed_by.strip():
        raise ProofReviewError(
            "INTENT_CONTRACT_CONFIRMATION_REQUIRED",
            "a named human confirmer is required",
        )
    ...
    destination = (
        workspace / ".factory" / "proof-review" / "contracts" / f"{contract_id}.json"
    )
    if destination.exists():
        raise ProofReviewError(
            "INTENT_CONTRACT_EXISTS", "intent contract ids are immutable"
        )
...
def verify_intent_contract(root: Path, contract_path: Path) -> dict[str, Any]:
    ...
    if (
        ...
        or not isinstance(digest, str)
        or digest != _sha(core)
        or value.get("human_confirmed") is not True
    ):
        return {
            "schema": INTENT_SCHEMA,
            "marker": "INTENT_CONTRACT_INVALID",
            "ok": False,
            "path": relative,
    ...
    if (
        ...
        or sha256(source_path.read_bytes()).hexdigest() != source["sha256"]
    ):
        return {
            "schema": INTENT_SCHEMA,
            "marker": "INTENT_CONTRACT_STALE",
            "ok": False,
            "path": relative,
            "reason": "source_drift",
        }
    return {
        "schema": INTENT_SCHEMA,
        "marker": "INTENT_CONTRACT_VERIFIED",
        "ok": True,
        "path": relative,
...
def _trajectory_configuration(
    trace: dict[str, Any], policy: dict[str, Any]
) -> tuple[list[Any], list[str], set[str], set[str], list[str], str]:
    events = trace.get("events")
    max_steps = policy.get("max_steps", MAX_EVENTS)
    if isinstance(max_steps, bool) or not isinstance(max_steps, int) or max_steps < 1:
        raise ProofReviewError(
            "TRAJECTORY_POLICY_INVALID", "max_steps must be an integer from 1 to 500"
        )
    if (
        not isinstance(events, list)
        or not events
        or len(events) > min(max_steps, MAX_EVENTS)
    ):
        raise ProofReviewError(
            "TRAJECTORY_EVENTS_INVALID", f"events must contain 1 to {MAX_EVENTS} items"
        )
...
def _trajectory_findings(
    ...
    for index, event in enumerate(events):
        if (
            ...
            raise ProofReviewError(
                "TRAJECTORY_EVENT_INVALID", f"event {index} must declare type and actor"
            )
        observed.append(event["type"])
        findings.extend(
            _event_policy_violations(
                event, index, allowed_tools, forbidden_tools, allowed_paths
            )
        )
        independent = independent or (
            event["type"] == "independent_audit" and event["actor"] != worker
        )
...
def prove_trajectory(
    root: Path, trace_path: Path, policy_path: Path, trajectory_id: str
) -> dict[str, Any]:
    """Evaluate a bounded agent trace against ordered, scoped, independently audited policy."""
    workspace = _workspace(root)
    if not _ID.fullmatch(trajectory_id):
        raise ProofReviewError(
            "TRAJECTORY_ID_INVALID", "trajectory id must be a lowercase identifier"
        )
    trace, trace_file, trace_relative = _load(workspace, trace_path, "trajectory trace")
    policy, policy_file, policy_relative = _load(
        workspace, policy_path, "trajectory policy"
    )
    events, required, allowed_tools, forbidden_tools, allowed_paths, worker = (
        _trajectory_configuration(trace, policy)
    )
    violations, independent = _trajectory_findings(
        events, required, allowed_tools, forbidden_tools, allowed_paths, worker
    )
    core = {
        ...
        "trace": {
            "path": trace_relative,
            "sha256": sha256(trace_file.read_bytes()).hexdigest(),
        },
        "policy": {
            "path": policy_relative,
            "sha256": sha256(policy_file.read_bytes()).hexdigest(),
        },
...
def verify_trajectory(root: Path, trajectory_path: Path) -> dict[str, Any]:
    ...
    if (
        value.get("schema") != TRAJECTORY_SCHEMA
        or not isinstance(digest, str)
        or digest != _sha(core)
    ):
        return {
            "schema": TRAJECTORY_SCHEMA,
            "marker": "AGENT_TRAJECTORY_INVALID",
            "ok": False,
            "path": relative,
            "reason": "receipt_digest",
        }
    ...
    return {
        "schema": TRAJECTORY_SCHEMA,
        "marker": "AGENT_TRAJECTORY_VERIFIED",
        "ok": True,
...
def create_quick_review(
    ...
    *,
    ...
    intake_parameters_path: Path | None = None,
    ...
    core = {
        ...
        "intent_contract": {
            ...
            "sha256": sha256(contract_file.read_bytes()).hexdigest(),
            "contract_sha256": contract["contract_sha256"],
        },
        "continuous_proof": {
            "path": continuous_path.relative_to(workspace).as_posix(),
            "sha256": sha256(continuous_path.read_bytes()).hexdigest(),
            "receipt_sha256": continuous["receipt_sha256"],
        },
        ...
        **({"intake_parameters": intake_binding} if intake_binding is not None else {}),
        ...
        "authority": dict(_AUTHORITY),
        "claim_limits": [
            "Review records are not unique users.",
            "No time, token, cost, quality, security, compliance, or productivity outcome is inferred.",
        ],
    }
    destination = (
        workspace / ".factory" / "proof-review" / "reviews" / f"{review_id}.json"
    )
    if destination.exists():
...
def _review_binding_failure(workspace: Path, value: dict[str, Any]) -> str | None:
    ...
    if not isinstance(contract, dict) or not isinstance(continuous, dict):
        return "bindings"
    if not _binding_matches(workspace, contract) or not _binding_matches(
        workspace, continuous
    ):
        return "binding_bytes"
    if not verify_intent_contract(workspace, Path(contract.get("path", ""))).get("ok"):
        return "intent_contract"
    if not verify_continuous_proof(workspace, Path(continuous.get("path", ""))).get(
        "ok"
    ):
    ...
    if isinstance(trajectory, dict) and (
        not _binding_matches(workspace, trajectory)
    ...
    if intake is not None:
        ...
        check = verify_intake_binding(
            workspace,
            Path(str(intake.get("path"))),
            scope_paths=value.get("changed_paths"),
...
def verify_quick_review(root: Path, review_path: Path) -> dict[str, Any]:
    ...
    if (
        ...
        or not isinstance(digest, str)
        or digest != _sha(core)
        or value.get("route") not in _ROUTES
    ):
        return {
            "schema": REVIEW_SCHEMA,
            "marker": "PROOF_REVIEW_INVALID",
            "ok": False,
            "path": relative,
            "reason": "receipt_digest",
        }
    ...
    if binding_failure:
        marker = (
            "PROOF_REVIEW_INVALID"
            if binding_failure == "bindings"
            else "PROOF_REVIEW_STALE"
        )
        return {
            "schema": REVIEW_SCHEMA,
            "marker": marker,
...
def promote_regression(
    root: Path, review_path: Path, capsule_id: str, confirmed_by: str, title: str
) -> dict[str, Any]:
    """Promote one current causal failure into an immutable, human-confirmed regression capsule."""
    workspace = _workspace(root)
    if not _ID.fullmatch(capsule_id):
        raise ProofReviewError(
            "REGRESSION_ID_INVALID", "capsule id must be a lowercase identifier"
        )
    if not confirmed_by.strip() or not title.strip():
        raise ProofReviewError(
            "REGRESSION_CONFIRMATION_REQUIRED", "named confirmer and title are required"
        )
    check = verify_quick_review(workspace, review_path)
    if not check.get("ok"):
        raise ProofReviewError(
            "REGRESSION_REVIEW_INVALID", "proof review must be current and verified"
        )
    review, review_file, review_relative = _load(workspace, review_path, "proof review")
    continuous, _, _ = _load(
        workspace, Path(review["continuous_proof"]["path"]), "continuous proof"
    )
    ...
    if not failures and not findings and review.get("route") == "review_ready":
        raise ProofReviewError(
            "REGRESSION_CAUSAL_FAILURE_REQUIRED",
            "a causal failure or deterministic finding is required",
        )
    core = {
        "schema": REGRESSION_SCHEMA,
        "marker": "REGRESSION_CAPSULE_PROMOTED",
        ...
        "proof_review": {
            ...
            "sha256": sha256(review_file.read_bytes()).hexdigest(),
            "review_sha256": review["review_sha256"],
        },
        "failure_classes": sorted(str(item) for item in failures),
        "deterministic_findings": sorted(
            {str(item.get("kind")) for item in findings if isinstance(item, dict)}
        ),
        "final_approval": False,
        "authority": dict(_AUTHORITY),
    }
    destination = (
        workspace / ".factory" / "proof-review" / "regressions" / f"{capsule_id}.json"
    )
    if destination.exists():
        raise ProofReviewError(
            "REGRESSION_CAPSULE_EXISTS", "regression capsule ids are immutable"
        )
...
def team_proof_inbox(root: Path) -> dict[str, Any]:
    ...
    paths = (
        sorted(directory.glob("*.json"))[: MAX_RECORDS + 1]
        if directory.is_dir()
        else []
    )
    ...
    return {
        "schema": INBOX_SCHEMA,
        "marker": "TEAM_PROOF_INBOX_READ_ONLY",
        "current_count": len(records),
        "stale_count": stale,
        "invalid_count": invalid,
        "truncated": truncated,
        "next_item": records[0] if records else None,
        "items": records,
        "authority": dict(_AUTHORITY),
        "claim_limits": [
            "Inbox records are not unique users.",
            "No effort, time, cost, savings, quality, or productivity value is inferred.",
        ],
    }
...
def install_hook_pack(root: Path) -> dict[str, Any]:
    ...
    files = {
        ...
        "github-copilot.json": json.dumps(
            {
                "version": 1,
                "events": {
                    "sessionStart": "python .factory/proof-review/hooks/event_writer.py session_start copilot",
                    "userPromptSubmitted": "python .factory/proof-review/hooks/event_writer.py prompt_submitted copilot",
                    "preToolUse": "python .factory/proof-review/hooks/event_writer.py pre_tool copilot",
                    "postToolUse": "python .factory/proof-review/hooks/event_writer.py post_tool copilot",
                    "agentStop": "python .factory/proof-review/hooks/event_writer.py agent_stop copilot",
                },
            },
            indent=2,
        ),
        "claude-code.json": json.dumps(
            {
                "hooks": {
                    "SessionStart": "python .factory/proof-review/hooks/event_writer.py session_start claude",
                    "PreToolUse": "python .factory/proof-review/hooks/event_writer.py pre_tool claude",
                    "PostToolUse": "python .factory/proof-review/hooks/event_writer.py post_tool claude",
                    "Stop": "python .factory/proof-review/hooks/event_writer.py agent_stop claude",
                }
            },
            indent=2,
        ),
        ...
        "generic-jsonl.json": json.dumps(
            {
                "schema": "factory.agent-hook-event.v1",
                "writer": ".factory/proof-review/hooks/event_writer.py",
                "output": ".factory/proof-review/hook-events.jsonl",
            },
            indent=2,
        ),
    }
    ...
    for name, content in files.items():
        ...
        if target.exists() and target.read_bytes() != data:
            raise ProofReviewError(
                "HOOK_PACK_CONFLICT",
                f"refusing to overwrite changed hook template: {name}",
            )
        _atomic(target, data)
        artifacts.append(
            {
    ...
    core = {
        "schema": HOOK_SCHEMA,
        "marker": "AGENT_HOOK_PACK_WRITTEN",
        "recorded_at": _now(),
        "adapters": [
            "github-copilot",
            "claude-code",
...
def create_proof_card(root: Path, review_path: Path, card_id: str) -> dict[str, Any]:
    ...
    if not _ID.fullmatch(card_id):
        raise ProofReviewError(
            "PROOF_CARD_ID_INVALID", "card id must be a lowercase identifier"
        )
    checked = verify_quick_review(workspace, review_path)
    if not checked.get("ok"):
        raise ProofReviewError(
            "PROOF_CARD_REVIEW_INVALID", "proof review must be current and verified"
        )
    review, _, _ = _load(workspace, review_path, "proof review")
    core = {
        "schema": CARD_SCHEMA,
        "marker": "SHAREABLE_PROOF_CARD_EXPORTED",
        "card_id": card_id,
        "recorded_at": _now(),
        "review_id": review["review_id"],
        "review_sha256": review["review_sha256"],
        "route": review["route"],
        "changed_path_count": len(review["changed_paths"]),
        "next_action": review["next_action"],
        "human_approval_required": True,
        "final_approval": False,
        "claim_limits": [
            "Ready means ready for human review only.",
            "No merge, deployment, compliance, production-readiness, user, savings, or productivity claim is made.",
        ],
    }
    ...
    colour = {
        "review_ready": "#17835c",
        "human_required": "#b33a3a",
        "reverification_required": "#b06d00",
        "evidence_required": "#3b67b1",
    }[card["route"]]
    ...
    return {
        **card,
        "artifacts": {
            "json": str(json_path),
            "markdown": str(destination / "card.md"),
            "svg": str(destination / "card.svg"),
        },
    }
...
def verify_proof_card(card_path: Path) -> dict[str, Any]:
    ...
    try:
        ...
        return {
            "schema": CARD_SCHEMA,
            "marker": "PROOF_CARD_INVALID",
            "ok": False,
            "reason": "unreadable",
        }
    digest = value.get("card_sha256") if isinstance(value, dict) else None
    core = (
        {key: item for key, item in value.items() if key != "card_sha256"}
        if isinstance(value, dict)
        else {}
    )
    ok = (
        value.get("schema") == CARD_SCHEMA
    ...
    return {
        "schema": CARD_SCHEMA,
        "marker": "PROOF_CARD_VERIFIED" if ok else "PROOF_CARD_INVALID",
        "ok": ok,
        "card_sha256": digest if ok else None,
        "reason": None if ok else "receipt_digest",
    }
...
def proof_review_projection(root: Path) -> dict[str, Any]:
    ...
    return {
        "current_count": inbox["current_count"],
        "stale_count": inbox["stale_count"],
        "invalid_count": inbox["invalid_count"],
        "next_item": inbox["next_item"],
        "regression_capsule_count": len(list(regressions.glob("*.json")))
