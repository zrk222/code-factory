...
def evaluate_migration(
    ...
    *,
    ...
    if (
        ...
        or not 1 <= len(required_invariants) <= 256
    ):
        return lane_result(
            "migration_integrity",
            "INCOMPLETE",
            "MIGRATION_INVARIANTS_MISSING",
            "Signed data invariants are absent.",
        )
    ...
    for item in required_invariants:
        ...
        if item_id in expected:
            return lane_result(
                "migration_integrity",
                "INCOMPLETE",
                "MIGRATION_DUPLICATE_INVARIANT",
                "Invariant IDs must be unique.",
            )
        expected[item_id] = require_str(
            item["expected_sha256"],
            "config.invariant.expected_sha256",
            minimum=64,
            maximum=64,
        )
    ...
    if not isinstance(observations, list) or not 1 <= len(observations) <= 256:
        return lane_result(
            ...
            "INCOMPLETE",
            "MIGRATION_INVARIANTS_MISSING",
            "Invariant query evidence is absent.",
        )
    actual: dict[str, str] = {}
    for item in observations:
        if not isinstance(item, dict):
            return lane_result(
                ...
                "INCOMPLETE",
                "MIGRATION_INVARIANT_INVALID",
                "An invariant result is malformed.",
            )
        ...
        if item_id in actual:
            return lane_result(
                ...
                "INCOMPLETE",
                "MIGRATION_DUPLICATE_INVARIANT",
                "Observed invariant IDs must be unique.",
            )
        actual[item_id] = require_str(
            item["actual_sha256"], "invariant.actual_sha256", minimum=64, maximum=64
        )
    ...
    if not isinstance(counts, list) or not 1 <= len(counts) <= 256:
        return lane_result(
            "migration_integrity",
            "INCOMPLETE",
            "MIGRATION_COUNTS_MISSING",
            "Record-count comparisons are absent.",
        )
    ...
    for item in counts:
        if not isinstance(item, dict):
            return lane_result(
                ...
                "A record-count comparison is malformed.",
            )
        ...
        if table in tables:
            return lane_result(
                "migration_integrity",
                "INCOMPLETE",
                "MIGRATION_DUPLICATE_COUNT",
                "Record-count tables must be unique.",
            )
        tables.add(table)
        old = require_int(
        ...
        if approved_counts is None or (old, new) != (
            approved_counts["before"],
            approved_counts["after"],
        ):
    ...
    if tables != set(expected_counts):
        return lane_result(
            ...
            "INCOMPLETE",
            "MIGRATION_COUNTS_MISSING",
            "Not all approved tables were checked.",
        )
    ...
    if not isinstance(readers, dict) or not isinstance(recovery, dict):
        return lane_result(
            "migration_integrity",
            "INCOMPLETE",
            "MIGRATION_REHEARSAL_INCOMPLETE",
            "Reader or recovery evidence is missing.",
        )
    ...
    invariant_failures = sorted(
        item_id for item_id, digest in expected.items() if actual.get(item_id) != digest
    )
    ...
    if (
        ...
        or artifact["history_valid"] is not True
        or artifact["drift_detected"] is not False
        or missing
        or unexpected_invariants
        or invariant_failures
        or losses
        or violations
        or readers.get("old") is not True
        or readers.get("new") is not True
        or recovery.get("exercised") is not True
        or recovery.get("succeeded") is not True
        or missing_catalog
        or invalid_catalog
