...
def _path_classification(raw_path: str, system: str, environ: Mapping[str, str]) -> str:
    ...
    if normalized.startswith("\\\\wsl$\\") or normalized.startswith(
        "\\\\wsl.localhost\\"
    ):
        return "wsl_unc"
    if normalized.startswith("\\\\"):
        return "windows_unc"
    if system.lower() == "linux" and (
        "WSL_DISTRO_NAME" in environ
        or "microsoft" in environ.get("WSL_INTEROP", "").lower()
    ):
        return "linux_wsl"
...
def _recommendations(
    *,
    ...
    generated = [
        item
        for item in managed_directory_summary
        if item["category"] == "generated" and int(item["bytes"]) > 0
    ]
    dependencies = [
        item
        for item in managed_directory_summary
        if item["category"] == "dependencies" and int(item["bytes"]) > 0
    ]
    if generated:
        ...
        recommendations.append(
            {
                ...
                "evidence": [
                    ...
                    f"scan_limited={str(scan_limited).lower()}",
                ],
        ...
        recommendations.append(
            {
                ...
                "state": "observed",
                "evidence": [
                    "No generated/dependency directory or remote-path heuristic crossed the advisor threshold."
                ],
...
def _safe_directories(
    current: Path, directory_names: list[str], state: _ScanState
) -> list[str]:
...
def _update_directory_summary(
    ...
    item = summary.setdefault(
        path, {"path": path, "category": category, "files": 0, "bytes": 0}
    )
...
def _observe_file(candidate: Path, workspace: Path, state: _ScanState) -> None:
    ...
    _update_directory_summary(
        state.top_level,
        top_level,
        _category_for_top_level(top_level),
        stat.st_size,
    )
...
def _scan_workspace(workspace: Path, max_files: int) -> tuple[_ScanState, bool]:
    ...
    for current_root, directory_names, file_names in os.walk(
        ...
        directory_names[:] = _safe_directories(
            Path(current_root), directory_names, state
        )
...
def inspect_workspace(
    ...
    *,
    ...
    if max_files < 1:
        raise WorkspaceAdvisorError(
            "WORKSPACE_ADVISOR_LIMIT_INVALID", "max_files must be positive"
        )
    ...
    path_classification = _path_classification(
        str(workspace), runtime_platform, environment
    )
    state, scan_limited = _scan_workspace(workspace, max_files)
    directories = sorted(
        state.top_level.values(),
        key=lambda item: (-int(item["bytes"]), str(item["path"])),
    )[:MAX_TOP_LEVEL_DIRECTORIES]
    managed_summary = sorted(
        state.managed_directories.values(),
        key=lambda item: (-int(item["bytes"]), str(item["path"])),
    )[:MAX_TOP_LEVEL_DIRECTORIES]
    large_files = sorted(state.large_files, key=lambda item: (-item[0], item[1]))[
        :MAX_LARGE_FILES
    ]
    ...
    payload["advice_sha256"] = sha256(
        _canonical(digest_source).encode("utf-8")
    ).hexdigest()
    return payload
...
def _require_child(root: Path, candidate: Path | str) -> Path:
    ...
    try:
        ...
        raise WorkspaceAdvisorError(
            "WORKSPACE_ADVISOR_OUTPUT_OUTSIDE_ROOT",
            "artifact directory must stay inside the workspace root",
        ) from exc
    return resolved
...
def write_workspace_advisor_artifacts(
    ...
    if report.get("schema") != WORKSPACE_ADVISOR_SCHEMA:
        raise WorkspaceAdvisorError(
            "WORKSPACE_ADVISOR_REPORT_INVALID", "report has an unexpected schema"
        )
