...
def _relative(root: Path, path: Path, field: str) -> tuple[Path, str]:
    ...
    try:
        ...
        raise SessionRecorderError(
            "SESSION_PATH_OUT_OF_SCOPE", f"{field} must stay inside the workspace"
        ) from exc
...
def _load_json(root: Path, path: Path, field: str) -> tuple[dict[str, Any], Path, str]:
    ...
    try:
        ...
        raise SessionRecorderError(
            "SESSION_INPUT_UNREADABLE", f"cannot read {field}: {relative}"
        ) from exc
    if not isinstance(value, dict):
        raise SessionRecorderError(
            "SESSION_INPUT_INVALID", f"{field} must be a JSON object"
        )
...
def _snapshot(root: Path) -> dict[str, str]:
    ...
    for path in sorted(root.rglob("*")):
        ...
        if len(records) > 10_000:
            raise SessionRecorderError(
                "SESSION_WORKSPACE_TOO_LARGE",
                "workspace observation exceeds 10000 files",
            )
    return records
...
def _argv(value: object, field: str) -> list[str]:
    if not isinstance(value, list) or not value or len(value) > 64:
        raise SessionRecorderError(
            "SESSION_INPUT_INVALID", f"{field} must contain 1 through 64 argv items"
        )
    if any(
        ...
        raise SessionRecorderError(
            "SESSION_INPUT_INVALID", f"{field} contains an invalid argv item"
        )
    return list(value)
...
def _validator_entries(value: dict[str, Any]) -> tuple[str, list[Any]]:
    if (
        ...
        raise SessionRecorderError(
            ...
            f"validator manifest must use {VALIDATOR_MANIFEST_SCHEMA}",
        )
...
def _run(argv: list[str], root: Path, timeout: int) -> dict[str, Any]:
    ...
    return {
        ...
        "stdout_bytes": len(b64decode(captures["stdout"])),
        "stderr_sha256": result["stderr_sha256"],
        "stderr_bytes": len(b64decode(captures["stderr"])),
    }
...
def _previous_session(root: Path) -> str | None:
    ...
    for candidate in (root / ".factory" / "session-recorder").glob("*/session.json"):
        ...
        if not isinstance(value, dict):
            continue
        recorded_at, run_id, digest = (
            value.get("recorded_at"),
            value.get("run_id"),
            value.get("session_sha256"),
        )
        if not all(
            isinstance(item, str) for item in (recorded_at, run_id, digest)
...
def _load_ready_admission(
    ...
    if ready.get("verdict") != "READY":
        raise SessionRecorderError(
            "SESSION_ADMISSION_NOT_READY",
            f"admission must verify READY: {ready.get('reason', ready.get('verdict'))}",
        )
...
def _failure_classes(
    ...
    if any(result["status"] == "timed_out" for result in validator_results):
        failures.add(FailureClass.RUNTIME_TIMEOUT.value)
    if any(
        result["status"] != "completed" or result["exit_code"] != 0
...
def _execute_observed(
    workspace: Path, argv: list[str], context: dict[str, Any]
) -> dict[str, Any]:
    ...
    scope_escapes = [
        item["path"]
        for item in changes
        if not _inside_scope(item["path"], context["scopes"])
    ]
    validator_results = [
        {"id": item["id"], **_run(item["argv"], workspace, item["timeout_seconds"])}
        for item in context["validators"]
    ]
    manifest_drifted = (
        _file_sha((workspace / context["manifest"]).resolve())
    ...
    return {
        ...
        "failures": _failure_classes(
            agent_result, validator_results, scope_escapes, manifest_drifted
        ),
    }


def _write_evidence_receipts(
    workspace: Path, run_id: str, context: dict[str, Any], observed: dict[str, Any]
) -> tuple[Path, Path]:
    output_dir = workspace / ".factory" / "session-recorder" / run_id
    result_core = {
        "schema": RESULT_RECEIPT_SCHEMA,
        "marker": "NO_RAW_CONTENT_RETAINED",
        "run_id": run_id,
        "admission": {
            "path": context["admission_relative"],
            "packet_sha256": context["ready"]["packet_sha256"],
            "pre_run_verdict": "READY",
        },
        "agent_command": observed["agent_result"],
        "workspace_delta": observed["changes"],
        "scope_escapes": observed["scope_escapes"],
        "authority": dict(_AUTHORITY),
        "scope_limits": [
            "Observed execution is not sandboxed execution.",
            "No raw command output, prompt text, credentials, or environment values are retained.",
        ],
    }
    result_path = _write(
        output_dir / "result.json", {**result_core, "result_sha256": _sha(result_core)}
    )
    verification_core = {
        "schema": VALIDATION_RECEIPT_SCHEMA,
        "run_id": run_id,
        "verifier_subject": context["verifier"],
        "manifest": {
            "path": context["manifest"],
            "sha256": context["manifest_sha256"],
            "drifted": observed["manifest_drifted"],
        },
        "validators": observed["validator_results"],
        "passed": not observed["failures"],
        "failure_classes": observed["failures"],
        "authority": dict(_AUTHORITY),
    }
    verification_path = _write(
        output_dir / "verification.json",
        {**verification_core, "verification_sha256": _sha(verification_core)},
    )
...
def _record_session(
    ...
    event_core = {
        ...
        "agent": context["agent"],
        "task_id": _event_task_id(context["request"]),
        "admission": {
            "path": context["admission_relative"],
            "packet_sha256": context["ready"]["packet_sha256"],
        },
        ...
        "verification": {
            "subject": context["verifier"],
            "receipt": {
                "path": verification_path.relative_to(workspace).as_posix(),
                "sha256": _file_sha(verification_path),
            },
        },
        "passed": not observed["failures"],
        "failure_classes": observed["failures"],
        "paths": context["scopes"],
    }
    ledger = record_bound_governed_event(
        workspace, {**event_core, "event_sha256": _sha(event_core)}
    )
    session_core = {
        ...
        "run_id": run_id,
        "recorded_at": recorded_at,
        "previous_session_sha256": _previous_session(workspace),
        "result": {
            "path": result_path.relative_to(workspace).as_posix(),
            "sha256": _file_sha(result_path),
        },
        ...
        "agent_event": {
            "path": Path(ledger["path"]).relative_to(workspace).as_posix(),
            "sha256": ledger["event"]["event_sha256"],
        },
        "passed": not observed["failures"],
        "failure_classes": observed["failures"],
        "authority": dict(_AUTHORITY),
        "scope_limits": [
            "Observed execution is not sandboxed execution.",
            "The host must enforce process, network, credential, identity, and filesystem isolation.",
        ],
    }
    session = {**session_core, "session_sha256": _sha(session_core)}
    session_path = _write(
        workspace / ".factory" / "session-recorder" / run_id / "session.json", session
    )
...
def run_observed_session(
    ...
    if not workspace.is_dir() or not _ID.fullmatch(run_id):
        raise SessionRecorderError(
            "SESSION_INPUT_INVALID", "root and lowercase run_id are required"
        )
    argv = _argv(command, "command")
    output_dir = workspace / ".factory" / "session-recorder" / run_id
    ledger_path = (
        workspace / ".factory" / "agent-licenses" / "events" / f"{run_id}.json"
    )
    if output_dir.exists() or ledger_path.exists():
        raise SessionRecorderError(
            "SESSION_RUN_EXISTS", "the admitted run id has already been consumed"
        )
    ...
    if context["request"].get("id") != run_id:
        raise SessionRecorderError(
            ...
            "run_id must exactly match the admitted request id",
        )
    observed = _execute_observed(workspace, argv, context)
    result_path, verification_path = _write_evidence_receipts(
        workspace, run_id, context, observed
    )
    return _record_session(
        workspace, run_id, context, observed, result_path, verification_path
    )
...
def verify_session_receipt(root: Path, receipt_path: Path) -> dict[str, Any]:
    ...
    if value.get("schema") != SESSION_RECEIPT_SCHEMA or digest != _sha(core):
        return {
            "schema": SESSION_RECEIPT_SCHEMA,
            "marker": "OBSERVED_SESSION_INVALID",
            "ok": False,
            "path": relative,
        }
    ...
    return {
        "schema": SESSION_RECEIPT_SCHEMA,
        "marker": "OBSERVED_SESSION_VERIFIED",
        "ok": True,
        "path": relative,
