...
def _bounded(value: object, field: str, maximum: int = 160) -> str:
    if not isinstance(value, str) or not value.strip() or len(value) > maximum:
        raise GraphForensicsError(
            "GRAPH_LINEAGE_INVALID",
...
def _hash(value: object, field: str) -> str:
    if not isinstance(value, str) or not _SHA.fullmatch(value):
        raise GraphForensicsError(
            "GRAPH_LINEAGE_INVALID", f"{field} must be a lowercase SHA-256 digest"
        )
    return value
...
def _items(raw: object, field: str) -> list[dict[str, Any]]:
    if (
        ...
        or len(raw) > MAX_STATE_ITEMS
        or any(not isinstance(item, dict) for item in raw)
    ):
        raise GraphForensicsError(
            "GRAPH_LINEAGE_INVALID",
            f"{field} must be a list of at most {MAX_STATE_ITEMS} objects",
        )
    return list(raw)
...
def _load(path: Path) -> dict[str, Any]:
    ...
    try:
        if not candidate.is_file() or candidate.stat().st_size > MAX_LINEAGE_BYTES:
            raise GraphForensicsError(
                "GRAPH_LINEAGE_UNREADABLE", "lineage file is missing or too large"
            )
        ...
        raise GraphForensicsError(
            "GRAPH_LINEAGE_UNREADABLE", "lineage file is not readable canonical JSON"
        ) from exc
    if not isinstance(value, dict):
        raise GraphForensicsError(
            "GRAPH_LINEAGE_INVALID", "lineage root must be one object"
        )
    return value
...
def _nonnegative(value: object, field: str) -> int:
    if not isinstance(value, int) or isinstance(value, bool) or value < 0:
        raise GraphForensicsError(
            "GRAPH_LINEAGE_INVALID", f"{field} must be a non-negative integer"
        )
    return value
...
def _normalize_read(item: dict[str, Any], field: str) -> dict[str, Any]:
    return {
        "key": _bounded(item.get("key"), f"{field}.key"),
        "version": _nonnegative(item.get("version"), "read version"),
        "sha256": _hash(item.get("sha256"), "read.sha256"),
    }
...
def _normalize_write(item: dict[str, Any], field: str) -> dict[str, Any]:
    ...
    if version <= previous:
        raise GraphForensicsError(
            "GRAPH_LINEAGE_INVALID", "write version must advance previous_version"
        )
    mode = item.get("mode", "replace")
    if mode not in {"replace", "reduce", "delete"}:
        raise GraphForensicsError(
            "GRAPH_LINEAGE_INVALID", "write mode must be replace, reduce, or delete"
        )
    ...
    return {
        ...
        "after_sha256": _hash(item.get("after_sha256"), "write.after_sha256"),
        "mode": mode,
...
def _normalize_evidence(item: dict[str, Any], _field: str) -> dict[str, str]:
    return {
        "path": _bounded(item.get("path"), "evidence.path", 320),
        "sha256": _hash(item.get("sha256"), "evidence.sha256"),
    }


def _normalize_effect(item: dict[str, Any], _field: str) -> dict[str, str]:
    return {
        "effect_id": _bounded(item.get("effect_id"), "side_effect.effect_id"),
        "idempotency_key": _bounded(
            item.get("idempotency_key"), "side_effect.idempotency_key", 240
        ),
        "status": _bounded(item.get("status"), "side_effect.status", 40),
    }


def _normalize_collection(
    raw: object, field: str, normalizer: Any, errors: list[str]
) -> list[dict[str, Any]]:
    try:
...
def _normalize_step(
    raw: object, offset: int, seen: set[int], errors: list[str]
) -> dict[str, Any] | None:
    ...
    try:
        ...
        if sequence < 1 or sequence in seen:
            raise GraphForensicsError(
                "GRAPH_LINEAGE_INVALID",
                f"steps[{offset}].sequence must be a unique positive integer",
            )
        node_id = _bounded(raw.get("node_id"), f"steps[{offset}].node_id")
        checkpoint_id = _bounded(
            raw.get("checkpoint_id"), f"steps[{offset}].checkpoint_id", 240
        )
    except GraphForensicsError as exc:
        errors.append(str(exc))
        return None
    seen.add(sequence)
    reads = _normalize_collection(
        raw.get("reads", []), f"steps[{offset}].reads", _normalize_read, errors
    )
    writes = _normalize_collection(
        raw.get("writes", []), f"steps[{offset}].writes", _normalize_write, errors
    )
    evidence = _normalize_collection(
        raw.get("evidence", []),
        f"steps[{offset}].evidence",
        _normalize_evidence,
        errors,
    )
    effects = _normalize_collection(
        raw.get("side_effects", []),
        f"steps[{offset}].side_effects",
        _normalize_effect,
        errors,
    )
    ...
    try:
        if not isinstance(decision, dict):
            raise GraphForensicsError(
                "GRAPH_LINEAGE_INVALID", f"steps[{offset}].decision must be an object"
            )
        normalized_decision = {
            "route": _bounded(decision.get("route"), "decision.route"),
            "reason": _bounded(decision.get("reason"), "decision.reason", 500),
        }
    except GraphForensicsError as exc:
        errors.append(str(exc))
    ...
    return {
        ...
        "reads": sorted(reads, key=lambda item: item["key"]),
        "writes": sorted(writes, key=lambda item: item["key"]),
        "evidence": sorted(evidence, key=lambda item: item["path"]),
        "side_effects": sorted(
            effects, key=lambda item: (item["effect_id"], item["idempotency_key"])
        ),
...
def _candidate_binding(
    value: dict[str, Any], expected: str | None, errors: list[str]
) -> str | None:
    declared = value.get("candidate_sha256")
    if declared is not None and (
        not isinstance(declared, str) or not _SHA.fullmatch(declared)
    ):
...
def _lineage_core(
    value: dict[str, Any], steps: list[dict[str, Any]], candidate: str | None
) -> dict[str, Any]:
    core = {
...
def verify_graph_lineage(
    ...
    normalized_steps = [
        ...
        if (step := _normalize_step(raw, offset, seen_sequences, errors)) is not None
    ]
    normalized_steps.sort(key=lambda item: item["sequence"])
    if normalized_steps and [item["sequence"] for item in normalized_steps] != list(
        range(1, len(normalized_steps) + 1)
    ):
    ...
    return {
        "schema": "factory.graph-lineage-verification.v1",
        "valid": not errors,
        "errors": errors,
        "run_id": value.get("run_id"),
        "graph_id": value.get("graph_id"),
        "lineage_sha256": calculated,
...
def _steps_for_seal(steps: list[object]) -> list[dict[str, Any]]:
    if not steps or len(steps) > MAX_STEPS:
        raise GraphForensicsError(
            "GRAPH_LINEAGE_INVALID", f"steps must contain 1..{MAX_STEPS} objects"
        )
    errors: list[str] = []
    seen: set[int] = set()
    normalized = [
        step
        for offset, raw in enumerate(steps)
        if (step := _normalize_step(raw, offset, seen, errors)) is not None
    ]
...
def _read_steps(path: Path) -> list[object]:
    try:
        ...
        raise GraphForensicsError(
            "GRAPH_LINEAGE_STEPS_UNREADABLE", "steps input must be readable JSON"
        ) from exc
    steps = source.get("steps") if isinstance(source, dict) else source
    if not isinstance(steps, list):
        raise GraphForensicsError(
            "GRAPH_LINEAGE_INVALID",
            "steps input must be a list or an object containing steps",
        )
    return steps
...
def _candidate_for_seal(candidate: str | None) -> str | None:
    if candidate is not None and (
        ...
        raise GraphForensicsError(
            "GRAPH_LINEAGE_INVALID",
            "candidate_sha256 must be a lowercase SHA-256 digest",
        )
    return candidate


def seal_graph_lineage(
    run_id: str,
    graph_id: str,
    steps_path: Path,
    out: Path,
    ...
    identity = {
        "schema": LINEAGE_SCHEMA,
        "run_id": _bounded(run_id, "run_id"),
        "graph_id": _bounded(graph_id, "graph_id"),
    }
    ...
    try:
        ...
        if not verification["valid"]:
            raise GraphForensicsError(
                "GRAPH_LINEAGE_INVALID", "; ".join(verification["errors"])
            )
...
def mission_history_steps(history: dict[str, Any]) -> list[dict[str, Any]]:
    ...
    if history.get("schema") != "factory.mission.graph.history.v1" or not isinstance(
        history.get("events"), list
    ):
        raise GraphForensicsError(
            "GRAPH_LINEAGE_HISTORY_INVALID",
            "mission history schema or events are invalid",
        )
    ...
    for offset, event in enumerate(history["events"], 1):
        if not isinstance(event, dict) or event.get("version") != offset:
            raise GraphForensicsError(
                "GRAPH_LINEAGE_HISTORY_INVALID",
                "mission events must be contiguous objects",
            )
        ...
        if not isinstance(receipt, dict):
            raise GraphForensicsError(
                "GRAPH_LINEAGE_HISTORY_INVALID", "mission event receipt is missing"
            )
        ...
        steps.append(
            {
                ...
                "writes": [
                    ...
                    },
                    {
                        ...
                        "previous_version": offset - 1,
                        "version": offset,
                        "before_sha256": previous_tip,
                        "after_sha256": event_sha,
                        "mode": "replace",
                        "reducer": None,
                    },
                ],
                ...
                "decision": {
                    "route": target,
                    "reason": f"guarded mission event {event_name}",
                },
            }
        )
    if not steps:
        raise GraphForensicsError(
            "GRAPH_LINEAGE_HISTORY_INVALID", "mission history contains no events"
        )
    return steps


def seal_mission_graph_lineage(
    mission_path: Path,
    root: Path,
    run_id: str,
    out: Path,
    ...
    try:
        with tempfile.NamedTemporaryFile(
            ...
            encoding="utf-8",
            dir=destination.parent,
            prefix=".mission-steps.",
            suffix=".json",
            delete=False,
        ) as handle:
            json.dump(steps, handle, ensure_ascii=False, sort_keys=True)
            step_file = Path(handle.name)
        result = seal_graph_lineage(
            run_id, str(history["mission_id"]), step_file, destination, candidate_sha256
        )
    finally:
    ...
    return {
        **result,
        "marker": "GRAPH_LINEAGE_MISSION_LEDGER_EXPORTED",
...
def _causal_cone(
    steps: list[dict[str, Any]], start: int, changed_keys: set[str]
...
def _divergence_index(
    ...
    for index in range(max(len(left), len(right))):
        if (
            index >= len(left)
            or index >= len(right)
            or _step_fingerprint(left[index]) != _step_fingerprint(right[index])
        ):
            return index
    return None
...
def _recovery_analysis(
    left: list[dict[str, Any]], right: list[dict[str, Any]], index: int | None
) -> tuple[dict[str, Any] | None, dict[str, Any]]:
    if index is None:
        return None, {
            "action": "no_recovery_required",
            "reason": "verified semantic steps are identical",
            "checkpoint_id": None,
            "rerun_nodes": [],
    ...
    changed_keys = {
        ...
        if step
        for item in [*step["reads"], *step["writes"]]
    }
    ...
    divergence = {
        "index": index,
        "sequence": candidate_step["sequence"] if candidate_step else None,
    ...
    recovery = {
        ...
        "reason": "fork before the first semantic divergence and rerun only its causal cone",
...
def graph_forensics(baseline_path: Path, candidate_path: Path) -> dict[str, Any]:
    ...
    if invalid:
        raise GraphForensicsError(
            "GRAPH_LINEAGE_INVALID", f"invalid lineage: {', '.join(invalid)}"
        )
    if baseline["graph_id"] != candidate["graph_id"]:
        raise GraphForensicsError(
            "GRAPH_LINEAGE_GRAPH_MISMATCH",
            "baseline and candidate must describe the same graph_id",
        )
    ...
    markers = [
        "GRAPH_FORENSICS_READ_ONLY",
        "GRAPH_FORENSICS_CAUSAL_CONE",
        "GRAPH_FORENSICS_CONCURRENCY_GUARD",
        "GRAPH_FORENSICS_AUTHORITY_RETAINED",
    ]
    if divergence is not None:
        markers.append("GRAPH_FORENSICS_FIRST_DIVERGENCE")
    core = {
        "schema": FORENSICS_SCHEMA,
        "marker": "GRAPH_FORENSICS_VERIFIED_SEMANTIC_TIME_TRAVEL",
        "markers": markers,
    ...
    return {
        **core,
        "forensics_sha256": _digest(core),
        "mermaid": _forensics_mermaid(divergence, recovery),
    }
...
def _forensics_mermaid(
    divergence: dict[str, Any] | None, recovery: dict[str, Any]
) -> str:
    if divergence is None:
        return 'flowchart LR\n    A["Baseline"] -->|semantically identical| B["Candidate"]\n'
    lines = [
        "flowchart LR",
        '    A["Verified baseline"] --> D["First divergence"]',
