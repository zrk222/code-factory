...
def _read_json(root: Path, value: Path) -> tuple[dict[str, Any], Path]:
    ...
    if source.stat().st_size > MAX_INPUT_BYTES:
        raise RevenueForgeError(
            "REVENUEFORGE_INPUT_TOO_LARGE", "evidence input exceeds 1 MiB"
        )
    try:
        payload = json.loads(source.read_text(encoding="utf-8-sig"))
    except (OSError, UnicodeDecodeError, json.JSONDecodeError) as exc:
        raise RevenueForgeError(
            "REVENUEFORGE_EVIDENCE_INVALID", "evidence input is not valid JSON"
        ) from exc
    if not isinstance(payload, dict):
        raise RevenueForgeError(
            "REVENUEFORGE_EVIDENCE_INVALID", "evidence input must be an object"
        )
...
def _required_object(payload: dict[str, Any], key: str) -> dict[str, Any]:
    ...
    if not isinstance(value, dict):
        raise RevenueForgeError(
            "REVENUEFORGE_EVIDENCE_INVALID", f"{key} must be an object"
        )
    return value
...
def _normalize_events(raw_events: object) -> list[dict[str, Any]]:
    if not isinstance(raw_events, list) or not raw_events or len(raw_events) > 500:
        raise RevenueForgeError(
            "REVENUEFORGE_EVIDENCE_INVALID", "events must contain 1-500 objects"
        )
...
def _replay_step(
    ...
    if not candidates:
        return {
            "step": event_type,
            "status": "unknown",
            "reason": "no build-bound observation supplied",
        }, prior_sequence
    ...
    if (
        event_type
        in {"purchase_started", "transaction_verified", "server_notification_verified"}
        and event["product_id"] not in product_ids
    ):
        reasons.append("product is not declared in the manifest")
    if (
        event_type in {"transaction_verified", "server_notification_verified"}
        and event["verified"] is not True
    ):
    ...
    return {
        ...
        "reason": "; ".join(reasons) if reasons else "observed and manifest-consistent",
    }, max(prior_sequence, event["sequence"])


def _replay_steps(
    events: list[dict[str, Any]], product_ids: set[str]
    ...
    for event_type in REQUIRED_JOURNEY:
        step, prior_sequence = _replay_step(
            event_type, by_type.get(event_type, []), product_ids, prior_sequence
        )
        steps.append(step)
...
def _validated_build(
    ...
    valid = (
        result["id"]
        and result["bundle_id"] == manifest["app"]["bundle_id"]
        and result["environment"] in {"sandbox", "testflight"}
    )
    if not valid:
        raise RevenueForgeError(
...
def replay_purchase_journey(
    root: Path, products_path: Path, events_path: Path, out: Path
) -> dict[str, Any]:
    ...
    optional = {
        event_type: "observed" if by_type.get(event_type) else "not_observed"
        for event_type in ("entitlement_expired", "entitlement_revoked")
    }
...
def _feedback_item(raw: object) -> dict[str, Any]:
    if not isinstance(raw, dict):
        raise RevenueForgeError(
            "REVENUEFORGE_TESTFLIGHT_INVALID", "each feedback item must be an object"
        )
...
def _feedback_fields(raw: dict[str, Any]) -> tuple[str, str, str, str]:
    ...
    kind, summary = (
        str(raw.get("kind") or "feedback").strip().lower(),
        str(raw.get("summary") or raw.get("comment") or "").strip()[:1000],
    )
    if (
        not external_id
        or not build_id
        or kind not in {"feedback", "screenshot", "crash"}
    ):
        raise RevenueForgeError(
...
def sync_testflight_evidence(
    root: Path, feedback_path: Path, out: Path
) -> dict[str, Any]:
    ...
    if not isinstance(items, list) or len(items) > 1000:
        raise RevenueForgeError(
            "REVENUEFORGE_TESTFLIGHT_INVALID",
            "items must be a list of at most 1000 feedback records",
        )
...
def evaluate_failure_matrix(
    root: Path, products_path: Path, evidence_path: Path, out: Path
) -> dict[str, Any]:
    ...
    rows = [
        _matrix_row(scenario, supplied.get(scenario)) for scenario in FAILURE_SCENARIOS
    ]
...
def _matrix_row(scenario: str, raw: object) -> dict[str, Any]:
    if raw is None:
        return {
            "scenario": scenario,
            "status": "unknown",
            "reason": "no observed negative-path evidence supplied",
        }
    if (
        not isinstance(raw, dict)
        or raw.get("observed") is not True
        or not isinstance(raw.get("passed"), bool)
    ):
        return {
            "scenario": scenario,
            "status": "unknown",
            "reason": "observed=true and boolean passed are required",
        }
    return {
        "scenario": scenario,
        "status": "pass" if raw["passed"] else "fail",
        "evidence_ref": str(raw.get("evidence_ref") or "")[:300] or None,
        "reason": str(raw.get("reason") or "observed result")[:500],
    }
...
def _policy_sources(payload: dict[str, Any]) -> dict[str, dict[str, Any]]:
    ...
    if not isinstance(sources, list) or not sources or len(sources) > 200:
        raise RevenueForgeError(
            "REVENUEFORGE_POLICY_INVALID", "sources must contain 1-200 objects"
        )
...
def watch_policy_drift(
    root: Path, registry_path: Path, snapshot_path: Path, out: Path
) -> dict[str, Any]:
...
def _accumulate_impacts(
    affected: dict[str, set[str]], impacts: list[dict[str, Any]]
) -> None:
    for impact in impacts:
...
def _parse_time(value: object, field: str) -> datetime:
    ...
    try:
        ...
        raise RevenueForgeError(
            "REVENUEFORGE_MEMORY_INVALID", f"{field} must be an ISO-8601 timestamp"
        ) from exc
    if parsed.tzinfo is None:
        raise RevenueForgeError(
            "REVENUEFORGE_MEMORY_INVALID", f"{field} must include a timezone"
        )
...
def _memory_fields(
    entry: dict[str, Any],
) -> tuple[str, str, str, str, str, datetime, list[Any]]:
    values = (
        str(entry.get("app_id") or "").strip(),
        str(entry.get("journey") or "").strip().lower(),
        str(entry.get("decision") or "").strip(),
        str(entry.get("resolution") or "").strip(),
        str(entry.get("approved_by") or "").strip(),
    )
    expires_at, receipts = (
        _parse_time(entry.get("expires_at"), "expires_at"),
        entry.get("evidence_receipts"),
    )
    if not all(values) or not isinstance(receipts, list) or not receipts:
        raise RevenueForgeError(
            "REVENUEFORGE_MEMORY_INVALID",
            "app_id, journey, decision, resolution, approved_by, expiry, and evidence receipts are required",
        )
...
def _memory_binding(workspace: Path, value: object) -> dict[str, str]:
    ...
    try:
        ...
        raise RevenueForgeError(
            "REVENUEFORGE_MEMORY_EVIDENCE_INVALID",
            "each evidence receipt must be sealed JSON",
        ) from exc
    if _sha(receipt) != expected or not str(receipt.get("schema", "")).startswith(
        "factory.revenueforge."
    ):
        raise RevenueForgeError(
            "REVENUEFORGE_MEMORY_EVIDENCE_INVALID", "receipt hash or schema is invalid"
        )
    return {
...
def promote_evidence_memory(root: Path, entry_path: Path, out: Path) -> dict[str, Any]:
    ...
    app_id, journey, decision, resolution, approved_by, expires_at, receipt_paths = (
        _memory_fields(entry)
    )
...
def query_evidence_memory(
    root: Path, app_id: str, journey: str, at: str | None = None
) -> dict[str, Any]:
    ...
    if not app or not lane:
        raise RevenueForgeError(
            "REVENUEFORGE_MEMORY_INVALID", "app_id and journey are required"
        )
...
def _memory_candidates(
    workspace: Path, app: str, lane: str, now: datetime
) -> tuple[list[dict[str, Any]], int]:
    candidates: list[dict[str, Any]] = []
    invalid = 0
    expected_scope = {"app_id": app, "journey": lane, "cross_tenant_reuse": False}
    for path in sorted(
        (workspace / ".factory" / "revenueforge" / "memory").glob("*.json")
    )[:500]:
        value = _read_memory(path)
        if value is None:
            invalid += 1
        elif (
            value["scope"] == expected_scope
            and _parse_time(value["expires_at"], "expires_at") > now
        ):
...
def _memory_status(candidates: list[dict[str, Any]]) -> str:
    ...
    return (
        "quarantined" if len(decisions) > 1 else "available" if candidates else "empty"
    )


def _memory_next_action(status: str) -> str:
    return {
        "quarantined": "human contradiction review",
        "available": "collect fresh build evidence",
    }.get(status, "run the evidence workflow")
...
def revenue_evidence_projection(root: Path) -> dict[str, Any]:
    ...
    result: dict[str, Any] = {
        "marker": "GRAPH_OPS_REVENUE_EVIDENCE_READ_ONLY",
        "invalid_count": 0,
    }
    for key, (filename, schema) in patterns.items():
        ...
        count = 0
        for path in sorted(
            (workspace / ".factory" / "revenueforge").glob(f"*/{filename}")
        )[:100]:
            try:
                value = json.loads(path.read_text(encoding="utf-8"))
                expected = value.pop("receipt_sha256")
