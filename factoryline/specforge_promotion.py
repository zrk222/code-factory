...
def verify_specforge_promotion(root: Path, spec_path: Path, forge_path: Path, out_path: Path) -> dict[str, Any]:
    ...
    if set(spec)!={"schema","intent","approval","obligations"} or spec.get("schema")!="factory.specline.delivery-packet.v1" or not isinstance(spec.get("intent"),dict) or not isinstance(spec.get("obligations"),list): raise RevenueForgeError("SPECFORGE_SPEC_INVALID","SpecLine packet must contain intent, approval, and obligations")
    digest=_sha(spec["intent"]); findings=[]
    if not isinstance(spec["approval"],dict) or spec["approval"].get("origin") not in {"human_confirmed","trusted_source"}: findings.append("E_SPECLINE_AUTHORITY_MISSING")
    if not spec["obligations"] or not all(isinstance(x,dict) and x.get("id") and x.get("forbidden_behavior") and x.get("gate") for x in spec["obligations"]): findings.append("E_SPECLINE_OBLIGATION_LOOSE")
    if set(forge)!={"schema","intent_sha256","state","gates"} or forge.get("schema")!="factory.forgeline.delivery-state.v1" or forge.get("intent_sha256")!=digest: findings.append("E_FORGELINE_INTENT_DRIFT")
    gates=forge.get("gates") if isinstance(forge.get("gates"),dict) else {}
    for gate in ("appforge_submission_integrity","appforge_capture_reconciliation"):
