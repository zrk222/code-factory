...
def verify_specforge_promotion(root: Path, spec_path: Path, forge_path: Path, out_path: Path) -> dict[str, Any]:
    ...
    if set(spec)!={"schema","intent","approval","obligations","required_gates"} or spec.get("schema")!="factory.specline.delivery-packet.v1" or not isinstance(spec.get("intent"),dict) or not isinstance(spec.get("obligations"),list) or not isinstance(spec.get("required_gates"),list) or not all(isinstance(x,str) and x for x in spec["required_gates"]): raise RevenueForgeError("SPECFORGE_SPEC_INVALID","SpecLine packet must contain intent, approval, obligations, and explicit required_gates")
