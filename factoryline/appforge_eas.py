...
def _local(root: Path, path: Path, *, exists: bool = True) -> Path:
    ...
    try:
        ...
        raise RevenueForgeError(
            "APPFORGE_EAS_PATH_REJECTED", "paths must remain inside the workspace"
        ) from exc
    if exists and not target.is_file():
        raise RevenueForgeError(
            "APPFORGE_EAS_INPUT_UNAVAILABLE", "input must be a regular workspace file"
        )
    return target
...
def _read_eas_config(root: Path, path: Path) -> tuple[dict[str, Any], Path]:
    ...
    if source.stat().st_size > MAX_BYTES:
        raise RevenueForgeError(
            "APPFORGE_EAS_INPUT_TOO_LARGE", "eas.json exceeds 1 MiB"
        )
    try:
        value = json.loads(source.read_text(encoding="utf-8-sig"))
    except (OSError, UnicodeDecodeError, json.JSONDecodeError) as exc:
        raise RevenueForgeError(
            "APPFORGE_EAS_INPUT_INVALID", "eas.json must be valid JSON"
        ) from exc
    if not isinstance(value, dict):
        raise RevenueForgeError(
            "APPFORGE_EAS_INPUT_INVALID", "eas.json must be a JSON object"
        )
    return value, source
...
def verify_eas_preflight(
    ...
    *,
    ...
    if secret_paths:
        findings.append(
            {
                "code": "APPFORGE_EAS_SECRET_IN_CONFIG",
                "detail": "credential-like keys are forbidden in eas.json: "
                + ", ".join(secret_paths),
            }
        )
    ...
    if not isinstance(build, dict) or not isinstance(build.get(build_name), dict):
        findings.append(
            {
                "code": "APPFORGE_EAS_BUILD_PROFILE_MISSING",
                "detail": f"build.{build_name} is required",
            }
        )
    ...
    if not isinstance(asc_app_id, (str, int)) or not str(asc_app_id).strip().isdigit():
        findings.append(
            {
                "code": "APPFORGE_EAS_ASC_APP_ID_MISSING",
    ...
    core: dict[str, Any] = {
        ...
        if not findings
        else "APPFORGE_EAS_PREFLIGHT_BLOCKED",
        "ok": not findings,
        ...
        "profiles": {
            "build": build_name,
            "submit": submit_name,
            "asc_app_id": str(asc_app_id).strip()
            if not findings and asc_app_id is not None
            else None,
        },
        "findings": findings,
        "human_handoff": [
    ...
    if out is not None:
        ...
        if target.exists():
            raise RevenueForgeError(
                "APPFORGE_EAS_OUTPUT_EXISTS",
                "preflight receipt destination already exists",
            )
...
def appforge_eas_projection(root: Path) -> dict[str, Any]:
    ...
    invalid: list[str] = []
    for path in sorted(
        (workspace / ".factory" / "appforge").rglob("*eas-preflight*.json")
    )[:100]:
        try:
