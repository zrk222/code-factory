...
def _canonical(value: object) -> bytes:
    try:
        ...
        raise LangGraphAssuranceError(
            INPUT_MARKER, "state values must be canonical JSON"
        ) from exc
...
def _identifier(value: object, label: str) -> str:
    if not isinstance(value, str) or not _TEXT.fullmatch(value):
        raise LangGraphAssuranceError(
            INPUT_MARKER, f"{label} must use 1-160 safe identifier characters"
        )
    return value
...
def _state(value: object, label: str) -> dict[str, Any]:
    if not isinstance(value, Mapping) or len(value) > _MAX_STATE_ITEMS:
        raise LangGraphAssuranceError(
            INPUT_MARKER,
            f"{label} must be an object with at most {_MAX_STATE_ITEMS} keys",
        )
...
def _inside(root: Path, value: Path | str, label: str, *, must_exist: bool) -> Path:
    ...
    if candidate.is_absolute():
        raise LangGraphAssuranceError(
            INPUT_MARKER, f"{label} must be workspace-relative"
        )
    try:
        ...
        raise LangGraphAssuranceError(
            INPUT_MARKER, f"{label} escapes the workspace"
        ) from exc
    if must_exist and not resolved.is_file():
        raise LangGraphAssuranceError(
            INPUT_MARKER, f"{label} must name an existing file"
        )
    return resolved
...
class LangGraphTransitionRecorder:
    ...
    def _start_superstep(self, superstep: int) -> None:
        if (
            not isinstance(superstep, int)
            or isinstance(superstep, bool)
            or superstep < 0
        ):
            raise LangGraphAssuranceError(
                INPUT_MARKER, "superstep must be a non-negative integer"
            )
    ...
    def _state_reads_and_writes(
        self, before: dict[str, Any], after: dict[str, Any]
    ) -> tuple[list[dict[str, object]], list[dict[str, object]]]:
        reads: list[dict[str, object]] = []
        writes: list[dict[str, object]] = []
        for key in sorted(set(before) | set(after)):
            prior_version, prior_hash = self._superstep_snapshot.get(
                key, (0, _sha(_MISSING))
            )
            ...
            if key in self._superstep_snapshot and before_hash != prior_hash:
                raise LangGraphAssuranceError(
                    INPUT_MARKER,
                    f"before_state does not match recorded version for {key}",
                )
    ...
    def _effects(side_effects: Iterable[Mapping[str, object]]) -> list[dict[str, str]]:
        ...
        for index, effect in enumerate(side_effects):
            if not isinstance(effect, Mapping) or set(effect) != {
                ...
                raise LangGraphAssuranceError(
                    INPUT_MARKER,
                    f"side_effects[{index}] must contain effect_id, idempotency_key, and status",
                )
            normalized.append(
                {
                    ...
                    "status": _identifier(
                        effect["status"], f"side_effects[{index}].status"
                    ),
                }
            )
        return sorted(
            normalized, key=lambda item: (item["effect_id"], item["idempotency_key"])
        )
    ...
    def _decision(decision: Mapping[str, object]) -> dict[str, str]:
        if not isinstance(decision, Mapping) or set(decision) != {"route", "reason"}:
            raise LangGraphAssuranceError(
                INPUT_MARKER, "decision must contain exactly route and reason"
            )
        return {
    ...
    def record_transition(
        ...
        *,
        ...
        return {
            ...
            "node_id": node,
            "state_keys": sorted(set(before) | set(after)),
        }
    ...
    def seal(self, root: Path, out: Path | str) -> dict[str, object]:
        ...
        if not workspace.is_dir():
            raise LangGraphAssuranceError(
                INPUT_MARKER, "workspace root must be an existing directory"
            )
        destination = _inside(workspace, out, "out", must_exist=False)
        if not self._steps:
            raise LangGraphAssuranceError(
                INPUT_MARKER, "at least one transition is required before sealing"
            )
        temporary: Path | None = None
        try:
            ...
            with tempfile.NamedTemporaryFile(
                ...
                prefix=".langgraph-steps.",
                suffix=".json",
                delete=False,
            ) as handle:
                json.dump(
                    {"steps": self._steps}, handle, ensure_ascii=False, sort_keys=True
                )
                temporary = Path(handle.name)
            sealed = seal_graph_lineage(
                self.run_id, self.graph_id, temporary, destination
            )
...
def _anomaly_cone(
    steps: list[dict[str, Any]], anomaly: dict[str, Any]
    ...
    if not isinstance(sequence, int):
        ...
        node = (
            writers[0] if isinstance(writers, list) and writers else "parallel-writers"
        )
        sequence = next(
            (step["sequence"] for step in steps if step["node_id"] == node), 1
        )
    start = next(
        (index for index, step in enumerate(steps) if step["sequence"] == sequence), 0
    )
    keys = (
        {str(anomaly["state_key"])}
        if isinstance(anomaly.get("state_key"), str)
        else set()
    )
    sequences: list[int] = []
    nodes: list[str] = []
...
def _incident_capsule(
    ...
    return {
        **core,
        ...
        "capsule_sha256": _sha(core),
    }


def _verified_comparison(
    reference_file: Path, resumed_file: Path
    ...
    try:
        ...
        if not reference["valid"] or not resumed["valid"]:
            raise GraphForensicsError(
                ...
                "reference and resumed lineages must both verify",
            )
        if reference["graph_id"] != resumed["graph_id"]:
            raise GraphForensicsError(
                "GRAPH_LINEAGE_GRAPH_MISMATCH",
                "reference and resumed graph_id values differ",
            )
        return (
            reference,
            resumed,
            graph_forensics(reference_file, resumed_file),
            graph_forensics(reference_file, reference_file),
        )
    except GraphForensicsError as exc:
        raise LangGraphAssuranceError(INPUT_MARKER, str(exc)) from exc


def _assurance_payload(
    reference: dict[str, Any],
    resumed: dict[str, Any],
    ...
    core = {
        ...
        "reference": {
            ...
            "lineage_sha256": reference["lineage_sha256"],
        },
        "resumed": {
            "run_id": resumed["run_id"],
            "lineage_sha256": resumed["lineage_sha256"],
        },
        "first_divergence": divergence,
        "anomalies": anomalies,
        "recovery_plan": recovery,
...
def verify_langgraph_resume_parity(
    ...
    destination = (
        _inside(workspace, out, "out", must_exist=False) if out is not None else None
    )
    reference, resumed, comparison, baseline = _verified_comparison(
        reference_file, resumed_file
    )
    ...
    if destination is not None:
        ...
        payload = {
            **payload,
            "path": str(destination.relative_to(workspace).as_posix()),
        }
    return payload
