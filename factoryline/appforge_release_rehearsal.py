...
CHANNELS = frozenset(
    {"testflight_internal", "testflight_external", "app_store", "beta_distribution"}
)
STAGES = (
    "local_readiness",
    "archive_export",
    "upload",
    "provider_processing",
    "tester_group_assignment",
    "tester_invitation_readback",
...
def _local(root: Path, path: Path, *, exists: bool = True) -> Path:
    ...
    try:
        ...
        raise RevenueForgeError(
            "APPFORGE_REHEARSAL_PATH_REJECTED", "paths must remain inside the workspace"
        ) from exc
    if exists and not target.is_file():
        raise RevenueForgeError(
            "APPFORGE_REHEARSAL_INPUT_UNAVAILABLE",
...
def _read_json(root: Path, path: Path, schema: str) -> tuple[dict[str, Any], Path]:
    ...
    if source.stat().st_size > MAX_BYTES:
        raise RevenueForgeError(
            "APPFORGE_REHEARSAL_INPUT_TOO_LARGE", "JSON input exceeds 1 MiB"
        )
    try:
        value = json.loads(source.read_text(encoding="utf-8-sig"))
    except (OSError, UnicodeDecodeError, json.JSONDecodeError) as exc:
        raise RevenueForgeError(
            "APPFORGE_REHEARSAL_INPUT_INVALID", "input must be valid JSON"
        ) from exc
    if not isinstance(value, dict) or value.get("schema") != schema:
        raise RevenueForgeError(
            "APPFORGE_REHEARSAL_SCHEMA_REJECTED", f"expected {schema}"
        )
    return value, source
...
def _text(value: object, field: str, *, limit: int = 160) -> str:
    ...
    if not result or len(result) > limit:
        raise RevenueForgeError(
            "APPFORGE_REHEARSAL_PROFILE_INVALID",
...
def _cider_manifest(root: Path, config: dict[str, Any]) -> dict[str, str]:
    if set(config) != {"manifest_path"}:
        raise RevenueForgeError(
            ...
            "Cider provider_config may contain only manifest_path",
        )
    declared = _text(
        config.get("manifest_path"), "provider_config.manifest_path", limit=512
    )
    path = _local(root, Path(declared))
    if path.suffix.lower() not in {".yaml", ".yml"}:
        raise RevenueForgeError(
            "APPFORGE_REHEARSAL_PROVIDER_INVALID",
            "Cider manifest_path must reference a YAML file",
        )
    if path.stat().st_size > MAX_BYTES:
        raise RevenueForgeError(
            "APPFORGE_REHEARSAL_INPUT_TOO_LARGE", "Cider manifest exceeds 1 MiB"
        )
    try:
        text = path.read_text(encoding="utf-8-sig")
    except (OSError, UnicodeDecodeError) as exc:
        raise RevenueForgeError(
            "APPFORGE_REHEARSAL_INPUT_INVALID", "Cider manifest must be UTF-8 text"
        ) from exc
    if re.search(
        ...
        raise RevenueForgeError(
            ...
            "Cider manifest must not contain credential-like keys",
        )
    return {
...
def _swiftlane_workflow(root: Path, config: dict[str, Any]) -> dict[str, Any]:
    if set(config) != {"workflow_path"}:
        raise RevenueForgeError(
            "APPFORGE_REHEARSAL_PROVIDER_INVALID",
            "Swiftlane provider_config may contain only workflow_path",
        )
    declared = _text(
        config.get("workflow_path"), "provider_config.workflow_path", limit=512
    )
    path = _local(root, Path(declared))
    if path.suffix.lower() != ".swift":
        raise RevenueForgeError(
            "APPFORGE_REHEARSAL_PROVIDER_INVALID",
            "Swiftlane workflow_path must reference a Swift file",
        )
    if path.stat().st_size > MAX_BYTES:
        raise RevenueForgeError(
            "APPFORGE_REHEARSAL_INPUT_TOO_LARGE", "Swiftlane workflow exceeds 1 MiB"
        )
    try:
        text = path.read_text(encoding="utf-8-sig")
    except (OSError, UnicodeDecodeError) as exc:
        raise RevenueForgeError(
            "APPFORGE_REHEARSAL_INPUT_INVALID", "Swiftlane workflow must be UTF-8 text"
        ) from exc
    steps = {
        "build": "Build(" in text,
        "test": "Test(" in text,
    ...
    if "Workflow(" not in text or not all(steps.values()):
        raise RevenueForgeError(
            "APPFORGE_REHEARSAL_PROVIDER_INVALID",
            "Swiftlane workflow must visibly declare Workflow, Build, Test, Archive, and ExportArchive stages",
        )
    return {
...
def _zealot_manifest(
    root: Path, config: dict[str, Any], candidate: dict[str, str]
) -> dict[str, str]:
    if set(config) != {"manifest_path"}:
        raise RevenueForgeError(
            "APPFORGE_REHEARSAL_PROVIDER_INVALID",
            "Zealot provider_config may contain only manifest_path",
        )
    declared = _text(
        config.get("manifest_path"), "provider_config.manifest_path", limit=512
    )
    ...
    if (
        ...
        raise RevenueForgeError(
            "APPFORGE_REHEARSAL_CANDIDATE_MISMATCH",
            "beta-distribution manifest must bind exactly the release candidate",
        )
    platform = _text(manifest.get("platform"), "platform", limit=16)
    if platform not in {"ios", "android", "macos", "windows", "linux"}:
        raise RevenueForgeError(
            "APPFORGE_REHEARSAL_PROVIDER_INVALID",
            "beta-distribution platform is unsupported",
        )
    artifact = manifest.get("artifact")
    distribution = manifest.get("distribution")
    if (
        not isinstance(artifact, dict)
        or set(artifact) != {"sha256"}
        or not isinstance(distribution, dict)
        or set(distribution) != {"channel", "audience_ref"}
    ):
        raise RevenueForgeError(
            "APPFORGE_REHEARSAL_PROVIDER_INVALID",
            "beta-distribution manifest has an invalid artifact or audience declaration",
        )
...
def _profile(
    ...
    if _secret_keys(profile):
        raise RevenueForgeError(
            "APPFORGE_REHEARSAL_SECRET_IN_PROFILE",
            "automation profile must not include credential-like keys",
        )
    if set(profile) != {
        "schema",
        "candidate",
        ...
        raise RevenueForgeError(
            "APPFORGE_REHEARSAL_PROFILE_INVALID",
            "automation profile has unsupported fields",
        )
    if profile.get("candidate") != candidate:
        raise RevenueForgeError(
            "APPFORGE_REHEARSAL_CANDIDATE_MISMATCH",
            "automation profile candidate must exactly match the release candidate",
        )
    provider = _text(profile.get("provider"), "provider", limit=32)
    if provider not in PROVIDERS:
        raise RevenueForgeError(
            "APPFORGE_REHEARSAL_PROVIDER_REJECTED",
            "provider must be fastlane, asc_cli, cider, swiftlane, or zealot",
        )
    channel = _text(profile.get("release_channel"), "release_channel", limit=32)
    if channel not in CHANNELS:
        raise RevenueForgeError(
            "APPFORGE_REHEARSAL_CHANNEL_INVALID", "release_channel is unsupported"
        )
    ...
    if provider == "cider":
        return {
            ...
            **_cider_manifest(root, config),
        }
    if provider == "swiftlane":
        return {
            "provider": provider,
            "release_channel": channel,
            **_swiftlane_workflow(root, config),
        }
    if provider == "zealot":
        if channel != "beta_distribution":
            raise RevenueForgeError(
                "APPFORGE_REHEARSAL_CHANNEL_INVALID",
                "Zealot rehearsal requires beta_distribution channel",
            )
        return {
            "provider": provider,
            "release_channel": channel,
...
def _validate_matrix(matrix: list[dict[str, str]], channel: str) -> None:
    ...
    if len(matrix) != len(STAGES) or [item.get("stage") for item in matrix] != list(
        STAGES
    ):
        raise RevenueForgeError(
            "APPFORGE_REHEARSAL_STATE_INVALID",
            "release rehearsal must contain the fixed ordered state matrix",
        )
    statuses = {str(item["stage"]): item.get("status") for item in matrix}
    if statuses["local_readiness"] != "ready":
        raise RevenueForgeError(
            "APPFORGE_REHEARSAL_STATE_INVALID", "local readiness must be ready"
        )
    if any(
        status == "ready"
        for stage, status in statuses.items()
        if stage != "local_readiness"
    ):
        raise RevenueForgeError(
            "APPFORGE_REHEARSAL_STATE_INVALID",
            "external provider stages cannot be marked ready by a local rehearsal",
        )
    ...
    actual_not_applicable = {
        stage for stage, status in statuses.items() if status == "not_applicable"
    }
    if actual_not_applicable != expected_not_applicable:
        raise RevenueForgeError(
            "APPFORGE_REHEARSAL_CHANNEL_INVALID",
            "release channel does not preserve its required provider-state boundaries",
        )
...
def create_release_rehearsal(
    ...
    assurance, assurance_source = _read_json(
        workspace, assurance_path, ASSURANCE_SCHEMA
    )
    if not _sealed(assurance, ASSURANCE_SCHEMA):
        raise RevenueForgeError(
            "APPFORGE_REHEARSAL_ASSURANCE_TAMPERED",
            "submission assurance receipt is not hash-valid",
        )
    if assurance.get("ok") is not True or assurance.get("candidate") != candidate:
        raise RevenueForgeError(
            "APPFORGE_REHEARSAL_ASSURANCE_BLOCKED",
            "submission assurance must be ready and bound to the exact candidate",
        )
    ...
    core: dict[str, Any] = {
        ...
        "sources": {
            ...
            "submission_assurance": {
                "path": assurance_source.relative_to(workspace).as_posix(),
                "sha256": _file_sha(assurance_source),
                "receipt_sha256": assurance["receipt_sha256"],
            },
            "automation_profile": {
                "path": profile_source.relative_to(workspace).as_posix(),
                "sha256": _file_sha(profile_source),
            },
        },
        "provider": provider,
        "state_matrix": matrix,
    ...
    if target.exists():
        raise RevenueForgeError(
            "APPFORGE_REHEARSAL_OUTPUT_EXISTS",
            "sealed rehearsal destination already exists",
        )
    ...
    if base.exists():
        for path in sorted(base.rglob("*release-rehearsal*.json"))[:100]:
            try:
                ...
                invalid.append(path.relative_to(workspace).as_posix())
    return {
        "schema": "factory.appforge.release-rehearsal-projection.v1",
        "marker": "APPFORGE_RELEASE_REHEARSAL_READ_ONLY",
        ...
        "claim_boundary": "Read-only local rehearsal status; not Fastlane/ASC execution, provider state, TestFlight delivery, App Review submission, or Apple approval.",
    }
