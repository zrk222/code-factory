...
def add_parser(sub: Any) -> None:
    ...
    proof_card = sub.add_parser(
        "proof-card", help="create a privacy-safe share card from one verified local E2E receipt"
    )
    ...
    adoption_status = adoption_sub.add_parser("status", help="read local aggregate activation milestones")
    ...
    adoption_record.add_argument("milestone", choices=["first_proof_completed", "seven_day_return"])
...
def run(args: Any) -> int:
    ...
    if args.cmd == "proof-card":
        from .adoption import AdoptionError, proof_card_from_receipt, record_adoption_event

        workspace = Path(args.root).resolve()
        try:
            result = proof_card_from_receipt(workspace, Path(args.receipt), Path(args.out_dir))
            ...
            print(json.dumps(error, indent=2, sort_keys=True) if args.json else f"proof card failed: {code}: {exc}", file=sys.stderr)
            return 2
        if args.json:
    ...
    if args.cmd == "adoption":
        from .adoption import AdoptionError, adoption_status, export_adoption_status, record_adoption_event

        workspace = Path(args.root).resolve()
        try:
            if args.adoption_cmd == "record":
                result = record_adoption_event(workspace, args.milestone, evidence_sha256=args.evidence_sha256)
            ...
            print(json.dumps(error, indent=2, sort_keys=True) if args.json else f"adoption failed: {code}: {exc}", file=sys.stderr)
            return 2
        if args.json:
            print(json.dumps(result, indent=2, sort_keys=True))
        else:
    ...
    if args.cmd == "counterexample":
        ...
        try:
            ...
            print(json.dumps(error, indent=2, sort_keys=True) if args.json else f"counterexample failed: {exc.code}: {exc}", file=sys.stderr)
            return 2
        if args.json:
    ...
    if args.cmd == "guardrail":
        ...
        try:
            if args.guardrail_cmd == "evaluate":
                principal = continuity_principal_from_args(args.subject, args.tenant, args.roles.split(","), args.purposes.split(","))
                payload = evaluate_guardrails(Path(args.manifest), Path(args.db), principal, changed_paths=args.changed)
            else:
                payload = verify_guardrail_evaluation(json.loads(Path(args.evaluation).read_text(encoding="utf-8")))
            ...
            print(json.dumps(error, indent=2, sort_keys=True) if args.json else f"guardrail failed: {error['code']}: {exc}", file=sys.stderr)
            return 2
        if args.json:
            print(json.dumps(payload, indent=2, sort_keys=True))
        else:
    ...
    try:
        ...
        print(json.dumps(error, indent=2, sort_keys=True) if args.json else f"resilience failed: {exc.code}: {exc}", file=sys.stderr)
        return 2
    if args.json:
        print(json.dumps(payload, indent=2, sort_keys=True))
    else:
