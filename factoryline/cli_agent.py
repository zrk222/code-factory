...
def add_parser(sub: Any) -> None:
    ...
    contract = agent_sub.add_parser("contract", help="validate one hash-bound Core-5 contract")
    ...
    plan = agent_sub.add_parser(
        "plan", help="compile a human-gated request-routing plan without dispatching work"
    )
    ...
    extended = agent_sub.add_parser(
        "extended", help="validate optional lanes 7-8 from supplied evidence into Receipt v2"
    )
    extended.add_argument("feature")
    extended.add_argument("--root", default=".")
    extended.add_argument("--evidence", help="JSON evidence object for the two extended lanes")
    ...
    extended.add_argument("--verify", help="verify an existing Receipt v2 payload instead of building one")
...
def run(args: Any) -> dict[str, Any]:
    ...
    if args.agent_cmd in {"contract", "attestation"}:
        from .agent_contract import validate_agent_contract, validate_verifier_attestation
