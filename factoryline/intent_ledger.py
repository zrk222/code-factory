...
def _text(value: object, field: str, *, maximum: int = MAX_TEXT) -> str:
    if not isinstance(value, str):
        raise IntentLedgerError(
            "INTENT_LEDGER_INPUT_INVALID", f"{field} must be a non-empty string"
        )
    result = " ".join(value.replace("\x00", " ").split())
    if not result or len(result) > maximum:
        raise IntentLedgerError(
            "INTENT_LEDGER_INPUT_INVALID",
...
def _clear_intent(value: str, field: str, *, require_action: bool = True) -> str:
    ...
    try:
        ...
        raise IntentLedgerError(
            "INTENT_LEDGER_INTENT_UNCLEAR", f"{field}: {exc.message}"
        ) from exc
...
def _changed_path(value: object) -> str:
    if not isinstance(value, str):
        raise IntentLedgerError(
            "INTENT_LEDGER_PATH_INVALID", "changed paths must be strings"
        )
    ...
    if (
        ...
        or ".." in result.split("/")
        or not _PATH.fullmatch(result)
    ):
        raise IntentLedgerError(
            "INTENT_LEDGER_PATH_INVALID",
...
def _paths(value: object) -> list[str]:
    if not isinstance(value, list) or not value:
        raise IntentLedgerError(
            "INTENT_LEDGER_PATH_INVALID",
            "changed must contain at least one workspace-relative path",
        )
    result = sorted({_changed_path(item) for item in value})
    if len(result) > MAX_CHANGED_PATHS:
        raise IntentLedgerError(
            "INTENT_LEDGER_PATH_LIMIT",
...
def _directory(root: Path, *, create: bool) -> Path:
    ...
    if not workspace.is_dir():
        raise IntentLedgerError(
            "INTENT_LEDGER_ROOT_INVALID",
            "root must name an existing workspace directory",
        )
...
def validate_intent_ledger_record(value: object) -> dict[str, Any]:
    ...
    if (
        ...
        or len(scope_limits) != 2
        or not all(isinstance(item, str) and item for item in scope_limits)
    ):
        raise IntentLedgerError(
...
def capture_intent_ledger(
    ...
    *,
    ...
    if confirmation != expected_confirmation:
        raise IntentLedgerError(
            "INTENT_LEDGER_CONFIRMATION_REQUIRED",
            f"confirmation must equal {expected_confirmation!r}",
        )
    paths = _paths(changed)
    timestamp = (
        (captured_at or datetime.now(timezone.utc)).astimezone(timezone.utc).isoformat()
    )
    core = _record_core(
        ...
        non_goal=_clear_intent(
            _text(non_goal, "non_goal"), "non_goal", require_action=False
        ),
...
def _latest_record(
    ...
    try:
        ...
        if size > MAX_RECORD_BYTES:
            raise IntentLedgerError(
                "INTENT_LEDGER_INVALID",
                f"Intent Ledger record exceeds {MAX_RECORD_BYTES} bytes",
            )
        ...
        if record["change_list"] != change_list:
            raise IntentLedgerError(
                "INTENT_LEDGER_INVALID",
                "Intent Ledger record change list does not match its file selector",
            )
        ...
        code = (
            exc.code if isinstance(exc, IntentLedgerError) else "INTENT_LEDGER_INVALID"
        )
    ...
    *,
    ...
    if invalid_code:
        finding = _state(
            "intent_ledger_invalid",
            "blocking",
            "The newest matching Intent Ledger record is malformed or tampered; no fallback record was used.",
            rejected_artifact=record_path,
        ...
        return _inspection(
            ...
            record=None,
            record_path=record_path,
            state="intent_ledger_invalid",
            findings=[finding],
            next_action=_next(
                "repair_intent_ledger_record",
                finding["message"],
                rejected_artifact=record_path,
            ),
            review={"available": False, "reason": "intent_ledger_invalid"},
            markers=[
                "INTENT_LEDGER_INVALID",
                "INTENT_LEDGER_FAIL_CLOSED",
                "INTENT_LEDGER_NO_EXECUTION",
            ],
            scope_limits=shared_limits,
        )
    if record is None:
        finding = _state(
            "uncontracted",
            "required",
            "No intact Intent Ledger record exists for this Change List; FactoryLine did not infer a behavioral promise.",
        )
        return _inspection(
            workspace=workspace,
            change_list=selected_change_list,
            ...
            findings=[finding],
            next_action=_next(
                "capture_intent", finding["message"], change_list=selected_change_list
            ),
            review={"available": False, "reason": "uncontracted"},
            markers=["INTENT_LEDGER_UNCONTRACTED", "INTENT_LEDGER_NO_EXECUTION"],
            scope_limits=shared_limits,
        )
    try:
        review_value = review_change(
            workspace, base=base, changed=changed if explicit else None
        )
        ...
        finding = _state(
            ...
            "blocking",
            "The current change set could not be mapped to proof; FactoryLine did not select a proof by guesswork.",
        ...
        return _inspection(
            ...
            record=record,
            record_path=record_path,
            state="change_review_unavailable",
            findings=[finding],
            next_action=_next(
                "inspect_change_set", finding["message"], failure_code=exc.code
            ),
            review={"available": False, "failure_code": exc.code, "message": str(exc)},
            markers=[
                "INTENT_LEDGER_CHANGE_REVIEW_UNAVAILABLE",
                "INTENT_LEDGER_NO_EXECUTION",
            ],
            scope_limits=shared_limits,
        )
    ...
    review_summary = {
        ...
        "stale_proof_ids": [
            item["proof_id"] for item in review_value["impact"]["rerun_proofs"]
        ],
    }
    if scope_escape_paths:
        finding = _state(
            "scope_escape",
            "blocking",
            "Current Change List paths exceed the declared Intent Ledger scope.",
            paths=scope_escape_paths,
        )
        return _inspection(
            workspace=workspace,
            change_list=selected_change_list,
            ...
            findings=[finding],
            next_action=_next(
                "amend_or_split_change_list",
                finding["message"],
                paths=scope_escape_paths,
            ),
            review=review_summary,
            markers=[
                "INTENT_LEDGER_SCOPE_ESCAPE",
                "INTENT_LEDGER_CHANGE_REVIEW_EXACT",
                "INTENT_LEDGER_NO_EXECUTION",
            ],
            scope_limits=shared_limits,
        )
    ...
    if stale_proof_ids:
        ...
        return _inspection(
            ...
            findings=[finding],
            next_action=_next(
                "rerun_stale_proof", finding["message"], proof_id=stale_proof_ids[0]
            ),
            review=review_summary,
            markers=[
                "INTENT_LEDGER_STALE_PROOF",
                "INTENT_LEDGER_CHANGE_REVIEW_EXACT",
                "INTENT_LEDGER_NO_EXECUTION",
            ],
            scope_limits=shared_limits,
    ...
    if review_summary["coverage_complete"] is False:
        ...
        finding = _state(
            "coverage_incomplete",
            "required",
            "Requirement coverage is absent or incomplete for this change set.",
        ...
        return _inspection(
            ...
            review=review_summary,
            markers=[
                "INTENT_LEDGER_COVERAGE_INCOMPLETE",
                "INTENT_LEDGER_CHANGE_REVIEW_EXACT",
                "INTENT_LEDGER_NO_EXECUTION",
            ],
            scope_limits=shared_limits,
    ...
    finding = _state(
        ...
        "review",
        "No declared local scope, stale-proof, or coverage gap was found.",
    )
    return _inspection(
        workspace=workspace,
        change_list=selected_change_list,
        ...
        findings=[finding],
        next_action=_next("review_packet", finding["message"]),
        review=review_summary,
        markers=[
            "INTENT_LEDGER_READY_FOR_HUMAN_REVIEW",
            "INTENT_LEDGER_CHANGE_REVIEW_EXACT",
            "INTENT_LEDGER_NO_EXECUTION",
        ],
        scope_limits=shared_limits,
