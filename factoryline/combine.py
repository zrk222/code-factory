...
def _task_input(value: dict[str, Any]) -> dict[str, Any]:
    if (
        set(value) != {"schema", "id", "description", "agents"}
        or value.get("schema") != COMBINE_TASK_SCHEMA
    ):
        raise CombineError(
            "COMBINE_TASK_INVALID",
            f"task must use {COMBINE_TASK_SCHEMA} and contain schema, id, description, and agents",
        )
    task_id = _text(value.get("id"), "id", maximum=96)
    if not _TASK_IDENTIFIER.fullmatch(task_id):
        raise CombineError(
            "COMBINE_TASK_INVALID",
            "task id must use lowercase letters, digits, and hyphens",
        )
    ...
    if not isinstance(raw_agents, list) or not 2 <= len(raw_agents) <= 8:
        raise CombineError(
            "COMBINE_TASK_INVALID",
            "agents must contain 2 through 8 declared identities",
        )
    agents = [
        normalize_agent_identity(item, f"agents[{index}]")
        for index, item in enumerate(raw_agents)
    ]
    hashes = [item["identity_sha256"] for item in agents]
    if len(set(hashes)) != len(hashes):
        raise CombineError(
            "COMBINE_TASK_INVALID", "agents must be unique declared identities"
        )
    return {
        "schema": COMBINE_TASK_SCHEMA,
        "id": task_id,
        "description": description,
        "agents": sorted(agents, key=lambda item: item["identity_sha256"]),
    }
...
def _task_core(value: dict[str, Any]) -> dict[str, Any]:
    ...
    if (
        set(value) != fields
        or value.get("schema") != COMBINE_TASK_SCHEMA
        or value.get("marker") != "COMBINE_TASK_SEALED"
    ):
    ...
    if [item["identity_sha256"] for item in normalized] != sorted(
        item["identity_sha256"] for item in normalized
    ):
        raise CombineError(
            "COMBINE_TASK_INVALID",
            "sealed task agents must be deterministically ordered",
        )
...
def seal_combine_task(
    root: Path, source_path: Path, *, out: Path | None = None
) -> dict[str, Any]:
    ...
    target = (
        Path(out) if out is not None else workspace / TASK_DIR / f"{task['id']}.json"
    )
    ...
    try:
        ...
        raise CombineError(
            "COMBINE_PATH_OUT_OF_SCOPE", "task output must remain inside workspace"
        ) from exc
    if target.exists():
        existing = _load_json(target)
        if existing.get("task_sha256") == sealed["task_sha256"]:
            return {
                "marker": "COMBINE_TASK_ALREADY_SEALED",
                "task": existing,
                "path": str(target.resolve()),
            }
        raise CombineError(
            "COMBINE_TASK_EXISTS",
            "task id is already bound to different immutable content",
        )
...
def _rank(rows: list[dict[str, Any]]) -> list[dict[str, Any]]:
    def key(row: dict[str, Any]) -> tuple[int, int, int, str]:
        ...
        severe = sum(
            item in {"hollow_test", "hollow_validator", "scope_escape"}
            for item in failures
        )
        return (
            0 if row["result"]["passed"] else 1,
            severe,
            len(failures),
            row["agent"]["identity_sha256"],
        )
...
def _scoreboard_core(
    task: dict[str, Any], rows: list[dict[str, Any]], *, now: datetime
) -> dict[str, Any]:
    ranked = _rank(rows)
    counts = Counter(
        item for row in ranked for item in row["result"]["failure_classes"]
    )
    summary = {
        ...
        "failure_class_counts": {
            ...
            for key in sorted(item.value for item in FailureClass)
        },
        "unobserved": {
...
def _event_rows(
    root: Path, task: dict[str, Any], event_paths: list[Path] | None
) -> list[dict[str, Any]]:
    events: list[dict[str, Any]] = []
    if event_paths is None:
        events = [
            event
            for event in load_governed_runs(root)
        ...
        for source in event_paths:
            try:
                ...
                event = _validate_ledger_event(
                    root, _load_json(path), require_evidence_files=True
                )
    ...
    for event in events:
        if event.get("task_id") != task["task_id"]:
            raise CombineError(
                "COMBINE_TASK_MISMATCH",
                "each event must bind exactly this sealed task id",
            )
        identity = event["agent"]["identity_sha256"]
        if identity not in expected:
            raise CombineError(
                "COMBINE_UNDECLARED_AGENT", "event agent is absent from the sealed task"
            )
        if identity in selected:
            raise CombineError(
                "COMBINE_DUPLICATE_RESULT",
                "exactly one event is allowed per declared agent",
            )
        selected[identity] = event
    missing = sorted(set(expected) - set(selected))
    if missing:
        raise CombineError(
            "COMBINE_EVENT_MISSING",
            "one governed event is required for each declared agent",
        )
        ...
        rows.append(
            {
                ...
                "event": {
                    ...
                    "verification_receipt_sha256": event["verification"]["receipt"][
                        "sha256"
                    ],
                },
                "result": {
                    "passed": event["passed"],
                    "failure_classes": event["failure_classes"],
...
def score_combine(
    ...
    *,
    ...
    target = (
        ...
        / SCOREBOARD_DIR
        / f"{task['task_id']}-{scoreboard['scoreboard_sha256'][:12]}.json"
    )
    target = target if target.is_absolute() else workspace / target
    try:
        target.resolve().relative_to(workspace)
        ...
        raise CombineError(
            ...
            "scoreboard output must remain inside workspace",
        ) from exc
...
def verify_combine_scoreboard(path: Path) -> dict[str, Any]:
    ...
    try:
        ...
        return {
            "schema": COMBINE_SCOREBOARD_SCHEMA,
            "marker": "COMBINE_SCOREBOARD_INVALID",
            "ok": False,
            "reason": getattr(exc, "code", "COMBINE_SCOREBOARD_INVALID"),
        }
    return {
        "schema": COMBINE_SCOREBOARD_SCHEMA,
        "marker": "COMBINE_SCOREBOARD_VERIFIED",
        "ok": True,
        "scoreboard_sha256": value["scoreboard_sha256"],
        "signature": "not_supplied",
    }


def seal_combine_scoreboard(
    path: Path,
    *,
    ...
    if not verified["ok"]:
        raise CombineError(
            "COMBINE_SIGNING_FAILED", "scoreboard must verify before sealing"
        )
    ...
    try:
        ...
        payload = {
            ...
            "module": "combine",
            "stage": "scoreboard",
            "feature": scoreboard["task"]["task_id"],
            "ok": True,
            "tenant_id": _text(tenant_id, "tenant_id"),
            "run_id": scoreboard["scoreboard_sha256"][:32],
            "ts": _iso(_now()),
            "subject_sha256": scoreboard["scoreboard_sha256"],
        }
    ...
    return {
        "schema": COMBINE_SCOREBOARD_SCHEMA,
        "marker": "COMBINE_SCOREBOARD_SEALED",
...
def combine_projection(root: Path) -> dict[str, Any]:
                ...
                "candidates": [
                    {
                        ...
                        "failure_classes": row["result"]["failure_classes"],
                    }
    ...
    return {
        "marker": "COMBINE_STATUS_READ_ONLY",
        "available": bool(scoreboards),
        "scoreboards": scoreboards,
        "authority": dict(_AUTHORITY),
        "scope_limits": [
            "Projection does not execute agent commands or use scores as a vendor-quality claim."
        ],
    }
