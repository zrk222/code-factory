    ...
    *,
    ...
    if missing_faults:
        return lane_result(
            ...
            details={"missing": missing_faults},
        )
    ...
    if artifact["max_concurrency"] > len(operations):
        return lane_result(
            ...
            details={
                "max_concurrency": artifact["max_concurrency"],
    ...
    for operation in operations:
        if not isinstance(operation, dict):
            return lane_result(
                ...
                "An operation record is malformed.",
            )
    ...
    if not any(count >= 2 for count in attempts.values()):
        return lane_result(
            "failure_recovery",
            "INCOMPLETE",
            "RECOVERY_DUPLICATE_NOT_EXERCISED",
            "No idempotency key was replayed.",
        )
    ...
    for condition in conditions:
        if not isinstance(condition, dict):
            return lane_result(
                "failure_recovery",
                "INCOMPLETE",
                "RECOVERY_CONDITION_INVALID",
                "A postcondition is malformed.",
            )
