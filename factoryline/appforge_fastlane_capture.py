...
def _local(root: Path, path: Path, *, exists: bool = True) -> Path:
    ...
    try:
        ...
        raise RevenueForgeError(
            "APPFORGE_FASTLANE_CAPTURE_PATH_REJECTED",
            "paths must remain inside the workspace",
        ) from exc
    if exists and not target.is_file():
        raise RevenueForgeError(
            "APPFORGE_FASTLANE_CAPTURE_INPUT_UNAVAILABLE",
...
def _read_json(root: Path, path: Path, schema: str) -> tuple[dict[str, Any], Path]:
    ...
    if source.stat().st_size > MAX_BYTES:
        raise RevenueForgeError(
            "APPFORGE_FASTLANE_CAPTURE_INPUT_TOO_LARGE", "JSON input exceeds 1 MiB"
        )
    try:
        value = json.loads(source.read_text(encoding="utf-8-sig"))
    except (OSError, UnicodeDecodeError, json.JSONDecodeError) as exc:
        raise RevenueForgeError(
            "APPFORGE_FASTLANE_CAPTURE_INPUT_INVALID", "input must be valid JSON"
        ) from exc
    if not isinstance(value, dict) or value.get("schema") != schema:
        raise RevenueForgeError(
            "APPFORGE_FASTLANE_CAPTURE_SCHEMA_REJECTED", f"expected {schema}"
        )
    return value, source
...
def _read_text(root: Path, path_value: object, field: str) -> tuple[Path, str]:
    ...
    if path.stat().st_size > MAX_BYTES:
        raise RevenueForgeError(
            "APPFORGE_FASTLANE_CAPTURE_INPUT_TOO_LARGE", f"{field} exceeds 1 MiB"
        )
    try:
        return path, path.read_text(encoding="utf-8-sig")
    except (OSError, UnicodeDecodeError) as exc:
        raise RevenueForgeError(
            "APPFORGE_FASTLANE_CAPTURE_INPUT_INVALID", f"{field} must be UTF-8 text"
        ) from exc
...
def _source_has_secret(text: str) -> bool:
    ...
    for raw_line in text.splitlines():
        ...
        if not any(hint in line for hint in _SECRET_HINTS):
            continue
        if re.search(
            r"(?:\b(?:token|password|secret|private_?key|api_?key|credential)\b\s*(?::|=|\())|\benv\s*\[",
            line,
        ):
            return True
    return False
...
def _lane_body(fastfile: str, lane: str) -> str:
    ...
    starts = re.compile(
        r"\b(?:do|if|unless|case|begin|while|until|for|def|class|module)\b"
    )
    ...
    raise RevenueForgeError(
        ...
        "Fastfile must contain one balanced declared capture-only lane",
    )


def _validate_snapfile(text: str) -> dict[str, bool]:
    checks = {
        "devices": bool(re.search(r"(?is)devices\s*\(.*?iPhone.*?iPad.*?\)", text))
        and bool(re.search(r"(?i)iPad[^\n\"]*13|13[^\n\"]*iPad", text)),
        "languages": bool(
            re.search(r"(?is)languages\s*\(.*?(?:en-US|en_US).*?\)", text)
        ),
        ...
        "clear_previous_screenshots": bool(
            re.search(r"(?i)clear_previous_screenshots\s*\(\s*true\s*\)", text)
        ),
        "override_status_bar": bool(
            re.search(r"(?i)override_status_bar\s*\(\s*true\s*\)", text)
        ),
        "stop_after_first_error": bool(
            re.search(r"(?i)stop_after_first_error\s*\(\s*true\s*\)", text)
        ),
    }
    if not all(checks.values()):
        missing = ", ".join(key for key, value in checks.items() if not value)
        raise RevenueForgeError(
            "APPFORGE_FASTLANE_CAPTURE_SNAPFILE_INVALID",
            f"Snapfile is missing required deterministic capture settings: {missing}",
        )
    return checks
...
def _validate_contract(
    ...
    expected = {
        "schema",
        "candidate",
        "surface_matrix_receipt_sha256",
        "storefront_story_receipt_sha256",
        "fastlane",
        "captures",
    }
    if (
        ...
        or value.get("surface_matrix_receipt_sha256") != matrix_sha
        or value.get("storefront_story_receipt_sha256") != story_sha
    ):
        raise RevenueForgeError(
            "APPFORGE_FASTLANE_CAPTURE_CONTRACT_INVALID",
...
def _validated_fastlane(value: object) -> dict[str, Any]:
    ...
    if not isinstance(fastlane, dict) or set(fastlane) != {
        ...
        raise RevenueForgeError(
            "APPFORGE_FASTLANE_CAPTURE_CONTRACT_INVALID",
            "fastlane must contain only declared capture inputs",
        )
    lane = _text(fastlane.get("capture_lane"), "fastlane.capture_lane", 80)
    if not re.fullmatch(r"[A-Za-z][A-Za-z0-9_]*", lane):
        raise RevenueForgeError(
            "APPFORGE_FASTLANE_CAPTURE_CONTRACT_INVALID",
            "fastlane.capture_lane must be alphanumeric with underscores",
        )
...
def _capture_contract_records(
    ...
    if not isinstance(captures, list) or not captures or len(captures) > 20:
        raise RevenueForgeError(
            "APPFORGE_FASTLANE_CAPTURE_CONTRACT_INVALID",
            "captures must contain 1-20 named outputs",
        )
...
def _capture_source_texts(
    ...
    fastfile_path, fastfile = _read_text(
        root, fastlane["fastfile_path"], "fastlane.fastfile_path"
    )
    ui_test_path, ui_test = _read_text(
        root, fastlane["ui_test_path"], "fastlane.ui_test_path"
    )
...
def _validate_ui_test(ui_test: str, captures: list[dict[str, str]]) -> None:
    if (
        ...
        or re.search(r"continueAfterFailure\s*=\s*true", ui_test)
    ):
        raise RevenueForgeError(
            "APPFORGE_FASTLANE_CAPTURE_UI_TEST_INVALID",
            "UI test must set up Snapshot, launch the app, and stop after failure",
        )
    missing = [
        item["snapshot_name"]
        for item in captures
        if not re.search(
            rf"\bsnapshot\s*\(\s*[\"']{re.escape(item['snapshot_name'])}[\"']\s*\)",
            ui_test,
        )
    ]
    if missing:
...
def _capture_source_receipts(
    ...
    return {
        "snapfile": {
            ...
            "sha256": _file_sha(snapfile_path),
            "settings": settings,
        },
        "fastfile": {
        ...
        "ui_test": {
            "path": ui_test_path.relative_to(root).as_posix(),
            "sha256": _file_sha(ui_test_path),
            "sealed_snapshot_count": len(captures),
...
def create_fastlane_capture_contract(
    root: Path,
    candidate_path: Path,
    surface_matrix_path: Path,
    storefront_story_path: Path,
    contract_path: Path,
    out_path: Path,
) -> dict[str, Any]:
    ...
    if (
        not _sealed(matrix, MATRIX_SCHEMA, "APPFORGE_SURFACE_MATRIX_WRITTEN")
        or matrix.get("candidate") != candidate
    ):
        raise RevenueForgeError(
            "APPFORGE_FASTLANE_CAPTURE_MATRIX_INVALID",
            "surface matrix must be hash-valid and bound to the exact candidate",
        )
    if (
        not _sealed(story, STORY_SCHEMA, "APPFORGE_STOREFRONT_STORY_READY")
        or story.get("candidate") != candidate
    ):
        raise RevenueForgeError(
            "APPFORGE_FASTLANE_CAPTURE_STORY_INVALID",
            "storefront story must be hash-valid, ready, and bound to the exact candidate",
        )
    scenes = story.get("scenes")
    if not isinstance(scenes, list) or not scenes:
        raise RevenueForgeError(
            "APPFORGE_FASTLANE_CAPTURE_STORY_INVALID",
            "storefront story must contain reviewed scenes",
        )
    scene_keys = {
        (str(scene.get("set_id")), str(scene.get("capture_id")))
        for scene in scenes
    ...
    if len(scene_keys) != len(scenes):
        raise RevenueForgeError(
            "APPFORGE_FASTLANE_CAPTURE_STORY_INVALID",
            "storefront story scenes must be unique",
        )
    contract, contract_source = _read_json(workspace, contract_path, CONTRACT_SCHEMA)
    fastlane, captures = _validate_contract(
        contract,
        candidate,
        matrix["receipt_sha256"],
    ...
    core: dict[str, Any] = {
        ...
        "sources": {
            ...
            "surface_matrix": {
                "path": matrix_source.relative_to(workspace).as_posix(),
                "receipt_sha256": matrix["receipt_sha256"],
                "sha256": _file_sha(matrix_source),
            },
            "storefront_story": {
                "path": story_source.relative_to(workspace).as_posix(),
                "receipt_sha256": story["receipt_sha256"],
                "sha256": _file_sha(story_source),
            },
            "capture_contract": {
    ...
    if target.exists():
        raise RevenueForgeError(
            "APPFORGE_FASTLANE_CAPTURE_OUTPUT_EXISTS",
            "sealed capture receipt destination already exists",
        )
    ...
    if base.exists():
        for path in sorted(base.rglob("*fastlane-capture*.json"))[:100]:
            try:
                    ...
                    current.append(
                        {
                            ...
                            "capture_count": len(value.get("captures", [])),
                ...
                invalid.append(path.relative_to(workspace).as_posix())
    return {
        "schema": "factory.appforge.fastlane-capture-projection.v1",
        "marker": "APPFORGE_FASTLANE_CAPTURE_READ_ONLY",
        ...
        "claim_boundary": "Read-only local capture-plan status; not Fastlane execution, a simulator/device run, screenshots, framing, Store media, TestFlight, App Review, or Apple approval evidence.",
    }
