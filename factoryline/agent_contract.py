...
def _canonical(value: object) -> bytes:
    try:
        ...
        raise AgentContractError(
            "AGENT_CONTRACT_INVALID", f"contract is not canonical JSON: {exc}"
        ) from exc
...
def _text(value: Any, label: str, *, max_len: int = 200) -> str:
    if not isinstance(value, str) or not value.strip() or len(value) > max_len:
        raise AgentContractError(
            ...
            f"{label} must be a non-empty string of at most {max_len} characters",
        )
    return value.strip()
...
def _digest(value: Any, label: str) -> str:
    ...
    if len(value) < 16 or any(char not in "0123456789abcdef" for char in value.lower()):
        raise AgentContractError(
            "AGENT_CONTRACT_INVALID", f"{label} must be a hexadecimal digest"
        )
    return value.lower()
...
def _bounded_cost(value: Any) -> float:
    if (
        ...
        or value != value
        or value < 0
        or value > MAX_COST_USD
    ):
        raise AgentContractError(
            "AGENT_CONTRACT_RAILS_ENFORCED",
            f"max_cost_usd must be between 0 and {MAX_COST_USD:g}",
        )
...
def _strings(value: Any, label: str, *, allow_empty: bool = True) -> list[str]:
    if (
        ...
        raise AgentContractError(
            "AGENT_CONTRACT_INVALID", f"{label} must be a list of non-empty strings"
        )
...
def _object(value: Any, label: str) -> dict[str, Any]:
    ...
    if SECRET_KEYS.intersection(value):
        raise AgentContractError(
            "AGENT_CONTRACT_SECRET_FIELD", f"{label} must not contain credential fields"
        )
    return value
...
def _core5(value: dict[str, Any]) -> dict[str, Any]:
    ...
    if value.get("schema") != AGENT_CONTRACT_SCHEMA:
        raise AgentContractError(
            "AGENT_CONTRACT_SCHEMA_MISMATCH", f"schema must be {AGENT_CONTRACT_SCHEMA}"
        )
    ...
    if any(source.startswith(("http:", "https:")) for source in sources):
        raise AgentContractError(
            "AGENT_CONTRACT_CONTEXT_UNTRUSTED",
            "context.sources must be local, reviewed identifiers",
        )
    ...
    if privacy not in PRIVACY_CLASSES:
        raise AgentContractError(
            "AGENT_CONTRACT_INVALID",
            "model.privacy_class must be standard, restricted, or local_only",
        )
    ...
    if wall not in {"isolated", "shared_readonly"}:
        raise AgentContractError(
            "AGENT_CONTRACT_INVALID",
            "harness.context_wall must be isolated or shared_readonly",
        )
    ...
    normalized = {
        ...
        "context": {
            ...
            "max_tokens": _bounded_int(
                context.get("max_tokens"), "context.max_tokens", 1, MAX_TOKENS
            ),
            "sources": sources,
        },
        "model": {
            ...
            "max_latency_ms": _bounded_int(
                model.get("max_latency_ms"), "model.max_latency_ms", 1, MAX_LATENCY_MS
            ),
        ...
        "tools": {
            "allow": _strings(tools.get("allow"), "tools.allow"),
            "deny": _strings(tools.get("deny"), "tools.deny"),
        },
        "harness": {
            "context_wall": wall,
        ...
        "handoff": {
            ...
            "output_schema": _text(
                handoff.get("output_schema"), "handoff.output_schema"
            ),
        },
    }
    if set(normalized["tools"]["allow"]).intersection(normalized["tools"]["deny"]):
        raise AgentContractError(
            "AGENT_CONTRACT_TOOL_CONFLICT",
            "tools.allow and tools.deny must be disjoint",
        )
    if (
        normalized["harness"]["context_wall"] == "isolated"
        and "creator_scratchpad" in normalized["context"]["sources"]
    ):
        raise AgentContractError(
            "AGENT_CONTRACT_CONTEXT_WALL",
            "isolated context may not include creator scratchpad",
        )
    return normalized
...
def validate_agent_contract(value: Path | dict[str, Any]) -> dict[str, Any]:
    ...
    if isinstance(value, (str, Path)):
        ...
        try:
            ...
            raise AgentContractError(
                "AGENT_CONTRACT_INVALID", f"cannot read contract: {exc}"
            ) from exc
    if not isinstance(value, dict):
        raise AgentContractError(
            "AGENT_CONTRACT_INVALID", "contract must be a JSON object"
        )
    core = _core5(value)
    ...
    if supplied is not None and supplied != digest:
        raise AgentContractError(
            "AGENT_CONTRACT_DIGEST_MISMATCH",
            "contract_digest does not match canonical contract",
        )
    ...
    *,
    ...
    if isinstance(value, (str, Path)):
        try:
            ...
            raise AgentContractError(
                "VERIFIER_ATTESTATION_INVALID", f"cannot read attestation: {exc}"
            ) from exc
    if not isinstance(value, dict):
        raise AgentContractError(
    ...
    if (
        ...
        raise AgentContractError(
            "VERIFIER_CONTEXT_WALL", "verifier must use a fresh isolated context wall"
        )
    context = _strings(
        value.get("verifier_context"), "verifier_context", allow_empty=False
    )
    ...
    if mission_digest is not None and mission != mission_digest:
        raise AgentContractError(
            "VERIFIER_ATTESTATION_BINDING",
            "mission digest does not match the requested mission",
        )
    if contract_digest is not None and contract != contract_digest:
        raise AgentContractError(
            "VERIFIER_ATTESTATION_BINDING",
            "contract digest does not match the requested contract",
        )
    ...
    return {
        **core,
        "attestation_digest": sha256(_canonical(core)).hexdigest(),
        "markers": [ATTESTATION_MARKER],
    }
