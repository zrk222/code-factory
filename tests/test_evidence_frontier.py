...
from factoryline.evidence_frontier import (
...
def _request(
    root: Path, evaluation: Path, experiments: list[dict], maximum: int = 8
) -> Path:
    value = {
...
def _experiment(
    ...
    *,
    ...
    if elapsed is not None:
        ...
        receipt.write_text(
            json.dumps({"historical": True, "elapsed_ms": elapsed}), encoding="utf-8"
        )
        measurement = {
            "elapsed_ms": elapsed,
            "receipt": {
    ...
    return {
        ...
        "kind": "test",
        "description": f"Run {identifier} only.",
        "predictions": predictions,
        "measurement": measurement,
    }


def test_frontier_selects_largest_separation_then_measured_elapsed_and_seals_output(
    tmp_path: Path,
):
    ...
    request = _request(
        ...
        [
            ...
            _experiment(
                "low-value",
                {"repair-a": "pass", "repair-b": "pass", "repair-c": "fail"},
                root=tmp_path,
            ),
        ],
    )
...
def test_frontier_halts_when_no_experiment_discriminates(tmp_path: Path):
    ...
    request = _request(
        ...
        [
            _experiment(
                "same",
                {"repair-a": "pass", "repair-b": "pass", "repair-c": "unknown"},
                root=tmp_path,
            ),
        ],
    )
...
def test_frontier_rejects_prediction_drift_and_evaluation_tampering(tmp_path: Path):
    ...
    bad = _request(
        ...
        [
            _experiment(
                "incomplete", {"repair-a": "pass", "repair-b": "fail"}, root=tmp_path
            ),
        ],
    )
...
def test_frontier_verification_detects_changed_bound_evaluation(tmp_path: Path):
    ...
    evaluation.write_text(
        evaluation.read_text(encoding="utf-8") + " ", encoding="utf-8"
    )
    ...
    assert any(
        "EVIDENCE_FRONTIER_EVIDENCE_HASH_MISMATCH" in item for item in result["errors"]
    )
