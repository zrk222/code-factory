...
from factoryline.architecture_health import collect_architecture_health, evaluate_architecture_health
...
def _policy(path: Path, **budgets: int | float) -> Path:
    payload = {
        ...
        "review_thresholds": {"markdown_python_ratio": 1.5, "cli_lines": 5000, "core_modules": 150},
...
def test_health_collects_files_and_excludes_package_init(tmp_path: Path) -> None:
    ...
    (tmp_path / "factoryline" / "__init__.py").write_text("__version__ = '1.0.0'\n", encoding="utf-8")
...
def test_health_blocks_budget_regression_but_keeps_debt_separate(tmp_path: Path) -> None:
    (tmp_path / "factoryline").mkdir()
    (tmp_path / "factoryline" / "cli.py").write_text("\n".join(["pass"] * 6), encoding="utf-8")
    (tmp_path / "factoryline" / "one.py").write_text("pass\n", encoding="utf-8")
    (tmp_path / "README.md").write_text("# readme\n", encoding="utf-8")
    result = evaluate_architecture_health(tmp_path, _policy(tmp_path / "policy.json", cli=5))
    assert result["decision"] == "BLOCKED"
    assert any(item["code"] == "E_ARCH_CLI_LINES_GROWTH" for item in result["regressions"])
...
def test_health_exposes_core_and_specialist_module_domains(tmp_path: Path) -> None:
    (tmp_path / "factoryline").mkdir()
    (tmp_path / "factoryline" / "appforge_demo.py").write_text("pass\n", encoding="utf-8")
...
def test_malformed_boundary_manifest_is_blocking(tmp_path: Path) -> None:
    ...
    (tmp_path / "architecture-boundaries.json").write_text("{not-json", encoding="utf-8")
    result = evaluate_architecture_health(tmp_path, _policy(tmp_path / "policy.json", cli=2))
    assert result["decision"] == "BLOCKED"
    assert any(item["code"] == "E_ARCH_BOUNDARY_MANIFEST_INVALID" for item in result["regressions"])


def test_architecture_health_cli_emits_machine_readable_receipt(capsys, tmp_path: Path) -> None:
    (tmp_path / "factoryline").mkdir()
    (tmp_path / "factoryline" / "cli.py").write_text("pass\n", encoding="utf-8")
    (tmp_path / "architecture-boundaries.json").write_text(json.dumps({
        "schema": "factory.module-boundaries.v1", "defaultDomain": "core", "domains": {}
    }), encoding="utf-8")
    (tmp_path / "architecture-policy.json").write_text(json.dumps({
        "schema": "factory.architecture-policy.v1",
        "baseline": {},
        "budgets": {"max_markdown_files": 1, "max_python_files": 2, "max_markdown_python_ratio": 2.0, "max_cli_lines": 2, "max_core_modules": 2},
        "review_thresholds": {"markdown_python_ratio": 1.5, "cli_lines": 5000, "core_modules": 150},
        "release": {"max_releases_30d": 4},
    }), encoding="utf-8")
