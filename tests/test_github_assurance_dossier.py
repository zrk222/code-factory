...
def _snapshot(*, checks: list[str] | None = None, signed: bool = True) -> dict:
    return {
        ...
        "capture": {
            "source": "github_api_export",
            "captured_by": "release.engineer",
            "source_reference": "rulesets-export-001",
        },
        "rulesets": [
            {
                "id": "main-protection",
                ...
                else ["factory-proof", "tests"],
                "required_workflows": ["verify"],
...
def _proof(root: Path) -> dict:
    return render_github_proof_review(
        review_change(root, changed=["factoryline/tool.py"]), HEAD
    )
...
def _exception(snapshot: dict, finding_ids: list[str], *, head: str = HEAD) -> dict:
    return {
        ...
        "id": "temporary-controls-exception",
        "approval": {"state": "approved", "approved_by": "security.owner"},
        "expires_at": "2026-08-20T10:00:00+00:00",
        "policy_sha256": policy_snapshot_sha256(snapshot),
        "head_sha": head,
        "finding_ids": finding_ids,
    }


def test_assurance_dossier_is_deterministic_and_writes_only_to_explicit_directory(
    tmp_path: Path,
) -> None:
    ...
    assert (
        "does not fetch or prove the live GitHub configuration"
        in dossier["dossier_markdown"]
    )
    artifacts = write_assurance_dossier_artifacts(
        dossier, tmp_path / ".factory" / "github-assurance"
    )
    assert set(artifacts["paths"]) == {"json", "markdown", "mermaid"}
    assert all(Path(path).is_file() for path in artifacts["paths"].values())


def test_assurance_dossier_reports_deterministic_high_drift_and_named_exception(
    tmp_path: Path,
) -> None:
    current = _snapshot(checks=["tests"], signed=False)
    unapproved = build_assurance_dossier(
        _proof(tmp_path), current, _snapshot(), now=NOW
    )
    ids = [item["id"] for item in unapproved["drift"]["findings"]]
    assert unapproved["status"] == "review_required"
    assert ids == [
        "ruleset:main-protection:check:factory-proof",
        "ruleset:main-protection:signed_commits",
    ]

    accepted = build_assurance_dossier(
        _proof(tmp_path), current, _snapshot(), [_exception(current, ids)], now=NOW
    )
    assert accepted["status"] == "exception_accepted"
    assert accepted["drift"]["unresolved_high_count"] == 0


def test_assurance_dossier_rejects_expired_or_wrongly_bound_exception_before_write(
    tmp_path: Path,
) -> None:
    current = _snapshot(checks=["tests"])
    exception = _exception(
        current, ["ruleset:main-protection:check:factory-proof"], head="0" * 40
    )
    with pytest.raises(GitHubAssuranceDossierError) as exc:
        build_assurance_dossier(
            _proof(tmp_path), current, _snapshot(), [exception], now=NOW
        )
    assert exc.value.code == "GITHUB_ASSURANCE_EXCEPTION_INVALID"


def test_assurance_dossier_cli_writes_receipt_then_returns_blocking_status(
    tmp_path: Path, capsys: pytest.CaptureFixture[str]
) -> None:
    review_path, current_path, baseline_path = (
...
def test_graph_ops_projects_valid_assurance_dossier_and_recommends_human_drift_resolution(
    tmp_path: Path,
) -> None:
    dossier = build_assurance_dossier(
        _proof(tmp_path), _snapshot(checks=["tests"]), _snapshot(), now=NOW
    )
    write_assurance_dossier_artifacts(
        dossier, tmp_path / ".factory" / "github-assurance"
    )
