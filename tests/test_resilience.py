...
def _write_lineage(path: Path, steps: list[dict]) -> Path:
    ...
    path.write_text(
        json.dumps({**core, "lineage_sha256": _digest(core)}), encoding="utf-8"
    )
    return path


def _step(
    sequence: int, node: str, *, superstep: int, reads=None, writes=None, effects=None
) -> dict:
    return {
        "sequence": sequence,
        "superstep": superstep,
        ...
        "reads": reads or [],
        "writes": writes or [],
        "evidence": [{"path": f"evidence/{node}.json", "sha256": H2}],
        "side_effects": effects or [],
        "decision": {"route": "next", "reason": "fixture"},
    }
...
def _steps() -> list[dict]:
    ...
    return [
        _step(
            ...
            "seed",
            superstep=1,
            writes=[_write("shared", 0, 1, H0, H1)],
            effects=[effect],
        ),
        _step(
            2,
        ...
        _step(
            ...
            writes=[_write("shared", 1, 3, H1, H0)],
        ),
    ]
...
def test_temporal_resilience_compiler_covers_declared_concurrency_and_is_locked(
    tmp_path: Path,
):
    lineage = _write_lineage(
        tmp_path / ".factory" / "graph-runs" / "checkout.json", _steps()
    )
...
def test_temporal_resilience_verifier_rejects_missing_schedule_tamper_and_stale_lineage(
    tmp_path: Path,
):
    lineage = _write_lineage(tmp_path / "checkout.json", _steps())
    out = tmp_path / "resilience.json"
    write_temporal_resilience_plan(
        compile_temporal_resilience_plan(tmp_path, lineage), out
    )
    ...
    assert (
        verify_temporal_resilience_plan(tmp_path, out)["marker"]
        == "TEMPORAL_RESILIENCE_PLAN_INCOMPLETE"
    )
    ...
    assert (
        verify_temporal_resilience_plan(tmp_path, out)["marker"]
        == "TEMPORAL_RESILIENCE_PLAN_TAMPERED"
    )
    ...
    assert (
        verify_temporal_resilience_plan(tmp_path, out)["marker"]
        == "TEMPORAL_RESILIENCE_SOURCE_STALE"
    )


def test_resilience_cli_writes_only_explicit_output_and_verifies(
    ...
    assert (
        main(
            [
                "resilience",
                "plan",
                str(lineage.relative_to(tmp_path)),
                "--root",
                str(tmp_path),
                "--out",
                str(out),
    ...
    assert (
        ...
        == "TEMPORAL_RESILIENCE_PLAN_COMPILED"
    )
    assert (
        main(
            [
    ...
    assert (
        json.loads(capsys.readouterr().out)["marker"]
        == "TEMPORAL_RESILIENCE_PLAN_VERIFIED"
    )
