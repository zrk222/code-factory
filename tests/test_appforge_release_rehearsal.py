...
from factoryline.appforge_release_rehearsal import (
    PROFILE_SCHEMA,
    ZEALOT_MANIFEST_SCHEMA,
    create_release_rehearsal,
    release_rehearsal_projection,
)
...
def _profile(
    ...
    *,
    provider: str = "asc_cli",
    channel: str = "testflight_external",
) -> Path:
    if provider == "asc_cli":
        ...
        _write(
            root / "beta-distribution.json",
            {
                "schema": ZEALOT_MANIFEST_SCHEMA,
                "candidate": candidate,
                "platform": "ios",
...
def test_release_rehearsal_seals_exact_candidate_and_preserves_external_state_boundaries(
    tmp_path: Path,
) -> None:
    candidate, candidate_path = _candidate(tmp_path)
    receipt = create_release_rehearsal(
        tmp_path,
        candidate_path,
        _assurance(tmp_path, candidate),
        _profile(tmp_path, candidate),
        Path(".factory/appforge/release-rehearsal.json"),
    )
    ...
    assert (
        release_rehearsal_projection(tmp_path)["latest"]["receipt_sha256"]
        == receipt["receipt_sha256"]
    )


def test_release_rehearsal_cli_writes_only_the_local_sealed_receipt(
    ...
    assert (
        main(
            [
                ...
                str(_assurance(tmp_path, candidate)),
                "--profile",
                str(_profile(tmp_path, candidate)),
                "--out",
...
def test_release_rehearsal_keeps_channel_specific_external_states_not_applicable(
    tmp_path: Path, channel: str, not_applicable: set[str]
) -> None:
    candidate, candidate_path = _candidate(tmp_path)
    receipt = create_release_rehearsal(
        tmp_path,
        candidate_path,
        _assurance(tmp_path, candidate),
        _profile(tmp_path, candidate, provider="fastlane", channel=channel),
        Path(f".factory/appforge/{channel}-release-rehearsal.json"),
    )
    states = {item["stage"]: item["status"] for item in receipt["state_matrix"]}
    assert {
        stage for stage, status in states.items() if status == "not_applicable"
    } == not_applicable
    assert receipt["provider"]["lane"] == "beta_release"


def test_release_rehearsal_binds_a_credential_free_cider_manifest_without_parsing_or_running_it(
    tmp_path: Path,
) -> None:
    candidate, candidate_path = _candidate(tmp_path)
    receipt = create_release_rehearsal(
        ...
        candidate_path,
        _assurance(tmp_path, candidate),
        _profile(tmp_path, candidate, provider="cider", channel="app_store"),
        Path(".factory/appforge/cider-release-rehearsal.json"),
    )
    ...
    with pytest.raises(RevenueForgeError) as raised:
        create_release_rehearsal(
            ...
            _assurance(tmp_path, candidate),
            profile,
            Path(".factory/appforge/cider-secret.json"),
        )
    assert raised.value.code == "APPFORGE_REHEARSAL_SECRET_IN_PROFILE"


def test_release_rehearsal_binds_a_complete_swiftlane_source_workflow_without_executing_it(
    tmp_path: Path,
) -> None:
    candidate, candidate_path = _candidate(tmp_path)
    receipt = create_release_rehearsal(
        ...
        candidate_path,
        _assurance(tmp_path, candidate),
        _profile(tmp_path, candidate, provider="swiftlane", channel="app_store"),
        Path(".factory/appforge/swiftlane-release-rehearsal.json"),
    )
    assert receipt["provider"]["provider"] == "swiftlane"
    assert receipt["provider"]["declared_source_steps"] == {
        "build": True,
        "test": True,
        "archive": True,
        "export_archive": True,
    ...
    with pytest.raises(RevenueForgeError) as raised:
        create_release_rehearsal(
            ...
            _assurance(tmp_path, candidate),
            profile,
            Path(".factory/appforge/swiftlane-incomplete.json"),
        )
    assert raised.value.code == "APPFORGE_REHEARSAL_PROVIDER_INVALID"


def test_release_rehearsal_binds_zealot_style_artifact_channel_and_audience_without_claiming_delivery(
    tmp_path: Path,
) -> None:
    candidate, candidate_path = _candidate(tmp_path)
    receipt = create_release_rehearsal(
        ...
        candidate_path,
        _assurance(tmp_path, candidate),
        _profile(tmp_path, candidate, provider="zealot", channel="beta_distribution"),
        Path(".factory/appforge/zealot-release-rehearsal.json"),
    )
    ...
    with pytest.raises(RevenueForgeError) as raised:
        create_release_rehearsal(
            ...
            candidate_path,
            _assurance(tmp_path, candidate),
            _profile(tmp_path, candidate, provider="zealot", channel="app_store"),
            Path(".factory/appforge/zealot-channel-invalid.json"),
        )
    assert raised.value.code == "APPFORGE_REHEARSAL_CHANNEL_INVALID"


def test_release_rehearsal_rejects_any_future_matrix_that_promotes_or_blurs_external_states(
    ...
    with pytest.raises(RevenueForgeError) as raised:
        create_release_rehearsal(
            ...
            _assurance(tmp_path, candidate),
            _profile(tmp_path, candidate),
            Path(".factory/appforge/invalid.json"),
        )
    assert raised.value.code == "APPFORGE_REHEARSAL_STATE_INVALID"
    assert not (tmp_path / ".factory" / "appforge" / "invalid.json").exists()


def test_release_rehearsal_rejects_tampered_assurance_and_credential_like_profile(
    tmp_path: Path,
) -> None:
    candidate, candidate_path = _candidate(tmp_path)
    ...
    with pytest.raises(RevenueForgeError) as unsupported_error:
        create_release_rehearsal(
            ...
            _assurance(tmp_path, candidate),
            profile,
            Path(".factory/appforge/c.json"),
        )
    assert unsupported_error.value.code == "APPFORGE_REHEARSAL_PROVIDER_INVALID"


def test_release_rehearsal_rejects_provider_and_candidate_scope_drift(
    tmp_path: Path,
) -> None:
    candidate, candidate_path = _candidate(tmp_path)
