...
from factoryline.appforge_fastlane_capture import (
    CONTRACT_SCHEMA,
    create_fastlane_capture_contract,
    fastlane_capture_projection,
)
...
def _candidate(root: Path) -> tuple[dict[str, str], Path]:
    ...
    return candidate, _write(
        root / "candidate.json", {"schema": CANDIDATE_SCHEMA, "candidate": candidate}
    )
...
def _receipts(
    root: Path, candidate: dict[str, str]
    ...
    matrix_core: dict[str, object] = {
        ...
        "candidate": candidate,
        "scenarios": [
            {"platform": "iphone", "configuration": "default appearance"},
            {"platform": "ipad", "configuration": "Split View / compact width"},
        ],
    }
...
def _fastlane_sources(
    root: Path, names: list[str], *, framing: str = "raw_only"
) -> None:
    ...
    action = (
        "capture_screenshots"
        if framing == "raw_only"
        else "capture_screenshots\n  frame_screenshots"
    )
    (root / "fastlane" / "Fastfile").write_text(
        f"lane :appforge_capture do\n  {action}\nend\n", encoding="utf-8"
    )
    (root / "AppForgeScreenshots.swift").write_text(
        ...
        + "\n".join(
            f'func test{index}() {{ snapshot("{name}") }}'
...
def _contract(
    ...
    *,
    framing: str = "raw_only",
) -> Path:
    captures = [
        {
            "set_id": scene["set_id"],
            "capture_id": scene["capture_id"],
            "snapshot_name": f"{index + 1:02d}-{scene['capture_id'].title()}",
        }
    ...
    value = {
        ...
        "candidate": candidate,
        "surface_matrix_receipt_sha256": json.loads(matrix.read_text(encoding="utf-8"))[
            "receipt_sha256"
        ],
        "storefront_story_receipt_sha256": json.loads(
            story.read_text(encoding="utf-8")
        )["receipt_sha256"],
        "fastlane": {
            "snapfile_path": "fastlane/Snapfile",
            "fastfile_path": "fastlane/Fastfile",
            "ui_test_path": "AppForgeScreenshots.swift",
            "capture_lane": "appforge_capture",
            "framing": framing,
        },
        "captures": captures,
    }
...
def test_fastlane_capture_contract_seals_sources_and_exact_story_coverage(
    tmp_path: Path,
) -> None:
    candidate, candidate_path = _candidate(tmp_path)
    matrix, story, scenes = _receipts(tmp_path, candidate)
    names = ["01-Home", "02-Workspace"]
    _fastlane_sources(tmp_path, names)
    receipt = create_fastlane_capture_contract(
        tmp_path,
        candidate_path,
        matrix,
        story,
        _contract(tmp_path, candidate, matrix, story, scenes),
        Path(".factory/appforge/fastlane-capture.json"),
    )
    ...
    assert (
        receipt["windows_operation"]["external_execution_requires_macos_xcode"] is True
    )
...
def test_fastlane_capture_contract_never_overwrites_sealed_receipt(
    tmp_path: Path,
) -> None:
    candidate, candidate_path = _candidate(tmp_path)
    matrix, story, scenes = _receipts(tmp_path, candidate)
...
def test_fastlane_capture_contract_cli_writes_only_local_receipt(
    tmp_path: Path, capsys: pytest.CaptureFixture[str]
) -> None:
    candidate, candidate_path = _candidate(tmp_path)
    matrix, story, scenes = _receipts(tmp_path, candidate)
    _fastlane_sources(tmp_path, ["01-Home", "02-Workspace"])
    contract = _contract(tmp_path, candidate, matrix, story, scenes)
    assert (
        main(
            [
                "revenue",
                ...
                str(contract),
                "--out",
                ".factory/appforge/cli-fastlane-capture.json",
                "--json",
            ]
        )
        == 0
    ...
    assert (
        ...
        == "APPFORGE_FASTLANE_CAPTURE_READY"
    )


def test_fastlane_capture_contract_rejects_missing_snapshot_and_coverage_drift(
    tmp_path: Path,
) -> None:
    candidate, candidate_path = _candidate(tmp_path)
    matrix, story, scenes = _receipts(tmp_path, candidate)
    _fastlane_sources(tmp_path, ["01-Home"])
    contract = _contract(tmp_path, candidate, matrix, story, scenes)
    with pytest.raises(RevenueForgeError) as raised:
        create_fastlane_capture_contract(
            ...
            story,
            contract,
            Path(".factory/appforge/rejected.json"),
        )
    ...
    with pytest.raises(RevenueForgeError) as coverage:
        create_fastlane_capture_contract(
            ...
            story,
            contract,
            Path(".factory/appforge/coverage.json"),
        )
...
@pytest.mark.parametrize(
    ...
    [
        (
            lambda root: (root / "fastlane" / "Fastfile").write_text(
                "lane :appforge_capture do\n  capture_screenshots\n  upload_to_app_store\nend\n",
                encoding="utf-8",
            ),
            "APPFORGE_FASTLANE_CAPTURE_LANE_INVALID",
        ),
        (
            lambda root: (root / "fastlane" / "Snapfile").write_text(
                'devices(["iPhone 17 Pro Max", "iPad Pro 13-inch (M4)"])\n',
                encoding="utf-8",
            ),
        ...
        (
            lambda root: (root / "AppForgeScreenshots.swift").write_text(
                'setupSnapshot(app)\napp.launch()\ncontinueAfterFailure = true\nsnapshot("01-Home")\nsnapshot("02-Workspace")',
                encoding="utf-8",
            ),
...
def test_fastlane_capture_contract_fails_closed_on_unsafe_lane_or_weak_config(
    ...
    with pytest.raises(RevenueForgeError) as raised:
        create_fastlane_capture_contract(
            ...
            story,
            _contract(tmp_path, candidate, matrix, story, scenes),
            Path(".factory/appforge/unsafe.json"),
        )
...
def test_fastlane_capture_contract_rejects_credential_like_ruby_forms(
    ...
    fastfile.write_text(
        fastfile.read_text(encoding="utf-8").replace(
            "capture_screenshots", f"capture_screenshots\n  {source}"
        ),
        encoding="utf-8",
    )
    with pytest.raises(RevenueForgeError) as raised:
        create_fastlane_capture_contract(
            ...
            story,
            _contract(tmp_path, candidate, matrix, story, scenes),
            Path(".factory/appforge/secret.json"),
        )
    assert raised.value.code == "APPFORGE_FASTLANE_CAPTURE_SECRET_IN_SOURCE"


def test_fastlane_capture_contract_detects_unsafe_action_after_nested_block(
    tmp_path: Path,
) -> None:
    ...
    matrix, story, scenes = _receipts(tmp_path, candidate)
    _fastlane_sources(tmp_path, ["01-Home", "02-Workspace"])
    (tmp_path / "fastlane" / "Fastfile").write_text(
        "lane :appforge_capture do\n  if true\n    capture_screenshots\n  end\n  upload_to_app_store\nend\n",
        encoding="utf-8",
    )
    with pytest.raises(RevenueForgeError) as raised:
        create_fastlane_capture_contract(
            ...
            story,
            _contract(tmp_path, candidate, matrix, story, scenes),
            Path(".factory/appforge/nested-unsafe.json"),
        )
    assert raised.value.code == "APPFORGE_FASTLANE_CAPTURE_LANE_INVALID"


def test_fastlane_capture_contract_allows_only_reviewed_framefile_lane(
    tmp_path: Path,
) -> None:
    candidate, candidate_path = _candidate(tmp_path)
    matrix, story, scenes = _receipts(tmp_path, candidate)
    _fastlane_sources(
        tmp_path, ["01-Home", "02-Workspace"], framing="reviewed_framefile"
    )
    receipt = create_fastlane_capture_contract(
        tmp_path,
        candidate_path,
        ...
        story,
        _contract(
            tmp_path, candidate, matrix, story, scenes, framing="reviewed_framefile"
        ),
        Path(".factory/appforge/framed-fastlane-capture.json"),
    )
