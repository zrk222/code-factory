...
def _manifest(root: Path) -> tuple[dict, datetime]:
    ...
    manifest = {
        ...
        "vex_policy": {"max_unresolved": {"critical": 0, "high": 0, "medium": 0, "low": 0}, "exceptions": []},
        "licenses": {"policy_sha256": hashlib.sha256(b"license-policy").hexdigest(), "entries": [{"component": "factoryline", "license": "Apache-2.0", "status": "allowed"}], "exceptions": []},
        "build": {"builder_id": "builder", "builder_version": "1", "toolchain_sha256": hashlib.sha256(b"python-3.11").hexdigest(), "command_sha256": hashlib.sha256(b"build").hexdigest(), "clean_environment": True, "reproducible": True, "rebuilds": [rebuild, {"id": "builder-b", "artifacts": [artifact]}]},
...
def test_supply_chain_passes_and_writes_hash_bound_receipt(tmp_path: Path) -> None:
    ...
    assert {check["id"] for check in result["checks"]} == {"SOURCE_MANIFEST_BOUND", "LOCKFILES_BOUND", "SBOM_BOUND", "VEX_POLICY_BOUND", "LICENSE_POLICY_BOUND", "REPRODUCIBLE_BUILDS", "ARTIFACT_SECRET_SCAN"}
    ...
    assert written["receipt_sha256"] == _digest({key: value for key, value in written.items() if key != "receipt_sha256"})
...
def test_rebuild_drift_and_license_policy_fail_closed(tmp_path: Path) -> None:
    ...
    drift["build"]["rebuilds"][1]["artifacts"] = [_descriptor(tmp_path, "dist/second-0.46.4.whl")]
...
def test_vulnerability_threshold_and_archive_secret_fail_closed(tmp_path: Path) -> None:
    manifest, now = _manifest(tmp_path)
    (tmp_path / "vex.json").write_text(json.dumps(build_vex([{ "vulnerability": "CVE-1", "component": "x", "status": "affected" }]) | {}), encoding="utf-8")
    ...
    manifest, now = _manifest(tmp_path)
    _write_zip(tmp_path, "dist/factoryline-0.46.4.whl", b"ghp_abcdefghijklmnopqrstuvwxyz")
    artifact = _descriptor(tmp_path, "dist/factoryline-0.46.4.whl")
    manifest["build"]["rebuilds"] = [{"id": "a", "artifacts": [artifact]}, {"id": "b", "artifacts": [artifact]}]
    blocked = evaluate_supply_chain(tmp_path, manifest, now=now)
...
def test_external_signed_attestation_is_verified_but_local_is_rejected(tmp_path: Path) -> None:
    ...
    keys = generate_key_material(out_dir=tmp_path / "keys", keyid="sca", identity="builder@example.test", issuer="factory.test")
    envelope = sign_payload(manifest, payload_type=PAYLOAD_TYPE, private_key_path=Path(keys["private_key"]), keyid="sca", identity="builder@example.test", issuer="factory.test")
    attestation = tmp_path / "attestation.json"
    attestation.write_text(json.dumps(envelope), encoding="utf-8")
    verified = verify_signed_supply_chain_attestation(attestation, Path(keys["trust_root"]), tmp_path, now=now)
    assert verified["state"] == "VERIFIED"
    local = copy.deepcopy(manifest)
    local["collector"] = {"id": "local", "role": "local_evaluator", "backend": "local"}
    local_envelope = sign_payload(local, payload_type=PAYLOAD_TYPE, private_key_path=Path(keys["private_key"]), keyid="sca", identity="builder@example.test", issuer="factory.test")
    attestation.write_text(json.dumps(local_envelope), encoding="utf-8")
    with pytest.raises(Exception) as error:
        verify_signed_supply_chain_attestation(attestation, Path(keys["trust_root"]), tmp_path, now=now)
