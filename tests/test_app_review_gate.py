...
from factoryline.app_review_gate import (
    RULES,
    app_review_gate_projection,
...
def _run(
    tmp_path: Path, checks: dict[str, object], *, observed: dict[str, str] | None = None
):
    contract = _write(
    ...
    evidence = _write(
        ...
        {"candidate": observed or CANDIDATE, "checks": checks},
    )
    return verify_app_review_readiness(
...
def test_every_rejection_derived_rule_fails_closed(
    ...
    assert [item["code"] for item in receipt["findings"]] == [
        f"APP_REVIEW_{missing.upper()}_UNPROVEN"
    ]


@pytest.mark.parametrize("not_true", [False, None, 1, "true", [], {}])
def test_non_boolean_truth_cannot_bypass_purchase_gate(
    tmp_path: Path, not_true: object
) -> None:
    checks: dict[str, object] = {key: True for key, _, _, _, _ in RULES}
    checks["physical_iphone_purchase"] = not_true
    receipt = _run(tmp_path, checks)
    assert any(
        item["code"] == "APP_REVIEW_PHYSICAL_IPHONE_PURCHASE_UNPROVEN"
...
def test_evidence_from_another_build_is_rejected(tmp_path: Path) -> None:
    receipt = _run(
        tmp_path,
        {key: True for key, _, _, _, _ in RULES},
        observed={**CANDIDATE, "build_number": "41"},
    )
    assert receipt["marker"] == "APP_REVIEW_BLOCKED"
...
def test_malformed_candidate_is_refused(tmp_path: Path) -> None:
    contract = _write(
        tmp_path / "contract.json",
        {
            "candidate": {"bundle_identifier": "com.example.app"},
            "applicability": APPLICABILITY,
        },
    )
    evidence = _write(
        tmp_path / "evidence.json", {"candidate": CANDIDATE, "checks": {}}
    )
...
def test_conditional_rule_cannot_disappear_by_omission(tmp_path: Path) -> None:
    contract = _write(
        tmp_path / "contract.json", {"candidate": CANDIDATE, "applicability": {}}
    )
    evidence = _write(
        tmp_path / "evidence.json",
        {"candidate": CANDIDATE, "checks": {key: True for key in ALWAYS}},
    )
    receipt = verify_app_review_readiness(
        tmp_path, contract, evidence, Path("gate.json")
    )
    assert receipt["marker"] == "APP_REVIEW_BLOCKED"
    assert sum(
        item["code"].endswith("_APPLICABILITY_UNREVIEWED")
        for item in receipt["findings"]
    ) == len(CONDITIONAL)
...
def test_named_non_applicable_rule_is_preserved_in_receipt(tmp_path: Path) -> None:
    applicability = {
        **APPLICABILITY,
        CONDITIONAL[0]: {
            ...
            "reviewed_by": "Release Owner",
            "rationale": "The candidate has no account or authentication capability.",
        },
    }
    contract = _write(
        tmp_path / "contract.json",
    ...
    evidence = _write(
        tmp_path / "evidence.json", {"candidate": CANDIDATE, "checks": checks}
    )
    receipt = verify_app_review_readiness(
        tmp_path, contract, evidence, Path("gate.json")
    )
