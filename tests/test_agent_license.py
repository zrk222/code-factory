...
def _record(
    ...
    *,
    ...
    result.write_text(
        json.dumps({"marker": "WORKER_RESULT", "id": identifier}), encoding="utf-8"
    )
    verifier.write_text(
        json.dumps({"marker": "INDEPENDENT_VERIFICATION", "id": identifier}),
        encoding="utf-8",
    )
    ...
    event.write_text(
        json.dumps(
            {
                "schema": "factory.agent-run.v1",
                "id": identifier,
                "agent": identity,
                "task_id": task_id,
                "admission": admission["path"],
                "result_receipt": str(result.relative_to(root)),
                "verification": {
                    "subject": "reviewer-beta",
                    "receipt": str(verifier.relative_to(root)),
                },
                "passed": passed,
                "failure_classes": failures or [],
            }
        ),
        encoding="utf-8",
    )
...
def test_clean_current_governed_runs_earn_scoped_autonomy_and_hash_verify(
    tmp_path: Path,
):
...
def test_severe_failure_writes_incident_and_demotes_immediately(tmp_path: Path):
    ...
    for index in range(20):
        _record(tmp_path, passport, AGENT, f"run-{index:02d}")
    severe = _record(
        tmp_path, passport, AGENT, "run-99", passed=False, failures=["hollow_test"]
    )
...
def test_record_requires_independent_verifier_without_writing_event(tmp_path: Path):
        ...
        json.dumps(
            {
                ...
                "task_id": None,
                "admission": admission["path"],
                "result_receipt": ".factory/fixture.json",
                "verification": {
                    "subject": "agent-alpha",
                    "receipt": ".factory/fixture.json",
                },
                "passed": True,
                "failure_classes": [],
            }
    ...
    assert (
        not list((tmp_path / ".factory" / "agent-licenses" / "events").glob("*.json"))
        if (tmp_path / ".factory" / "agent-licenses" / "events").exists()
        else True
    )
...
def test_admission_caps_declared_agent_autonomy_by_license(tmp_path: Path):
    ...
    with pytest.raises(AdmissionError) as raised:
        prepare_admission(
            tmp_path, passport, _request(tmp_path, AGENT, "autonomy-request")
        )
