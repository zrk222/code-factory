...
from factoryline.proof_continuity_ledger import (
...
def _oracle(root: Path) -> Path:
    ...
    handoff = capture_intent_handoff(
        root, intent, AGENT, "restore", Path(".factory/oracles/handoffs/restore.json")
    )
    rules = {
        "requirements": [
            {
        ...
        "forbidden_behaviors": [
            {
                "id": "double-charge",
                "statement": "Restore never creates a new charge.",
                "origin": "human_confirmed",
                "effect": "blocking",
                "source_id": "original-intent",
                "critical": True,
        ...
        "gates": [
            {
                "id": "restore-rate",
                "statement": "Restore evidence meets the approved floor.",
                "origin": "human_confirmed",
                "effect": "blocking",
                "source_id": "original-intent",
                "critical": True,
        ...
        "exceptions": [
            {
                "id": "none",
                "statement": "No exception is approved.",
                "origin": "human_confirmed",
                "effect": "advisory",
                "source_id": "original-intent",
                "critical": False,
        ...
        "negative_cases": [
            {
                "id": "expired",
                "statement": "Expired access must not restore paid access.",
                "origin": "human_confirmed",
                "effect": "blocking",
                "source_id": "original-intent",
                "critical": True,
        ...
        "invariants": [
            {
                "id": "revision",
                "statement": "Evidence binds to the reviewed revision.",
        ...
        "tests": [
            {
                "id": "restore-flow",
                "statement": "The restore test fails if expired access is accepted.",
                "origin": "human_confirmed",
                "effect": "blocking",
                "source_id": "original-intent",
                "critical": True,
    ...
    payload = {
        ...
        "approval_rationale": "The owner reviewed the exact restoration safety boundary.",
        "scope_paths": ["."],
...
def _evidence(root: Path) -> Path:
    core = {
        "schema": "factory.test-evidence.v1",
        "marker": "TEST_EVIDENCE_READY",
        "candidate": {"source_commit": REVISION},
        "result": "declared local evidence",
    }
        ...
        "obligations": [
            {
                "id": "restore-proof",
                "source_id": "original-intent",
                "requirement_id": "restore",
                "forbidden_behavior_id": "double-charge",
                "gate_id": "restore-rate",
                "test_id": "restore-flow",
                "evidence_id": "restore-evidence",
            }
        ],
        "approved_by": "Release Owner",
...
def test_proof_continuity_seals_a_repository_level_chain_and_projects_graph_mcp(
    tmp_path: Path,
) -> None:
    oracle = _oracle(tmp_path)
    evidence = _evidence(tmp_path)
    receipt = seal_proof_continuity(
        tmp_path,
        _input(tmp_path, oracle, evidence),
        Path(".factory/proof-continuity/contracts/restore.json"),
    )
    ...
    mcp = dispatch(
        {
            ...
            "id": 1,
            "method": "tools/call",
            "params": {"name": "factory.proof_continuity_status", "arguments": {}},
        },
        tmp_path,
    )
...
def test_proof_continuity_reopens_on_later_contradiction_and_demands_supervision(
    tmp_path: Path,
) -> None:
    oracle = _oracle(tmp_path)
    evidence = _evidence(tmp_path)
    contract = seal_proof_continuity(
        tmp_path,
        _input(tmp_path, oracle, evidence),
        Path(".factory/proof-continuity/contracts/restore.json"),
    )
    later = tmp_path / "runtime-observation.log"
    later.write_text("duplicate charge observed", encoding="utf-8")
    observed = _write(
        tmp_path / "observation.json",
        {
            "schema": "factory.proof-continuity-observation-input.v1",
            "id": "duplicate-charge",
            "contract_sha256": contract["receipt_sha256"],
            "obligation_id": "restore-proof",
            "outcome": "contradicted",
            "kind": "runtime",
            "evidence_path": "runtime-observation.log",
            "evidence_sha256": hashlib.sha256(later.read_bytes()).hexdigest(),
            "observed_at": "2026-09-03T12:00:00Z",
            "reviewed_by": "Release Owner",
            "consequence": "Charge boundary needs a named remediation review.",
        },
    )
    receipt = record_proof_continuity_observation(
        tmp_path,
        Path(contract["path"]),
        observed,
        Path(".factory/proof-continuity/observations/duplicate-charge.json"),
    )

    assert receipt["marker"] == "E_PROOF_CONTINUITY_REOPENED"
    assert receipt["verdict"] == "BLOCKED"
    assert receipt["incident"] == {
        "open": True,
...
def test_proof_continuity_rejects_agent_provenance_gaps_stale_evidence_and_unknown_obligations(
    tmp_path: Path,
) -> None:
    oracle = _oracle(tmp_path)
    evidence = _evidence(tmp_path)
    contract_input = _input(tmp_path, oracle, evidence)
    value = json.loads(contract_input.read_text(encoding="utf-8"))
    value["obligations"][0]["gate_id"] = "missing"
    _write(contract_input, value)
    with pytest.raises(ProofContinuityError) as missing:
        seal_proof_continuity(
            tmp_path,
            contract_input,
            Path(".factory/proof-continuity/contracts/rejected.json"),
        )
    assert missing.value.code == "PROOF_CONTINUITY_CHAIN_INVALID"
    contract_input = _input(tmp_path, oracle, evidence)
    value = json.loads(contract_input.read_text(encoding="utf-8"))
    value["subject"]["revision"] = "b" * 40
    _write(contract_input, value)
    with pytest.raises(ProofContinuityError) as stale:
        seal_proof_continuity(
            tmp_path,
            contract_input,
            Path(".factory/proof-continuity/contracts/revision-mismatch.json"),
        )
    assert stale.value.code == "PROOF_CONTINUITY_EVIDENCE_REVISION_MISMATCH"


def test_proof_continuity_cli_keeps_observation_and_status_local(
    tmp_path: Path, capsys: pytest.CaptureFixture[str]
) -> None:
    oracle = _oracle(tmp_path)
    evidence = _evidence(tmp_path)
    source = _input(tmp_path, oracle, evidence)
    assert (
        main(
            [
                "proof-continuity",
                ...
                str(source),
                "--out",
                ".factory/proof-continuity/contracts/cli.json",
                "--json",
            ]
        )
        == 0
