...
def _manifest(
    ...
    **overrides: object,
    ...
    value: dict[str, object] = {
        ...
        "negative": {
            "argv": negative or [sys.executable, "-c", "import sys; sys.exit(1)"]
        },
...
def test_native_e2e_gate_proves_a_declared_negative_case_and_writes_explicit_artifacts(
    tmp_path: Path,
) -> None:
    ...
    assert validated_manifest["approval"] == {
        "state": "approved",
    ...
    assert (
        "does not enforce host or process network isolation"
        in receipt["scope_limits"][1]
    )
...
def test_native_e2e_gate_reports_hollow_when_negative_command_succeeds(
    tmp_path: Path,
) -> None:
...
def test_native_e2e_gate_records_positive_failures_and_timeouts_without_claiming_success(
    tmp_path: Path,
) -> None:
    ...
    assert (failed["marker"], failed["run_state"], failed["ok"]) == (
        "E2E_POSITIVE_FAILED",
        "positive_nonzero",
        False,
    )
    ...
    assert (timed["marker"], timed["run_state"], timed["ok"]) == (
        "E2E_POSITIVE_TIMEOUT",
        "positive_timeout",
        False,
    )
...
def test_native_e2e_gate_rejects_unapproved_or_unsafe_manifests_before_execution(
...
def test_native_e2e_gate_reports_missing_declared_artifact_and_rejects_tampered_receipts(
    tmp_path: Path,
) -> None:
    manifest = _manifest(
        tmp_path, positive=[sys.executable, "-c", "pass"], artifacts=["missing.png"]
    )
    receipt = verify_e2e_proof(tmp_path, manifest)

    assert (receipt["marker"], receipt["run_state"], receipt["ok"]) == (
        "E2E_ARTIFACT_MISSING",
        "artifact_missing",
        False,
    )
...
def test_native_e2e_cli_is_json_safe_and_returns_one_for_a_hollow_check(
