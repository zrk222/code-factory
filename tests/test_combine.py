...
from factoryline.combine import (
    score_combine,
    seal_combine_task,
...
def _task(root: Path) -> Path:
    ...
    source.write_text(
        json.dumps(
            {
                "schema": "factory.combine-task.v1",
                "id": "approval-tracker",
                "description": "Build the sealed approval tracker task.",
...
def test_combine_scores_exact_governed_events_and_rejects_tampering(tmp_path: Path):
    ...
    _record(
        ...
        "bravo-run",
        passed=False,
        failures=["wrong_output"],
        task_id="approval-tracker",
    )
