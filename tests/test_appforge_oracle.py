...
from factoryline.appforge_oracle import (
    appforge_oracle_projection,
...
def test_projection_rejects_changed_authority_sources(tmp_path, source):
    ...
    verify_appforge_oracle_authority(
        tmp_path, authority, out=Path(".factory/appforge/oracle-authority.json")
    )
...
def _contract(root: Path) -> Path:
    ...
    original.write_text(
        "The app must make subscription restore clear and safe.", encoding="utf-8"
    )
    ...
    handoff = capture_intent_handoff(
        ...
        AGENT,
        "appforge-intake",
        Path(".factory/oracles/handoffs/appforge.json"),
    )
    ...
    source = _write(
        root / "oracle-input.json",
        {
            "schema": "factory.oracle-contract-input.v1",
            "id": "appforge-review",
            "version": 1,
            "approved_by": "Release Owner",
            "approval_rationale": "The release owner checked user intent, policy source, and the candidate review gates.",
            "scope_paths": ["."],
            ...
            "forbidden_behaviors": [
                rule("double-charge", "Restore does not create a charge.")
            ],
            "gates": [
                rule(
                    "accessibility",
            ...
            "exceptions": [
                {
                    "id": "offline-note",
                    "statement": "Offline recovery needs a named future review.",
                    "origin": "human_confirmed",
                    "effect": "advisory",
                    "source_id": "original-intent",
                    "critical": False,
            ...
            "negative_cases": [
                rule("expired", "Expired entitlements cannot restore access.")
            ],
            "invariants": [
                rule("candidate", "Evidence stays bound to the exact candidate.")
            ],
            "tests": [
                rule(
                    "restore-test",
                    "Restore test must fail when expired access is accepted.",
    ...
    return (
        ...
        / seal_oracle_contract(
            root, source, Path(".factory/oracles/contracts/appforge.json")
        )["path"]
    )


def _authority(
    root: Path, contract: Path, candidate: dict[str, str] = CANDIDATE
) -> Path:
    return _write(
        root / "authority.json",
        {
...
def test_appforge_authority_binds_candidate_and_named_policy_source(
    tmp_path: Path,
) -> None:
    receipt = verify_appforge_oracle_authority(
        tmp_path,
        _authority(tmp_path, _contract(tmp_path)),
        candidate=CANDIDATE,
        out=Path(".factory/appforge/oracle-authority.json"),
    )
...
def test_appforge_authority_fails_closed_when_candidate_or_policy_authority_changes(
    tmp_path: Path,
) -> None:
    contract = _contract(tmp_path)
    receipt = verify_appforge_oracle_authority(
        tmp_path,
        _authority(tmp_path, contract),
        candidate={**CANDIDATE, "build_number": "43"},
    )

    assert receipt["ok"] is False
    assert receipt["marker"] == "APPFORGE_ORACLE_AUTHORITY_BLOCKED"
    assert any(
        item["code"] == "APPFORGE_ORACLE_CANDIDATE_MISMATCH"
