...
def _validators(root: Path, *, passing: bool = True) -> Path:
    ...
    code = (
        "from pathlib import Path; assert Path('product.txt').read_text() == 'ready'"
        if passing
        else "raise AssertionError('wrong output')"
    )
    path.write_text(
        json.dumps(
            {
...
def test_observed_session_binds_pre_run_admission_delta_validators_and_license_event(
    tmp_path: Path,
) -> None:
    ...
    result = run_observed_session(
        ...
        [
            sys.executable,
            "-c",
            "from pathlib import Path; Path('product.txt').write_text('ready')",
        ],
        "observed-run",
    )
    ...
    observed = json.loads(
        (tmp_path / receipt["result"]["path"]).read_text(encoding="utf-8")
    )
    assert observed["admission"]["pre_run_verdict"] == "READY"
    assert observed["workspace_delta"] == [
        {
            "after_sha256": observed["workspace_delta"][0]["after_sha256"],
            "path": "product.txt",
            "status": "created",
        }
    ]
    assert (
        observed["scope_limits"][0] == "Observed execution is not sandboxed execution."
    )
    assert "Path('product.txt')" not in json.dumps(observed)
    assert verify_session_receipt(tmp_path, Path(result["path"]))["ok"] is True


def test_observed_session_classifies_validator_failure_without_calling_it_hollow(
    tmp_path: Path,
) -> None:
...
def test_gauntlet_draft_uses_structural_cli_evidence_and_withholds_route_execution(
    tmp_path: Path,
) -> None:
...
def test_cli_draft_and_wrap_are_exposed(tmp_path: Path, capsys) -> None:
    ...
    assert (
        json.loads(capsys.readouterr().out)["draft"]["marker"]
        == "GAUNTLET_DRAFT_CREATED"
    )
...
def test_claude_hook_plugin_hashes_input_without_retaining_it(tmp_path: Path) -> None:
    ...
    record = next(
        (tmp_path / ".factory" / "session-recorder" / "claude-hooks").rglob("*.json")
    )
...
def test_observed_session_rejects_reused_or_mismatched_admission_before_execution(
    tmp_path: Path,
) -> None:
    validators = _validators(tmp_path)
    ...
    with pytest.raises(ValueError, match="already been consumed"):
        run_observed_session(
            ...
            [
                sys.executable,
                "-c",
                "from pathlib import Path; Path('should-not-exist.txt').write_text('bad')",
            ],
            "single-use-run",
        )
    ...
    with pytest.raises(ValueError, match="exactly match"):
        run_observed_session(
            mismatch_root, other_admission, other_validators, command, "different-name"
        )
...
def test_observed_session_detects_validator_manifest_tampering(tmp_path: Path) -> None:
    ...
    result = run_observed_session(
        tmp_path, admission, validators, command, "manifest-drift"
    )
...
def test_session_chain_uses_recorded_time_not_lexical_run_id(tmp_path: Path) -> None:
    ...
    for run_id, recorded_at, digest in (
        ...
        path.write_text(
            json.dumps(
                {"run_id": run_id, "recorded_at": recorded_at, "session_sha256": digest}
            ),
            encoding="utf-8",
        )
