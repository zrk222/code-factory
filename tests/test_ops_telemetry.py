...
def test_lifecycle_command_family_never_retains_free_form_prompt(tmp_path: Path) -> None:
    from factoryline.ops_telemetry import _command_family

    assert _command_family(["create", "a private customer prompt", "--root", str(tmp_path)]) == "create"
def test_lifecycle_failure_is_recorded_without_changing_exit_semantics(tmp_path: Path, capsys) -> None:
    assert main(["architecture", "health", "--root", str(tmp_path), "--policy", "missing.json"]) == 2
    capsys.readouterr()
    inventory = lifecycle_inventory(tmp_path)
    assert inventory["receipt_count"] == 1
