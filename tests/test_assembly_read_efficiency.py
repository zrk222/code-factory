...
HELPERS = (
    "_append_oracle_firewall",
    "_append_operations_controls",
    "_append_lifecycle_events",
    "_append_repair_loops",
)
...
def test_graph_reads_shared_stores_once_and_preserves_legacy_output(
...
def test_next_graph_request_reads_fresh_invalid_evidence(tmp_path, monkeypatch):
    ...
    assert dict(counts) == {name: 2 for name in READERS}
    assert (
        second["mission_control"]["blockers"]["operations_invalid"]
        > first["mission_control"]["blockers"]["operations_invalid"]
    )
...
def test_explicit_empty_observation_does_not_trigger_a_second_read(
...
def test_profile_has_stable_fingerprints_and_no_raw_evidence(tmp_path, monkeypatch):
    ...
    assert first["reader_elapsed_ns"] == 7
    assert second["reader_elapsed_ns"] == 70
    assert len(first["spans"]) == 7
    assert all(
        set(span) == {"name", "elapsed_ns", "output_sha256"} for span in first["spans"]
    )
...
def test_profile_fingerprint_changes_with_evidence(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr(
        control, "operations_control_projection", lambda root: {"invalid_count": 7}
    )
...
def test_profile_does_not_export_private_values(tmp_path, monkeypatch):
    monkeypatch.setattr(
        control,
        "operations_control_projection",
        lambda root: {"private": "never-display-this-value"},
    )
    assert "never-display-this-value" not in json.dumps(
