...
def _completed_mission(tmp_path: Path):
    ...
    validation.write_text(
        json.dumps(_validation_for(mission, "builder", "verifier", evidence)),
        encoding="utf-8",
    )
...
def test_graph_ops_links_local_product_mission_and_valid_completion_exactly(
    tmp_path: Path,
):
    ...
    assert first["agentic_control"]["feature_count"] == 15
    ...
    for requirement_id in selected_requirements:
        assert (
            completion_id,
            f"requirement:{project}:{requirement_id}",
            "verifies",
        ) in edges
...
def test_graph_ops_marks_stale_proof_and_prioritizes_rerun_without_execution(
    tmp_path: Path,
):
    ...
    proof = next(
        ...
        if node["id"] == f"proof:{receipt['proof_key']}"
    )
...
def test_graph_ops_projects_proof_delta_no_gain_halt_telemetry(tmp_path: Path):
    ...
    create_proof_delta(
        ...
        repair,
        failure,
        mission["completion_contract"]["criteria"][0]["id"],
        delta,
    )
    ...
    assert (
        build_proof_delta_telemetry(
            verify_proof_delta(tmp_path, delta), telemetry["proofDeltaSha256"]
        )
        == telemetry
    )
    ...
    guard = next(
        node for node in snapshot["nodes"] if node["kind"] == "proof_delta_guard"
    )
    assert guard["facts"] == telemetry
    blocker = next(
        node for node in snapshot["nodes"] if node["kind"] == "proof_delta_blocker"
    )
    assert blocker["facts"]["candidateHash"] == "b" * 64
    assert any(
        edge["target"] == guard["id"] and edge["relation"] == "projects_telemetry"
        for edge in snapshot["edges"]
    )
    assert snapshot["authority"]["execution"] is False


def test_graph_ops_impact_maps_only_explicit_changed_input_edges_to_stale_reruns(
    ...
    assert [item["proof_id"] for item in impact["matched_proofs"]] == [
        f"proof:{receipt['proof_key']}"
    ]
    assert [item["proof_id"] for item in impact["rerun_proofs"]] == [
        f"proof:{receipt['proof_key']}"
    ]
    ...
    code = main(
        ["graph", "impact", "--root", str(tmp_path), "--changed", "input.txt", "--json"]
    )
    payload = json.loads(capsys.readouterr().out)
    assert code == 0
    assert (
        payload["impact_sha256"]
        == graph_ops_impact(tmp_path, ["input.txt"])["impact_sha256"]
    )
...
def test_graph_ops_reports_compact_error_for_malformed_json(tmp_path: Path):
    ...
    assert snapshot["source_errors"] == [
        {"source": ".factory/proofs/broken.json", "code": "SOURCE_UNREADABLE"}
    ]


def test_graph_ops_projects_external_runtime_as_observed_read_only_evidence(
    tmp_path: Path,
):
    ...
    node = next(
        item for item in snapshot["nodes"] if item["kind"] == "external_runtime"
    )
...
def test_graph_ops_fails_closed_for_untraceable_or_malformed_intent_receipts(
    tmp_path: Path,
):
...
def test_graph_ops_prefers_explicit_factoryline_adapter_over_legacy_forge_receipt(
    tmp_path: Path,
):
    ...
    legacy.write_text(
        json.dumps({"phase": "verify", "verified": True}) + "\n" + legacy_line + "\n",
        encoding="utf-8",
    )
    ...
    adapter = {
        ...
        "outputs": {
            "intent_trace": {
                ...
                "intent_hash": "c" * 64,
                "obligations": "1/1",
                "forge_receipt_sha256": hashlib.sha256(
                    legacy_line.encode("utf-8")
                ).hexdigest(),
                "ts": "2026-08-24T12:01:00Z",
                "authority": {
                    key: False
    ...
    (adapter_dir / f"forgeline-{feature}-ship-adapter.json").write_text(
    ...
    assert (
        traces[0]["facts"]["forge_receipt_source"]
        == ".forge/adapter-feature/receipts.jsonl"
    )
    ...
    assert (
        source["facts"]["sha256"] == traces[0]["facts"]["observed_forge_receipt_sha256"]
    )
...
def test_graph_ops_rejects_malformed_factoryline_adapter_without_legacy_fallback(
    tmp_path: Path,
):
...
def test_graph_ops_marks_adapter_hash_mismatch_without_trusting_traceability(
    tmp_path: Path,
):
...
def test_graph_ops_marks_adapter_claim_mismatch_without_trusting_traceability(
    tmp_path: Path,
):
    ...
    assert trace["facts"]["provenance_status"] == "mismatch"
    assert (
        trace["facts"]["observed_forge_receipt_sha256"]
        == trace["facts"]["forge_receipt_sha256"]
    )
    assert snapshot["facts"]["intent_trace_binding_mismatch_count"] == 1
    assert snapshot["recommendation"]["action"] == "repair_intent_trace_binding"
...
def test_graph_ops_exposes_declared_gate_state_without_running_commands(tmp_path: Path):
    plan_dir = tmp_path / ".factory" / "proof-plans"
    plan_dir.mkdir(parents=True)
    (plan_dir / "blocked.json").write_text(
        json.dumps(
            {
                "schema": "factory.proof-plan.v1",
                "items": [
...
def test_graph_ops_projects_release_workflow_before_a_named_feature_decision(
    ...
    monkeypatch.setattr(
        "factoryline.mission_control_status._release_workflow_integrity", blocked
    )
    snapshot = graph_ops_snapshot(tmp_path)
    node = next(
        item for item in snapshot["nodes"] if item["kind"] == "release_decision"
    )
    ...
    monkeypatch.setattr(
        "factoryline.mission_control_status._release_workflow_integrity",
        lambda _root: {
            "schema": "factory.release_integrity.v1",
            ...
            "external_requirements": [
                "VSCE_PAT remains an unobserved protected-environment requirement."
            ],
        },
    )
    healthy = graph_ops_snapshot(tmp_path)
    healthy_node = next(
        item for item in healthy["nodes"] if item["kind"] == "release_decision"
    )
    assert healthy_node["facts"]["state"] == "FEATURE_DECISION_REQUIRED"
    assert healthy_node["facts"]["feature_required"] is True
    assert (
        healthy_node["facts"]["next_action"]
        == "factory release decision <feature> --root . --json"
    )
    assert healthy_node["facts"]["external_requirements"] == [
        "VSCE_PAT remains an unobserved protected-environment requirement."
    ]
    ...
    absent_node = next(
        item for item in absent["nodes"] if item["kind"] == "release_decision"
    )
...
def test_graph_ops_visual_template_is_accessible_and_uses_text_nodes_only():
    ...
    assert (
        "registerFactoryWebMcp();registerMobileEvidenceWebMcp();load();loadDeveloperMemory();"
        in page
    )
    ...
    assert (
        "factory judgment safety-case --root . --changed <path> --change-profile .factory/judgment/change-profile.json --json"
        in page
    )
    assert "renderJudgment(nodes)" in page
    assert (
        "cannot infer intent, promote or waive a decision, execute a repair, approve code, merge, publish, deploy, sign, message, or access credentials"
        in page
    )
    ...
    assert (
        "No local Forge ship receipt is present. Intent traceability is unverified."
        in page
    )
...
def test_graph_ops_projects_sealed_admission_without_changing_its_base_graph(
    tmp_path: Path,
):
...
def test_graph_ops_exposes_verified_proofsearch_winner_and_candidate_controls(
    tmp_path: Path,
):
    ...
    winner = next(
        node
        for node in snapshot["nodes"]
        if node["kind"] == "repair_candidate" and node["status"] == "winner"
    )
...
def test_graph_ops_projects_a_verified_evidence_frontier_without_execution(
    tmp_path: Path,
):
    ...
    request = _request(
        ...
        [
            _experiment(
                "targeted",
                {"repair-a": "pass", "repair-b": "fail", "repair-c": "fail"},
                root=tmp_path,
            ),
        ],
    )
    ...
    decision = next(
        node for node in snapshot["nodes"] if node["kind"] == "evidence_frontier"
    )
    assert decision["facts"]["next_experiment"] == "targeted"
    assert all(value is False for value in decision["facts"]["authority"].values())
    experiment = next(
        node for node in snapshot["nodes"] if node["kind"] == "evidence_experiment"
    )
    assert experiment["status"] == "next"
    assert experiment["facts"]["execution_allowed"] is False


def test_graph_ops_projects_a_supervised_reality_check_without_rerunning_it(
    tmp_path: Path,
):
    ...
    directory = tmp_path / ".factory" / "reality"
    directory.mkdir(parents=True)
    (directory / "approval.reality.json").write_text(
...
def test_graph_ops_projects_survival_cards_without_running_or_signing_them(
    tmp_path: Path,
):
    ...
    assert all(
        value is False
        for key, value in card["facts"]["authority"].items()
        if key not in {"execution", "test_execution"}
    )


def test_graph_ops_projects_counterexamples_guardrails_and_temporal_resilience_read_only(
    tmp_path: Path,
):
    from test_counterexample import _source as counterexample_source
    from test_guardrails import (
        _manifest as guardrail_manifest,
        _principal as guardrail_principal,
        _store as guardrail_store,
    )
    ...
    from factoryline.resilience import (
    ...
    write_counterexample_plan(
        compile_counterexample_plan(tmp_path, source), counterexample_out
    )
    ...
    evaluation = evaluate_guardrails(
        manifest,
        db,
        guardrail_principal("reader", ("reader",)),
        changed_paths=["src/checkout/submit.py"],
    )
    ...
    lineage = _write_lineage(
        tmp_path / ".factory" / "graph-runs" / "checkout.lineage.json",
        resilience_steps(),
    )
    resilience_out = tmp_path / ".factory" / "resilience" / "checkout.json"
    write_temporal_resilience_plan(
        compile_temporal_resilience_plan(tmp_path, lineage), resilience_out
    )
    before = {
        path.relative_to(tmp_path): path.read_bytes()
        for path in tmp_path.rglob("*")
    ...
    assert before == after
    assert {"counterexample_plan", "guardrail_evaluation", "temporal_resilience"} <= {
...
def test_graph_ops_starts_with_plain_language_progressive_disclosure():
    html = (Path(__file__).parents[1] / "factoryline" / "graph_ops.html").read_text(
        encoding="utf-8"
    )
