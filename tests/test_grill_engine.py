...
from factoryline.grill_engine import answer_question, confirm_grill, next_question, start_grill


def test_grill_is_one_question_at_a_time_and_requires_confirmation(tmp_path: Path) -> None:
    ...
    for question in started["questions"]:
        if question["required"]:
            answer_question(tmp_path, "audit-slice", question["id"], f"Human answer for {question['id']}", answered_by="owner")
