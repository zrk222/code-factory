...
from factoryline.runtime_audit import EVALUATORS, execute_runtime_audit, evaluate_runtime_audit, runtime_audit_status
...
def test_runtime_boundary_is_joined_without_promoting_supervision(monkeypatch, tmp_path):
    for kind in LANES:
        monkeypatch.setitem(EVALUATORS, kind, lambda artifact, config, engine, engine_version, k=kind: {"lane": k, "state": "FAIL" if artifact["bad"] else "PASS", "finding": f"NEG_{k}" if artifact["bad"] else "HELD", "consequence": "caught", "details": {}})
    plan = _plan()
    plan["environment"] = {"kind": "local_test", "digest": "e" * 64, "origins": ["http://127.0.0.1"]}
    plan["runtime_boundary"] = {"requested_isolation": "supervised_subprocess", "attestation_required": True}
    executions = {"executions": [{"id": kind, "kind": kind, "target": _command(kind), "known_bad": _command(kind, True)} for kind in LANES]}
    plan_digest = sha256_bytes(canonical_bytes(plan))
    executions["plan_sha256"] = plan_digest
    executions["runtime_boundary"] = capture_supervised_attestation(attestation_id="run", candidate_sha256=plan["candidate_sha256"], plan_sha256=plan_digest, environment_sha256=plan["environment"]["digest"])
...
def test_execute_runtime_audit_binds_verified_plan_digest(monkeypatch, tmp_path):
    ...
    monkeypatch.setattr("factoryline.runtime_audit.evaluate_runtime_audit", lambda current, execution, workspace: {"schema": "factory.runtime-audit-receipt.v1", "decision": "BLOCKED", "lanes": [], "authority": "none", "release_approval": False})
    result = execute_runtime_audit(tmp_path / "plan.json", tmp_path / "trust.json", "t" * 64, tmp_path, "e" * 64, run_root)
