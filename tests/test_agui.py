...
from factoryline.agui import AguiError, build_release_completed_event, build_release_interrupt_event, build_review_events, validate_agui_events
...
def test_review_events_are_deterministic_and_ordered() -> None:
    ...
    assert first == build_review_events(status, run_id="run-01", surface="mission_control")
    assert [event["type"] for event in first] == ["RUN_STARTED", "STATE_SNAPSHOT", "REVIEW_CARD", "RUN_FINISHED"]
...
def test_review_events_reject_sensitive_fields() -> None:
    with pytest.raises(AguiError) as error:
        validate_agui_events([
            {"schema": "factory.agui.events.v1", "runId": "run-01", "sequence": 0, "type": "RUN_STARTED", "payload": {"token": "secret"}, "eventId": "bad", "payloadSha256": "bad"},
        ])
...
def test_release_gate_interrupt_is_human_owned_and_bounded() -> None:
    envelope = release_gate_input_required({
        ...
        "blockers": [{"lane": "failure_recovery", "code": "RETRY_NOT_PROVEN", "severity": "HIGH"}],
