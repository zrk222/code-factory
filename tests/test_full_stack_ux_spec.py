...
def _direct_spec(root: Path) -> Path:
    for path in (
        ...
        root / "screens" / "iphone.png",
        root / "screens" / "ipad.png",
        root / "fastlane.json",
    ):
    ...
    value = {
        ...
        "metadata": {
            "name": "demo",
            "targetAppId": "com.example.demo",
            "version": "1.2.3",
        },
        "harnessConfig": {
            "visualMedia": {
                "storyboardManifest": "storyboard.json",
                "screenshotPaths": ["screens/iphone.png", "screens/ipad.png"],
            },
            "privacyToListing": {
                "declaredPermissions": ["camera"],
                "appStorePrivacyLabelHash": digest,
            },
            "releaseChain": {
                "xcodeSigningVerified": True,
                "fastlaneReceiptPath": "fastlane.json",
            },
            "designSystem": {
                "designTokensHash": digest,
                "hierarchyRulesVerified": True,
            },
            "productionSignal": {
                "telemetryProvider": "local-export",
                "crashReportingVerified": True,
            },
            "androidParity": {
                "gradleBuildVariant": "release",
                "adbDeviceLogsHash": digest,
            },
        },
        "triageRules": [
            {
                ...
                "severity": "BLOCKER",
                "failureMessage": "iPad media is missing",
            }
        ],
    }
...
def test_current_legacy_ssat_projects_to_all_six_mobile_categories(tmp_path: Path):
    ...
    source.write_text(
        (Path(__file__).parents[1] / "full-stack-ux-harness-v1.ssat.yaml").read_text(
            encoding="utf-8"
        ),
        encoding="utf-8",
    )
...
def test_direct_contract_blocks_unknown_category_and_receipt_is_immutable(
    tmp_path: Path,
):
    source = _direct_spec(tmp_path)
...
def test_cli_spec_validate_and_verify_are_read_only_replay_commands(
