...
def _control(cid: str = "CTRL-001", *, threshold: int = 0, severity: str = "HIGH", provenance: str = "human_confirmed", evidence: list[str] | None = None) -> dict:
    return {
        "id": cid,
        ...
        "gate": {"id": f"gate-{cid}", "kind": "receipt", "threshold": {"operator": "lte", "value": threshold}},
        "test": {"id": f"test-{cid}", "kind": "independent_receipt"},
        "required_evidence": evidence or ["receipt"],
        "severity": severity,
        "provenance": {"kind": provenance, "source": "human policy", "source_sha256": "a" * 64},
        "remediation": f"Supply a fresh receipt for {cid}",
    }


def _pack(*, controls: list[dict] | None = None, kind: str = "human_confirmed", author: str = "alice", approver: str = "bob", parent: dict | None = None) -> dict:
    return {
        "schema": "factory.policy-pack.v1",
        "pack_id": "team-baseline",
        "version": "1.0.0",
        "provenance": {"kind": kind, "source": "reviewed baseline", "source_sha256": "b" * 64},
        "authorship": {"author": author},
        "approval": {"approver": approver, "decision": "approved", "approved_at": "2026-01-01T00:00:00Z", "approval_sha256": "c" * 64},
        "parent": parent,
...
def _sealed_receipt(root: Path, control_id: str = "CTRL-001") -> str:
    ...
    payload = {"control_id": control_id, "verdict": "PASS", "source": "independent-run", "generated_at": "2026-01-01T00:00:00Z"}
...
def test_provenance_and_inheritance_are_visible(tmp_path: Path) -> None:
    ...
    _write(child, _pack(controls=[_control("CTRL-CHILD")], parent={"path": "parent.json", "sha256": hashlib.sha256(parent.read_bytes()).hexdigest()}))
    ...
    assert {item["id"] for item in resolved["controls"]} == {"CTRL-PARENT", "CTRL-CHILD"}
    ...
    _write(nested_child, _pack(controls=[], parent={"path": "../org-parent.json", "sha256": hashlib.sha256(nested_parent.read_bytes()).hexdigest()}))
    assert "CTRL-ORG" in {item["id"] for item in load_policy_pack(tmp_path, "controls/nested.json")["controls"]}
...
def test_weakening_and_deletion_fail_closed(tmp_path: Path) -> None:
    ...
    child["parent"] = {"path": "parent.json", "sha256": hashlib.sha256(parent.read_bytes()).hexdigest()}
    ...
    deleted_value["parent"] = {"path": "parent.json", "sha256": hashlib.sha256(parent.read_bytes()).hexdigest()}
...
def test_evaluation_blocks_without_evidence_and_reports_drift(tmp_path: Path) -> None:
    ...
    _write(policy, _pack())
    evaluation = evaluate_controls(tmp_path, "controls/policy.json", baseline={"effective_sha256": "d" * 64}, event={"kind": "deployment", "actor": "agent", "commit": "abc"})
    ...
    assert evaluation["authority"] == {"release": False, "merge": False, "deploy": False, "credentials": False}
    passed = evaluate_controls(tmp_path, "controls/policy.json", evidence_paths=[_sealed_receipt(tmp_path)])
...
def test_exception_sod_ttl_and_expiry(tmp_path: Path) -> None:
    ...
    exception = create_exception(tmp_path, "controls/policy.json", "CTRL-001", owner="ops", reason="hotfix", scope="release/123", ttl_days=7, evidence_path="evidence.txt", author="alice", approver="bob", out="controls/exception.json")
    assert exception["status"] == "ACTIVE"
    assert exception["approval"]["approver"] == "bob"
    result = evaluate_controls(tmp_path, "controls/policy.json", exception_paths=["controls/exception.json"])
    assert result["controls"][0]["status"] == "EXEMPT"
    with pytest.raises(ControlsError) as error:
        create_exception(tmp_path, "controls/policy.json", "CTRL-001", owner="ops", reason="bad", scope="all", ttl_days=31, evidence_path="evidence.txt", author="alice", approver="bob")
...
def test_graph_dossier_and_projection(tmp_path: Path) -> None:
    ...
    assert relations[:6] == ["defines", "forbids", "guards", "challenged_by", "evidenced_by", "decides"]
...
def test_fleet_coverage(tmp_path: Path) -> None:
    for name in ("one", "two"):
        ...
        _write(repo / "controls" / "policy.json", _pack(controls=[_control("CTRL-001" if name == "one" else "CTRL-002")]))
    fleet = {"schema": "factory.control-fleet.v1", "baseline_controls": ["CTRL-001", "CTRL-002"], "repositories": [{"id": "one", "path": "repos/one", "policy": "controls/policy.json"}, {"id": "two", "path": "repos/two", "policy": "controls/policy.json"}]}
