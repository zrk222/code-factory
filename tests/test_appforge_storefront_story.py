...
from factoryline.appforge_storefront_story import (
    CONTRACT_SCHEMA,
    EVIDENCE_SCHEMA,
    RECEIPT_SCHEMA,
    storefront_story_projection,
    verify_storefront_story,
)
...
def _media(root: Path) -> Path:
    core = {
        ...
        "media_sets": {
            ...
            "ipad": {
                "captures": [{"id": "ipad-workspace", "journey": "core workspace"}]
            },
        },
    }
...
def _contract(
    root: Path, media: Path, *, headline: str = "Your calm workspace"
) -> Path:
    ...
    return _write(
        root / "story.json",
        {
            ...
            "scenes": [
                {
                    "set_id": "iphone",
                    "capture_id": "phone-first",
                    "story_beat": "mission",
                    "headline": headline,
                    "supporting_copy": "See the first useful next step.",
                    "claim_kind": "experience",
                    "evidence_refs": [],
                },
                {
                    "set_id": "ipad",
                    "capture_id": "ipad-workspace",
                    "story_beat": "agency",
                    "headline": "Work with context",
                    "supporting_copy": "Open the work that matters without losing your place.",
                    "claim_kind": "feature",
                    "evidence_refs": ["evidence-reference.md"],
                },
            ],
        },
    )
...
def _evidence(root: Path, contract: Path) -> Path:
    (root / "evidence-reference.md").write_text(
        "Local feature walkthrough reference.", encoding="utf-8"
    )
    return _write(
        root / "evidence.json",
        {
...
def test_storefront_story_binds_every_capture_to_one_reviewed_scene(
    ...
    receipt = verify_storefront_story(
        tmp_path,
        candidate,
        media,
        contract,
        evidence,
        Path(".factory/appforge/storefront-story.json"),
    )
    ...
    assert (
        main(
            [
                ...
                "evidence.json",
                "--out",
                ".factory/appforge/cli-storefront-story.json",
                "--json",
            ]
        )
        == 0
    ...
    assert (
        ...
        == "APPFORGE_STOREFRONT_STORY_READY"
    )
