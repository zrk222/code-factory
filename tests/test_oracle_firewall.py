...
@pytest.mark.parametrize(
    "mutation", ["gates", "provenance", "sources", "duplicate", "original", "groups"]
)
def test_rehashed_contract_must_still_obey_constructor_rules(tmp_path, mutation):
    ...
    if mutation == "gates":
        ...
        payload["rules"]["gates"][0]["origin"] = "agent_proposed"
    elif mutation == "sources":
        payload["sources"] = []
    elif mutation == "duplicate":
        payload["sources"].append(dict(payload["sources"][0]))
    elif mutation == "original":
        payload["sources"][0]["id"] = "replacement"
    else:
        payload["rules"].pop("tests")
...
def _handoff(root: Path) -> Path:
    ...
    intent.write_text(
        "Users must be able to restore a subscription without a new purchase.\n",
        encoding="utf-8",
    )
    result = capture_intent_handoff(
        ...
        AGENT,
        "ios-intake",
        Path(".factory/oracles/handoffs/ios-intake.json"),
    )
...
def _input(
    ...
    *,
    ...
    exception_effect: str = "advisory",
) -> Path:
    rules = {
        "requirements": [
            {
                "id": "restore",
                "statement": "Restore succeeds for an entitled account.",
        ...
        "forbidden_behaviors": [
            {
                "id": "double-charge",
                "statement": "A restore must not create another charge.",
                "origin": "human_confirmed",
                "effect": "blocking",
                "source_id": "original-intent",
                "critical": True,
        ...
        "gates": [
            {
                "id": "restore-rate",
                "statement": "Restore evidence rate is at least the approved floor.",
                "origin": "human_confirmed",
                "effect": "blocking",
                "source_id": "original-intent",
                "critical": True,
        ...
        "exceptions": [
            {
                "id": "offline-note",
                "statement": "Offline handling remains advisory until a named decision is recorded.",
                "origin": "human_confirmed",
                "effect": exception_effect,
                "source_id": "original-intent",
                "critical": False,
            }
        ],
        "negative_cases": (
            [
                {
                    ...
                    "statement": "An expired entitlement must not restore paid access.",
                    "origin": "human_confirmed",
                    "effect": "blocking",
                    "source_id": "original-intent",
                    "critical": True,
            ...
            if include_negative
                ...
                {
                    "id": "new-negative",
                    "statement": "A new non-equivalent negative case cannot replace the required case.",
                    "origin": "human_confirmed",
                    "effect": "blocking",
                    "source_id": "original-intent",
                    "critical": True,
        ...
        "invariants": [
            {
                "id": "candidate-bound",
                "statement": "Evidence must bind to the exact submitted candidate.",
        ...
        "tests": [
            {
                "id": "restore-flow",
                "statement": "The restore flow must fail when implementation accepts expired access.",
                "origin": "human_confirmed",
                "effect": "blocking",
                "source_id": "original-intent",
                "critical": True,
    ...
    payload = {
        ...
        "approval_rationale": "The product owner reviewed the original intent and the strict restore safety boundary.",
        "scope_paths": ["."],
...
def _contract(
    root: Path,
    *,
    out: str = ".factory/oracles/contracts/original.json",
    **kwargs: object,
) -> Path:
...
def test_handoff_or_contract_seals_exact_original_intent_and_rejects_agent_authority(
    tmp_path: Path,
) -> None:
    contract = _contract(tmp_path)
    verified = verify_oracle_contract(tmp_path, contract)
    handoff = verify_intent_handoff(
        tmp_path, tmp_path / ".factory/oracles/handoffs/ios-intake.json"
    )

    assert handoff["ok"] is True
    assert verified["ok"] is True
    assert (
        verified["contract"]["handoff"]["original_intent"]["sha256"]
        == hashlib.sha256(
            (tmp_path / "briefs/original-intent.md").read_bytes()
        ).hexdigest()
    )
    ...
    with pytest.raises(OracleFirewallError) as raised:
        seal_oracle_contract(
            tmp_path, source, Path(".factory/oracles/contracts/agent-gate.json")
        )
    assert raised.value.code == "ORACLE_PROVENANCE_INVALID"


def test_drift_blocks_threshold_lowering_and_removed_negative_case_with_source_justification(
    tmp_path: Path,
) -> None:
    ...
    candidate = (
        tmp_path
        / seal_oracle_contract(
            tmp_path, source, Path(".factory/oracles/contracts/candidate.json")
        )["path"]
    )
    drift = compare_oracle_contracts(
        tmp_path, prior, candidate, Path(".factory/oracles/drifts/latest.json")
    )

    assert drift["marker"] == "E_ORACLE_WEAKENING"
    assert drift["verdict"] == "BLOCKED"
    assert {item["code"] for item in drift["findings"]} >= {
        "threshold_lowered",
        "negative_case_removed",
    }
    assert all(
        item["justification"]["approved_by"] == "Release Owner"
        for item in drift["findings"]
    )


def test_drift_requires_review_for_same_id_semantic_rewrites_for_every_blocking_rule_class(
    tmp_path: Path,
) -> None:
    prior = _contract(tmp_path, out=".factory/oracles/contracts/prior.json")
    source = _input(tmp_path, tmp_path / ".factory/oracles/handoffs/ios-intake.json")
    candidate_input = json.loads(source.read_text(encoding="utf-8"))
    candidate_input["requirements"][0]["statement"] = (
        "Restore may fail for an entitled account."
    )
    candidate_input["forbidden_behaviors"][0]["statement"] = (
        "A restore may create another charge."
    )
    candidate_input["invariants"][0]["statement"] = (
        "Evidence may bind to a different candidate."
    )
    ...
    rewritten = [
        item for item in drift["findings"] if item["code"] == "blocking_rule_rewritten"
    ]
    assert drift["marker"] == "ORACLE_DRIFT_REVIEW_REQUIRED"
    assert drift["verdict"] == "REVIEW_REQUIRED"
    assert {item["group"] for item in rewritten} >= {
        "requirements",
        "forbidden_behaviors",
        "invariants",
        "gates",
...
def test_added_blocking_rule_requires_review_and_never_clears(tmp_path: Path) -> None:
    ...
    added = [
        item
        for item in drift["review_findings"]
        if item["code"] == "blocking_rule_added"
    ]
    assert len(added) == 1
    ...
    assert (
        added[0]["after"]["statement"]
        == "The final receipt must remain available for audit."
    )


def test_advisory_removal_or_rewrite_requires_review_and_never_clears(
    tmp_path: Path,
) -> None:
    handoff = _handoff(tmp_path)
    source = _input(tmp_path, handoff)
    prior_input = json.loads(source.read_text(encoding="utf-8"))
    prior_input["requirements"].append(
        {
            "id": "advisory-note",
            "statement": "A non-blocking operator note remains visible.",
            "origin": "human_confirmed",
            "effect": "advisory",
            "source_id": "original-intent",
            "critical": False,
    ...
    prior = (
        tmp_path
        / seal_oracle_contract(
            tmp_path, source, Path(".factory/oracles/contracts/prior.json")
        )["path"]
    )
    candidate_input = json.loads(source.read_text(encoding="utf-8"))
    candidate_input["requirements"] = [
        item
        for item in candidate_input["requirements"]
        if item["id"] != "advisory-note"
    ]
    _write(source, candidate_input)
    removed = (
        tmp_path
        / seal_oracle_contract(
            tmp_path, source, Path(".factory/oracles/contracts/removed.json")
        )["path"]
    )
    removal = compare_oracle_contracts(tmp_path, prior, removed)
    assert removal["verdict"] == "REVIEW_REQUIRED"
    assert any(
        item["code"] == "advisory_rule_removed" for item in removal["review_findings"]
    )

    candidate_input = prior_input
    candidate_input["requirements"][-1]["statement"] = (
        "The operator note changes and must be reviewed."
    )
    _write(source, candidate_input)
    rewritten = (
        tmp_path
        / seal_oracle_contract(
            tmp_path, source, Path(".factory/oracles/contracts/rewritten.json")
        )["path"]
    )
    rewrite = compare_oracle_contracts(tmp_path, prior, rewritten)
    assert rewrite["verdict"] == "REVIEW_REQUIRED"
    assert any(
        item["code"] == "blocking_rule_rewritten" for item in rewrite["review_findings"]
    )


def test_shadow_oracle_challenge_is_implementation_targeted_and_fails_on_survivor(
    tmp_path: Path,
) -> None:
    contract = _contract(tmp_path)
    plan = compile_oracle_challenge(
        tmp_path, contract, Path(".factory/oracles/challenges/restore.json")
    )
    result = _write(
        tmp_path / "challenge-result.json",
        {
            "schema": "factory.oracle-challenge-result.v1",
            "challenge_sha256": plan["challenge_sha256"],
            "worker_subject": "worker-alpha",
            "verifier_subject": "verifier-beta",
            "target": "implementation",
            "cases": [
                {"id": item["id"], "outcome": "survived" if index == 0 else "killed"}
    ...
    assert [item["relation"] for item in gate["boundary_cases"]] == [
        "below",
        "at",
        "above",
    ]
...
def test_shadow_oracle_challenge_rejects_a_stale_plan_even_when_every_case_is_killed(
    tmp_path: Path,
) -> None:
    contract = _contract(tmp_path)
    plan = compile_oracle_challenge(
    ...
    result = _write(
        tmp_path / "challenge-result.json",
        {
            ...
            "target": "implementation",
            "cases": [
                {"id": item["id"], "outcome": "killed"} for item in plan["cases"]
            ],
        },
    )
    ...
    captured.write_text(
        "A changed bound source invalidates the previously sealed contract.\n",
        encoding="utf-8",
    )
...
@pytest.mark.parametrize(
    "mutation", ["missing", "extra", "reordered", "duplicate", "altered"]
)
def test_rehashed_challenge_plan_must_exactly_match_current_contract(
...
def test_challenge_result_rejects_non_exact_case_rows(
    tmp_path: Path, mutation: str
) -> None:
    contract = _contract(tmp_path)
    plan = compile_oracle_challenge(
        tmp_path, contract, Path(".factory/oracles/challenges/exact.json")
    )
...
def test_oracle_incident_demotes_declared_agent_and_projects_read_only_status(
    tmp_path: Path,
) -> None:
    prior = _contract(tmp_path, out=".factory/oracles/contracts/prior.json")
    handoff = tmp_path / ".factory/oracles/handoffs/ios-intake.json"
    candidate = (
        ...
        / seal_oracle_contract(
            tmp_path,
            _input(tmp_path, handoff, threshold=90),
            Path(".factory/oracles/contracts/candidate.json"),
        )["path"]
    )
    drift = compare_oracle_contracts(
        tmp_path, prior, candidate, Path(".factory/oracles/drifts/blocked.json")
    )
...
def test_oracle_incident_rejects_stale_or_unrelated_drift_without_demotion(
    tmp_path: Path,
) -> None:
    prior = _contract(tmp_path, out=".factory/oracles/contracts/prior.json")
    source = _input(
        tmp_path, tmp_path / ".factory/oracles/handoffs/ios-intake.json", threshold=90
    )
    candidate = (
        tmp_path
        / seal_oracle_contract(
    ...
    assert (
        sorted(incidents.glob("*.json")) if incidents.exists() else []
    ) == before_incidents

    stale = _write(
        tmp_path / ".factory/oracles/drifts/stale.json",
        {
            ...
            "contracts": {
                "prior": {"path": "x", "contract_sha256": "0" * 64},
                "candidate": {"path": "y", "contract_sha256": "1" * 64},
            },
            "findings": [],
    ...
    stale_payload["drift_sha256"] = hashlib.sha256(
        json.dumps(stale_payload, sort_keys=True, separators=(",", ":")).encode("utf-8")
    ).hexdigest()
...
def test_oracle_incident_rejects_self_hashed_drift_not_reconstructed_from_contracts(
    tmp_path: Path,
) -> None:
    prior = _contract(tmp_path, out=".factory/oracles/contracts/prior.json")
    source = _input(
...
def test_full_init_creates_deliberately_incomplete_appforge_authority_workspace(
    tmp_path: Path,
) -> None:
    source = tmp_path / "original.md"
    source.write_text(
        "The user wants accessible subscription restore and a clear review path.",
        encoding="utf-8",
    )

    result = initialize_oracle_firewall(
        tmp_path,
        Path(".factory/oracles/init/ios"),
        source,
        AGENT,
        "ios-release",
        ["."],
    ...
    assert (
        json.loads(
            (workspace / "oracle-contract-input.json").read_text(encoding="utf-8")
        )["requirements"]
        == []
    )
    assert (workspace / "appforge-policy-authority-template.json").is_file()
    assert "does not approve" in (workspace / "NEXT_STEPS.md").read_text(
        encoding="utf-8"
    )
