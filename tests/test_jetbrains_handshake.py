...
from factoryline.jetbrains_handshake import (
...
def _scope(root: Path) -> Path:
    ...
    return _write(
        root / ".factory/repair-sandboxes/scope.json",
        {
            key: value
            for key, value in scope.items()
            if key not in {"scope_markdown", "mermaid"}
        },
    )
...
def _e2e(root: Path, *, hollow: bool = False) -> Path:
    manifest = _write(
        root / "proof.json",
        {
            ...
            "id": "checkout",
            "approval": {"state": "approved", "approved_by": "Ada"},
            "working_directory": ".",
            "timeout_seconds": 5,
            ...
            "negative": {
                "argv": [
                    "python",
                    "-c",
                    "pass" if hollow else "import sys; sys.exit(1)",
                ]
            },
            "artifact_paths": [],
        },
    ...
    return _write(
        root / ".factory/e2e-proof/receipt.json",
...
def _intent(root: Path) -> None:
    capture_intent_ledger(
        root,
        change_list="Checkout hardening",
        changed=["app.py"],
        confirmed_by="Ada",
        promise="Checkout rejects expired cards.",
        non_goal="No billing migration.",
        failure_case="An expired card is rejected.",
        confirmation="CAPTURE Checkout hardening",
    )


def test_mission_is_scope_bound_and_tells_junie_not_to_weaken_tests(
    tmp_path: Path,
) -> None:
...
def test_mission_binds_hash_valid_appforge_context_without_granting_agent_authority(
    tmp_path: Path,
) -> None:
    scope = _scope(tmp_path)
    brief = _write(
        tmp_path / "appforge-brief.json",
        {
            ...
            "screens": [
                {"id": "review", "user_goal": "understand whether evidence is complete"}
            ],
        },
    )
...
def test_handshake_blocks_scope_escape_analysis_regression_and_hollow_test(
    tmp_path: Path,
) -> None:
    scope = _scope(tmp_path)
    sarif = _sarif(
        tmp_path,
        results=[{"ruleId": "UnsafeCall", "level": "error", "baselineState": "new"}],
    )
    receipt = _e2e(tmp_path, hollow=True)
    result = evaluate_jetbrains_handshake(
        tmp_path, scope, ["app.py", "release.yml"], sarif, receipt
    )
...
def test_handshake_ready_requires_intent_analysis_execution_and_non_hollow_e2e(
    ...
    monkeypatch.setattr(
        "factoryline.jetbrains_handshake.inspect_intent_ledger",
        lambda *_args, **_kwargs: {
            "state": "ready_for_human_review",
            "inspection_sha256": "a" * 64,
            "next_action": {"action": "review_packet"},
            "record": None,
        },
    )
...
def test_handshake_accepts_sonarqube_as_a_verified_analysis_source(
    ...
    result = evaluate_jetbrains_handshake(
        tmp_path, scope, ["app.py"], _sarif(tmp_path, tool="SonarQube for IDE"), receipt
    )
    assert result["verdict"] == "ready_for_human_review"
    assert result["analysis"]["provider"] == "sonarqube"


def test_handshake_fails_closed_on_tampered_scope_and_missing_evidence(
    tmp_path: Path,
) -> None:
    scope = _scope(tmp_path)
    ...
    scope = _scope(tmp_path)
    result = evaluate_jetbrains_handshake(
        tmp_path, scope, ["app.py"], _sarif(tmp_path, success=False)
    )
