...
from factoryline.capability_evidence import (
...
def test_structural_audit_hashes_files_without_execution_or_inflated_claim(
    tmp_path: Path,
):
    _manifest(tmp_path)
...
@pytest.mark.parametrize(
    ("path", "code"),
    [
        ("../outside.py", "E_CAPABILITY_EVIDENCE_PATH"),
        ("missing.py", "E_CAPABILITY_EVIDENCE_MISSING"),
    ],
)
...
def test_whole_manifest_is_validated_before_any_command_runs(
    ...
    manifest.write_text(json.dumps(raw), encoding="utf-8")
    monkeypatch.setattr(
        "factoryline.capability_evidence.run_bounded_command",
        lambda *args, **kwargs: pytest.fail("command ran before validation completed"),
    )
    with pytest.raises(CapabilityEvidenceError) as caught:
...
def test_cli_returns_stable_json_and_nonzero_for_failed_execution(
...
def test_repository_manifest_binds_all_public_maturity_classes():
    ...
    assert {item["maturity"] for item in result["claims"]} == {
