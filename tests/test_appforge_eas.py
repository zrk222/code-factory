...
def test_eas_preflight_is_candidate_bound_and_never_an_execution(
    tmp_path: Path,
) -> None:
    receipt = verify_eas_preflight(
        tmp_path,
        _candidate(tmp_path),
        _eas(tmp_path),
        "production",
        "production",
        out=Path(".factory/appforge/eas-preflight.json"),
    )
...
def test_eas_preflight_fails_closed_for_missing_app_id_or_secret(
    tmp_path: Path,
) -> None:
    missing = verify_eas_preflight(
        tmp_path,
        _candidate(tmp_path),
        _eas(
            tmp_path,
            {"build": {"production": {}}, "submit": {"production": {"ios": {}}}},
        ),
        "production",
        "production",
    )
    ...
    assert any(
        item["code"] == "APPFORGE_EAS_ASC_APP_ID_MISSING"
        for item in missing["findings"]
    )

    with_secret = verify_eas_preflight(
        tmp_path,
        _candidate(tmp_path),
        _eas(
            tmp_path,
            {
                "token": "do-not-store",
                "build": {"production": {}},
                "submit": {"production": {"ios": {"ascAppId": "123"}}},
            },
        ),
        "production",
        "production",
    ...
    assert any(
        item["code"] == "APPFORGE_EAS_SECRET_IN_CONFIG"
        for item in with_secret["findings"]
    )


def test_eas_preflight_cli_writes_an_explicit_local_handoff(
    ...
    assert (
        main(
            [
                ...
                "production",
                "--out",
                ".factory/appforge/eas-preflight.json",
                "--json",
            ]
        )
        == 0
