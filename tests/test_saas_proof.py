...
from factoryline.saas_proof import (
    SaasProofError,
    saas_proof_projection,
    verify_saas_proof,
)
...
def _contract() -> dict:
    return {
        ...
        "provider": {
            ...
            "audience": "demo-api",
            "pkce_required": True,
            "claims": {"subject": "sub", "tenant": "org_id", "roles": "roles"},
        },
...
def _evidence() -> dict:
    ...
    for sequence, event_type in enumerate(types, 1):
        events.append(
            {
                "id": f"e{sequence}",
                "provider_event_id": f"p{sequence}",
                "sequence": sequence,
                "type": event_type,
                "subject": "u1",
                "tenant": "t1",
                ...
                "audience": "demo-api" if event_type == "auth_success" else None,
...
def test_provider_neutral_lifecycle_verifies_and_projects(tmp_path: Path):
    receipt = verify_saas_proof(
        tmp_path,
        _write(tmp_path / "contract.json", _contract()),
        _write(tmp_path / "evidence.json", _evidence()),
        Path(".factory/saas-proof/latest.json"),
    )
...
def test_missing_observation_and_stale_entitlement_block(tmp_path: Path):
    evidence = _evidence()
    evidence["events"] = [
        item for item in evidence["events"] if item["type"] != "webhook_verified"
    ]
...
def test_raw_tokens_and_non_pkce_contracts_fail_closed(tmp_path: Path):
    ...
    with pytest.raises(SaasProofError) as error:
        verify_saas_proof(
            tmp_path,
            _write(tmp_path / "contract2.json", _contract()),
            _write(tmp_path / "evidence2.json", evidence),
            Path("out.json"),
        )
...
def test_cross_tenant_journey_and_missing_build_binding_fail_closed(tmp_path: Path):
    ...
    assert receipt["verdict"] == "blocked"
    assert "SAAS_PROOF_CROSS_IDENTITY_OR_TENANT_JOURNEY" in {
    ...
    with pytest.raises(SaasProofError) as error:
        verify_saas_proof(
            tmp_path,
            _write(tmp_path / "contract2.json", _contract()),
            _write(tmp_path / "evidence2.json", missing_build),
            Path("out2.json"),
        )
