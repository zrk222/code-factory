...
def test_public_audit_condition_count_is_recomputed_from_source():
    ...
    spec = importlib.util.spec_from_file_location(
        "audit_condition_inventory", module_path
    )
...
def test_publication_versions_and_citation_are_synchronized():
    ...
    package_version = _match(
        ROOT / "factoryline" / "__init__.py", r'^__version__ = "([^"]+)"$'
    )
    ...
    assert (
        package["runtimeArguments"][0]["value"]
        == f"factoryline-code-factory=={pyproject_version}"
    )
...
def test_python_wheel_data_is_explicit_and_does_not_depend_on_package_discovery():
    ...
    assert (
        'factoryline = ["builtin_packs/**/*.json", "builtin_packs/**/*.yaml", "data/*.json", "hosted_console.html", "graph_ops.html"]'
        in project
    )
...
def test_public_ctas_are_outcome_led_and_preserve_proof_boundaries():
    ...
    assert {
        ...
        "design-review",
        "ui-quality",
    }.issubset(vscode_package["keywords"])
    ...
    assert (
        "FactoryLine AI Proof is free, local IDE Guardian + AI proof for JetBrains."
        in intellij_plugin
    )
    assert 'width="40"' in (
        ROOT
    ...
    operations = (ROOT / "docs" / "ENTERPRISE_TEAMS_OPERATIONS.md").read_text(
        encoding="utf-8"
    )
...
def test_vscode_supply_chain_is_patched_and_audited_before_tests():
    ...
    auditor = (ROOT / "editors/vscode/scripts/audit-lockfile.mjs").read_text(
        encoding="utf-8"
    )
    assert (
...
def test_openvsx_workflow_seals_a_tested_immutable_candidate_before_manual_publish():
    workflow = (ROOT / ".github" / "workflows" / "openvsx.yml").read_text(
        encoding="utf-8"
    )
    ...
    assert (
        workflow.index("npm ci")
        < workflow.index("npm run audit")
    ...
    assert workflow.index(
        "Require the scoped Open VSX publisher token before candidate work"
    ) < workflow.index("Install, audit, test, and package")


def test_marketplace_workflow_uses_current_gradle_action_and_scoped_secret():
    workflow = (ROOT / ".github" / "workflows" / "jetbrains-marketplace.yml").read_text(
        encoding="utf-8"
    )
...
def test_intellij_compatibility_reuses_verified_package_with_cached_dependencies():
    ...
    assert (
        "./gradlew buildPlugin verifyPlugin -PfactorylineVerificationProduct"
        not in workflow
    )
...
def test_hosted_release_and_editor_versions_are_declared():
    ...
    hosted_workflow = (ROOT / ".github" / "workflows" / "hosted-adapter.yml").read_text(
        encoding="utf-8"
    )
...
def test_jetbrains_listing_is_outcome_led_and_first_proof_is_discoverable():
    ...
    screenshot_brief = (
        ROOT / "docs" / "JETBRAINS_MARKETPLACE_SCREENSHOTS.md"
    ).read_text(encoding="utf-8")
    ...
    assert (
        "OAuth/OIDC identity, tenant authorization, checkout, webhook, entitlement, feature access, and revocation"
        in plugin_xml
    )
...
def test_intellij_build_uses_the_gradle_9_5_compatible_kotlin_plugin_line():
    settings = (ROOT / "editors" / "intellij" / "settings.gradle.kts").read_text(
        encoding="utf-8"
    )
    build = (ROOT / "editors" / "intellij" / "build.gradle.kts").read_text(
        encoding="utf-8"
    )
...
def test_marketplace_acquisition_kit_uses_real_product_assets_and_observed_metrics_only():
    kit = (ROOT / "docs" / "JETBRAINS_MARKETPLACE_ACQUISITION_KIT.md").read_text(
        encoding="utf-8"
    )
    baseline = json.loads(
        (ROOT / "docs" / "JETBRAINS_MARKETPLACE_MEASUREMENT.json").read_text(
            encoding="utf-8"
        )
    )
...
def test_jetbrains_price_is_owner_locked_reproducible_and_not_claimed_live():
    sample = json.loads(
        (ROOT / "docs" / "JETBRAINS_PRICING_BENCHMARK.json").read_text(encoding="utf-8")
    )
    ...
    assert (
        round((average - 5.95) / average, 10) == sample["discount_from_sample_average"]
    )
...
def test_jetbrains_paid_launch_is_complete_but_cannot_activate_early():
    plan = json.loads(
        (ROOT / "docs" / "JETBRAINS_MONETIZATION_2027.json").read_text(encoding="utf-8")
    )
    runbook = (ROOT / "docs" / "JETBRAINS_MONETIZATION_2027.md").read_text(
        encoding="utf-8"
    )
    readme = (ROOT / "README.md").read_text(encoding="utf-8")
    active_xml = (
    ...
    staged_xml = (
        ...
        / "intellij"
        / "monetization"
        / "plugin-product-descriptor-2027.xml"
    ).read_text(encoding="utf-8")
    ...
    assert plan["plugin"]["current_free_version"] == "0.9.1"
    ...
    assert (
        "$5.95 USD per named seat/month or $60 USD per named seat/year"
        not in active_xml
    )
    assert (
        "Optional paid features are not active in this Marketplace build" in active_xml
    )
...
def test_jetbrains_reviewer_and_growth_docs_keep_external_approval_and_reviews_honest():
    reviewer = (ROOT / "docs" / "JETBRAINS_REVIEWER_SUMMARY.md").read_text(
        encoding="utf-8"
    )
    compliance = (
        ROOT / "docs" / "JETBRAINS_MARKETPLACE_COMPLIANCE_0_8_19.md"
    ).read_text(encoding="utf-8")
    growth = (ROOT / "docs" / "JETBRAINS_POST_RELEASE_GROWTH.md").read_text(
        encoding="utf-8"
    )
...
def test_zenodo_metadata_and_visual_evidence_are_publicly_archivable():
    ...
    assert "or claim a measured rejection-rate reduction" in metadata["description"]
...
def test_codex_usage_sample_is_privacy_safe_and_does_not_invent_savings():
    sample = json.loads(
        (ROOT / "docs" / "CODEX_USAGE_SAMPLE.json").read_text(encoding="utf-8")
    )

    assert sample["scope"]["assembly_invocations"] == 19
    assert (
        sum(sample["assembly_invocations"][key] for key in ("real", "help", "dry_run"))
        == 19
    )
