...
def _setup(tmp_path: Path) -> tuple[Path, dict, Path, Path, Path]:
    ...
    mission.write_text(
        json.dumps({"schema": "factory.mission.v1", "id": "checkout-proof"}),
        encoding="utf-8",
    )
        ...
        json.dumps(
            {
                ...
                else [{"id": "unit", "passed": True}, {"id": "policy", "passed": True}],
                "verdict": verdict,
                "harness_attestation": {
                    "runner": "external-supervised",
...
def test_verifier_plane_binds_distinct_worker_bundle_and_evidence(
    tmp_path: Path,
) -> None:
...
def test_verifier_plane_rejects_self_verification(tmp_path: Path) -> None:
    ...
    worker = _worker(root, session, candidate)
    verifier = _verifier(
        root, session, worker, bundle, evidence, verifier_id="worker-1"
    )
...
def test_verifier_plane_rejects_usage_above_the_bound_token_ceiling(
    tmp_path: Path,
) -> None:
    root, session, candidate, bundle, evidence = _setup(tmp_path)
    worker = _worker(root, session, candidate)
...
def test_verifier_plane_rejects_passed_verdict_with_failed_check(
    tmp_path: Path,
) -> None:
    root, session, candidate, bundle, evidence = _setup(tmp_path)
    worker = _worker(root, session, candidate)
...
def test_verifier_cli_reports_stalled_progress_without_claiming_execution(
...
def test_verifier_cli_returns_zero_only_for_bound_evidence(
    tmp_path: Path, capsys: pytest.CaptureFixture[str]
) -> None:
    root, session, candidate, bundle, evidence = _setup(tmp_path)
    worker = _worker(root, session, candidate)
...
def test_graph_ops_shows_bound_verifier_sessions_as_runtime_unattested(
    tmp_path: Path,
) -> None:
    ...
    session_node = next(
        node for node in snapshot["nodes"] if node["kind"] == "verifier_session"
    )
    assert session_node["status"] == "runtime-unattested"
    assert snapshot["facts"]["verifier_session_count"] == 1
    assert (
        snapshot["recommendation"]["action"] == "collect_independent_verifier_evidence"
    )
