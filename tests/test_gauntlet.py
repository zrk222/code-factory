...
def _reality() -> dict[str, object]:
    return {
        ...
        "intent_assertions": [
            {
                "id": "manager-approved",
                "statement": "Manager approval is recorded.",
                "evidence": "positive",
            },
            {
                "id": "non-manager-blocked",
                "statement": "Non-manager approval is rejected.",
                "evidence": "negative",
            },
        ],
...
def _write(
    root: Path, *, hollow: bool = False, continuity: dict[str, object] | None = None
) -> Path:
    (root / "approval.e2e.json").write_text(
        json.dumps(_e2e(hollow=hollow)), encoding="utf-8"
    )
    (root / "approval.reality.json").write_text(
        json.dumps(_reality()), encoding="utf-8"
    )
...
def _verified_continuity(root: Path) -> dict[str, object]:
    ...
    promoter = ContinuityPrincipal(
        "continuity-promoter", "team-a", ("promoter",), (purpose,)
    )
    store.record(
        writer,
        {
        ...
        idempotency_key="approved-context",
        record_id="approved-context",
    )
...
def test_gauntlet_proposal_binds_declared_intent_taxonomy_and_exact_e2e_bytes_without_execution(
    tmp_path: Path,
) -> None:
...
def test_gauntlet_proposal_fails_closed_when_declared_e2e_bytes_drift(
    tmp_path: Path,
) -> None:
    proposal_path = _proposal(tmp_path)
    ...
    assert (verification["ok"], verification["marker"]) == (
        False,
        "GAUNTLET_PROPOSAL_STALE",
    )


def test_gauntlet_optionally_binds_only_verified_redacted_continuity_metadata(
    tmp_path: Path,
) -> None:
    ...
    assert (
        gauntlet_status(tmp_path, "approval-gauntlet")["entries"][0]["continuity"][
            "bound"
        ]
        is True
    )
    ...
    with (
        store._session() as db
    ):  # Test only: force stale metadata after a sealed proposal.
        db.execute(
            "UPDATE continuity_records SET expires_at = ? WHERE record_id = ?",
            ("2020-01-01T00:00:00Z", "approved-context"),
        )
    stale = verify_gauntlet_proposal(tmp_path, proposal_path)
    assert (stale["ok"], stale["reason"]) == (False, "GAUNTLET_CONTINUITY_STALE")


def test_gauntlet_requires_named_admission_before_running_any_command(
    tmp_path: Path,
) -> None:
    proposal_path = _proposal(tmp_path)
...
def test_admitted_gauntlet_survives_and_emits_public_offline_verifiable_card(
    tmp_path: Path,
) -> None:
    proposal_path = _proposal(tmp_path)
    ...
    assert (
        Path(result["artifacts"]["svg"]).read_text(encoding="utf-8").startswith("<svg")
    )
    assert Path(
        write_survival_card_artifacts(card, tmp_path / "copied-card")["markdown"]
    ).is_file()
    assert (
        challenge_survival_card(Path(result["path"]))["marker"]
        == "GAUNTLET_CARD_MUTATION_REJECTED"
    )
    with pytest.raises(GauntletError) as exc:
...
def test_hollow_case_is_visible_on_card_and_never_claims_survival(
    tmp_path: Path,
) -> None:
...
def test_survival_card_rejects_tamper_and_optional_dsse_binds_exact_card_hash(
    tmp_path: Path,
) -> None:
    proposal_path = _proposal(tmp_path)
    result = run_gauntlet(tmp_path, proposal_path, _admission(tmp_path, proposal_path))
    ...
    try:
        ...
        Exception
    ) as exc:  # pragma: no cover - optional enterprise extra may be absent.
    ...
    verified = verify_survival_card(
        card_path, envelope_path=envelope, trust_root_path=Path(material["trust_root"])
    )
    assert verified["signature"]["verification"] == "offline_dsse_ed25519"


def test_survival_card_rejects_resealed_promise_reality_mismatch(
    tmp_path: Path,
) -> None:
    proposal_path = _proposal(tmp_path)
    result = run_gauntlet(tmp_path, proposal_path, _admission(tmp_path, proposal_path))
    ...
    core = {
        ...
        if key not in {"card_sha256", "card_markdown", "card_svg"}
    }
...
def test_gauntlet_status_is_read_only_and_projects_existing_card(
    tmp_path: Path,
) -> None:
    proposal_path = _proposal(tmp_path)
    ...
    assert status["entries"][0]["continuity"] == {
        "bound": False,
...
def test_gauntlet_cli_requires_admission_then_exposes_only_public_card_facts(
    ...
    assert (
        main(
            [
                "gauntlet",
                "run",
                str(proposal.relative_to(tmp_path)),
                "--root",
                str(tmp_path),
                "--json",
            ]
    ...
    assert (
        json.loads(capsys.readouterr().err)["marker"] == "GAUNTLET_ADMISSION_REQUIRED"
    )
    assert (
        main(
            [
                ...
                "qa-owner",
                "--rationale",
                "Run the declared local proof.",
                "--confirmation",
                "ADMIT approval-gauntlet",
                "--json",
            ]
        )
        == 0
    ...
    assert (
        main(
            [
                ...
                str(tmp_path),
                "--admission",
                str(admission.relative_to(tmp_path)),
                "--json",
            ]
        )
        == 0
