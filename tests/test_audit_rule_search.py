...
def test_search_is_bounded_lane_filtered_and_hash_addressed():
    ...
    assert all(rule["lane"] == "authorization_tenant_isolation" for rule in payload["rules"])
...
def test_search_can_surface_cross_cutting_rules_and_keeps_context_small():
    ...
    assert all("rejectionCondition" in rule and "practicalQuestion" in rule for rule in payload["rules"])
    if payload["rules"]:
