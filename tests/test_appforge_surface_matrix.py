...
from factoryline.appforge_surface_matrix import (
    create_surface_matrix,
    surface_matrix_projection,
)
...
def test_surface_matrix_expands_the_exact_ready_native_surface_receipt(
    ...
    receipt = create_surface_matrix(
        tmp_path,
        candidate,
        Path(".factory/appforge/native-surface.json"),
        Path(".factory/appforge/surface-matrix.json"),
    )

    assert receipt["marker"] == "APPFORGE_SURFACE_MATRIX_WRITTEN"
    assert {item["configuration"] for item in receipt["scenarios"]} >= {
        "Split View / compact width",
        "Dynamic Type accessibility size",
        "VoiceOver",
    }
    assert all(
        item["required_evidence"] == "supervised physical-device capture"
    ...
    assert (
        main(
            [
                ...
                "candidate.json",
                "--native-surface",
                ".factory/appforge/native-surface.json",
                "--out",
                ".factory/appforge/cli-surface-matrix.json",
                "--json",
            ]
        )
        == 0
...
def test_surface_matrix_rejects_unsealed_or_mismatched_native_surface(
    tmp_path: Path,
) -> None:
    candidate = _candidate(tmp_path)
    ...
    with pytest.raises(RevenueForgeError, match="hash-valid, ready"):
        create_surface_matrix(
            tmp_path, candidate, bad, Path(".factory/appforge/surface-matrix.json")
        )
...
def test_surface_matrix_projection_excludes_tampered_receipt(tmp_path: Path) -> None:
    ...
    (directory / "surface-matrix.json").write_text(
        json.dumps(
            {
                "schema": "factory.appforge.surface-matrix-receipt.v1",
                "receipt_sha256": "0" * 64,
            }
        ),
        encoding="utf-8",
