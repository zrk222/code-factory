...
from factoryline.appforge_native_surface import (
    CONTRACT_SCHEMA,
    EVIDENCE_SCHEMA,
    native_surface_projection,
    verify_native_surface,
)
            ...
            "storyboard": [
                {
                    ...
                    "platform": "iphone",
                    "journey": "first value",
                    "user_value": "understand today",
                },
                {
                    "id": "ipad-workspace",
                    "platform": "ipad",
                    "journey": "core workspace",
                    "user_value": "work with context",
                },
            ],
        },
    )
    ...
    return _write(
        root / "evidence.json",
        {
            ...
            "review": {
                ...
                "accessibility_fallbacks": True,
                "material_hierarchy": True,
                "storyboard_truth": True,
            },
        },
    )
...
def test_native_surface_binds_intent_source_platform_and_storyboard(
    tmp_path: Path,
) -> None:
    ...
    receipt = verify_native_surface(
        tmp_path,
        candidate,
        contract,
        evidence,
        Path(".factory/appforge/native-surface.json"),
    )
...
def test_native_surface_blocks_geometry_glass_and_icon_shortcuts(
    tmp_path: Path,
) -> None:
    candidate = _candidate(tmp_path)
    source = _source(
...
""",
    ...
    assert {
        "APPFORGE_NATIVE_FIXED_SCREEN_GEOMETRY",
        "APPFORGE_NATIVE_ADAPTIVE_API_MISSING",
        "APPFORGE_NATIVE_ICON_LABEL_REVIEW_REQUIRED",
    } <= codes


def test_native_surface_blocks_when_required_accessibility_signals_are_absent(
    tmp_path: Path,
) -> None:
    candidate = _candidate(tmp_path)
    source = _source(
    ...
    assert (
        sum(
            item["code"] == "APPFORGE_NATIVE_ACCESSIBILITY_SIGNAL_MISSING"
...
def test_native_surface_rejects_tampered_contract_binding_and_cli_round_trip(
    ...
    assert (
        main(
            [
                ...
                "evidence.json",
                "--out",
                ".factory/appforge/cli-native-surface.json",
                "--json",
            ]
        )
        == 0
    ...
    assert (
        json.loads(capsys.readouterr().out)["marker"] == "APPFORGE_NATIVE_SURFACE_READY"
    )
...
def test_native_surface_projection_excludes_tampered_receipt(tmp_path: Path) -> None:
    ...
    (directory / "native-surface.json").write_text(
        json.dumps(
            {
                "schema": "factory.appforge.native-surface-receipt.v1",
                "receipt_sha256": "0" * 64,
            }
        ),
        encoding="utf-8",
