...
def _api_call(app, method, path, *, body=b"", headers=None):
    ...
    result = b"".join(
        app(
            environ,
            lambda status, values: captured.update(status=status, headers=values),
        )
    )
...
def test_hosted_environment_contract_requires_matching_installation_maps():
    env = {
        "FACTORY_OIDC_ISSUER": "https://id.example",
        "FACTORY_OIDC_AUDIENCE": "factory",
        "FACTORY_WEBHOOK_SECRETS_JSON": json.dumps(
            {"4421": "hosted-webhook-secret-32-bytes!!"}
        ),
...
def test_jwks_cache_refreshes_and_rejects_cache_older_than_hard_limit():
    ...
    transport = Transport(
        [
            Response(200, {"keys": [{"kid": "one"}]}),
            PRAssuranceError("E_HTTP_UNAVAILABLE", "down"),
        ]
    )
...
def test_github_app_publisher_uses_short_lived_credentials_without_returning_them():
    ...
    transport = Transport(
        [Response(201, {"token": "installation-secret"}), Response(201, {"id": 991})]
    )
    publisher = GitHubAppPublisher(
        app_id=17, private_key_pem=pem, transport=transport, clock=lambda: 1_800_000_000
    )
    check_id = publish_check(
        publisher,
        4421,
        "acme/app",
        {"schema": "x", "marker": "y", "name": "check", "head_sha": "a" * 40},
    )
    assert check_id == 991
...
def test_wsgi_routes_require_bearer_and_never_accept_tenant_headers(monkeypatch):
    ...
    status, accepted = _api_call(
        app, "POST", "/v1/github/webhooks", body=body, headers=wsgi_headers
    )
    ...
    assert decided["markers"] == [
        ...
        "JWKS_ROTATION_PINNED",
        "HOSTED_AUTH_BOUNDARY",
    ]


def test_service_rejects_self_approval_and_dispatch_retains_classified_failure(
    monkeypatch,
):
    store = FakeStore()
    ...
    with pytest.raises(PRAssuranceError) as self_approval:
        service.decide(
            "approval-1", _token(private), {"decision": "approved", "reason": "mine"}
        )
    assert self_approval.value.code == "E_SELF_APPROVAL"
    store.outbox = [
        OutboxRecord("o-1", "tenant-a", "a-1", 4421, "acme/app", {"name": "check"}, 1)
    ]

    class Publisher:
        def publish(self, *args):
            raise PRAssuranceError(
                "E_GITHUB_PUBLISH", "failed with installation-secret"
            )

    service.publisher = Publisher()
    assert service.dispatch("tenant-a") == [
        {"marker": "OUTBOX_PENDING", "error_code": "E_GITHUB_PUBLISH"}
    ]
