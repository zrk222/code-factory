...
from factoryline.appforge_mobile_evidence import (
    _sha as _canonical_sha,
    mobile_evidence_projection,
    verify_mobile_evidence,
)
from factoryline.cli import main


CANDIDATE = {
    "bundle_identifier": "com.example.mobile",
    "version": "1.0.0",
    "build_number": "100",
    "source_commit": "a" * 40,
}
...
def _contract(root: Path) -> Path:
    return _write(
        root / "contract.json",
        {
            "schema": "factory.appforge.mobile-evidence-contract.v1",
            "candidate": CANDIDATE,
...
def _evidence(root: Path, contract: Path) -> Path:
    reports = [
        _report(
            ...
            "xcodebuild",
            ["ios"],
            {"build", "tests", "accessibility", "design_system_conformance"},
        ),
        _report(
            root,
            "android_gradle",
        ...
        _report(
            ...
            "fastlane",
            ["ios"],
            {"snapshot", "device_frames", "layout", "contrast", "store_assets"},
        ),
        _report(
            root,
            "device_cloud",
    ...
    return _write(
        root / "evidence.json",
        {
            "schema": "factory.appforge.mobile-evidence-input.v1",
            "candidate": CANDIDATE,
...
def test_mobile_evidence_normalizes_ios_android_and_store_gates(tmp_path: Path) -> None:
    ...
    receipt = verify_mobile_evidence(
        tmp_path,
        candidate,
        contract,
        _evidence(tmp_path, contract),
        Path(".factory/appforge/mobile-evidence.json"),
    )
    assert receipt["ok"] is True
    assert receipt["marker"] == "APPFORGE_MOBILE_EVIDENCE_READY"
    assert set(receipt["contract"]["platforms"]) == {"android", "ios"}
    assert (
        mobile_evidence_projection(tmp_path)["latest"]["receipt_sha256"]
        == receipt["receipt_sha256"]
    )


def test_mobile_evidence_fails_closed_when_a_source_report_changes(
    tmp_path: Path,
) -> None:
    candidate, contract = _candidate(tmp_path), _contract(tmp_path)
    ...
    receipt = verify_mobile_evidence(
        ...
        contract,
        evidence,
        Path(".factory/appforge/mobile-evidence-blocked.json"),
    )
    assert receipt["ok"] is False
    assert any(
        item["code"] == "APPFORGE_MOBILE_EVIDENCE_SOURCE_STALE"
        for item in receipt["findings"]
    )


def test_mobile_evidence_cli_exposes_the_same_candidate_bound_gate(
    ...
    assert (
        main(
            [
                ...
                evidence.name,
                "--out",
                ".factory/appforge/cli-mobile-evidence.json",
                "--json",
            ]
        )
        == 0
...
def test_mobile_evidence_does_not_use_android_passes_for_missing_ios_checks(
    tmp_path: Path,
) -> None:
    candidate, contract = _candidate(tmp_path), _contract(tmp_path)
    evidence = _evidence(tmp_path, contract)
    ...
    receipt = verify_mobile_evidence(
        tmp_path, candidate, contract, evidence, Path("blocked.json")
    )
    assert receipt["ok"] is False
    assert any(
        "ios:ios_native lacks passed evidence" in item["detail"]
        for item in receipt["findings"]
    )


def test_mobile_evidence_rejects_failed_check_even_when_another_report_passes(
    tmp_path: Path,
) -> None:
    candidate, contract = _candidate(tmp_path), _contract(tmp_path)
    evidence = _evidence(tmp_path, contract)
    ...
    assert receipt["ok"] is False
    assert any(
        item["code"] == "APPFORGE_MOBILE_EVIDENCE_CHECK_FAILED"
        for item in receipt["findings"]
    )
...
def test_mobile_evidence_rejects_nonfinite_production_metrics(tmp_path: Path) -> None:
    ...
    assert receipt["ok"] is False
    assert any(
        item["code"] == "APPFORGE_MOBILE_EVIDENCE_REPORT_INVALID"
        for item in receipt["findings"]
    )


def test_mobile_evidence_projection_uses_newest_receipt_not_lexical_path(
    tmp_path: Path,
) -> None:
    candidate, contract = _candidate(tmp_path), _contract(tmp_path)
    first = verify_mobile_evidence(
        tmp_path,
        candidate,
        contract,
        _evidence(tmp_path, contract),
        Path(".factory/appforge/z-old-mobile-evidence.json"),
    )
    second = verify_mobile_evidence(
        tmp_path,
        candidate,
        contract,
        _evidence(tmp_path, contract),
        Path(".factory/appforge/a-new-mobile-evidence.json"),
    )
    import os
...
def test_mobile_evidence_requires_local_build_and_signing_per_platform(
    tmp_path: Path,
) -> None:
    candidate, contract = _candidate(tmp_path), _contract(tmp_path)
    evidence = _evidence(tmp_path, contract)
...
def test_mobile_evidence_requires_platform_native_tool_provenance(
    tmp_path: Path,
) -> None:
    candidate, contract = _candidate(tmp_path), _contract(tmp_path)
    evidence = _evidence(tmp_path, contract)
...
def test_mobile_projection_replays_receipt_and_masks_a_newer_forged_ready_value(
    tmp_path: Path,
) -> None:
    candidate, contract = _candidate(tmp_path), _contract(tmp_path)
    first = verify_mobile_evidence(
        ...
        contract,
        _evidence(tmp_path, contract),
        Path(".factory/appforge/old-mobile-evidence.json"),
    )
    forged = dict(first)
    forged["path"] = None
    forged["reports"] = []
    forged["receipt_sha256"] = _canonical_sha(
        {
            key: item
