...
def _contract(root: Path) -> Path:
    ...
    handoff = capture_intent_handoff(
        root, intent, SENDER, "intake", Path(".factory/oracles/handoffs/intake.json")
    )
    source = _write(
        root / "contract-input.json",
        {
            ...
            "version": 1,
            "approved_by": "Owner",
            "approval_rationale": "Lock the restore safety rule.",
            "scope_paths": ["."],
            ...
            "requirements": [
                {
                    "id": "restore",
                    "statement": "Restore succeeds only for an entitlement.",
            ...
            "forbidden_behaviors": [
                {
                    "id": "charge",
                    "statement": "Restore never creates a purchase.",
                    "origin": "human_confirmed",
                    "effect": "blocking",
                    "source_id": "original-intent",
                    "critical": True,
            ...
            "negative_cases": [
                {
                    "id": "expired",
                    "statement": "Expired access is refused.",
                    "origin": "human_confirmed",
                    "effect": "blocking",
                    "source_id": "original-intent",
                    "critical": True,
            ...
            "tests": [
                {
                    "id": "restore-test",
                    "statement": "The test fails on expired access.",
                    "origin": "human_confirmed",
                    "effect": "blocking",
                    "source_id": "original-intent",
                    "critical": True,
    ...
    return (
        ...
        / seal_oracle_contract(
            root, source, Path(".factory/oracles/contracts/restore.json")
        )["path"]
    )
...
def _handoff(root: Path, contract: Path, *, blocking_unknown: bool = False) -> Path:
    payload = _write(
        root / "semantic-input.json",
        {
            ...
            "goal": "Verify that restore preserves the entitlement boundary.",
            "context_urn": "urn:factory:subscription-restore:v1",
            "context_source_id": "original-intent",
            "scope_paths": ["src", "tests"],
            "allowed_actions": ["inspect", "test"],
            "sensitivities": [
                {
                    "id": "expired",
                    "when": "entitlement is expired",
                    "impact": "the negative case must reject the action",
                }
            ],
            "epistemic": {
                "known": [
                    {
                        "id": "intent",
                        "statement": "The sealed intent prohibits a second purchase.",
                        "source_id": "original-intent",
                    }
                ],
                "unknown": [
                    {
                        "id": "provider-state",
                        "statement": "The current provider entitlement state is not in the local receipt.",
                        "impact": "Do not claim a live restore succeeded.",
                ...
                "uncertain": [
                    {
                        "id": "sandbox-parity",
                        "statement": "Sandbox behavior may differ from a provider environment.",
                        "impact": "Independent runtime evidence remains required.",
                    }
                ],
                "capability_limits": [
                    "This local worker cannot inspect a provider account or approve a release."
                ],
            },
        },
    )
    return (
        root
        / seal_semantic_handoff(
            root, payload, Path(".factory/semantic-authority/handoffs/restore.json")
        )["path"]
    )
    ...
    *,
    ...
    payload = _write(
        root / "lease-input.json",
        {
            ...
            "delegatee": RECEIVER,
            "scope_paths": ["src", "tests"],
            "allowed_actions": actions or ["inspect", "test"],
            "expires_at": expires_at
            or (_now() + timedelta(minutes=20)).isoformat().replace("+00:00", "Z"),
            "approval_origin": "human_confirmed",
            "approved_by": "Owner",
            "rationale": "Bound a local verifier to the original restore task.",
        },
    )
    return (
        root
        / seal_authority_lease(
            root, payload, Path(".factory/semantic-authority/leases/restore.json")
        )["path"]
    )
...
def test_handoff_and_lease_only_admit_exact_bound_context_scope_and_actor(
    tmp_path: Path,
) -> None:
    ...
    binding = {
        "lease_path": lease.relative_to(tmp_path).as_posix(),
        "lease_sha256": lease_value["lease_sha256"],
        "action_id": "bound-restore-check",
        "action": "test",
        "context_urn": "urn:factory:subscription-restore:v1",
    }
    assert (
        verify_semantic_binding(tmp_path, binding, RECEIVER, ["tests"])["action"]
        == "test"
    )
    assert (
        authorize_semantic_action(tmp_path, lease, _request(scope_paths=["outside"]))[
            "reason"
        ]
        == "scope_escape"
    )
    assert (
        authorize_semantic_action(
            tmp_path, lease, _request(context_urn="urn:factory:other:v1")
        )["reason"]
        == "context_mismatch"
    )
    other = {**RECEIVER, "subject": "untrusted"}
    assert (
        authorize_semantic_action(tmp_path, lease, _request(actor=other))["reason"]
        == "lease_subject_mismatch"
    )


def test_lease_rejects_expiration_ungranted_and_external_actions(
    tmp_path: Path,
) -> None:
    handoff = _handoff(tmp_path, _contract(tmp_path))
    ...
    lease = _lease(tmp_path, handoff)
    assert (
        authorize_semantic_action(tmp_path, lease, _request(action="repair"))["reason"]
        == "action_not_granted"
    )


def test_blocking_unknown_prevents_lease_or_action_until_a_new_handoff_is_sealed(
    tmp_path: Path,
) -> None:
...
def test_existing_hash_valid_lease_fails_closed_when_its_handoff_contains_a_blocking_unknown(
    tmp_path: Path,
) -> None:
    handoff = _handoff(tmp_path, _contract(tmp_path))
    lease = _lease(tmp_path, handoff)
    ...
    assert (
        authorize_semantic_action(tmp_path, lease, _request())["reason"]
...
def test_recorded_action_is_immutable_and_replay_is_blocked(tmp_path: Path) -> None:
    ...
    lease_value = json.loads(lease.read_text(encoding="utf-8"))
    output = (
        Path(".factory/semantic-authority/decisions")
        / f"{lease_value['lease_sha256']}-check-restore.json"
    )
    ...
    assert {"semantic_handoff", "authority_lease", "semantic_decision"} <= {
...
def test_agent_proposed_source_cannot_become_semantic_authority(tmp_path: Path) -> None:
    ...
    input_path = _write(
        tmp_path / "bad-semantic-input.json",
        {
            "schema": "factory.semantic-handoff-input.v1",
            "id": "bad",
            "oracle_contract": contract.relative_to(tmp_path).as_posix(),
            "sender": SENDER,
            "receiver": RECEIVER,
            "performative": "REQUEST",
            "goal": "Inspect the restore flow.",
            "context_urn": "urn:factory:subscription-restore:v1",
            "context_source_id": "missing-agent-source",
            "scope_paths": ["src"],
            ...
            "epistemic": {
                ...
                "unknown": [
                    {
                        "id": "state",
                        "statement": "Provider state is unknown.",
                        "impact": "No live conclusion.",
                        "blocking": True,
                    }
                ],
                "uncertain": [
                    {
                        "id": "runtime",
                        "statement": "Runtime parity is unknown.",
                        "impact": "Need evidence.",
                    }
                ],
                "capability_limits": ["No provider access."],
            },
        },
    )
    with pytest.raises(SemanticAuthorityError) as rejected:
        seal_semantic_handoff(
            tmp_path, input_path, Path(".factory/semantic-authority/handoffs/bad.json")
        )
    assert rejected.value.code == "E_SEMANTIC_AUTHORIZATION"
